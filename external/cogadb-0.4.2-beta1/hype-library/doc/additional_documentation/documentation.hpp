/***********************************************************************************************************
Copyright (c) 2012, Sebastian\htmlonly Bre&szlig;\endhtmlonly\latexonly Bre{\ss}\endlatexonly, Otto-von-Guericke University of Magdeburg, Germany. All rights reserved.
 ...
 *  \author    Sebastian\htmlonly Bre&szlig;\endhtmlonly\latexonly Bre{\ss}\endlatexonly
 *  \version   0.1
 *  \date      2012
 ...
 *	It is a research prototype developed by the <a href="http://www.uni-magdeburg.de/unimagdeburg/en/Otto_von_Guericke+University+Magdeburg.html">Otto-von-Guericke University Magdeburg</a> 
 * in collaboration with <a href="http://www.tu-ilmenau.de/en/international/">Ilmenau University of Technology</a>. 
 * 
 ...
 * - <b>New:</b> Supports the following compilers: g++ (>=4.5), clang, and Visual C++
...
<li><a name="bress:2012:GvD:decision_model"></a>Sebastian Bre&szlig;, Siba Mohammad, and
  Eike Schallehn.
...
<!-- <li><a name="siba:2012:GvD:cloud_dbms"></a>Siba Mohammad, Sebastian Bre&szlig;, and
  Eike Schallehn.
...
<li><a name="bress:towards_hybrid_query_processing"></a>Sebastian Bre&szlig;, Eike Schallehn, and
  Ingolf Geist.
 ...
 * \tableofcontents
 *
 * HyPE uses <a href="http://www.cmake.org/">CMake</a> as a cross-platform build system.
 ...
 *
 * There might also be precompiled binaries for your platform and toolchain.<br/>
 * <b>Note</b> that C++ libraries for one platform (e.g. Windows) built with different
 * toolchains or merely different toolchain versions are generally not interchangeable.
 *
 * \section sec Prerequisites
 *
 * HyPE depends on the following third-party libraries and tools:
 *   - <a href="http://www.boost.org/">boost::filesystem, boost::system, boost::thread, boost::program_options, boost::chrono</a>
 ...
 *   - <a href="http://www.alglib.net/">ALGLIB</a> (included, usually no need install separately)
 *   - <a href="http://www.stack.nl/~dimitri/doxygen/">Doxygen</a> (only if you would like to build the documentation)
 *
 ...
 * We advise you to do a CMake out-of-source-tree build, for instance using the following
 ...
 * cmake ../
 * @endcode
 * If you encounter any errors during configuration, or want to tweak HyPE's build system you may want to run `ccmake`
 ...
 *   - <a href="http://www.mingw.org/">Minimal GNU for Windows (32-bit)</a>, GCC 4.7.2
 *   - <a href="http://www.microsoft.com/visualstudio/">Visual Studio C++ 2010 Express</a> (32-bit)
 *
 * Naturally, other toolchains like MinGW-64 and newer versions of Visual Studio
 * might work as well.
 *
 * In any case, if you would like to build HyPE's documentation, install Doxygen via its
 * Windows installer.
 ...
 * They do not necessarily have to be installed into the MinGW/MSYS path hierarchy.
 *
 * Boost <b>must</b> be built for the MinGW toolchain, Visual Studio builds will
 * not work.
 * If you cannot find an appropriate binary, build the binaries from a MinGW command shell
 * (cmd.exe):
 * @code
 * cd boost_1_53_0
 * boostrap mingw
 ...
 * At first you must remove Loki's `src/LevelMutex.cpp`, since it is broken on MinGW and
 ...
 *
 * Now _Configure_ again - Boost should be properly configured now but not the Loki library.
 ...
 *
 * To install HyPE using the Visual Studio C++ toolchain, first build MSVC versions of
 * the dependant libraries.
 *
 * Boost <b>must</b> be built for the MSVC toolchain, MinGW builds will
 * not work.
 * If you cannot find an appropriate binary, build the sources from Windows command shell
 * (cmd.exe):
 * @code
 * cd boost_1_53_0
 * boostrap
 ...
 * HyPE should now <b>Configure</b> properly and you can click <b>Generate</b>.
 * 
 * In Visual Studio, you can now open the generated solution file `HyPE.sln` (located in the
 * build directory) and build the entire solution or a specific target.<br/>
 * <b>Note</b>: In order to run the test suite, copy all necessary DLLs to
 * `hype-library/build/examples/unitests/` (or whatever your build directory is).
 *
 * <b>Note also</b>, that when performing a _Debug_ build, all dependent libraries must be
 * debug versions as well.
 * Debug versions of Boost are automatically selected by the build system, but Loki library
...
/*! \page page_tutorial Tutorial: How to use HyPE
   ...
   * 
	* <b>ATTENTION: HyPE uses the thread library of boost for it's advanced features. There is a bug concerning applications compiled with the g++ compiler, because boost thread does not properly export all symbols until version 1.48. 
	* The Bug was fixed in <a href="http://www.boost.org/users/history/version_1_49_0.html">Boost 1.49</a>. 
	* The workaround is to statically link against boost thread. Further details can be found <a href="https://svn.boost.org/trac/boost/ticket/2309">here</a>.</b>
	* 
...
g++ -g  -Wl,-rpath,${PATH_TO_HYPE_LIB}/lib -Wall -Werror -o <you application's name> <object files> -I${PATH_TO_HYPE_LIB}/include -Bstatic -lboost_thread -pthread -Bdynamic -L${PATH_TO_HYPE_LIB}/lib -lhype -lboost_system -lboost_filesystem -lboost_program_options-mt -lloki -lrt
	*	@endcode
	 ...
	 * 
	 * The general concept of HyPE is to make decisions for your applications, which algorithm (processing device) should be used to perform an operation.
	 * Therefore, you first have to specify the Operations you wish to make decisions for and second you have to register your available algorithms for these operations.
	 * First we need a reference to the global Scheduler:
	 * @code
hype::Scheduler& scheduler=hype::Scheduler::instance();
	*	@endcode
	
	 * HyPE uses two major abstractions: First, a DeviceSpecification, which defines information to a processing device, e.g., a CPU or GPU.
	 * Second, is an AlgorithmSpecification, which encapsulates algorithm specific information, e.g., the name, the name of the operation 
	 * the algorithm belongs to as well as the learning and the load adaption strategy.
	 * 
...
AlgorithmSpecification gpu_alg("GPU_Algorithm",
	...
	*	@endcode
 * 
 * Note that the GPU algorithm is only executable on the GPU and hence, should be assigned only to 
 * DeviceSpecifcations of ProcessingDeviceType GPU. <b>ATTENTION: the algorithm name in the 
 * AlgorithmSpecification has to be unique!</b> 
 * Let's assume that our CPU algorithm runs only 
 * on the CPU and the GPU algorithms runs only on the GPU. We define this by calling the method Scheduler::addAlgorithm:
...
scheduler.addAlgorithm(gpu_alg, gpu_dev_spec); //add GPU Algorithm to GPU Processing Device
	*	@endcode
	 ...
	 * First, we have to identify the parameters of a data set that have a high impact on the algorithms execution time. 
	 * In case of our sorting example, we identify the size of the input array as the most important feature value. Note that HyPE supports n feature values (n>=1).
	 * 
	 * To tell HyPE the feature value(s) of the data set that is to be processed, we have to store them in a hype::Tuple object. By convention, the first entry 
	 * quantifies the size of the input data, and the second (if any) should contain the selectivity of a database operator.
	 * @code
hype::Tuple t;
t.push_back(Size_of_Input_Dataset);//for our sort operation, we only need the data size
	*	@endcode
	 * 
	 * Now, HyPE knows about your hardware and your algorithms. We can now let HyPE do scheduling decisions. 
	 * HyPE needs two informations to perform scheduling decisions: First is a OperatorSpecification, which 
	 * defines the operation that should be executed ("SORT"), and the feature vector of the input data (t). 
	 * Furthermore, we have to specify the location of the input data as well as the desired location for 
...
OperatorSpecification op_spec("SORT", 
	...
	*	@endcode	
	 * The second information, which HyPE needs, is a specification of constraints on the processing devices. 
	 * For some applications, operations cannot be executed on all processing devices for arbitrary data. For example,
	 * if a GPU has not enough memory to process a data set, the operation will fail (and will probably slow down 
	 * other operations). Since HyPE cannot know this, because it does not know the semantic of the operations, the user 
	 * can specify constraints, on which type of processing device the operation may be executed. 
	 * In our case, we have no constraints and just default construct a DeviceConstraint object.
	 * @code
...
SchedulingDecision sched_dec = scheduler.getOptimalAlgorithm(op_spec, dev_constr);
	 * @endcode
	 * 
	*  Note that the application <b>always</b> has to execute the algorithm HyPE chooses, otherwise, all 
	*  following calls to sched_dec.getOptimalAlgorithm() will have undefined behavior.
	*  Since HyPE uses a feedback loop to refine the estimations of algorithm execution times, you have 
	*  to measure the execution times of your algorithms and pass them back to HyPE.
...
alg_measure.afterAlgorithmExecution();  //has to be directly after algorithm termination
	*	@endcode
	 * The AlgorithmMeasurement object starts a timer and afterAlgorithmExecution() stops the timer. Note that the constructor of the AlgorithmMeasurement object needs a SchedulingDecision as parameter.
	 * When we put the usage of the SchedulingDecision together, we get the following code skeleton:
	 * @code
if(sched_dec.getNameofChoosenAlgorithm()=="CPU_Algorithm"){
	...
	alg_measure.afterAlgorithmExecution(); 
	  ...
	  * @endcode
	   ...
	   * Some applications have their own time measurement routines and wish to use their own timer framework. To support such applications,
		* HyPE offers a direct way to add a measured execution time in nanoseconds for a corresponding SchedulingDecision:
	   * @code
...
scheduler.addObservation(sched_dec,end-begin);
	   ...
	   * The complete source code of this example can be found in the documentation \ref online_learning.cpp and in the examples directory of HyPE (examples/use_as_online_framework/online_learning.cpp).
		* 
 * */
...
/*! \page page_existing_plugins_hype Available Components
  ...
  \section sec_statistical_method Statistical Methods
	 ...
	 A Statistical Method learns the relation between the feature values of the input dataset and an algorithms execution time. It is a central part of HyPE, implementing the learning based execution time estimation. 
	  * Hence, it is crucial to select the appropriate statistical method depending on the algorithm and the application environment.
	  * Currently, HyPE supports one dimensional Least Square Method and Multi Linear Fitting. Statistical methods are defined in the type hype::StatisticalMethods.
	  * 
		\subsection subsec_statistical_method_leastsquares Least Square Method
		...
		\subsection subsec_statistical_method_multi_linear_fitting Multi Linear Fitting 
		HyPE uses the multi linear fitting functionality of the ALGLIB Project. You should choose Multi Linear Fitting, if an algorithm depends on multiple input features, such as selections (data size, selectivity).
		<b>Note that Multi Linear Fitting is currently limited to two features, but will support more in future versions of HyPE.</b>
		 
  \section sec_recomputation_heuristic Recomputation Heuristics
   A Recomputation Heuristic implements the load adaption functionality of HyPE. If the load situation of a system dramatically changes, then it is very likely that the execution time of algorithm will change as well.
	 * To ensure sufficiently exact estimations, the learned approximation functions have to be updated. However, there is now 'perfect' point in time when to recompute the approximation functions. 
	 * Therefore, the user can select a Recomputation Heuristic, which is appropriate for the application. Recomputation heuristics are defined in the type hype::RecomputationHeuristics.
	 * 
	 \subsection subsec_recomputation_heuristic_periodic Periodic Recomputation
	  * The Periodic Recomputation Heuristic will recompute the approximation function of an algorithm after X executions of this algorithm. X is called Recomputation 
	  * Period and can be configured as well (see \ref page_configure_hype  for details). 
	  * You should use this Recomputation Heuristic if you want that HyPE refines its estimations at runtime to adapt at changing data, load, etc.
		 
	 \subsection subsec_recomputation_heuristic_oneshot Oneshot Recomputation
	 ...
	 \subsection subsec_recomputation_heuristic_error_based Error based Recomputation (under development)

  \section sec_optimization_criterion Optimization Criterions
   An Optimization Criterion specifies what an "optimal" algorithm for your application is. Should it be the fastest? 
	Or would you like to select algorithms in a way that the throughput of your system is optimized? 
	Therefore, we implemented several strategies to make HyPE configurable and better usable for a wide range of applications. 
	Optimization criteria are defined in the type hype::OptimizationCriterions. 
	 ...
	 \subsection subsec_optimization_criterion_WTAR Waiting Time Aware Response Time 
	  Waiting Time Aware Response Time (WTAR) is an extension of the simple response time algorithm. WTAR takes into account the load 
	  on all processing devices and allocates for an operation O the processing device, were the sum of the waiting time, 
	  until the previous oeprators finished, and the estimated execution time of O is minimal. This is the recommended optimization algorithm for HyPE. 
	
	 \subsection subsec_optimization_criterion_RR Round Robin
	  * The round robin strategy allocates processing devices for operations in turns, distributing a workload of operations on all processing devices. 
	  * This approach works well in case operation need roughly the same time on all processing devices (e.g., on homogeneous hardware). 
	  * However, in case one processing device is significantly faster than the other processing devices, the round strategy will under utilize the 
	  * faster processing device, and over utilize the slower processing devices.

 *  \subsection subsec_optimization_criterion_TBO Threshold-based Outsourcing
 *  Threshold-based Outsourcing is an extension of Response Time. The idea is to force the use of a slower processing device 
 *  to relieve the fastest processing device and distribute the workload on all available processing devices. However, the algorithm has to ensure
 *  that the operation's response time does not significantly increase. Therefore, an operation may be executed on a slower processing device, 
 ...
 * 
 * Probability-based Outsourcing computes for each scheduling decision the estimated execution times of the avaialble algorithms. Then, each algorithm gets 
 * assigned a probability, depending on the estimated execution time. Faster algorithms (on faster processing devices) get a higher probability to be executed 
 * then slower algorithms. Depending on the probability, an algorithm is chosen randomly for execution.
 * 
	  * 
	  * 
*/
...
/*! \page page_configure_hype Configure HyPE
   ...
   * HyPE can be configured in four ways. First, modify the Static_Configuration of HyPE, which sets default values for all variables. 
	* Second, update the configuration at runtime. The class Runtime_Configuration provides methods to change all modifiable variables. 
	* Note that not all variables are modifiable during runtime.
	* Third, create a configuration file 'hype.conf', and add the variables with their corresponding values. 
	* Note, that the structure of the file for each line is variable_name=value and one line may at most contain one assignment.
	...
	* - create a configuration file 'hype.conf', and add the variables with their corresponding values
 	* 		- <b>help</b>                        produce help message
 	* 		- <b>length_of_trainingphase</b> set the number algorithms executions to complete training
 	* 		- <b>history_length</b> set the number of measurement pairs that are kept in the history (important for precision of approximation functions)
 	* 		- <b>recomputation_period</b> set the number of algorithm executions to trigger recomputation
 	* 		- <b>algorithm_maximal_idle_time</b> set maximal number of operation executions, where an algorithm was not executed; forces retraining of algorithm
 	* 		- <b>retraining_length</b> set the number of algorithm executions needed to complete a retraining phase (load adaption feature)
	* 		- <b>ready_queue_length</b> set the number of operators that are queued on a processing device, before scheduling decision stops scheduling 
	* 			new operators (The idea is to wait how the done scheduling decisions turn out and to adjust the scheduling accordingly)
 * 	* 		- <b>print_algorithm_statistics</b> set the mode for storing algorithm statistics, true means algorithms statistics are dumped to the output directory of HyPE, false disables the feature (default)
	...
	* 		- HYPE_READY_QUEUE_LENGTH set the number of operators that are queued on a processing device, before scheduling decision stops scheduling 
	* 			new operators (The idea is to wait how the done scheduling decisions turn out and to adjust the scheduling accordingly)
	* 		- HYPE_PRINT_ALGORITHM_STATISTICS set the mode for storing algorithm statistics, true means algorithms statistics are dumped to the output directory of HyPE, false disables the feature (default)
	* 
	* 
*/
...
/* \page page_structure_hype Structure of HyPE
	* 
	* HyPE is structured to be a DBMS independent library, meaning templates and functions can be used without modifcation. 
	* However, to acheive this goal, the user has to implement a mapping layer, between HyPE and the hybrid DBMS:
	* 
	* \htmlonly
 ...
 *    \section sec_statistical_method Statistical Methods
 * HyPE learns the correlation between features of the input data set and the resulting execution time 
 * of an algorithm on a specific processing device. To allow the user to fine tune the statistical method, 
 * HyPE provides a plug-in architecture, where the user can choose either from the set of implemented 
 * statistical methods, or alternatively, implement and integrate the preferred statistical method in HyPE.
 * 
 * To create a new statistical method, the user has to inherit from the abstract base class 
 * hype::core::StatisticalMethod and implement its pure virtual methods. 
 * Since HyPE uses a plug-in architecture based on factories, a static member function <b>create</b> should 
 * be defined, which returns a pointer to a new instance of your new statistical method (e.g., Least_Squares_Method_1D).
	   * @code
...
namespace hype{
	...
	* After the user added the class, he needs to extend the enumeration hype::StatisticalMethods::StatisticalMethod
	* in file global_definitions.hpp by a new member, which identifies the plug-in. Finally, the user has to register the 
	* plug-in in the class hype::core::PluginLoader in file pluginloader.cpp.
 * 
   *   \section sec_recomputation_heuristic Recomputation Heuristics
 * HyPE is capable of refining estimated execution times at run-time. Depending on the application, a run-time refinement is
 * beneficial or causes only additional overhead. To support a wide range of applications, HyPE allows to fine tune 
 * the runtime refinement on a per algorithm basis (e.g, for the same operation, we can have runtime adaption on the CPU, 
 * but not on the GPU.)
 * 
 * HyPE provides a plug-in architecture, where the user can choose either from the set of implemented 
 * recomputation heuristics, or alternatively, implement and integrate the preferred recomputation heuristic in HyPE.
 * 
 * To create a new recomputation heuristic, the user has to inherit from the abstract base class 
 * hype::core::RecomputationHeuristic and implement its pure virtual methods. 
 * Since HyPE uses a plug-in architecture based on factories, a static member function <b>create</b> should 
 * be defined, which returns a pointer to a new instance of your new recomputation heuristic (e.g., Oneshotcomputation).
 * 
	* @code
...
namespace hype{
	...
	* @endcode
	* After the user added the class, he needs to extend the enumeration hype::RecomputationHeuristics::RecomputationHeuristic
	* in file global_definitions.hpp by a new member, which identifies the plug-in. Finally, the user has to register the 
	* plug-in in the class hype::core::PluginLoader in file pluginloader.cpp.
	...
	*   \section sec_optimization_criterion Optimization Criterions
	*   
	* 	 To add a new optimization criterion, the user has to inherit from the abstract class hype::core::OptimizationCriterion and implement its pure vitual methods.
	* 	 Since HyPE uses a plug-in architecture based on factories, a static member function <b>create</b> should be defined, which returns a pointer to a new instance of 
	*   your new optimization criterion, which we will call "NewResponseTime".
...
namespace hype{
	namespace core{
	   ...
	   * @endcode
	* 
	* After the user added the class, he needs to extend the enumeration hype::OptimizationCriterions::OptimizationCriterion 
	* in file global_definitions.hpp by a new member, which identifies the plug-in. Finally, the user has to register the 
	* plug-in in the the class hype::core::PluginLoader in file pluginloader.cpp.
	* 
*/
...
/*! \page page_faq FAQ
   * 
	...
	*  HyPE is licenced under the <a href="http://www.gnu.org/licenses/lgpl-3.0.txt">GNU LESSER GENERAL PUBLIC LICENSE - Version 3</a>. 
	...
	*  For information about the project, technical questions and bug reports: please contact the development team via <a href="http://wwwiti.cs.uni-magdeburg.de/~bress/">Sebastian\htmlonly Bre&szlig;\endhtmlonly\latexonly Bre{\ss}\endlatexonly</a>.
	* 
*/
