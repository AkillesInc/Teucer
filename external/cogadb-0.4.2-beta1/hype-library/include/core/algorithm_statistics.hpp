 ...
 * 
 * 
 *  \brief     This class represents an information collection of an algorithm. Hence, it is the central place where all statistical information for one algorithm is stored.
 *  \details   The algorithm and the operation the algorithm belongs to are responsible to update the statistical information. Note, that a stemod::core::AlgorithmMeasurement
 *  				object is used for the execution time measurement on the user side and automatically triggers the insertion of a new MeasurementPair as soon as the measurement 
 * 				is stopped.
 *
 ...
 *  \version   0.1
 *  \date      2012
 *  \todo		This class is designed to be a member for algorithm that encapsulates statistical information. It is NOT meaningful to create
 * 				an AlgorithmStatistics object on the heap. Therefore, heap based objects should be forbidden by making the operators new and new[] private. Furtheremore, 
 * 				the AlgorithmStatistics object should not be copyable, so making the copy cosntructur and copy assignment operator private is necessary as well. 
 *  \copyright GNU LESSER GENERAL PUBLIC LICENSE - Version 3, http://www.gnu.org/licenses/lgpl-3.0.txt
 */
class AlgorithmStatistics{ // : private boost::noncopyable{ //make AlgorithmStatistics not copyable
	...
	AlgorithmStatistics();
		...
		unsigned int number_of_terminated_executions_of_this_algorithm_;
                ...
                std::string getReport(const std::string& operation_name, const std::string& algorithm_name, const std::string indent_str="") const;
...
public:
    ...
    AlgorithmStatisticsPtr getAlgorithmStatistics(const DeviceSpecification& dev_spec, const std::string& alg_name);
private:
    AlgorithmStatisticsManager();
    AlgorithmStatisticsManager(const AlgorithmStatisticsManager&);
    typedef std::map<AlgCostModelIdentifier,AlgorithmStatisticsPtr> AlgorithmStatisticsMap;
