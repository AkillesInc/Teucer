...
namespace hype{
	namespace core{
		 ...
		 * 
		 * 
		 *  \brief     This class represents an operation, which abstracts from algorithms and summarizes them under an abstract task, like sorting, join, selection, projection etc. 
		 *  \details   An operation gets assigned two things. First is a list of algorithms, which represent the set of algorithms available to process operation O, which is 	
		 *					basically the algorithm pool of the decision model. Second is an optimization_criterion, which is implemented by drived classes of  
		 *					stemod::core::OptimizationCriterion. The optimization criterion takes the computed estimated execution times of each algorithm and decides on the optimal 
		 *					algorithm, where the user specifies what is optimal by specifying an OptimizationCriterion.
		 *					
		 *					Note that the stemod::core::OptimizationCriterioc can be exchanged at run-time, because the Operation class uses the pointer to 
		 *             implementation technique (or pimpl-idiom). This makes it highly adaptable.
		...
		class Operation{
			...
			public:
				typedef std::map<std::string,boost::shared_ptr<Algorithm> > MapNameToAlgorithm;
				...
				/*! \brief returns true if an algrithm named "name_of_algorithm" is registered for this operation and false otherwise*/
			...
			private:
				//std::map<std::string,boost::shared_ptr<Algorithm> > map_algorithmname_to_pointer_;
				MapNameToAlgorithm map_algorithmname_to_pointer_;
				boost::shared_ptr<OptimizationCriterion_Internal> ptr_to_optimization_criterion_;
