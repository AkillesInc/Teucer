...
/*!
* \brief The namespace stemod contains all classes and functions that belongs to the framework.
*/
namespace hype{
/*!
* \brief The namespace stemod::core contains all classes and functions that belong to the core framework.
*/
	namespace core{
 ...
 * 
 * 
 *  \brief     This class represents an algorithm. It abstracts important features and encapsulates information of the algorithm. At the same time, it is highly adaptable.
 *  \details   An algorithm gets assigned three things. First is a statistical method, which dictates an approximation function and takes care of recomputing 
 *					the approximation function. Second is a recomputation heuristic, which decides, when to recompute an algorithm. Third is a statistic, which stores past    	
 * 				observed execution tiems ofthe algorithm together with an abstraction of the input data set in a measurement pair list. This list is implemented by the class
 *					stemod::core::AlgorithmStatistics. The algorithm has to update it's statistics every time, a new observation (an instance of class
 *					stemod::core::MeasurementPair) is added. Hence, the Algorithm class provides the basis for the decision component, which is done by an
 * 				optimization_criterion, which is implemented by drived classes of stemod::core::OptimizationCriterion.
 *
 ...
 */
		class Algorithm{
			...
			public:
			/*! \brief Initializes Algorithm according to Parameters {@link name_of_algorithm}, {@link name_of_statistical_method}, {@link name_of_recomputation_strategy}.
			 * \param name_of_algorithm name of the algorithm, it has to be unique!
			 * \param name_of_statistical_method the name of a supported statistical method
			 * \param name_of_recomputation_strategy name of the recomputation strategy
			 */
			...
			bool setStatisticalMethod(boost::shared_ptr<StatisticalMethod_Internal> ptr_statistical_method);

			bool setRecomputationHeuristic(boost::shared_ptr<RecomputationHeuristic_Internal> ptr_recomp_heuristic);
			...
			AlgorithmStatistics& getAlgorithmStatistics(){
				return *statistics_;
			}
...
//                        {
//                            std::stringstream ss;
//                            ss << "Algorithm: " << this->name_ << " (Operation: " << this->operation_.getName() << ")" << std::endl;
//                            ss << "StatisticalMethod: " << this->ptr_statistical_method_->
//                            
//                        }
			...
			boost::shared_ptr<RecomputationHeuristic_Internal> ptr_recomputation_heristic_;
