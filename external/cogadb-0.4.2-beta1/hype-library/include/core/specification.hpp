...
namespace hype{
	...
	/*!
	 *  \brief     An AlgorithmSpecification specifies all relevant information about an algorithm, 
	 * such as the algorithm's name or the name of the operation the algorithms belongs to.
	...
	class HYPE_EXPORT AlgorithmSpecification{
		public:
		/*! \brief constructs an AlgorithmSpecification object by assigning necessary informations to all fields of the object 	
	  ...
	  /*!
		*  \brief returns the name of the statistical method that is used for the algorithm
		*/		
		const std::string getStatisticalMethodName() const throw();
	  /*!
		*  \brief returns the name of the recomputation heuristic that is used for the algorithm
		*/			
		const std::string getRecomputationHeuristicName() const throw();
	  /*!
		*  \brief returns the name of the optimization criterion of the operation the algorithm belongs to
		*/		
	  ...
	  /*!  \brief the optimization criterion of the operation the algorithm belongs to*/				
	...
	/*!
	 *  \brief    A OperatorSpecification defines the operator that the user wants to execute.
	 *  \details  Hence, it contains the name of the operation and the features of the input data set as well as the
	 * 			  two memory ids, where the first identifies where the input data is stored, 
	 * 			  and the second specifies where the input data is stored. Note that HyPE needs 
	 * 			  this information to estimate the overhead of data transfers in case the data needs 
	...
	public:
		/*! \brief constructs an OperatorSpecification object by assigning necessary informations to all fields of the object 	
	...
	/*!
	 *  \brief    A DeviceSpecification defines a processing device that is available for performing computations. 
	 *  \details  It consists of a ProcessingDeviceID, which has to be unique, a processing device type (e.g., CPU or GPU) 
	 * 			  and a memory id, which specifies the memory that the processing devices uses. By convention, the host's CPU has
	 * 			  the processing device id of 0, is a processing device from type CPU and the CPU's main memory has memory id 0.
	 *  \author    Sebastian Breß
	 *  \version   0.2
	 *  \date      2013
	 *  \copyright GNU LESSER GENERAL PUBLIC LICENSE - Version 3, http://www.gnu.org/licenses/lgpl-3.0.txt
	...
	struct HYPE_EXPORT DeviceSpecification{
		public:
		/*! \brief constructs an DeviceSpecification object by assigning necessary informations to all fields of the object 	
		 ...
		 */	
		DeviceSpecification(ProcessingDeviceID pd,
	  ...
	  /*!
		*  \brief implicit conversion to an object of type ProcessingDeviceID
		*/			
		operator ProcessingDeviceID(); 
	  /*!
		*  \brief implicit conversion to an object of type ProcessingDeviceType
		*/				
		operator ProcessingDeviceType();
	  /*!
		*  \brief implicit conversion to an object of type ProcessingDeviceMemoryID
		*/				
	  ...
	  /*!
	   ...
	   /*! \brief the processing device's ProcessingDeviceID*/			
		...
		/*! \brief function pointer that returns the available memory of the processing device, ignored if NULL */	
	...
	/*!
	 *  \brief     A DeviceConstraint restricts the type of processing device, which HyPE may choose to process an operator.
	 *  \details   This is especially important if an algorithms does not support a certain data type on a certain processing device
	 *  (e.g., no filter operations on an array of strings on the GPU). On default construction, no constraint is defined. 
	 *  \author    Sebastian Breß
	 *  \version   0.2
	 *  \date      2013
	 *  \copyright GNU LESSER GENERAL PUBLIC LICENSE - Version 3, http://www.gnu.org/licenses/lgpl-3.0.txt
	...
	struct HYPE_EXPORT DeviceConstraint{
		public:
		/*! \brief constructs an DeviceConstraint object by assigning necessary informations to all fields of the object 	
	  ...
	  /*!
		*  \brief implicit conversion to an object of type DeviceTypeConstraint
	  ...
	  /*!
		...
		*/				
		...
		/*! \brief the memory id of the memory, where the data should be processed*/		
		ProcessingDeviceMemoryID pd_mem_constr_; //restrict to devices that use a certain memory
