...
namespace hype{
	...
	namespace core{
		 ...
		 * 
		 * 
		 *  \brief     This class represents a scheduling decision for an operation w.r.t. a user specified set of features of the input data.
		 *  \details   A user has to execute the algortihm suggested by the SchedulingDecision, or the library will not work. The 
		 *		 			user can determine the algorithm to excute by calling getNameofChoosenAlgorithm(). The user can then measure the execution 
			 *					time of the algorithm by using an AlgorithmMeasurement object.
		 *  \author    Sebastian Breß
		 *  \version   0.1
		 *  \date      2012
		 *  \todo change interface, so it stores a pointer to the choosen algorithm, and update algorithm timestamps in the constructor!
		...
		class HYPE_EXPORT SchedulingDecision{
			public:
				/*! \brief constructs a SchedulingDecision object by assigning neccessary informations to all fields of the object 	
				 * \param alg_ref Reference to Algorithm that was choosen
				 * \param estimated_time_for_algorithm estimated execution time for algorithm
				 * \param feature_values features of the input data set that were previously specified by the user
				 */	
				SchedulingDecision(Algorithm& alg_ref, const EstimatedTime& estimated_time_for_algorithm, const Tuple& feature_values, MemoryChunkPtr mem_chunk=MemoryChunkPtr());
				...
				/*! \brief returns the feature values that were the basis for this decision
...
//                                SchedulingDecision(const hype::core::SchedulingDecision&);
//                                
//                                SchedulingDecision& operator=(const hype::core::SchedulingDecision&);
