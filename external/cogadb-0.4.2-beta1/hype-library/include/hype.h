...
extern "C" {
    ...
    typedef struct C_AlgorithmSpecification {
        void* ptr;
    } C_AlgorithmSpecification;

    /*! \brief constructs an C_AlgorithmSpecification object by assigning necessary informations to all fields of the object 	
     * \param alg_name name of the algorithm
     * \param op_name name of the operation the algorithms belongs to
     * \param stat_meth the statistical method used for learning the algorithms behavior (optional)
     * \param recomp_heur the recomputation heuristic used for adapting the algorithms approximation function (optional) 
     * \param opt_crit the optimization criterion of the operation the algorithm belongs to (optional)
     */
    ...
    typedef struct C_DeviceSpecification {
        void* ptr;
    } C_DeviceSpecification;

    /*! \brief constructs an C_DeviceSpecification object by assigning necessary informations to all fields of the object 	
    ...
    C_DeviceSpecification* hype_createDeviceSpecification(ProcessingDeviceID pd,
	  ...
	  /*!
            ...
            */	
    ProcessingDeviceID hype_DeviceSpecification_getProcessingDeviceID(C_DeviceSpecification* dev_spec);
	  /*!
            *  \brief returns the processing device's device type
            */			
    ProcessingDeviceType hype_DeviceSpecification_getDeviceType(C_DeviceSpecification* dev_spec);
	  /*!
            *  \brief returns the processing device's memory id
            */			
    ProcessingDeviceMemoryID hype_DeviceSpecification_getMemoryID(C_DeviceSpecification* dev_spec);
	  /*!
    ...
    typedef struct C_OperatorSpecification {
        void* ptr;
    } C_OperatorSpecification;

    /*! \brief constructs an C_OperatorSpecification object by assigning necessary informations to all fields of the object 	
    ...
    typedef struct C_DeviceConstraint {
        void* ptr;
    } C_DeviceConstraint;

    /*! \brief constructs an C_DeviceConstraint object by assigning necessary informations to all fields of the object 	
     * \param dev_constr a device type constraint (e.g., CPU_ONLY or ANY_DEVICE for now restriction)
     * \param pd_mem_constr memory id, where the data should be stored when processed (experimental)
     */
    C_DeviceConstraint* hype_createDeviceConstraint(DeviceTypeConstraint dev_constr,
     ...
     * \return returns true on success and false otherwise	
     */
    int hype_setStatisticalMethod(const char* name_of_algorithm, C_DeviceSpecification* dev_spec,
     ...
     * \return returns true on success and false otherwise	
     */
    int hype_setRecomputationHeuristic(const char* name_of_algorithm, C_DeviceSpecification* dev_spec,
     ...
     */
    double hype_getEstimatedExecutionTime(const C_OperatorSpecification* op_spec, const char* alg_name, C_DeviceSpecification* dev_spec);
