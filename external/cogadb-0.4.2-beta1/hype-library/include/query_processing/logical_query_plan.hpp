    ...
    namespace queryprocessing {
		...
		struct print_NodePtr_functor {
			...
			void operator()(NodePtr ptr) const {
                for (unsigned int i = 0; i < ptr->getLevel(); i++)
					(*out) << "\t";
				//std::cout << "Operation: " << ptr->getOperationName();
                                (*out) << ptr->toString(true);
                if (ptr->getDeviceConstraint().getDeviceTypeConstraint() != hype::ANY_DEVICE) {
					(*out) << "\t[" << hype::util::getName(ptr->getDeviceConstraint().getDeviceTypeConstraint()) << "]";
				}
				(*out) << std::endl;
			}
                    std::ostream* out;
		...
		struct init_output_stream_NodePtr_functor {
                    ...
                    init_output_stream_NodePtr_functor(std::ostream* output_stream) : out(output_stream){
		...
		struct print_graph_NodePtr_functor {
			...
			void operator()(NodePtr ptr) const
			{
				...
				if (ptr->getLeft())
					stream << "\tnode_" << ptr << " -> node_" << ptr->getLeft() << ":n [label=\" L\"];" << std::endl;
				if (ptr->getRight())
					stream << "\tnode_" << ptr << " -> node_" << ptr->getRight() << ":n [label=\" R\"];" << std::endl;
			}
		};
		...
		struct Construct_Physical_Query_Plan_functor {
            ...
            Construct_Physical_Query_Plan_functor(std::ostream& output_stream) : root_(), map_log_op_to_phy_op_(), out(output_stream) {
            ...
            void operator()(TypedNodePtr ptr, bool chopped = false) {
                if (!hype::core::quiet && hype::core::verbose && hype::core::debug) std::cout << typeid (ptr).name();

                if (ptr) {
                    //std::cout << "Allocate Processor for Node: " << ptr->getOperationName() << std::endl;

                    TypedOperatorPtr left_op;
                    TypedOperatorPtr right_op;

                    TypedNodePtr left = boost::static_pointer_cast<typename TypedNodePtr::element_type > (ptr->getLeft());
                    TypedNodePtr right = boost::static_pointer_cast<typename TypedNodePtr::element_type > (ptr->getRight());
                    if (left != NULL) {
                        typename Map::iterator it = map_log_op_to_phy_op_.find(left);
                        if (it != map_log_op_to_phy_op_.end()) {
                            left_op = it->second;
                        }else{
                            HYPE_FATAL_ERROR("Operator " << left->getOperationName() << " not found!",std::cout);
                        }

                        if (chopped && left_op == 0) {
                            std::cout << "Child operation of " << ptr.get()->getOperationName() << " not ready: " << left.get()->getOperationName() << std::endl;
                            // exit(-1);
                        }
                    }
                    if (right != NULL) {
                        ...
                        if (it != map_log_op_to_phy_op_.end()){
                            ...
                            HYPE_FATAL_ERROR("Operator " << right->getOperationName() << " not found!",std::cout);
                        }
                    }

                    TypedOperatorPtr phy_op_ptr = ptr->getOptimalOperator(left_op, right_op, ptr->getDeviceConstraint());
...
//                    assert(map_log_op_to_phy_op_.find(ptr)==map_log_op_to_phy_op_.end());
                    ...
                    root_ = phy_op_ptr; //update root (only important thing that the correct root node is stored in here at the end, when get function is called)
                    if (chopped) {
                    phy_op_ptr->run();
                    }
                }
        ...
        inline bool recursiveAssign(hype::query_optimization::QEP_Node* qep_node, NodePtr node);
	...
	template <typename Type>
        class LogicalQueryPlan {
			public:
				...
				typedef typename OperatorMapper_Helper_Template<Type>::TypedOperatorPtr TypedOperatorPtr;
				typedef typename OperatorMapper_Helper_Template<Type>::TypedNodePtr TypedNodePtr;
				typedef typename OperatorMapper_Helper_Template<Type>::PhysicalQueryPlanPtr PhysicalQueryPlanPtr;                          

				LogicalQueryPlan(TypedNodePtr root, std::ostream& output_stream = std::cout) : root_(root), out(&output_stream) {
					root_->setLevel(0);
					...
					traverse(init_output_stream_NodePtr_functor(out));
                                ...
                                ~LogicalQueryPlan(){
                                        this->reverse_traverse(Cleanup_NodePtr_functor());
                                }
                                

				const PhysicalQueryPlanPtr convertToPhysicalQueryPlan() {
					Construct_Physical_Query_Plan_functor<Type> functor = reverse_traverse(Construct_Physical_Query_Plan_functor<Type>(*out));
					TypedOperatorPtr root = functor.getRootOperator();
					PhysicalQueryPlanPtr ptr(new PhysicalQueryPlan<Type>(root, *out));
					return ptr;
				}
                                
//                                static const TypedOperatorPtr converQEPNodeToPhysicalOperator(hype::query_optimization::QEP_Node* qep_node){
//                                    
//                                }
//                                
//                                static bool recursiveAssign(hype::query_optimization::QEP_Node* qep_node, TypedNodePtr node){
//            
...
//                                            hype::query_optimization::QEP_Node* qep_left = node->getLeft()->toQEP_Node();
//                                            if(qep_left){
...
//                                            hype::query_optimization::QEP_Node* qep_right = node->getRight()->toQEP_Node();
//                                            if(qep_right){
...
//                                }
                                ...
                                const PhysicalQueryPlanPtr convertQEPToPhysicalQueryPlan(hype::query_optimization::QEPPtr qep){

                                    TypedOperatorPtr root; // = converQEPNodeToPhysicalOperator(ptr->getRoot());
                                ...
                                TypedOperatorPtr recursiveConvert(hype::query_optimization::QEP_Node* qep_node, TypedNodePtr logical_node){
                                    assert(qep_node!=NULL);
                                    assert(logical_node!=NULL);
                                    assert(qep_node->number_childs<=2);
                                    assert(qep_node->sched_dec_ptr!=NULL);
                                    
                                    //handle special case, where the physical plan contains a node that was not in the logical plan
                                    //currently, this is only for inserted copy operations
                                    if(qep_node->op_spec.getOperatorName()!=logical_node->getOperationName()){
                                        //is copy operation?
                                        if(!hype::core::quiet && hype::core::verbose && hype::core::debug)
                                            std::cout << "QEP Operationname: " << qep_node->op_spec.getOperatorName() << "\tLogical Operator Name: " << logical_node->getOperationName() << std::endl;
                                        assert(qep_node->op_spec.getOperatorName()=="COPY_CP_CPU" || qep_node->op_spec.getOperatorName()=="COPY_CPU_CP");
                                    ...
                                    if(qep_node->number_childs==0){
                                        result_physical_operator = logical_node->createPhysicalOperator(TypedOperatorPtr(), TypedOperatorPtr(), *qep_node->sched_dec_ptr);
                                        result_physical_operator->setLogicalOperator(logical_node);
                                    //unary operator?
                                    }else if(qep_node->number_childs==1){

                                        TypedNodePtr left_child_logical_node = boost::dynamic_pointer_cast<typename TypedNodePtr::element_type > (logical_node->getLeft());
                                        assert(qep_node->childs[0]!=NULL);
                                        TypedOperatorPtr child = recursiveConvert(qep_node->childs[0], left_child_logical_node);
                                        
                                        result_physical_operator = logical_node->createPhysicalOperator(child, TypedOperatorPtr(), *qep_node->sched_dec_ptr);
                                        result_physical_operator->setLogicalOperator(logical_node);
        
                                    //binary operator?
                                    }else if(qep_node->number_childs==2){
                                        TypedNodePtr left_child_logical_node = boost::dynamic_pointer_cast<typename TypedNodePtr::element_type > (logical_node->getLeft());
                                        TypedNodePtr right_child_logical_node = boost::dynamic_pointer_cast<typename TypedNodePtr::element_type > (logical_node->getRight());
                                                                              
                                        assert(qep_node->childs[0]!=NULL);
                                        assert(qep_node->childs[1]!=NULL);
                                        TypedOperatorPtr left_child = recursiveConvert(qep_node->childs[0], left_child_logical_node);
                                        TypedOperatorPtr right_child = recursiveConvert(qep_node->childs[1], right_child_logical_node);
                                        
                                        
                                        result_physical_operator = logical_node->createPhysicalOperator(left_child, right_child, *qep_node->sched_dec_ptr);
                                        result_physical_operator->setLogicalOperator(logical_node);
                                        
                                    }
				    if(result_physical_operator){
					result_physical_operator->setOutputStream(*out); //logical_node->getOutputStream());
                                ...
                                hype::query_optimization::QEPPtr convertToQEP(){
                                     ...
                                     return hype::query_optimization::QEPPtr(new hype::query_optimization::QEP(qep_root));
                                }
                                   

            const PhysicalQueryPlanPtr runChoppedPlan() {
                double begin = double(hype::core::getTimestamp());
                //Construct_Physical_Query_Plan_functor<Type> functor = query_chopping_traversal(Construct_Physical_Query_Plan_functor<Type>());
                
                Node::chopOffAndExecute(this->getRoot(),out);
                this->getRoot()->waitForSelf();
                
                OperatorPtr root_node = this->getRoot()->getPhysicalOperator();
                TypedOperatorPtr root = boost::dynamic_pointer_cast<TypedOperator<Type> >(root_node);
                PhysicalQueryPlanPtr ptr(new PhysicalQueryPlan<Type>(root, *out));
                ...
                while (!queue.empty()) {
                    ...
                    if (currentNode.get() != NULL) {
                        TypedNodePtr left = boost::static_pointer_cast<typename TypedNodePtr::element_type > (currentNode->getLeft());
                        TypedNodePtr right = boost::static_pointer_cast<typename TypedNodePtr::element_type > (currentNode->getRight());
            ...
            static void chop(UnaryFunction *f, TypedNodePtr nodePtr) {
                TypedNodePtr left = boost::static_pointer_cast<typename TypedNodePtr::element_type > (nodePtr->getLeft());
                TypedNodePtr right = boost::static_pointer_cast<typename TypedNodePtr::element_type > (nodePtr->getRight());
                if (left != NULL && right != NULL) {
                    boost::thread_group g;
                    g.add_thread(new boost::thread(boost::bind(&LogicalQueryPlan<Type>::chop<UnaryFunction>, f, left)));
                    g.add_thread(new boost::thread(boost::bind(&LogicalQueryPlan<Type>::chop<UnaryFunction>, f, right)));
                    g.join_all();
                } else if (left != NULL) {
                    chop(f, left);
                }
            ...
            UnaryFunction traverse_inorder(UnaryFunction f, TypedNodePtr node) {
                ...
                traverse_inorder(f, boost::dynamic_pointer_cast<typename TypedNodePtr::element_type > (node->getLeft()));
                                    f(node);
                traverse_inorder(f, boost::dynamic_pointer_cast<typename TypedNodePtr::element_type > (node->getRight()));
                return f;
           }
				...
				UnaryFunction traverse_preorder(UnaryFunction f) {
                    ...
                    if (curr.get() != NULL) {
							...
							nodeStack.push(curr);
                        curr = boost::static_pointer_cast<typename TypedNodePtr::element_type > (curr->getLeft());
							continue;
						}
						if (!nodeStack.size()) {
							return f;
						...
						curr = nodeStack.top();
						nodeStack.pop();
                    curr = boost::static_pointer_cast<typename TypedNodePtr::element_type > (curr->getRight());
					}
					return f;
				}
				...
				void print() {
					...
					traverse_preorder(print_NodePtr_functor(out));
