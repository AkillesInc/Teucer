...
namespace hype {
    namespace queryprocessing {
        ...
        public:
            ...
            PhysicalQueryPlan(TypedOperatorPtr root, std::ostream& output_stream) : root_(root), out(&output_stream) {
                timeNeeded = 0;
                timeEstimated = 0;
            }
            ...
            void print() {
                ...
                if (timeNeeded > 0) {
                    ...
                    root_->print(*out);
                    (*out) << "Estimated Time (total): " << timeEstimated / 1000000 << "ms (" << timeEstimated << "ns)" << std::endl;
                }
            }
            ...
            void printResults(bool show_timings=false, bool show_cardinalities=false, bool show_estimation_error=false) {
                (*out) << std::string(80,'=') << std::endl; 
                (*out) << "Query Execution Plan:" << std::endl;
                root_->printResult(*out, 0, show_timings, show_cardinalities, show_estimation_error);
                if(show_estimation_error){
                (*out) << "Estimated Query Response Time: " << timeEstimated / 1000000 << "ms (" << timeEstimated << "ns)" << std::endl;
                (*out) << "Measured Query Response Time: " << (timeNeeded) / 1000000 << "ms (" << timeNeeded << "ns)" << std::endl;
                (*out) << "Estimation Error: " << 1 - (double(timeEstimated) / timeNeeded) << std::endl;
                }
                (*out) << std::string(80,'=') << std::endl; 
            }
