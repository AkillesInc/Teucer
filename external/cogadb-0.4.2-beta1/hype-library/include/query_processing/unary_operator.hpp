...
namespace hype {
    namespace queryprocessing {
        ...
        template <typename OperatorInputType, typename OperatorOutputType> //, typename TLogicalOperation>
        class UnaryOperator : public TypedOperator<OperatorOutputType> { //uses 1D Learning Method
        ...
        public:
            ...
            virtual bool run() {
                ...
                if (!hype::core::Runtime_Configuration::instance().isQueryChoppingEnabled() && child_) {
                    child_->run();
                }
...
//                }
                        out << std::endl;
                if (child_) child_->printResult(out, tree_level + 1, show_timings, show_cardinalities, show_estimation_error);
            }
...
//	    		std::cout << "start_this_operator_timestamp: " << start_this_operator_timestamp << std::endl;
//	    		std::cout << "end_child_operator_timestamp: " << end_child_operator_timestamp << std::endl;
...
//	    			HYPE_FATAL_ERROR("Current operator startet before child operator finished!", std::cout);
	    		...
	    		if(start_this_operator_timestamp>end_child_operator_timestamp)
	    			ret+=double(start_this_operator_timestamp-end_child_operator_timestamp);
	    	}
	    	return ret;
	    }  
            ...
            virtual std::list<TypedOperator<OperatorOutputType>& > getOperatorQueue(){
                    std::list<TypedOperator<OperatorOutputType>& > list = child_->getOperatorQueue();
