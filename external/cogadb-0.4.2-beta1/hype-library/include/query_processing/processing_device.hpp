	...
	namespace queryprocessing
	{
		...
		class ProcessingDevice
		{
			...
			public:
				...
				void run(); //thread
				bool isIdle();
				/*! \todo Implement function! sum all Estimated Execution times of operators in operator list! OR better updated on add/remove from queue! 
				 * Finally, add the rest time of current oeprator, which is provided by getEstimatedTimeUntilOperatorCompletion()*/
			...
			private:
				...
				/*! \todo Implement function! store start timestamp of operator currently running and the Estiamted Execution Time for him -> compute rest time using current timestamp*/	
				...
				double total_measured_execution_time_for_all_executed_operators_; //used to determine how processing devices were utilized
		...
		/*! */
		ProcessingDevicePtr getProcessingDevice(const hype::core::DeviceSpecification& dev); //can later be queried by using algorithmptr->getDeviceSpecification();
