...
//                threads.join_all();
                ...
                //save one thread creating for one binary operator without loosing parallelism
                if (left_child_) {
                    //left_child_->run();
                    threads.add_thread(new boost::thread(boost::bind(&TypedOperator<OperatorInputTypeLeftChild>::run, left_child_.get())));
                }
                ...
                //this->setTimeEstimated(double(this->getEstimatedExecutionTime().getTimeinNanoseconds()));
                bool retVal = (*this)();
            ...
            virtual void print(std::ostream& out, unsigned int tree_level) const {
                ...
                out << this->getAlgorithmName() << " ET: " << this->getEstimatedExecutionTime().getTimeinNanoseconds()
...
//                    out << "\t";
//                }
            virtual void printResult(std::ostream& out, unsigned int tree_level, bool show_timings, bool show_cardinalities, bool show_estimation_error) {
                for (unsigned int i = 0; i < tree_level; ++i) {
                    out << "\t";
                }
                if(this->sched_dec_.getDeviceSpecification().getDeviceType()==hype::CPU){
                    out << "\033[44m"; //highlight following text in blue
                }else if(this->sched_dec_.getDeviceSpecification().getDeviceType()==hype::GPU){
...
//               out << this->logical_operator_->toString(true); //getAlgorithmName();
               out << this->getAlgorithmName();
               out << " [" << util::getName(this->sched_dec_.getDeviceSpecification().getDeviceType()) 
                         << (int) this->sched_dec_.getDeviceSpecification().getProcessingDeviceID()
                         << "]";
               if(this->has_aborted_){
                   out << " [Aborted]";
               }
               ...
               if(show_timings){
                   ...
                   out << this->getTimeNeeded()/(1000*1000) << "ms]"; // << st::endl;
               }
               out << "\t[SD: " << this->getSchedulingDelay()/(1000*1000) << "ms]";
               if(show_estimation_error){
                   out << "\t[Estimated Time: " << this->getEstimatedExecutionTime().getTimeinNanoseconds()/(1000*1000) << "ms]"; // << st::endl;
                   out << "\t[Estimation Error: " <<  double(this->getTimeNeeded()-this->getEstimatedExecutionTime().getTimeinNanoseconds()) / this->getTimeNeeded() << "ms]";
               }
               if(show_cardinalities){
                   out << ";\t[Result Size: " << this->getResultSize() << "rows]";
                   out << ";\t[Estimated Result Size: " << this->getFeatureValues()[0] << "rows]";
               }
               
               
                
                assert(!this->getFeatureValues().empty());
//                out << this->getAlgorithmName() << " ET: " << this->getTimeEstimated()
//                        << "\tMT: " << this->getTimeNeeded()
//                        << "\tEE: " << 1 - (this->getTimeEstimated() / this->getTimeNeeded())
//                        << "\tEC: " << this->getFeatureValues()[0];
//                if (this->getResultSize() != -1) {
//                    out << "\tRR: " << this->getResultSize();
//                }
                out << std::endl;
                if (left_child_) left_child_->printResult(out, tree_level + 1, show_timings, show_cardinalities, show_estimation_error);
                if (right_child_) right_child_->printResult(out, tree_level + 1, show_timings, show_cardinalities, show_estimation_error);
            }
...
//                } else {
	    	...
	    	if(left_child_){
	    		...
	    		uint64_t end_child_operator_timestamp = left_child_->getEndTimestamp();
	    		//consider precision jitter of clock in assertion
//	    		if(start_this_operator_timestamp+1000<end_child_operator_timestamp)
//	    			HYPE_FATAL_ERROR("Current operator startet before child operator finished!", std::cout);
	    		//assume no time has passed in jitter interval, otherwise add the scheduling delay
	    		if(start_this_operator_timestamp>end_child_operator_timestamp)
	    			left_child_delay=double(start_this_operator_timestamp-end_child_operator_timestamp);
	    	}
	    	if(right_child_){
	    		uint64_t start_this_operator_timestamp = this->getBeginTimestamp();
	    		uint64_t end_child_operator_timestamp = right_child_->getEndTimestamp();
	    		//consider precision jitter of clock in assertion
//	    		if(start_this_operator_timestamp+1000<end_child_operator_timestamp)
//	    			HYPE_FATAL_ERROR("Current operator startet before child operator finished!", std::cout);
	    		//assume no time has passed in jitter interval, otherwise add the scheduling delay
	    		if(start_this_operator_timestamp>end_child_operator_timestamp)
	    			right_child_delay=double(start_this_operator_timestamp-end_child_operator_timestamp);
	    	}
                
                
                if (left_child_ && right_child_) {
                    if (left_child_->getRecursiveExecutionTimeInNanoseconds()+left_child_delay > right_child_->getRecursiveExecutionTimeInNanoseconds()+right_child_delay) {
                        executiontime += left_child_->getRecursiveExecutionTimeInNanoseconds()+left_child_delay;
                    } else {
                        executiontime += right_child_->getRecursiveExecutionTimeInNanoseconds()+right_child_delay;
                    }
                } else {
                    if (left_child_) {
                        executiontime += left_child_->getRecursiveExecutionTimeInNanoseconds()+left_child_delay;
            ...
            virtual double getRecursiveEstimationTimeInNanoseconds() {
                ...
                if (!hype::core::Runtime_Configuration::instance().isQueryChoppingEnabled()) {
                    if (left_child_) {
                        estimationTime += left_child_->getRecursiveEstimationTimeInNanoseconds();
                    }
                    if (right_child_) {
                        estimationTime += right_child_->getRecursiveEstimationTimeInNanoseconds();
                    }
                } else {
                    if (left_child_ && right_child_) {
                        if (left_child_->getRecursiveEstimationTimeInNanoseconds() > right_child_->getRecursiveEstimationTimeInNanoseconds()) {
                            estimationTime += left_child_->getRecursiveEstimationTimeInNanoseconds();
                        } else {
                            estimationTime += right_child_->getRecursiveEstimationTimeInNanoseconds();
                        }
	    	...
	    	double left_child_total_exec_time=0;
	    	double right_child_scheduling_delay=0;
	    	double right_child_total_exec_time=0;
	    	if(left_child_){
	    		left_child_scheduling_delay=left_child_->getTotalSchedulingDelay();
	    		//left_child_scheduling_delay+=getSchedulingDelay();
                        left_child_total_exec_time=left_child_->getRecursiveExecutionTimeInNanoseconds();
	    	}
	    	if(right_child_){
	    		right_child_scheduling_delay=right_child_->getTotalSchedulingDelay();
                        right_child_total_exec_time=left_child_->getRecursiveExecutionTimeInNanoseconds();
	    		//right_child_scheduling_delay+=getSchedulingDelay();
	    	}
	    	//return sum of max latencies/delays until now, which is exactly the penalty on the response time of the query
	    	ret=std::max(left_child_scheduling_delay,right_child_scheduling_delay);
	    	...
	    	if(left_child_){
	    		uint64_t start_this_operator_timestamp = this->getBeginTimestamp();
	    		uint64_t end_child_operator_timestamp = left_child_->getEndTimestamp();
//                        if(start_this_operator_timestamp<=0){ HYPE_FATAL_ERROR("Invalid begin timestamp! for " << this->getAlgorithmName(), std::cout); }
//                        if(end_child_operator_timestamp<=0){ HYPE_FATAL_ERROR("Invalid end timestamp! for " << this->getAlgorithmName(), std::cout); }
	    		//consider precision jitter of clock in assertion
//	    		if(start_this_operator_timestamp+1000<end_child_operator_timestamp)
//	    			HYPE_FATAL_ERROR("Current operator startet before child operator finished!", std::cout);
	    		//assume no time has passed in jitter interval, otherwise add the scheduling delay
	    		if(start_this_operator_timestamp>end_child_operator_timestamp)
	    			left_child_scheduling_delay+=double(start_this_operator_timestamp-end_child_operator_timestamp);
	    	}
	    	if(right_child_){
	    		uint64_t start_this_operator_timestamp = this->getBeginTimestamp();
	    		uint64_t end_child_operator_timestamp = right_child_->getEndTimestamp();
...
//                        if(end_child_operator_timestamp<=0){ HYPE_FATAL_ERROR("Invalid end timestamp! for " << this->getAlgorithmName(), std::cout); }
	    		//assume no time has passed in jitter interval, otherwise add the scheduling delay
	    		if(start_this_operator_timestamp>end_child_operator_timestamp)
	    			right_child_scheduling_delay+=double(start_this_operator_timestamp-end_child_operator_timestamp);
	    	}
                //the delay starts after the second child finished!
	    	ret=std::min(left_child_scheduling_delay,right_child_scheduling_delay);
	    	return ret;
            }
