...
class Generic_Operation_Benchmark{
	...
	typedef typename OperatorMapper_Helper_Template<Type>::TypedNodePtr TypedNodePtr;
        ...
        typedef std::vector<hype::queryprocessing::ProcessingDevicePtr> ProcessingDevices;

	Generic_Operation_Benchmark(const std::string& operation_name,
                                                          //alg specs all have to have the same operation!
							  const AlgorithmSpecifications& alg_specs_, const DeviceSpecifications& dev_specs_) : 	
...
//            assert(hype::core::Runtime_Configuration::instance().getMaximalReadyQueueLength()>=alg_specs_.size()*hype::core::Runtime_Configuration::instance().getTrainingLength());
            ...
            for(unsigned int i=0;i<dev_specs_.size();++i){
                ProcessingDevicePtr proc_dev_ptr = core::Scheduler::instance().getProcessingDevices().getProcessingDevice(dev_specs_[i].getProcessingDeviceID());
                assert(proc_dev_ptr!=NULL);
                //if the following assertion is true, we can use the device id for fast lookup!
            ...
            if(!alg_specs_.empty()){
                for(unsigned int i=0;i<alg_specs_.size();++i){
                    //check whether the algorithm specification belongs to the specified operation 
        ...
        for(unsigned int i=0;i<dev_specs_.size();++i){
             for(unsigned int j=0;j<alg_specs_.size();++j){
                ...
                if(!hype::Scheduler::instance().setStatisticalMethod(alg_specs_[j].getAlgorithmName(),dev_specs_[i],stemod_statistical_method_)){
                        std::cout << "Error setting StatisticalMethod " << stemod_statistical_method_ << " for algorithm: " << alg_specs_[j].getAlgorithmName() << std::endl; return -1;
                }
                ...
                if(!hype::Scheduler::instance().setRecomputationHeuristic(alg_specs_[j].getAlgorithmName(),dev_specs_[i],stemod_recomputation_heuristic_)){
                        std::cout << "Error setting RecomputationHeuristic " << stemod_statistical_method_ << " for algorithm: " << alg_specs_[j].getAlgorithmName() << std::endl; return -1;
                }
             }    
        }  
	...
	for(unsigned int i=0;i<NUMBER_OF_SORT_OPERATIONS_IN_WORKLOAD_;i++){
		...
		if(sched_config_==HYBRID){ //CPU_ONLY,GPU_ONLY,HYBRID)
			//cout << "scheduling operator " << i << std::endl;
                        const unsigned int number_of_algorithms_per_device=alg_specs_.size();
                        const unsigned int number_of_devices=dev_specs_.size();
                        const unsigned int total_number_of_algorithms=number_of_algorithms_per_device*number_of_devices;
			const unsigned int number_of_training_operations = (hype::core::Runtime_Configuration::instance().getTrainingLength()*total_number_of_algorithms)+1; //*number of algortihms per operation (2)
				...
				if(!hype::core::quiet)
					...
					//wait until training operations finished
                                        ...
                                        while(continue_loop){
                                            ...
                                            for(unsigned int i=0;i<proc_devs_.size();++i){
                                                assert(proc_devs_[i]!=NULL);
                                                //if at least one processing device is not finished, wait and continue loop!
...
//					}	
			...
			TypedOperatorPtr phy_scan; //TypedOperatorPtr phy_scan = scan->getOptimalOperator(TypedOperatorPtr(),TypedOperatorPtr());
                        if(scan) 
                           phy_scan = scan->getOptimalOperator(TypedOperatorPtr(),TypedOperatorPtr());
			TypedOperatorPtr phy_op = current_operator->getOptimalOperator(phy_scan,TypedOperatorPtr());

                        ProcessingDeviceID dev_id = phy_op->getDeviceSpecification().getProcessingDeviceID();
...
//	}
	...
	//std::cout << "stat: cpu " << !cpu->isIdle() << " gpu " << !gpu->isIdle() << std::endl; 
        //if(!core::quiet)
        {
            ...
            for(unsigned int i=0;i<proc_devs_.size();++i){
                assert(proc_devs_[i]!=NULL);
                ProcessingDeviceID proc_dev_id = proc_devs_[i]->getProcessingDeviceID();
                queryprocessing::VirtualProcessingDevicePtr virt_dev_ptr = core::Scheduler::instance().getProcessingDevices().getVirtualProcessingDevice(proc_dev_id);
                queryprocessing::ProcessingDevicePtr phy_dev_ptr = core::Scheduler::instance().getProcessingDevices().getProcessingDevice(proc_dev_id);
                assert(virt_dev_ptr!=NULL);
                assert(phy_dev_ptr!=NULL);
                virt_dev_ptr->print();
                std::cout << "\t" << "Total Processing Time: " << phy_dev_ptr->getTotalProcessingTime() << "ns (" << phy_dev_ptr->getTotalProcessingTime()/(1000*1000*1000) << "s)" << std::endl;
            }        
        }
...
//	gpu->stop();
	...
	double relative_error_gpu_algorithm = 0;
        ...
        std::vector<double> relative_errors(alg_specs_.size()*dev_specs_.size());
	...
	if(sched_config_==HYBRID){ //a training phase only exists when the decision model is used
		...
		time_for_training_phase=end_training_timestamp-begin_benchmark_timestamp;
                for(unsigned int i=0;i<dev_specs_.size();++i){
                     for(unsigned int j=0;j<alg_specs_.size();++j){
                          relative_errors[i*alg_specs_.size()+j]=hype::Report::instance().getRelativeEstimationError(alg_specs_[j].getAlgorithmName(),dev_specs_[i]);
                     }    
                }  
		//relative_error_cpu_parallel_algorithm = hype::Report::instance().getRelativeEstimationError(cpu_algorithm_name_,cpu_dev_spec_);
		//relative_error_gpu_algorithm = hype::Report::instance().getRelativeEstimationError(gpu_algorithm_name_,gpu_dev_spec_);
	}
	...
	std::cout << "Time for Workload: " <<  end_benchmark_timestamp-begin_benchmark_timestamp << "ns (" 
        ...
        for(unsigned int i=0;i<proc_devs_.size();++i){
                percentaged_execution_times[i]=proc_dev_execution_times[i]/total_processing_time_forall_devices;
                equal_processing_utilization_error+=std::pow(percentaged_execution_times[i]-double(1/proc_devs_.size()),2);
        }  
...
//	}
        ...
        for(unsigned int i=0;i<proc_devs_.size();++i){
                 std::cout << "Time for " << hype::util::getName(dev_specs_[i].getDeviceType()) <<"_"<< (int)dev_specs_[i].getProcessingDeviceID() << ": " << proc_dev_execution_times[i] << "ns" << std::endl;
        }  
        std::cout << "----------------------------------------------------------------------" << std::endl;
        //print percentaged execution time w.r.t. execution time spend on all processing devices
        for(unsigned int i=0;i<proc_devs_.size();++i){
                 std::cout << "Utilization of "  << hype::util::getName(dev_specs_[i].getDeviceType()) <<"_"<< (int)dev_specs_[i].getProcessingDeviceID() << ": " << percentaged_execution_times[i]*100 << "%" << std::endl;
        }
        std::cout << "Equal Processing Device Utilization Error: " << equal_processing_utilization_error << " (ideal: 0, worst: ?)" << std::endl;
        std::cout << "----------------------------------------------------------------------" << std::endl;
        //print relative estimation errors
        for(unsigned int i=0;i<dev_specs_.size();++i){
             for(unsigned int j=0;j<alg_specs_.size();++j){
                 std::cout << "Relative Estimation Error " << alg_specs_[j].getAlgorithmName() << " on " << hype::util::getName(dev_specs_[i].getDeviceType()) <<"_" <<  (int)dev_specs_[i].getProcessingDeviceID() << ": " << relative_errors[i*alg_specs_.size()+j]*100 << "%" << std::endl;
             }    
        }  
        std::cout << "----------------------------------------------------------------------" << std::endl;
...
//		<< relative_error_gpu_algorithm		
		...
		<< "execution_time_training_only_in_ns" << "\t"	
                ...
                //print total execution times (ns)
                for(unsigned int i=0;i<proc_devs_.size();++i){
                        str_stream_header_line << "total_time_" << hype::util::getName(dev_specs_[i].getDeviceType()) <<"_"<< (int)dev_specs_[i].getProcessingDeviceID() << "\t";
                }  
                //print percentaged execution time w.r.t. execution time spend on all processing devices
                for(unsigned int i=0;i<proc_devs_.size();++i){
                       str_stream_header_line << "spent_time_on_"  << hype::util::getName(dev_specs_[i].getDeviceType()) <<"_"<< (int)dev_specs_[i].getProcessingDeviceID() << "_in_percent" << "\t";
                }
                //print relative estimation errors
                for(unsigned int i=0;i<dev_specs_.size();++i){
                     for(unsigned int j=0;j<alg_specs_.size();++j){
                          str_stream_header_line << "average_estimation_error_" << alg_specs_[j].getAlgorithmName() << "_on_" << hype::util::getName(dev_specs_[i].getDeviceType()) <<"_" <<  (int)dev_specs_[i].getProcessingDeviceID() << "\t";
                     }    
                }  
//		<< "total_time_cpu"  << "\t"
//		<< "total_time_gpu"  << "\t"
...
//                //print total execution times (ns)
//                for(unsigned int i=0;i<proc_devs_.size();++i){
//                        cout << "total_time_" << hype::util::getName(dev_specs_[i].getDeviceType()) <<"_"<< (int)dev_specs_[i].getProcessingDeviceID() << "\t";
//                }  
//                //print percentaged execution time w.r.t. execution time spend on all processing devices
//                for(unsigned int i=0;i<proc_devs_.size();++i){
//                       cout << "spent_time_on_"  << hype::util::getName(dev_specs_[i].getDeviceType()) <<"_"<< (int)dev_specs_[i].getProcessingDeviceID() << "_in_percent" << "\t";
//                }
//                //print relative estimation errors
//                for(unsigned int i=0;i<dev_specs_.size();++i){
//                     for(unsigned int j=0;j<alg_specs_.size();++j){
//                          cout << "average_estimation_error_" << alg_specs_[j].getAlgorithmName() << "_on_" << hype::util::getName(dev_specs_[i].getDeviceType()) <<"_" <<  (int)dev_specs_[i].getProcessingDeviceID() << "\t";
//                     }    
//                }  
////		<< "total_time_cpu"  << "\t"
////		<< "total_time_gpu"  << "\t"
