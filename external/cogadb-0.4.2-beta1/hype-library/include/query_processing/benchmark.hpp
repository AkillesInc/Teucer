...
//typedef boost::shared_ptr<Vec> VecPtr;
	...
	Operation_Benchmark(const std::string& operation_name,
							  const std::string& cpu_algorithm_name, const std::string& gpu_algorithm_name) : 	
								...
								cpu_dev_spec_( DeviceSpecification (hype::PD0,hype::CPU,hype::PD_Memory_0) ),
								gpu_dev_spec_( DeviceSpecification (hype::PD1,hype::GPU,hype::PD_Memory_1)  ),		
                                cpu(),
                                gpu(),
								datasets(),
								operator_queries_(),
...
//                  std::cout << "HyPE: FATAL ERROR: Operation_Benchmark: At least one Processing Device not found! "
//                       << "This class assumes you setup the algorithms and processing devices already, and that " 
//                       << "you have one CPU (DeviceSpecification (hype::PD0,hype::CPU,hype::PD_Memory_0)) "
//                       << "and one GPU (DeviceSpecification(hype::PD1,hype::GPU,hype::PD_Memory_1))"
//                       << std::endl;
//                  std::exit(-1);
//              }   
...
//            gpu=hype::queryprocessing::getProcessingDevice(DeviceSpecification (hype::PD1,hype::GPU,hype::PD_Memory_1));
	...
	//Physical_Operator_Map_Ptr operator_map_;
		...
		return (uint64_t)dur.count();
		  ...
		  virtual DataSetType generate_dataset(unsigned int size_in_number_of_bytes) = 0;
...
std::vector<DataSetType> generate_random_datasets(unsigned int max_size_of_dataset_in_byte, unsigned int number_of_datasets){
	std::vector<DataSetType> datasets;
	...
	unsigned int tic=max_size_of_dataset_in_byte/number_of_datasets;
		for(unsigned int i=0;i<number_of_datasets;i++){
			DataSetType vec_ptr = this->generate_dataset(max_size_of_dataset_in_byte-i*tic); //(unsigned int) (rand()%max_size_in_number_of_elements) );
			assert(vec_ptr!=NULL);
	...
	//we don't want the OS to swap out our data to disc that's why we lock it
        ...
        if(ret!=0){
		std::cout << "FATAL ERROR! Could not lock main memory! Try run as root." << std::endl;
		exit(-1);
	}
	...
	//if(!scheduler.setOptimizationCriterion("MERGE","Throughput")) std::cout << "Error" << std::endl;
        ...
        for(unsigned int i=0;i<dev_specs.size();++i){
                if(dev_specs[i].getDeviceType()==hype::CPU){
                    if(!hype::Scheduler::instance().setStatisticalMethod(cpu_algorithm_name_,dev_specs[i],stemod_statistical_method_)){
                            std::cout << "Error" << std::endl; return -1;
                    }
                }else if(dev_specs[i].getDeviceType()==hype::GPU){
#ifdef ENABLE_GPU_ACCELERATION
                    if(!hype::Scheduler::instance().setStatisticalMethod(gpu_algorithm_name_,dev_specs[i],stemod_statistical_method_)){
                            std::cout << "Error" << std::endl; return -1;
                    }
#endif
                }
        ...
        for(unsigned int i=0;i<dev_specs.size();++i){
                if(dev_specs[i].getDeviceType()==hype::CPU){
                    if(!hype::Scheduler::instance().setRecomputationHeuristic(cpu_algorithm_name_,dev_specs[i],stemod_recomputation_heuristic_)){
                            std::cout << "Error" << std::endl; return -1;
                    }
                }else if(dev_specs[i].getDeviceType()==hype::GPU){
#ifdef ENABLE_GPU_ACCELERATION
                    if(!hype::Scheduler::instance().setRecomputationHeuristic(gpu_algorithm_name_,dev_specs[i],stemod_recomputation_heuristic_)){
                            std::cout << "Error" << std::endl; return -1;
                    }
#endif
                }
...
//	} else std::cout << "Success..." << std::endl;
	...
	for(unsigned int i=0;i<NUMBER_OF_SORT_OPERATIONS_IN_WORKLOAD_;i++){
		...
		*/
		...
		TypedNodePtr scan_left=boost::static_pointer_cast<typename TypedNodePtr::element_type>(current_operator->getLeft());
		TypedNodePtr scan_right=boost::static_pointer_cast<typename TypedNodePtr::element_type>(current_operator->getRight());
		...
		if(sched_config_==HYBRID){ //CPU_ONLY,GPU_ONLY,HYBRID)
			...
			if(scan_left) phy_left_scan = scan_left->getOptimalOperator(TypedOperatorPtr(),TypedOperatorPtr());
			if(scan_right) phy_right_scan = scan_right->getOptimalOperator(TypedOperatorPtr(),TypedOperatorPtr());
                        
			TypedOperatorPtr phy_op = current_operator->getOptimalOperator(phy_left_scan,phy_right_scan);
			...
			TypedOperatorPtr phy_op = current_operator->getOptimalOperator(phy_left_scan,phy_right_scan, hype::CPU_ONLY);
			...
			*/
			...
			TypedOperatorPtr phy_op = current_operator->getOptimalOperator(phy_left_scan,phy_right_scan, hype::GPU_ONLY);
