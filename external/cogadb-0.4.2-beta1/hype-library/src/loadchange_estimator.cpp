...
namespace hype{
	...
	LoadChangeEstimator::LoadChangeEstimator(unsigned int size_of_circular_buffer) : last_load_factors_() {
	...
	double LoadChangeEstimator::getLoadModificator() const throw(){
		if(last_load_factors_.empty()){
			...
			double sum=0;
			for(it=last_load_factors_.begin();it!=last_load_factors_.end();++it){
				sum+=*it;
				//cout << "DEBUG: Load Factor: " << *it << endl;
			}
			//cout << "DEBUG: Result: " << sum/last_load_factors_.size() << endl;
			return sum/last_load_factors_.size(); //return average of last_load_factors_
		}
	}

	void LoadChangeEstimator::add(const MeasurementPair& mp) throw(){
		last_load_factors_.push_back(mp.getMeasuredTime().getTimeinNanoseconds()/mp.getEstimatedTime().getTimeinNanoseconds());
	}
