...
namespace hype{
	...
	namespace core{
			...
			MemoryChunk::MemoryChunk(size_t estimated_memory_capacity_a,
				    ...
				    uint64_t allocation_id_a) 
                        : estimated_memory_capacity(estimated_memory_capacity_a),
                          mem_id(mem_id_a), allocation_id(allocation_id_a)
                        {
                        }

                        DeviceMemory::DeviceMemory(size_t total_memory_in_bytes_a, ProcessingDeviceMemoryID mem_id)
                        : allocated_memory_chunks(), total_memory_in_bytes(total_memory_in_bytes_a),
                          unique_allocation_id_(0), mem_id_(mem_id)
                        {
			...
			size_t DeviceMemory::getEstimatedUsedMemoryInBytes() const throw(){
                            ...
                            for(cit=this->allocated_memory_chunks.begin();cit!=this->allocated_memory_chunks.end();++cit){
			...
			MemoryChunkPtr DeviceMemory::allocateMemory(size_t number_of_bytes){
                            ...
                            if(number_of_bytes<=free_mem){
                                MemoryChunkPtr mem_chunk(new MemoryChunk(number_of_bytes, mem_id_ , getUniqueMemoryAllocationID()));
			...
			DeviceMemoryPtr DeviceMemories::getDeviceMemory(ProcessingDeviceMemoryID mem_id){
			...
			bool DeviceMemories::addDeviceMemory(ProcessingDeviceMemoryID mem_id, size_t total_memory_in_bytes){
