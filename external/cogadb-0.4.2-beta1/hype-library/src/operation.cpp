	...
	namespace core{
		...
		bool Operation::addAlgorithm(const std::string& name_of_algorithm, 
			...
			if(it==map_algorithmname_to_pointer_.end()){ //algorithm does not exist
				//alg = boost::shared_ptr<Algorithm>(new Algorithm(name_of_algorithm,name_of_statistical_method,name_of_recomputation_strategy,*this));
				...
				map_algorithmname_to_pointer_[name_of_algorithm]=boost::shared_ptr<Algorithm>(new Algorithm(name_of_algorithm,name_of_statistical_method,name_of_recomputation_strategy,*this,comp_dev));
				return true;
		...
		bool Operation::setNewOptimizationCriterion(const std::string& name_of_optimization_criterion){
			...
			ptr_to_optimization_criterion_=boost::shared_ptr<OptimizationCriterion_Internal>( getNewOptimizationCriterionbyName(name_of_optimization_criterion));
		...
		uint64_t Operation::getCurrentTimestamp() const throw(){
			return logical_time_;
                ...
                const std::string Operation::toString(){
                    std::stringstream ss;
                    ss << this->name_ << " (Optimization Criterion: " << this->ptr_to_optimization_criterion_->getName() << ")";
                    return ss.str();
                }
