			...
			std::string map_environment_variable_name_to_option_name(const std::string& s){
				...
				if(s=="STEMOD_LENGTH_OF_TRAININGPHASE" || s=="HYPE_LENGTH_OF_TRAININGPHASE"){
					...
					return "ready_queue_length";
                                ...
                                } else if (s == "STEMOD_PRINT_ALGORITHM_STATISTICS" || s == "HYPE_PRINT_ALGORITHM_STATISTICS") {
                            ...
                            enablePullBasedQueryChopping_(false)
	{
		...
		desc.add_options()
			 ...
			 ("ready_queue_length", boost::program_options::value<unsigned int>(), "set the queue length of operators that may be concurrently scheduled (then, clients are blocked on a processing device)")
                         ...
                         ("print_algorithm_statistics", boost::program_options::value<bool>(), "set the mode for storing algorithm statistics, true means algorithms statistics are dumped to the output directory of HyPE, false disables the feature (default)")
                         ("reuse_performance_models", boost::program_options::value<bool>(), "set the mode for the collected observations that serve as basis for the performance models, true means collected observations are stored on disk and are reused at system startup to skip the training phase, false disables the feature (default)")          
                         ("track_memory_usage", boost::program_options::value<bool>(), "enables or disables book keeping of used memory of processing devices (disabled by default)")      
		;
		...
		if (vm.count("ready_queue_length")) {
			...
			if (!quiet) cout << "ready_queue_length was not specified, using default value...\n";
                ...
                if (vm.count("default_optimization_criterion")) {
                    if (!quiet) cout << "Default Optimization Criterion: "
                    ...
                    if (!quiet) cout << "default_optimization_criterion was not specified, using default value...\n";
                }
                ...
                if (vm.count("print_algorithm_statistics")) {
                    ...
                    if (!quiet) cout << endl;
                    print_algorithm_statistics_report_ = vm["print_algorithm_statistics"].as<bool>();
                } else {
                    if (!quiet) cout << "print_algorithm_statistics was not specified, using default value (no)...\n";
                }
		...
		if (vm.count("reuse_performance_models")) {
		   ...
		   this->store_performance_models_=vm["reuse_performance_models"].as<bool>();
                    ...
                    if (!quiet) cout << "reuse_performance_models was not specified, using default value...\n";
		}
		...
		if (vm.count("track_memory_usage")) {
		   ...
		   this->track_memory_usage_=vm["track_memory_usage"].as<bool>();
                    ...
                    if (!quiet) cout << "track_memory_usage was not specified, using default value...\n";
		}
        ...
        bool Runtime_Configuration::setPullBasedQueryChoppingEnabled(bool value) throw () {
        ...
        bool Runtime_Configuration::setDataPlacementDrivenOptimization(bool value) throw (){
