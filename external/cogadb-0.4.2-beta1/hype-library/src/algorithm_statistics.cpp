...
std::string AlgorithmStatistics::getReport(const std::string& operation_name, const std::string& algorithm_name, const std::string indent_str) const{
...
//	}
        ...
        std::vector<MeasuredTime> measured_times = this->executionHistory_.getColumnMeasurements();
	...
	ss  << indent_str << "==========================================================================" << endl
            << indent_str << "\t" << "Report for Algorithm '" << algorithm_name << "'" << endl
            << indent_str << "\t" << "Average Relative Error: " << getAverageRelativeError() << endl //sum/num_valid_samples << endl  //sum/double(relative_errors_.size()) << endl
            << indent_str << "\t" << "Number of Recomputations: " << number_of_recomputations_ << endl
            << indent_str << "\t" << "Number of Decisions for this Algorithm: " << number_of_decisions_for_this_algorithm_ << endl
            << indent_str << "\t" << "Average Execution Time for this Algorithm: " << avg_exec_time/(1000*1000) << "ms" << endl
            << indent_str << "\t" << "Total Execution Time of this Algorithm: " << total_execution_time_/(1000*1000*1000) << "s" << endl;
            //<< "**************************************************************************" 
            //<< endl;
        return ss.str();
}
...
bool AlgorithmStatistics::storeFeatureVectors(const std::string& path){
    ...
    if(!quiet) std::cout << "Store performance model to '" << path << "' [" << executionHistory_.feature_values_.size() << " observations]"  << std::endl;
    std::ofstream outfile (path.c_str(), std::ios_base::binary | std::ios_base::out);
    
    boost::archive::binary_oarchive oa(outfile);
    std::vector<Tuple> feature_values(executionHistory_.feature_values_.begin(),executionHistory_.feature_values_.end());
    std::vector<EstimatedTime> estimated_execution_times(executionHistory_.estimated_times_.begin(),executionHistory_.estimated_times_.end());
    std::vector<MeasuredTime> measured_execution_times(executionHistory_.measured_times_.begin(),executionHistory_.measured_times_.end());
...
bool AlgorithmStatistics::loadFeatureVectors(const std::string& path){
       		...
       		if ( !boost::filesystem::exists( path ) ){ 
                    std::cout << "Error loading performance model: path '" << path << "' does not exist!" << std::endl; 
                    return false;
                }
		...
		infile.close();
                ...
                executionHistory_.feature_values_.insert(executionHistory_.feature_values_.begin(),feature_values.begin(),feature_values.end());
                executionHistory_.estimated_times_.insert(executionHistory_.estimated_times_.begin(),estimated_execution_times.begin(),estimated_execution_times.end());
                executionHistory_.measured_times_.insert(executionHistory_.measured_times_.begin(),measured_execution_times.begin(),measured_execution_times.end());
                                
                if(executionHistory_.feature_values_.size()<core::Runtime_Configuration::instance().getTrainingLength()){
                    HYPE_WARNING("Insufficient number of data points for '"<<path<<"', cannot compute cost model!",std::cout);
                    return false;
                }
    
                std::cout << "Successfully load data for cost model '" << path << "' [" << executionHistory_.feature_values_.size() << " observations]" << std::endl;
                return true;
}

    AlgCostModelIdentifier::AlgCostModelIdentifier() : external_alg_name(), pd_t(), pd_m(){
...
AlgorithmStatisticsManager::AlgorithmStatisticsManager(){
    ...
    AlgorithmStatisticsPtr AlgorithmStatisticsManager::getAlgorithmStatistics(const DeviceSpecification& dev_spec, const std::string& alg_name){
                ...
                if(it!=alg_statistics_.end()){
                    //if(!it->second) it->second=AlgorithmStatisticsPtr(new AlgorithmStatistics());
                    return it->second;
                }else{
