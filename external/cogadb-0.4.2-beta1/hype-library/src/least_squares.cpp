...
namespace hype{
	...
	Least_Squares_Method_1D::Least_Squares_Method_1D() : StatisticalMethod_Internal("Least Squares 1D"), timeestimationpolynomial_(), degree_of_polynomial_(2),polynomial_computed_(false) {

	}

	Least_Squares_Method_1D::~Least_Squares_Method_1D(){}

	const EstimatedTime Least_Squares_Method_1D::computeEstimation(const Tuple& input_values){
		if(input_values.size()!=1){
			std::cout << "FATAL ERROR: Least_Squares_Method_1D cannot handle input dimensions other than 1" << std::endl;
			exit(-1);
		} 
		...
		if(polynomial_computed_){
			returnval = alglib::barycentriccalc(timeestimationpolynomial_,input_values[0]);
		}
	...
	void Least_Squares_Method_1D::retrain(){
	...
	bool Least_Squares_Method_1D::recomuteApproximationFunction(Algorithm& algorithm){
		...
		std::vector<MeasuredTime> measured_times = algorithm.getAlgorithmStatistics().executionHistory_.getColumnMeasurements();
		int number_of_measurement_pairs = measured_times.size(); //= algorithm.getAlgorithmStatistics().executionHistory_.measured_times_.size();
		std::vector<Tuple> tuples = algorithm.getAlgorithmStatistics().executionHistory_.getColumnFeatureValues();
		int number_of_feature_values = tuples.size(); 
		
		assert(number_of_feature_values==number_of_measurement_pairs); //number of measured execution times = number of processed data sets
	...
	//cout << "Length Array x: " << x.length() << "   Length Array y: " << y.length() << endl;
    ...
    alglib::polynomialfit(x, y, m, info, p, rep);
		...
		cout << "FATAL ERROR! Least_Squares_Method_1D::recomuteApproximationFunction() " << endl
