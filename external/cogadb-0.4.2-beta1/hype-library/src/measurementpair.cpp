...
namespace hype{
	...
	MeasurementPair::MeasurementPair() : feature_values_(), measured_time_(0), estimated_time_(0) {

	}

	MeasurementPair::MeasurementPair(const Tuple& feature_values, MeasuredTime measured_time,EstimatedTime estimated_time)
	 : feature_values_(feature_values), measured_time_(measured_time), estimated_time_(estimated_time)
	{
	...
	const std::string MeasurementPair::toPlainString() const{
		...
		result+=boost::lexical_cast<std::string>(measured_time_.getTimeinNanoseconds());
		result+="\t";	
		result+=boost::lexical_cast<std::string>(estimated_time_.getTimeinNanoseconds());
		return result;
	}
...
std::ostream& operator<< (std::ostream &out, MeasurementPair &pair)
{
    ...
    out << pair.getMeasuredTime().getTimeinNanoseconds() << ", " 
    	  << pair.getEstimatedTime().getTimeinNanoseconds() << ")"; // << std::endl;
