...
namespace hype{
	...
	//speichert zeit auf cpu und gpu (momentan bloß pro algorithmus, aber spielt keine Rolle wenn wir eh nur einen CPU und einen GPU algorithmus haben)
	std::map<std::string,double> object_map;

	Throughput2::Throughput2(std::string name_of_operation) : OptimizationCriterion_Internal(std::string("Throughput2"),name_of_operation) {
		//OptimizationCriterionFactorySingleton::Instance().Register("Response Time",&Throughput2::create);
	}

	const SchedulingDecision Throughput2::getOptimalAlgorithm_internal(const Tuple& input_values, Operation& op, DeviceTypeConstraint dev_constr){
		...
		for(it=map_execution_times_to_algorithm_name.begin();it!=map_execution_times_to_algorithm_name.end();it++){
			...
			if(it->first+object_map[it->second]<min_time){ //CHANGED! object map added
				min_time=it->first;
