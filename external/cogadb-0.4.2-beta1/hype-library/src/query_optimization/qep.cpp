...
namespace hype{
	...
	namespace query_optimization{
                        ...
                        QEP_Node::QEP_Node(const OperatorSpecification& op_spec_arg, const DeviceConstraint& dev_constr_arg, bool input_data_cached_arg)
                        : op_spec(op_spec_arg), dev_constr(dev_constr_arg), parent(NULL), childs(), number_childs(0),
                        alg_ptr(NULL), cost(0), sched_dec_ptr(NULL), user_payload(), c_user_payload(NULL), input_data_cached(input_data_cached_arg){
                            
                        }
                        
                        
                        QEP_Node::QEP_Node(const QEP_Node& other) : op_spec (other.op_spec), 
                        ...
                        void QEP_Node::assignAlgorithm(core::Algorithm* alg_ptr){
                            boost::lock_guard<boost::mutex> lock(global_mutex_estimated_execution_time_of_algorithm);
                        ...
                        void QEP_Node::assignAlgorithm(core::Algorithm* alg_ptr, const EstimatedTime est_exec_time){
                            this->alg_ptr=alg_ptr;
                            //std::cout << "AssignAlg: " << alg_ptr->toString() << std::endl;
...
//                        void QEP_Node::assignAlgorithm(const OperatorSpecification& op_spec, const SchedulingDecision& sched_dec){
//                            
//                        }
                        ...
                        void QEP_Node::setParent(QEP_Node* parent){
                            this->parent=parent;
                        }
                        ...
                        double QEP_Node::computeCost(){
                            if(number_childs==0){
                                ...
                                for(unsigned int i=0;i<number_childs;++i){
                                    max_cost_of_childs=std::max(max_cost_of_childs, childs[i]->computeCost());
                                }
                       ...
                       bool QEP_Node::isInputDataCached() const{
                           ...
                           //so we the largest input data is missing, even if the perior filter operators 
                           ...
                           for(unsigned int i=0;i<number_childs;++i){
                               ...
                               if(childs[i]->sched_dec_ptr && childs[i]->sched_dec_ptr->getDeviceSpecification().getDeviceType()!=hype::GPU){
                                   return false;
                               }
                           }
                           return ret;
                        ...
                        QEP_Node::~QEP_Node(){
                            ...
                            //algorithms are explicitely managed by their operations, which are in turn managed by the central scheduler instance!

                            if(sched_dec_ptr){
                                core::Scheduler::instance().getProcessingDevices().removeSchedulingDecision(*sched_dec_ptr);
                        ...
                        std::string QEP_Node::toString(){
                            ...
                            if(this->sched_dec_ptr){
                                ss << alg_ptr->getName() << "(" <<  op_spec.getFeatureVector() << "), Cost: " << this->cost/(1000*1000) << "ms"; 
                                ss << " at device: " << (int)sched_dec_ptr->getDeviceSpecification().getProcessingDeviceID()
                                   << " (" << util::getName(sched_dec_ptr->getDeviceSpecification().getDeviceType()) << ", "
                                   << "MemoryID: " << (int)sched_dec_ptr->getDeviceSpecification().getMemoryID() << ")";
                                if(this->c_user_payload){
                                    ss << " User Payload (C Interface): " << this->c_user_payload; // << std::endl;
                                }
                            }else if(alg_ptr){
                                //ss << alg_ptr->getName() << ": " << this->cost;
                                ss << alg_ptr->getName() << "(" <<  op_spec.getFeatureVector() << "): " << this->cost; 
                            }else{
                                //ss << op_spec.getOperatorName();
                                ss << op_spec.getOperatorName() << "(" <<  op_spec.getFeatureVector() << ")" << util::getName(this->dev_constr);
                            }
                            return ss.str();
                        }
   ...
   /* ============================================================================ */
                    ...
                    struct Print_QEP_Functor{
                        ...
                        Print_QEP_Functor(std::string& str, bool enable_colors_a = true, bool print_node_numbers_a = false) 
                        : result_string(str),
                        ...
                        void operator()(QEP_Node* node, unsigned int level){
                            ...
                            if(enable_colors){
                                if(node->sched_dec_ptr){
                                    if(node->sched_dec_ptr->getDeviceSpecification().getDeviceType()==hype::CPU){
                                        ss << "\033[44m"; //highlight following text in blue
                                    }else if(node->sched_dec_ptr->getDeviceSpecification().getDeviceType()==hype::GPU){
                    ...
                    std::string QEP::toString(unsigned int indent, bool enable_colors, bool print_node_numbers){
                        if(root_==NULL){
                            return std::string();
                        }
                        std::string result;
                        ...
                        std::stringstream ss;
                        ss << std::endl << "Estimated Cost: " << this->computeCost(); // << std::endl;
...
////                              }
//                            if(childs[i]){
//                               ss << QEP(new QEP_Node(*childs[i])).toString(indent+1);
//                            }   
//                        }
//
//                        
//                        return ss.str();
                    ...
                    std::vector<hype::query_optimization::QEP_Node*> QEP::getLeafNodes(){
                            std::list<hype::query_optimization::QEP_Node*> order = this->reverselevelorder_traversal();
                    ...
                    size_t QEP::getNumberOfOperators(){
                        if(root_==NULL){
                            ...
                            root_->getTotalNumberofChilds();
                        }
                    }
                    
                    QEP_Node* QEP::getRoot(){
                        return this->root_;
                    }
                    ...
                    void QEP::setRoot(QEP_Node* new_root){
                        this->root_=new_root;
                    }
   ...
   /* ============================================================================ */
                ...
                void advanceAlgorithmIDs(std::vector<int>& current_algorithm_ids, const std::vector<int>& number_of_algorithms, int begin_index = 0){
                    assert(current_algorithm_ids.size()==number_of_algorithms.size());
                    if(begin_index==current_algorithm_ids.size())
                        return;
                    if(begin_index<current_algorithm_ids.size()){
                        if(current_algorithm_ids[begin_index] < number_of_algorithms[begin_index]-1){
                           current_algorithm_ids[begin_index]++; 
                        }else{
                            current_algorithm_ids[begin_index]=0;
                            advanceAlgorithmIDs(current_algorithm_ids, number_of_algorithms, begin_index+1);        
                        }
                    }   
                }
                    ...
                    hype::core::OperatorSequence createOperatorSequence(QEP& plan){
                        ...
                        std::list<hype::query_optimization::QEP_Node*> order = plan.levelorder_traversal(); //plan.levelorder_traversal();
                        std::list<hype::query_optimization::QEP_Node*>::iterator it;
                        for(it=order.begin();it!=order.end();++it){
                    ...
                    std::vector<std::vector<hype::core::AlgorithmPtr> > getAvailableAlgorithmsPerOperator(hype::core::OperatorSequence& op_seq){
                        std::vector<std::vector<hype::core::AlgorithmPtr> > available_algorithms_per_operator(op_seq.size());
                        for(unsigned int i=0;i<op_seq.size();++i){
                            //core::Scheduler::instance().get
                            const core::Scheduler::MapNameToOperation& map_operationname_to_operation_=hype::core::Scheduler::instance().getOperatorMap();
                            core::Scheduler::MapNameToOperation::const_iterator it = map_operationname_to_operation_.find(op_seq[i].first.getOperatorName());
                            ...
                            const std::vector<hype::core::AlgorithmPtr> alg_ptrs = it->second->getAlgorithms();
                            for(unsigned int j=0;j<alg_ptrs.size();++j){
                                if(op_seq[i].second.getDeviceTypeConstraint()==hype::ANY_DEVICE 
                                  || (alg_ptrs[j]->getDeviceSpecification().getDeviceType()==hype::CPU) //always add the CPU algorithms to avoid errors for CPU Only operations 
                                  || (op_seq[i].second.getDeviceTypeConstraint()==hype::GPU_ONLY && alg_ptrs[j]->getDeviceSpecification().getDeviceType()==hype::GPU)){
                    ...
                    void createPhysicalQueryPlan(QEP& current_plan, const std::vector<std::vector<hype::core::AlgorithmPtr> >& available_algorithms_per_operator, const std::vector<int>& current_algorithm_ids){
//                                QEP current_plan(plan);
                                std::list<hype::query_optimization::QEP_Node*> order = current_plan.levelorder_traversal(); //current_plan.reverselevelorder_traversal(); //plan.levelorder_traversal();
                                std::list<hype::query_optimization::QEP_Node*>::iterator it;
                                unsigned int operator_index=0;
                                for(it=order.begin();it!=order.end();++it,++operator_index){
                                    if(!hype::quiet && hype::verbose && hype::debug)
                                        std::cout << (*it)->op_spec.getOperatorName() <<  " (" << (*it)->op_spec.getFeatureVector() << ")" << std::endl;
                                    //fetch algorithm
                                    hype::core::AlgorithmPtr alg_ptr = available_algorithms_per_operator[operator_index][current_algorithm_ids[operator_index]];
                    ...
                    void createSchedulingDecisionsforQEP(QEP& plan){
                        ...
                        std::list<hype::query_optimization::QEP_Node*> order = plan.levelorder_traversal(); //current_plan.reverselevelorder_traversal(); //plan.levelorder_traversal();
                        std::list<hype::query_optimization::QEP_Node*>::iterator it;
                        unsigned int operator_index=0;
                        for(it=order.begin();it!=order.end();++it,++operator_index){
                            (*it)->sched_dec_ptr= new core::SchedulingDecision(*((*it)->alg_ptr), core::EstimatedTime((*it)->cost), (*it)->op_spec.getFeatureVector());
                        }
                    }
...
//                                    //fetch algorithm
//                                    
//                                    SchedulingDecision sched_dec = core::Scheduler::instance().getOptimalAlgorithm((*it)->op_spec, (*it)->dev_constr);
//                                    (*it)->sched_dec_ptr=new core::SchedulingDecision(sched_dec);
//                                    core::AlgorithmPtr alg_ptr = core::Scheduler::instance().getAlgorithm(hype::util::toInternalAlgName(sched_dec.getNameofChoosenAlgorithm(), sched_dec.getDeviceSpecification()));
//                                    assert(alg_ptr!=NULL);
...
//                       order = current_plan.levelorder_traversal(); //current_plan.reverselevelorder_traversal(); //plan.levelorder_traversal();
//                        for(it=order.begin();it!=order.end();++it){
//                            if((*it)->sched_dec_ptr==NULL){
//                                if((*it)->alg_ptr){
//                                    (*it)->sched_dec_ptr=new core::SchedulingDecision(*((*it)->alg_ptr), core::EstimatedTime((*it)->cost), (*it)->op_spec.getFeatureVector()); 
//                                }else{
//                                    HYPE_FATAL_ERROR("In Greedy Strategy: Found Node without assigned algorithm!",std::cout);
//                                }
//                            }
//                            
//                        }
...
//                }
                ...
                void optimizeQueryPlanGreedy(QEP& plan){
                            ...
                            //for(unsigned int j=0;j<op_seq.size();++j){
                                ...
                                std::list<hype::query_optimization::QEP_Node*> order = current_plan.reverselevelorder_traversal(); //plan.levelorder_traversal();
                                std::list<hype::query_optimization::QEP_Node*>::iterator it;
                                ...
                                for(it=order.begin();it!=order.end();++it){
                                    ...
                                    //does not display the load, only this algorithm's 
                                    //execution time
                                    (*it)->assignAlgorithm(alg_ptr.get(), sched_dec.getEstimatedExecutionTimeforAlgorithm());
                                }
                ...
                size_t countNumberOfConstraints(const std::vector<DeviceTypeConstraint>& dev_type_constraints, DeviceTypeConstraint constr){
                ...
                void optimizeQueryPlanGreedyChainer(QEP& plan){
                            ...
                            //for(unsigned int j=0;j<op_seq.size();++j){
                                ...
                                for(it=order.begin();it!=order.end();++it){
                                    ...
                                    //DeviceTypeConstraint dev_constr_of_op=(*it)->dev_constr.getDeviceTypeConstraint();
                                    //DeviceTypeConstraint  (*it)->dev_constr.getDeviceTypeConstraint()
                                    if(!util::isChainBreaker((*it)->op_spec.getOperatorName())){
                                        std::vector<DeviceTypeConstraint> dev_type_constraints((*it)->number_childs,ANY_DEVICE);
                                        bool at_least_one_child_is_chain_breaker = false;
                                        for(unsigned int i=0;i<(*it)->number_childs;++i){
                                            if((*it)->childs[i]){
                                                if(!util::isChainBreaker((*it)->childs[i]->op_spec.getOperatorName())){
                                                    assert((*it)->childs[i]->sched_dec_ptr!=NULL);
                                                    dev_type_constraints[i]=util::getDeviceConstraintForProcessingDeviceType((*it)->childs[i]->sched_dec_ptr->getDeviceSpecification().getDeviceType());
                                                }else{
                                        ...
                                        size_t num_cpu_only = countNumberOfConstraints(dev_type_constraints,CPU_ONLY);
                                        size_t num_gpu_only = countNumberOfConstraints(dev_type_constraints,GPU_ONLY);
                                        if(at_least_one_child_is_chain_breaker){
                                            ...
                                            //if we find a non cahin breaking operator, and the device constraint is
                                            //not ANY_DEVICE, we found a hint and use it, otherwise, we set a device constraint ourselves
                                            if((*it)->dev_constr==hype::ANY_DEVICE){
                                                //use GPU in case more than halve of child operators were executed on GPU
...
//                                                }
                ...
                void optimizeQueryPlanAccelerateCriticalPath(QEP& plan, unsigned int& leaf_number_of_new_path){
...
//                                std::list<hype::query_optimization::QEP_Node*>::iterator it;
                                ...
                                if(!hype::quiet && hype::verbose && hype::debug) std::cout << "Number of Leafs: " << leafs.size() << std::endl;
                                ...
                                for(unsigned int i=0;i<qep_candidates.size();++i){
                                    ...
                                    std::vector<hype::query_optimization::QEP_Node*> leaves  = qep_candidates[i]->getLeafNodes();
                                    if(!hype::quiet && hype::verbose && hype::debug)
                                    {
                                        std::cout << "Input Plan for Leaf " << leaves[i]->toString() << "    " << (void*)leaves[i] << "(i="<< i <<  "):" << std::endl; 
                                        std::cout << qep_candidates[i]->toString() << std::endl; 
                                    }
                                    if(!leaves[i]->isRoot()){ // && !util::isChainBreaker(leaves[i]->parent->op_spec.getOperatorName())){
                                        ...
                                        while(current_ancestor!=NULL){
                                            ...
                                            && !util::isChainBreaker(current_ancestor->op_spec.getOperatorName())){
                                    ...
                                    if(!hype::quiet && hype::verbose && hype::debug)
                                    {
                                        std::cout << "Annotated Plan for Leaf " << leaves[i]->toString() << "    " << (void*)leaves[i] << "(i="<< i <<  "):" << std::endl; 
                                        std::cout << qep_candidates[i]->toString() << std::endl; 
                                        std::cout << "Plan for Leaf " << leaves[i]->toString() << "    " << (void*)leaves[i] << "(i="<< i <<  "):" << std::endl; 
                                    }
                ...
                hype::query_optimization::QEP_Node* getParentFetchJoin(hype::query_optimization::QEP_Node* current_node){
                    hype::query_optimization::QEP_Node* ancestor = current_node->parent;
                    while(ancestor!=NULL && ancestor->op_spec.getOperatorName()!="COLUMN_FETCH_JOIN" && ancestor->op_spec.getOperatorName()!="FETCH_JOIN"){
...
//                                QEP current_plan(plan);          
//                                std::vector<hype::query_optimization::QEP_Node*> leafs = current_plan.getLeafNodes();
//                                for(unsigned int i=0;i<leafs.size();++i){
//                                    hype::query_optimization::QEP_Node* fetch_join = getParentFetchJoin(leafs[i]);
//                                    if(fetch_join){
...
//                                }
                                ...
                                QEP current_plan(plan);  
                                std::list<hype::query_optimization::QEP_Node*> list_nodes = current_plan.reverselevelorder_traversal();
                                std::vector<hype::query_optimization::QEP_Node*> nodes(list_nodes.begin(),list_nodes.end());
                                
                                for(unsigned int i=0;i<nodes.size();++i){
                                    //hype::query_optimization::QEP_Node* fetch_join = getParentFetchJoin(nodes[i]);
                                    if(nodes[i]){
...
//                void optimizeQueryPlanAccelerateCriticalPathDataPlacementAware(QEP& plan, unsigned int& leaf_number_of_new_path){
//                    
////                                std::list<hype::query_optimization::QEP_Node*> order = plan.reverselevelorder_traversal(); //plan.levelorder_traversal();
////                                std::list<hype::query_optimization::QEP_Node*>::iterator it;
//                                
...
//                }
             ...
             void optimizeQueryPlanGPUBestEffort(QEP& plan){
                ...
                void optimizeQueryPlanAccelerateCriticalPathRecursive(QEP& plan){
                    ...
                    size_t num_iterations = std::min(plan.getLeafNodes().size(),max_number_of_gpu_accelerated_paths);
			...
			{
                        std::list<hype::query_optimization::QEP_Node*> order = cpu_only_qep->reverselevelorder_traversal(); //plan.levelorder_traversal();
                        std::list<hype::query_optimization::QEP_Node*>::iterator it;
...
//                        std::cout << cpu_only_qep->toString() << std::endl;
		    ...
		    //create hybrid plans
                    for(unsigned int i=0;i<num_iterations;++i){
                        ...
                        std::vector<hype::query_optimization::QEP_Node*> leafs = current_qep->getLeafNodes();
//                        for(unsigned int j=0;j<gpu_path_leaves.size();++j){
//                            leafs[j]->dev_constr=hype::CPU_ONLY;
//                        }        
                        for(unsigned int j=0;j<gpu_path_leaves.size();++j){
//                            //only override constrained no other hint is present
...
//                                leafs[gpu_path_leaves[j]]->parent->dev_constr=hype::GPU_ONLY;
                            ...
                            hype::query_optimization::QEP_Node* current_ancestor=leafs[gpu_path_leaves[j]]->parent;
                            while(current_ancestor!=NULL){
                                if(current_ancestor->dev_constr==hype::ANY_DEVICE){
                                    current_ancestor->dev_constr=hype::GPU_ONLY;
                                    break;
                        ...
                        optimizeQueryPlanAccelerateCriticalPath(*current_qep, leaf_number_of_new_path);

                        
                        double cost = current_qep->computeTotalCost(); //current_qep->computeCost();
                        if (!hype::quiet && hype::verbose && hype::debug) 
                        {
...
//                        }else{
////                            if(!hype::quiet && hype::verbose && hype::debug)
//                                std::cout << "Found no better plan in iteration " << i << "! Returning plan..." << std::endl;
//                            break;
                        }
                    }
                    assert(best_qep_candidate!=NULL);
		    if(cpu_only_qep->computeTotalCost()>=best_qep_candidate->computeTotalCost()){
			...
			plan=*cpu_only_qep;
                ...
                void optimizeQueryPlanAccelerateCriticalPath_old(QEP& plan){
                            ...
                            //for(unsigned int j=0;j<op_seq.size();++j){
                                ...
                                for(it=order.begin();it!=order.end();++it){
                                    ...
                                    SchedulingDecision sched_dec = core::Scheduler::instance().getOptimalAlgorithm((*it)->op_spec, hype::CPU_ONLY); //(*it)->dev_constr);
                                    (*it)->sched_dec_ptr=new core::SchedulingDecision(sched_dec);
                                    core::AlgorithmPtr alg_ptr = core::Scheduler::instance().getAlgorithm(hype::util::toInternalAlgName(sched_dec.getNameofChoosenAlgorithm(), sched_dec.getDeviceSpecification()));
                                    assert(alg_ptr!=NULL);
                                    //decision is load aware, if WTAR is used. 
                                ...
                                //TODO: traverse plan, identify critical path, and assign the critical path GPU operators
                ...
                void optimizeQueryPlanInteractive_old(QEP& plan){
                    ...
                    std::list<hype::query_optimization::QEP_Node*> order = current_plan.preorder_traversal();
                    
                    std::vector<hype::query_optimization::QEP_Node*> indexed_order(order.begin(),order.end());
                    ...
                    const core::Scheduler::MapNameToOperation& map_operationname_to_operation_=hype::core::Scheduler::instance().getOperatorMap();
                    core::Scheduler::MapNameToOperation::const_iterator it = map_operationname_to_operation_.find(node->op_spec.getOperatorName());
                    if(it==map_operationname_to_operation_.end()){
                        //error, return NUll Pointer
                        HYPE_FATAL_ERROR("INVALID OPERATIONNAME!",std::cout);
                        return;
                    ...
                    for(unsigned int i=0;i<alg_ptrs.size();++i){
                        std::cout << i << ". " << util::getName(alg_ptrs[i]->getDeviceSpecification().getDeviceType()) 
                                  << alg_ptrs[i]->getName() << std::endl;
                    }
                    
                    std::cout << "Choose Algorithm for Operator " << node->toString() <<  ":"  << std::endl;
                    ...
                    //TODO: FIXME: Scheduling Decision has to be removed from book keeping mechanism
                    if( node->sched_dec_ptr) delete node->sched_dec_ptr;
                    node->sched_dec_ptr= new core::SchedulingDecision(*(node->alg_ptr), core::EstimatedTime(node->cost), node->op_spec.getFeatureVector());
                    ...
                    //inside plan    
                        ...
                        insertCopyOperatorInnerNode(node);
                        if(node->parent->op_spec.getMemoryLocation()!=node->op_spec.getMemoryLocation()){
                        }
                    }         
                    }
                ...
                void optimizeQueryPlanInteractive(QEP& plan){
                    ...
                    if(!hype::quiet && hype::verbose && hype::debug){
                        ...
                        std::vector<std::vector<hype::core::AlgorithmPtr> > available_algorithms_per_operator = getAvailableAlgorithmsPerOperator(op_seq);
                        
                        //init array for number of algorithms per operator
                        std::vector<int> number_of_algorithms(op_seq.size());
                        for(unsigned int i=0;i<op_seq.size();++i){
                            number_of_algorithms[i] = available_algorithms_per_operator[i].size(); // number_of_algorithms[i];
                        }
                        
                        std::list<hype::query_optimization::QEP_Node*> level_order = plan.levelorder_traversal(); 
                        std::list<hype::query_optimization::QEP_Node*> pre_order = plan.preorder_traversal();
                        std::list<hype::query_optimization::QEP_Node*>::iterator it;
                        std::list<hype::query_optimization::QEP_Node*>::iterator inner_it;
                        std::vector<unsigned int> operator_id_mapping_array(op_seq.size());
                        unsigned int operator_index=0;
                        for(it=level_order.begin();it!=level_order.end();++it,++operator_index){
                            unsigned int inner_operator_index=0;
                            for(inner_it=pre_order.begin();inner_it!=pre_order.end();++inner_it,++inner_operator_index){
                                if(*it==*inner_it){
...
//                                    operator_id_mapping_array[operator_index]=inner_operator_index;
                                    std::cout << "LevelOrder: " <<  (*it)->toString() << " (index " << operator_index << ")" << std::endl
                                              << "PreOrder: " <<  (*inner_it)->toString() << " (index " << inner_operator_index << ")" << std::endl
                                              << "Map Array: " << inner_operator_index << "->" << operator_index << std::endl;
                                }
                            }
                        }
...
//                        }
                        ...
                        //std::cout << "Backtracking exploring " << number_of_plans << " plans" << std::endl;
                        while(true){
                            ...
                            //create plan
                            //for(unsigned int j=0;j<op_seq.size();++j){
                            createPhysicalQueryPlan(current_plan, available_algorithms_per_operator, current_algorithm_ids);
                    ...
                    std::cout << "transformed id: " <<  operator_id_mapping_array[node_number] << std::endl;
...
//                    
//                    const core::Scheduler::MapNameToOperation& map_operationname_to_operation_=hype::core::Scheduler::instance().getOperatorMap();
//                    core::Scheduler::MapNameToOperation::const_iterator it = map_operationname_to_operation_.find(op_seq[node_number].first.getOperatorName());
//                    if(it==map_operationname_to_operation_.end()){
//                        //error, return NUll Pointer
//                        HYPE_FATAL_ERROR("INVALID OPERATIONNAME!",std::cout);
//                        return;
...
//                    std::cout << "Available Operators: " << std::endl;
                    ...
                    const std::vector<hype::core::AlgorithmPtr> alg_ptrs = available_algorithms_per_operator[node_number]; //it->second->getAlgorithms();
                    for(unsigned int i=0;i<alg_ptrs.size();++i){
                        std::cout << i << ". " << util::getName(alg_ptrs[i]->getDeviceSpecification().getDeviceType()) 
                                  << alg_ptrs[i]->getName() << std::endl;
                    }
                    
                    std::cout << "Choose Algorithm for Operator " << op_seq[node_number].first.getOperatorName() <<  ":"  << std::endl;
                    unsigned int algorithm_id=0;
                    std::cin >> user_input; 
                    if(user_input=="quit"){
                        break;
...
//                                std::cout << "Total Cost: " << cost_current_plan/(1100*1000) << "ms" << std::endl;
//                            }
//                            
...
//                                optimal_plan=current_plan;
//                                if(!hype::quiet && hype::verbose && hype::debug)
//                                    std::cout << "================ New Optimal Plan ===============" << std::endl;
//                            }
                ...
                void optimizeQueryPlanBacktracking(QEP& plan){
                    ...
                    if(!hype::quiet && hype::verbose && hype::debug){
                        ...
                        for(unsigned int i=0;i<op_seq.size();++i){
                            number_of_plans*= available_algorithms_per_operator[i].size(); // number_of_algorithms[i];
                        }
                ...
                bool verifyPlan(QEP& plan){
                    //check the plan whether all QEP_Nodes have a valid Scheduling Decision and that no Device Constraint is violated
                    std::list<hype::query_optimization::QEP_Node*> order = plan.levelorder_traversal(); //current_plan.reverselevelorder_traversal(); //plan.levelorder_traversal();
                    std::list<hype::query_optimization::QEP_Node*>::iterator it;
                    unsigned int operator_index=0;
                    for(it=order.begin();it!=order.end();++it,++operator_index){
                        ...
                        if((*it)->sched_dec_ptr==NULL){
                            std::cerr << "Error! Node '" << (*it)->toString() << "' has no valid Scheduling Decision!" << std::endl;
                            return false;
                        }
                        //device constraints considered?
                        if(!util::satisfiesDeviceConstraint((*it)->sched_dec_ptr->getDeviceSpecification(), (*it)->dev_constr)){
                            HYPE_WARNING("Node '" << (*it)->toString() 
                                << "' does not satisfy its DeviceConstraint!"
                                << "This can happen in case a co-processor has not enough memory",std::cerr);
                            //return false; 
                        }
...
//                    return;
    ...
    //                        optimizeQueryPlanAccelerateCriticalPath(plan,i);
                    ...
                    //std::cout << "Result Plan (" << heuristic_name << "): " << std::endl << plan.toString() << std::endl;
                    //std::cout << "Time for optimization using heuristic '" << heuristic_name << "': " <<  double(end-begin)/(1000*1000) << "ms" << std::endl;
                }
                        ...
                        void insertCopyOperatorLeaveNode(hype::query_optimization::QEP_Node* leave){
                            ...
                            if(leave->alg_ptr->getDeviceSpecification().getMemoryID()!=hype::PD_Memory_0){
                                std::string copy_type = util::getCopyOperationType(hype::PD0,leave->alg_ptr->getDeviceSpecification().getProcessingDeviceID());
                                assert(!copy_type.empty());
                                DeviceSpecification dev_spec = hype::core::Scheduler::instance().getDeviceSpecificationforCopyType(copy_type);
                                core::AlgorithmPtr copy_alg_ptr = hype::core::Scheduler::instance().getAlgorithm(util::toInternalAlgName(copy_type,dev_spec ));
                                ...
                                OperatorSpecification copy_op_spec(copy_type,
                                                                    t,
                                                                    hype::PD_Memory_0, //memory id of input data
                                                                    leave->alg_ptr->getDeviceSpecification().getMemoryID()); //memory id of result data
                                
                                hype::query_optimization::QEP_Node* copy_node = new hype::query_optimization::QEP_Node(copy_op_spec, hype::core::DeviceConstraint());
                                copy_node->assignAlgorithm(copy_alg_ptr.get());
                                //SchedulingDecision copy_operator(*copy_alg_ptr,copy_alg_ptr->getEstimatedExecutionTime(t),t);
                                //result_plan->push_back(copy_operator); 
//                                hype::query_optimization::QEP_Node* old_parent = leave->parent;
//                                hype::query_optimization::QEP_Node* old_child = leave;
                                leave->setChilds(&copy_node,1);

                            }
                        }
                        void insertCopyOperatorRootNode(hype::query_optimization::QEP& qep, hype::query_optimization::QEP_Node* root){
      
                            if(root->alg_ptr->getDeviceSpecification().getMemoryID()!=hype::PD_Memory_0){
                                //cout << "Insert COPY Operation to transfer result to CPU" << endl;
                                std::string copy_type = util::getCopyOperationType(root->alg_ptr->getDeviceSpecification().getProcessingDeviceID(),hype::PD0); //assume that PD0 is CPU!
                                assert(!copy_type.empty());
                                DeviceSpecification dev_spec = hype::core::Scheduler::instance().getDeviceSpecificationforCopyType(copy_type);
                                core::AlgorithmPtr copy_alg_ptr = hype::core::Scheduler::instance().getAlgorithm(util::toInternalAlgName(copy_type,dev_spec ));
                                assert(copy_alg_ptr!=NULL);
                                ...
                                OperatorSpecification copy_op_spec(copy_type,
                                                                    t,
                                                                    root->alg_ptr->getDeviceSpecification().getMemoryID(), //memory id of input data
                                ...
                                //InternalPhysicalOperator copy_operator(copy_alg_ptr, t, copy_alg_ptr->getEstimatedExecutionTime(t).getTimeinNanoseconds());
                                //SchedulingDecision copy_operator(*copy_alg_ptr,copy_alg_ptr->getEstimatedExecutionTime(t),t);
                            }
...
//                                //cout << "Insert COPY Operation to transfer result to CPU" << endl;
//                                std::string copy_type = util::getCopyOperationType(last_operator.alg_ptr->getDeviceSpecification().getProcessingDeviceID(),hype::PD0); //assume that PD0 is CPU!
//                                assert(!copy_type.empty());
//                                DeviceSpecification dev_spec = getDeviceSpecificationforCopyType(copy_type);
//                                AlgorithmPtr copy_alg_ptr = this->getAlgorithm(util::toInternalAlgName(copy_type,dev_spec ));
//                                assert(copy_alg_ptr!=NULL);
//                                //cout << "COPY Operator " << copy_alg_ptr.get() << endl;
//                                Tuple t;
...
//                            }
                        ...
                        void insertCopyOperatorInnerNode(hype::query_optimization::QEP_Node* inner_node){
                                ...
                                if(!util::isCopyOperation(inner_node->alg_ptr->getName())){
                                    ProcessingDeviceMemoryID mem_id_current_node=inner_node->alg_ptr->getDeviceSpecification().getMemoryID();
                                    ProcessingDeviceMemoryID mem_id_parent_node=inner_node->parent->alg_ptr->getDeviceSpecification().getMemoryID();
                                    if(mem_id_current_node!=mem_id_parent_node){
                                        ...
                                        std::string copy_type = util::getCopyOperationType(inner_node->alg_ptr->getDeviceSpecification().getProcessingDeviceID(),inner_node->parent->alg_ptr->getDeviceSpecification().getProcessingDeviceID());
                                        assert(!copy_type.empty());
                                        DeviceSpecification dev_spec = hype::core::Scheduler::instance().getDeviceSpecificationforCopyType(copy_type);
                                        core::AlgorithmPtr copy_alg_ptr = hype::core::Scheduler::instance().getAlgorithm(util::toInternalAlgName(copy_type,dev_spec ));
                                        assert(copy_alg_ptr!=NULL);
                                        ...
                                        copy_node->assignAlgorithm(copy_alg_ptr.get());
                                        if(!hype::quiet && hype::verbose && hype::debug)
                                            std::cout << "create Copy Operation: " << copy_node->toString() << std::endl;
                                        ...
                                        //InternalPhysicalOperator copy_operator(copy_alg_ptr, t, copy_alg_ptr->getEstimatedExecutionTime(t).getTimeinNanoseconds());
                                        //SchedulingDecision copy_operator( (*(copy_alg_ptr.get())), copy_alg_ptr->getEstimatedExecutionTime(t),t);
                                        //cout << "insert " << copy_operator.getNameofChoosenAlgorithm() << " between " << result_plan->back().getNameofChoosenAlgorithm() << " and " << current_operator.getNameofChoosenAlgorithm()   << endl;
                                        //result_plan.push_back(copy_operator);
                                    }
                                }
                        }
