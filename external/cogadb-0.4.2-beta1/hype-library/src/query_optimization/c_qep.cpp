...
C_QEP_Node* hype_createQEPNode(C_OperatorSpecification* op_spec, C_DeviceConstraint* dev_constr){
    C_QEP_Node* ret = (C_QEP_Node*) malloc(sizeof(C_QEP_Node));
    
    if(op_spec==NULL || dev_constr==NULL)
        return NULL;
...
void hype_setParents(C_QEP_Node* node, C_QEP_Node** parents, size_t num_parents){
...
C_QEP_Node** hype_getParents(C_QEP_Node* node, size_t* num_parents){
    ...
    //Note: C interface more general than implementation, currently, only 1 parent per node is supported
...
void hype_setChildren(C_QEP_Node* node, C_QEP_Node** childs, size_t num_childs){
        QEP_Node* qep_node = (QEP_Node*) node->ptr;
...
void hype_addChild(C_QEP_Node* node, C_QEP_Node* child){
     ...
     if(!qep_node->addChild(qep_child)){
         HYPE_FATAL_ERROR("Maximal Number of Children Exceeded! For QEP Node: " << qep_node->toString(),std::cerr);
     }    
}
...
C_QEP_Node** hype_getChildren(C_QEP_Node* node, size_t* num_children){
    ...
    C_QEP_Node** qep_children = (C_QEP_Node**) malloc(sizeof(C_QEP_Node**)*qep_node->number_childs);
...
//    std::cout << "Optimizing Query Plan: " << std::endl;
    ...
    optimizeQueryPlan(query_plan_to_optimize, opt_heu); //BACKTRACKING); //GREEDY_HEURISTIC); //BACKTRACKING);
...
C_SchedulingDecision* hype_getSchedulingDecision(C_QEP_Node* node){
        ...
        //create a copy of hte SchedulingDecision, because it will be deleted when the query plan is destructed
        c_sched_dec->ptr=new hype::core::SchedulingDecision(*qep_node->sched_dec_ptr);
        return c_sched_dec;
}
...
static int hype_traverseQEP_(C_QEP_Node* root, int level, hype_qep_node_function_ptr func_ptr){
    int i=0;
    int error_code=0;
...
int hype_QEPNodeSetPayload(C_QEP_Node* node, void* payload){
   ...
   //std::cout << "Assign Payload " << payload << " to Node: " << qep_node->toString() << std::endl;
...
//   }
   ...
   //std::cout << "Fetch Payload " << payload << " from Node: " << qep_node->toString() << std::endl;
   return payload;
}
