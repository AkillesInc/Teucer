...
namespace hype{
	namespace core {
        ...
        private:
            ...
            const EstimatedTime getKNearestNeighbourEstimation(const Tuple& input_values, size_t k);
            struct Record {
                ...
                double computeWeightedDistance(const Record& rec, const std::vector<double>& weights) const;
...
//            bool operator<(const Record& rec_l, const Record& rec_r);
        ...
        template <size_t NUMBER_OF_FEATURES>     
        KNN_Regression_Impl<NUMBER_OF_FEATURES>::Record::Record(const Tuple& input_values_a, double measured_time_a)
        ...
        template <size_t NUMBER_OF_FEATURES>     
        double KNN_Regression_Impl<NUMBER_OF_FEATURES>::Record::computeDistance(const Record& rec) const{
            double sum=0;
            double tmp=0;
            for(unsigned int i=0;i<NUMBER_OF_FEATURES;++i){
        ...
        template <size_t NUMBER_OF_FEATURES>     
        double KNN_Regression_Impl<NUMBER_OF_FEATURES>::Record::computeWeightedDistance(const Record& rec, const std::vector<double>& weights) const{
            double sum=0;
            double tmp=0;
            for(unsigned int i=0;i<NUMBER_OF_FEATURES;++i){
        ...
        template <size_t NUMBER_OF_FEATURES>     
        bool KNN_Regression_Impl<NUMBER_OF_FEATURES>::Record::operator<(const Record& rec) const{
            for(unsigned int i=0;i<NUMBER_OF_FEATURES;++i){
        ...
        template <size_t NUMBER_OF_FEATURES>
        KNN_Regression_Impl<NUMBER_OF_FEATURES>::KNN_Regression_Impl() : records_(), feature_weights_(){
        ...
        template <size_t NUMBER_OF_FEATURES>
        const EstimatedTime KNN_Regression_Impl<NUMBER_OF_FEATURES>::getNearestNeighbourEstimation(const Tuple& input_values) {
...
//        template <size_t NUMBER_OF_FEATURES>
//        bool KNN_Regression_Impl<NUMBER_OF_FEATURES>::operator<(const Record& rec_l, const Record& rec_r){
//            return rec_l.operator <(rec_r);
//        }
        
        template <size_t NUMBER_OF_FEATURES>
        const EstimatedTime KNN_Regression_Impl<NUMBER_OF_FEATURES>::getKNearestNeighbourEstimation(const Tuple& input_values, size_t k){
            typedef pair<double,Record> DistanceRecord;
            //Record with largest distance to query point is always at the top of the priority queue
            std::priority_queue<DistanceRecord> nearest_neighbours;
            Record rec(input_values, 0);
            for(unsigned int i=0;i<records_.size();++i){
                //double distance = records_[i].computeDistance(rec);
                double distance = records_[i].computeWeightedDistance(rec,this->feature_weights_);
...
//                if(!quiet && verbose && debug) std::cout << "NN: " << nearest_neighbours.top().second.toString() << " (distance: " << nearest_neighbours.top().first <<  ")" << std::endl;
//                nearest_neighbours.pop();
//            }
...
//            std::cout << "records_.size(): " << records_.size() << " k: " << k << " #nn: " << nns.size()  << std::endl;
            if(records_.size()>=k){
                assert(nns.size()==k);
            }
            double sum=0;
...
//            }
        ...
        template <size_t NUMBER_OF_FEATURES>
        const EstimatedTime KNN_Regression_Impl<NUMBER_OF_FEATURES>::computeEstimation(const Tuple& input_values) {
...
//            k=std::max(k,size_t(1));
            ...
            //std::cout << "KNN search time:" << double(end-begin)/(1000*1000) << "ms" << std::endl; 
            return est;
//            double min_dist = std::numeric_limits<double>::max();
//            unsigned int nearest_neighbour_index=0;
//            Record rec(input_values, 0);
...
//            return EstimatedTime(records_[nearest_neighbour_index].measured_time);
        ...
        template <size_t NUMBER_OF_FEATURES>
        bool KNN_Regression_Impl<NUMBER_OF_FEATURES>::recomuteApproximationFunction(Algorithm& algorithm) {
            
		std::vector<Tuple> features = algorithm.getAlgorithmStatistics().executionHistory_.getColumnFeatureValues();
		std::vector<MeasuredTime> measurements = algorithm.getAlgorithmStatistics().executionHistory_.getColumnMeasurements();
                records_.clear();
		for(unsigned int i=0;i<features.size();++i){
                    records_.push_back(Record(features[i], measurements[i].getTimeinNanoseconds()));
                }
                //compute maximal value for each feature
                std::vector<double> max_feature_values(NUMBER_OF_FEATURES,std::numeric_limits<double>::min());
	...
	KNN_Regression::KNN_Regression() : StatisticalMethod_Internal("KNN_Regression"),polynomial_computed_(false), knn_regression_model_() {
	...
	KNN_Regression::~KNN_Regression(){
        ...
        const EstimatedTime KNN_Regression::computeEstimation(const Tuple& input_values) {

            if(knn_regression_model_==NULL){
                if(input_values.size()==1){
                    knn_regression_model_=KNN_Regression_ModelPtr(new KNN_Regression_Impl<1>());
                }else if (input_values.size()==2){
                    knn_regression_model_=KNN_Regression_ModelPtr(new KNN_Regression_Impl<2>());
                }else if (input_values.size()==3){
                    knn_regression_model_=KNN_Regression_ModelPtr(new KNN_Regression_Impl<3>());
                }else if (input_values.size()==4){
                    knn_regression_model_=KNN_Regression_ModelPtr(new KNN_Regression_Impl<4>());
                }else if (input_values.size()==5){
                    knn_regression_model_=KNN_Regression_ModelPtr(new KNN_Regression_Impl<5>());
                }
            }
            
            //negative estimation is invalid value, allowing to distinguish between a regular estimation and a dummy value, which is returned if Algorithm is in trainingphase
            //double returnval=-1;
	...
	bool KNN_Regression::inTrainingPhase() const throw(){
		return !polynomial_computed_; //if polynomial is not computed, we are in the initial trainingphase
	}

	void KNN_Regression::retrain(){
		polynomial_computed_=false;
	}

        std::string KNN_Regression::getName() const{
        ...
        bool KNN_Regression::recomuteApproximationFunction(Algorithm& algorithm){
            ...
            if (knn_regression_model_ == NULL) {
                assert(!algorithm.getAlgorithmStatistics().executionHistory_.getColumnFeatureValues().empty());
                size_t num_of_features = algorithm.getAlgorithmStatistics().executionHistory_.getColumnFeatureValues().front().size();
