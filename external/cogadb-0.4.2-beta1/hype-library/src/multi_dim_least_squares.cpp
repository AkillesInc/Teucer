...
namespace hype{
	...
	Least_Squares_Method_2D::Least_Squares_Method_2D() : StatisticalMethod_Internal("Least Squares 2D"),degree_of_polynomial_(2),polynomial_computed_(false), objHeArray_() {

	}

	Least_Squares_Method_2D::~Least_Squares_Method_2D(){}

	const EstimatedTime Least_Squares_Method_2D::computeEstimation(const Tuple& input_values){
		if(input_values.size()!=2){
			std::cout << "FATAL ERROR: Least_Squares_Method_2D cannot handle input dimensions other than 2" << std::endl;
...
//		return EstimatedTime(returnval);
	...
	void Least_Squares_Method_2D::retrain(){
	...
	bool Least_Squares_Method_2D::recomuteApproximationFunction(Algorithm& algorithm){
//			int intMax = (int) vecValue->size();
//			if(intMax == 0 || (int)vecFeature->size() % intMax != 0){return false;}
//			int intMaxInner = (int)vecFeature->size() /  intMax;
		   ...
		   polynomial_computed_=true;
			...
			objArrayMulti.setlength(features.size(),features[0].size()); //(intMax, intMaxInner); 
			objArrayMulti.setcontent(features.size(),features[0].size(), util::begin_ptr(features_array));
			...
			objArray.setcontent(measurements_array.size(), util::begin_ptr(measurements_array));
			...
			//std::cout << "Ausgabe" << objResult[0] << "  "<< objResult[1]  << "  " << objResult[2] << "\n";
