...
/************** Algorithm Specification **********/
	...
	AlgorithmSpecification::AlgorithmSpecification(const std::string& alg_name,
...
const std::string AlgorithmSpecification::getOptimizationCriterionName() const throw()
{
	return getName(this->opt_crit_);
}

const std::string AlgorithmSpecification::getRecomputationHeuristicName() const throw()
{
	return getName(this->recomp_heur_);
}

const std::string AlgorithmSpecification::getStatisticalMethodName() const throw()
{
...
/************** Operator Specification **********/
		...
		OperatorSpecification::OperatorSpecification(const std::string& operator_name, 
...
ProcessingDeviceMemoryID OperatorSpecification::getMemoryLocation() const throw()
{
...
DeviceSpecification::DeviceSpecification(ProcessingDeviceID pd,
...
ProcessingDeviceType DeviceSpecification::getDeviceType() const throw()
{
		...
		DeviceSpecification::operator ProcessingDeviceID(){
			return this->pd_;
} 
		DeviceSpecification::operator ProcessingDeviceType(){
			return this->pd_t_;
}
		DeviceSpecification::operator ProcessingDeviceMemoryID(){
			return this->pd_m_;
}

		bool DeviceSpecification::operator== (const DeviceSpecification& dev_spec) const{
			return  		(this->pd_==dev_spec.pd_)
						&& (this->pd_t_==dev_spec.pd_t_)
...
DeviceConstraint::DeviceConstraint(DeviceTypeConstraint dev_constr,
											  ProcessingDeviceMemoryID pd_mem_constr) : dev_constr_(dev_constr), pd_mem_constr_(pd_mem_constr)
{		
}
		...
		DeviceConstraint::operator DeviceTypeConstraint()
		{
			return this->dev_constr_;
		}
