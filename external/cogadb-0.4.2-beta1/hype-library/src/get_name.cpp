...
namespace hype{
	...
	const std::string getName(StatisticalMethod x){
		...
		const char * const statistical_method_names[] = {"Least Squares 1D","Least Squares 2D","KNN_Regression"};
	...
	const std::string getName(RecomputationHeuristic x){
		...
		const char * const recomputation_heuristic_names[] = {"Oneshot Recomputation","Periodic Recomputation"};
...
//	};
	...
	const std::string getName(OptimizationCriterion x){
		...
		const char * const optimization_criterion_names[] = {"Response Time","WaitingTimeAwareResponseTime","Throughput","Simple Round Robin","ProbabilityBasedOutsourcing","Throughput2"};
...
//	};
	...
	const std::string getName(ProcessingDeviceType x){
		//	enum ProcessingDeviceType{CPU,GPU,FPGA,NP,XEON_PHI,DMA};	//CPU,GPU,FPGA, Network Processor
	...
	const std::string getName(DeviceTypeConstraint x){
		//	enum DeviceTypeConstraint{ANY_DEVICE,CPU_ONLY,GPU_ONLY,FPGA_ONLY,NP_ONLY,XEON_PHI_ONLY};
		const char * const names[] = {"ANY_DEVICE","CPU_ONLY","GPU_ONLY","FPGA_ONLY","NP_ONLY","XEON_PHI_ONLY"};
...
//                    } QueryOptimizationHeuristic;
	...
	const std::string getName(QueryOptimizationHeuristic x){
		//	enum DeviceTypeConstraint{ANY_DEVICE,CPU_ONLY,GPU_ONLY,FPGA_ONLY,NP_ONLY,XEON_PHI_ONLY};
		const char * const names[] = {"GREEDY_HEURISTIC", "BACKTRACKING", "TWO_COPY_HEURISTIC", "CPU_CP_SEQUENCE_ALLOCATION","INTERACTIVE_USER_OPTIMIZATION","GREEDY_CHAINER_HEURISTIC","CRITICAL_PATH_HEURISTIC","BEST_EFFORT_GPU_HEURISTIC"};
