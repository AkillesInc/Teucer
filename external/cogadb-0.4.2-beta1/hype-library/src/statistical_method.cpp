...
namespace hype{
	...
	StatisticalMethod_Internal::StatisticalMethod_Internal(const std::string& name) : name_(name){
	}
...
const boost::shared_ptr<StatisticalMethod_Internal> getNewStatisticalMethodbyName(std::string name){
	...
	StatisticalMethod_Internal* ptr = StatisticalMethodFactorySingleton::Instance().CreateObject(name); //.( 1, createProductNull );
