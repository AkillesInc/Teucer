...
namespace hype{
	...
	ProbabilityBasedOutsourcing::ProbabilityBasedOutsourcing(const std::string& name_of_operation) : OptimizationCriterion_Internal(std::string("ProbabilityBasedOutsourcing"),name_of_operation), random_number_generator_() {
		//OptimizationCriterionFactorySingleton::Instance().Register("Response Time",&ProbabilityBasedOutsourcing::create);
	}

	const SchedulingDecision ProbabilityBasedOutsourcing::getOptimalAlgorithm_internal(const Tuple& input_values, Operation& op, DeviceTypeConstraint dev_constr){
		//idea: compute a probability for each algorithm depending on its estimated execution time
		...
		std::vector<AlgorithmPtr> alg_ptrs_unfiltered = op.getAlgorithms();
                ...
                //pre filter algorithms, so only algorithms are considered that satisfy the device constraint
		for(unsigned int i=0;i<alg_ptrs_unfiltered.size();i++){
                    if(util::satisfiesDeviceConstraint(alg_ptrs_unfiltered[i]->getDeviceSpecification(), dev_constr)){
                        alg_ptrs.push_back(alg_ptrs_unfiltered[i]);
                    }
                    //
                }
                if(alg_ptrs.empty()){
                    HYPE_FATAL_ERROR("Unsatisfiable device constraint: " << util::getName(dev_constr),std::cerr);
                }
		...
		std::map<DeviceSpecification,double> map_compute_device_to_estimated_waiting_time;
		
		for(unsigned int i=0;i<alg_ptrs.size();i++){
			estimated_execution_times[i]=alg_ptrs[i]->getEstimatedExecutionTime(input_values).getTimeinNanoseconds();
		}
		
		double est_Execution_Time_SUM=std::accumulate(estimated_execution_times.begin(),estimated_execution_times.end(),double(0));
		
		for(unsigned int i=0;i<alg_ptrs.size();i++){
			probabilities[i]=1-(estimated_execution_times[i]/est_Execution_Time_SUM);
		}		
		...
		return SchedulingDecision(*optimal_algorithm_ptr,EstimatedTime(estimated_execution_times[selected_algorithm_index]),input_values);
