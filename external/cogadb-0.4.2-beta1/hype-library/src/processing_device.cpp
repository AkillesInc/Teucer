...
namespace hype{
	namespace queryprocessing{
		...
		ProcessingDevice::ProcessingDevice(const core::DeviceSpecification& dev_spec) : operators_(), new_operator_available_(), operator_queue_full_(), operator_mutex_(), thread_(),
			...
			total_measured_execution_time_for_all_executed_operators_(0),
                        ...
                        device_operator_queue_(DeviceOperatorQueues::instance().getDeviceOperatorQueue(dev_spec.getMemoryID()))
		{
		...
		{	/* \todo this method has to wait, since it is not meaningful to store the whole workload (since we don't have meaningful estimations at the beginning of the application)*/
			assert(op!=NULL);
...
#else				//notify that operator has finished and that a new one can be added to the operator queue
		...
		double hype::queryprocessing::ProcessingDevice::getTotalProcessingTime() const
		{
			...
			return total_measured_execution_time_for_all_executed_operators_;
                ...
                ProcessingDeviceID hype::queryprocessing::ProcessingDevice::getProcessingDeviceID() const throw(){
                    return proc_dev_id_;
                }
                
		ProcessingDevicePtr getProcessingDevice(const hype::core::DeviceSpecification& dev)
		{
...
//                        core::Scheduler::ProcessingDevices::PhysicalDevices::iterator phy_it;
//                        if(!(*it)->second.second.empty()) return *((*it)->second.second.begin());
...
//                    }
                    ...
                    return core::Scheduler::instance().getProcessingDevices().getProcessingDevice(dev.getProcessingDeviceID());
