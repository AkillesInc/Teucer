...
namespace hype{
	namespace core{
		...
		Scheduler::Scheduler() : map_operationname_to_operation_(), 
                                          map_statisticalmethodname_to_statisticalmethod_(),
                                          proc_devs_(),
                                          dma_cpu_cp_(hype::PD_DMA0,hype::DMA, 
                                                      hype::PD_NO_Memory),
                                          dma_cp_cpu_(hype::PD_DMA1,hype::DMA, 
                                                      hype::PD_NO_Memory),
                                          global_operator_stream_(),
                                          operator_stream_mutex_(),
                                          scheduling_thread_mutex_(), 
                                          scheduling_thread_cond_var_(),
                                          scheduling_thread_(new boost::thread(boost::bind(&Scheduler::scheduling_thread, this)))
                {
			...
			PluginLoader::loadPlugins();
                        ...
                        {
                        AlgorithmSpecification copy_operator("COPY_CPU_CP",
                                                             "COPY_CPU_CP",
                        ...
                        AlgorithmSpecification copy_operator("COPY_CP_CPU",
                                                             "COPY_CP_CPU",
                                                             hype::Least_Squares_1D,
                                                             hype::Periodic,
                                                             hype::ResponseTime);
                        ...
                        {
                        AlgorithmSpecification copy_operator("COPY_CP_CP",
                                                             "COPY_CP_CP",
                                                             hype::Least_Squares_1D,
                                                             hype::Periodic,
                                                             hype::ResponseTime);
...
//                        //DUMMY Operator doing nothing, e.g., the root node of a MonetDB MAL plan
//                        {
//                        AlgorithmSpecification dummy_operator("DUMMY_ALGORITHM",
//                                                             "DUMMY_OPERATOR",
//                                                             hype::Least_Squares_1D,
//                                                             hype::No_Recomputation, //we do not want any overhead for this
//                                                             hype::ResponseTime);
//                        //We currently assume that the DMA controller for copying 
//                        //data from CP to CPU also always performs copies CP to CP
//                        /* \todo Verify!*/
//                        /*FIXME: DUMMY OPERATOR has ZERO execution cost, we 
//                         * just assign it to a DMA Controlelr for now (which is 
...
//                        }
		...
		bool Scheduler::addAlgorithm(const AlgorithmSpecification& alg_spec, const DeviceSpecification& dev_spec){	
			...
			std::string internal_algorithm_name=hype::util::toInternalAlgName(alg_spec.getAlgorithmName(),dev_spec);
                        
			if(!this->proc_devs_.exists(dev_spec)){
				if(!this->proc_devs_.addDevice(dev_spec)){
					std::cout << "FATAL ERROR! Failed to add Processing Device while creating Algorithm '" << alg_spec.getAlgorithmName() << "'" << std::endl;
				}
			}
                        if(!DeviceMemories::instance().existDeviceMemory(dev_spec.getMemoryID())){
                            if(!DeviceMemories::instance().addDeviceMemory(dev_spec.getMemoryID(), dev_spec.getTotalMemoryCapacity())){
                                HYPE_WARNING("Failed to add DeviceMemory while creating Algorithm '" << alg_spec.getAlgorithmName() << "'",std::cerr);
                            }
                        }
			...
			return op->addAlgorithm(internal_algorithm_name, dev_spec, alg_spec.getStatisticalMethodName(), alg_spec.getRecomputationHeuristicName());
		}
	...
	/*
		...
		bool Scheduler::setOptimizationCriterion(const std::string& name_of_operation, const std::string& name_of_optimization_criterion){
...
//			StatisticalMethodMap::iterator it = map_statisticalmethodname_to_statisticalmethod_.find(name_of_operation);
			...
			boost::shared_ptr<OptimizationCriterion_Internal> opt_crit = getNewOptimizationCriterionbyName(name_of_optimization_criterion);
			if(opt_crit){
				//StatisticalMethodMap::iterator it = map_statisticalmethodname_to_statisticalmethod_.find(name_of_operation);
		...
		bool Scheduler::setRecomputationHeuristic(const std::string& name_of_algorithm, const std::string& name_of_recomputation_strategy){
	
			boost::lock_guard<boost::mutex> lock(global_mutex);
			//const std::string name_of_algorithm(hype::util::toInternalAlgName(sched_dec.getNameofChoosenAlgorithm(),sched_dec.getDeviceSpecification())); 
			AlgorithmPtr alg_ptr = this->getAlgorithm(name_of_algorithm);
			if(!alg_ptr){
				cout << "Error in Scheduler::setRecomputationHeuristic(): Algorithm " << name_of_algorithm << " not found!" << endl;
				return false;
			}
			boost::shared_ptr<RecomputationHeuristic_Internal> recomp_heuristic = getNewRecomputationHeuristicbyName(name_of_recomputation_strategy);
		...
		const SchedulingDecision Scheduler::getOptimalAlgorithm(const OperatorSpecification& op_spec, const DeviceConstraint& dev_constr){
			...
			if(it==map_operationname_to_operation_.end()){ //operation does not exist
                ...
                void advanceAlgorithmIDs(vector<int>& current_algorithm_ids, const vector<int>& number_of_algorithms, int begin_index = 0){
                ...
                void addSchedulingDecisionVectorToLoadTracker(SchedulingDecisionVectorPtr phy_plan){
                    for(unsigned int i=0;i<phy_plan->size();++i){
                        core::Scheduler::instance().getProcessingDevices().addSchedulingDecision((*phy_plan)[i]); 
                    }
                }
                
                
		const SchedulingDecisionVectorPtr Scheduler::getOptimalAlgorithm(const OperatorSequence& op_seq, const QueryOptimizationHeuristic& heuristic){
                    
                    SchedulingDecisionVectorPtr result_plan(new SchedulingDecisionVector()); // (new SchedulingDecisionVector(op_seq.size()));
                    
                    if(heuristic==GREEDY_HEURISTIC){
                        for(unsigned int i=0;i<op_seq.size();++i){
                            SchedulingDecision sched_dec = this->getOptimalAlgorithm(op_seq[i].first, op_seq[i].second);
                        ...
                        vector<vector<AlgorithmPtr> > available_algorithms_per_operator(op_seq.size());
                        for(unsigned int i=0;i<op_seq.size();++i){
                            MapNameToOperation::iterator it = this->map_operationname_to_operation_.find(op_seq[i].first.getOperatorName());
                            ...
                            for(unsigned int j=0;j<alg_ptrs.size();++j){
                                if(util::satisfiesDeviceConstraint(alg_ptrs[j]->getDeviceSpecification(), op_seq[i].second.getDeviceTypeConstraint())){
...
//                                  || (op_seq[i].second.getDeviceTypeConstraint()==hype::CPU_ONLY && alg_ptrs[j]->getDeviceSpecification().getDeviceType()==hype::CPU) 
...
//                                }
                        ...
                        for(unsigned int i=0;i<number_of_plans;++i){
                            ...
                            for(unsigned int j=0;j<op_seq.size();++j){
                                ...
                                //fetch algorithm
                                AlgorithmPtr alg_ptr = available_algorithms_per_operator[j][current_algorithm_ids[j]];
                                //compute cost
                                double estimated_time = alg_ptr->getEstimatedExecutionTime(op_seq[j].first.getFeatureVector()).getTimeinNanoseconds();
                                //current_plan.push_back(make_pair(alg_ptr,estimated_time));
                                InternalPhysicalOperator phy_op(alg_ptr,op_seq[j].first.getFeatureVector(),estimated_time);
                                ...
                                //cout << current_plan.back().first->getName() << ": " << estimated_time/(1000*1000) << "ms" << endl;
                            }
                            //compute cost
...
//                            for(unsigned int j=0;j<op_seq.size();++j){
//                                if(current_algorithm_ids[j] < number_of_algorithms[j]){
...
//                            cout << "feature Vector Size: " << op_seq[i].first.getFeatureVector().size() << endl;
//                            result_plan->push_back(SchedulingDecision(*(optimal_plan[i].alg_ptr), hype::core::EstimatedTime(optimal_plan[i].cost), op_seq[i].first.getFeatureVector()));
////                            SchedulingDecision s(*(optimal_plan[i].first), hype::core::EstimatedTime(optimal_plan[i].second), op_seq[i].first.getFeatureVector());
...
//                        }
                        ...
                        for(unsigned int i=0;i<optimal_plan.size();++i){
                            //create SchedulingDecision for this algorithm
//                            cout << "feature Vector Size: " << op_seq[i].first.getFeatureVector().size() << endl;
                            result_plan->push_back(SchedulingDecision(*(optimal_plan[i].alg_ptr), hype::core::EstimatedTime(optimal_plan[i].cost), optimal_plan[i].feature_vector));
//                            SchedulingDecision s(*(optimal_plan[i].first), hype::core::EstimatedTime(optimal_plan[i].second), op_seq[i].first.getFeatureVector());
//                            result_plan->push_back(s);
                        }
...
//                        
//                                                
//                        QEP_Node(const QEP_Node& other) : op_spec (other.op_spec), 
...
//		}
		...
		bool Scheduler::addObservation(const SchedulingDecision& sched_dec, const double& measured_execution_time){
			...
			if(mem_chunk){
                            DeviceMemoryPtr dev_mem = DeviceMemories::instance().getDeviceMemory(mem_chunk->mem_id);
                            dev_mem->releaseMemory(mem_chunk);
                        }
                            
                        //Scheduling decisions return the external algorithm name, so to compute the internal algorithm name, we need the external name and the device 
                        //specification to get a unique string for the algorithm associated with the processing device
			const std::string name_of_algorithm(hype::util::toInternalAlgName(sched_dec.getNameofChoosenAlgorithm(),sched_dec.getDeviceSpecification())); 
...
//                        std::cout << "Operation: '" << it->second->getName() << "'" << std::endl;
                ...
                void Scheduler::setGlobalLoadAdaptionPolicy(RecomputationHeuristic recomp_heur){
                    boost::lock_guard<boost::mutex> lock(global_mutex);
                    MapNameToOperation::iterator it;
                    for(it=map_operationname_to_operation_.begin();it!=map_operationname_to_operation_.end();++it){
                        ...
                        for (unsigned int i=0;i<algs.size();++i){
                            algs[i]->setRecomputationHeuristic(getNewRecomputationHeuristicbyName(name_of_recomputation_strategy));
                            //std::cout << algs[i]->toString(1) << endl; //algs[i]->getName() << endl;
                ...
                bool Scheduler::registerMemoryCostModel(const AlgorithmSpecification& alg_spec, const DeviceSpecification& dev_spec, MemoryCostModelFuncPtr memory_cost_model){
			boost::lock_guard<boost::mutex> lock(global_mutex);
			...
			return true;
                ...
                void Scheduler::addIntoGlobalOperatorStream(const std::list<queryprocessing::NodePtr>& operator_list){
                    	//boost::lock_guard<boost::mutex> lock(global_mutex);
                        {   //boost::lock_guard<boost::mutex> lock(global_mutex);
                            boost::lock_guard<boost::mutex> lock2(this->operator_stream_mutex_);
                ...
                const std::list<std::pair<std::string,double> > Scheduler::getAverageEstimationErrors() const{
                    boost::lock_guard<boost::mutex> lock(global_mutex);
...
//                        std::cout << "Operation: '" << it->second->getName() << "'" << std::endl;
                        ...
                        std::vector<AlgorithmPtr> algs = it->second->getAlgorithms();
                        for (unsigned int i=0;i<algs.size();++i){
                            result.push_back(std::make_pair(algs[i]->getName(), algs[i]->getAlgorithmStatistics().getAverageRelativeError())); // << endl; //algs[i]->getName() << endl;
                             
                        }
                    }
                    return result;
                }
                const std::list<std::pair<std::string,double> > Scheduler::getTotalProcessorTimes() const{
                    boost::lock_guard<boost::mutex> lock(global_mutex);
...
//                        }
//                        {
//                            boost::lock_guard<boost::mutex> lock(this->operator_stream_mutex_);
...
//                                if (logical_operator->getLeft()){ //if left child is valid (has to be by convention!), add input data size
//                                    t.push_back(logical_operator->getLeft()->getOutputResultSize());
//                                    if (logical_operator->getRight()) { //if right child is valid (not null), add input data size for it as well
...
//                                //we purposefully break the chain if one algorithm is much faster than the other
//                                SchedulingDecision sched_dec_cpu_only = hype::Scheduler::instance().getOptimalAlgorithm(op_spec, hype::CPU_ONLY);
//                                SchedulingDecision sched_dec_gpu_only = hype::Scheduler::instance().getOptimalAlgorithm(op_spec, logical_operator->getDeviceConstraint()); //hype::GPU_ONLY);
//                                if(sched_dec_gpu_only.getDeviceSpecification().getDeviceType()==CPU){
//                                    //not enough memory in GPU 
//                                        op = logical_operator->getPhysicalOperator(sched_dec_cpu_only);
//                                        sched_dec=new SchedulingDecision(sched_dec_cpu_only);
...
//                                        op = logical_operator->getPhysicalOperator(sched_dec_gpu_only);
//                                        sched_dec=new SchedulingDecision(sched_dec_gpu_only);
...
//                                }
                                ...
                                queryprocessing::ProcessingDevicePtr proc_dev = this->proc_devs_.getProcessingDevice(op->getDeviceSpecification().getProcessingDeviceID());
                                queryprocessing::VirtualProcessingDevicePtr virt_proc_dev = this->proc_devs_.getVirtualProcessingDevice(op->getDeviceSpecification().getProcessingDeviceID());
                                assert(proc_dev!=NULL);
                                assert(virt_proc_dev!=NULL);
                                
                                if(!hype::core::quiet && hype::core::verbose && hype::core::debug)
                                std::cout << "Scheduling Thread: schedule operator " << logical_operator->toString(true)
                                          << " to device " << (int) op->getDeviceSpecification().getProcessingDeviceID() 
                                          << " (" << util::getName(op->getDeviceSpecification().getDeviceType()) << ")" << std::endl;
                                
                                proc_dev->addOperator(op);
                                virt_proc_dev->addRunningOperation(sched_dec);
#else
                                DeviceOperatorQueuePtr dev_op_queue = DeviceOperatorQueues::instance().getDeviceOperatorQueue(sched_dec.getDeviceSpecification().getMemoryID());
                                ...
                                if(!hype::core::quiet && hype::core::verbose && hype::core::debug)
                                std::cout << "Scheduling Thread: schedule operator " << logical_operator->toString(true)
                                          << " to device group " << (int) op->getDeviceSpecification().getMemoryID() 
                                          << " (Devices attached to memory " << (int)op->getDeviceSpecification().getMemoryID() << ")" << std::endl;
#endif
               ...
               DeviceSpecification& Scheduler::getDeviceSpecificationforCopyType(const std::string& copy_type){
                   if(copy_type=="COPY_CPU_CP"){
                       ...
                       return this->dma_cp_cpu_;
                        ...
                        HYPE_FATAL_ERROR(std::string("INVALID COPY OPERATION TYPE: ")+copy_type,std::cout);
                        return this->dma_cp_cpu_;
                   }
               }
               

                void Scheduler::insertCopyOperationAtBegin(SchedulingDecisionVectorPtr result_plan, const SchedulingDecision& first_operator){
                    if(first_operator.getDeviceSpecification().getMemoryID()!=hype::PD_Memory_0){
                        std::string copy_type = util::getCopyOperationType(hype::PD0,first_operator.getDeviceSpecification().getProcessingDeviceID());
                ...
                void Scheduler::insertCopyOperationAtBegin(InternalPhysicalPlan& result_plan, const InternalPhysicalOperator& first_operator){
                    if(first_operator.alg_ptr->getDeviceSpecification().getMemoryID()!=hype::PD_Memory_0){
                        std::string copy_type = util::getCopyOperationType(hype::PD0,first_operator.alg_ptr->getDeviceSpecification().getProcessingDeviceID());
                        assert(!copy_type.empty());
                        DeviceSpecification dev_spec = getDeviceSpecificationforCopyType(copy_type);
                        AlgorithmPtr copy_alg_ptr = this->getAlgorithm(util::toInternalAlgName(copy_type,dev_spec ));
                        assert(copy_alg_ptr!=NULL);
                ...
                void Scheduler::insertCopyOperationAtEnd(SchedulingDecisionVectorPtr result_plan, const SchedulingDecision& last_operator){
                    if(last_operator.getDeviceSpecification().getMemoryID()!=hype::PD_Memory_0){
                        //cout << "Insert COPY Operation to transfer result to CPU" << endl;
                        std::string copy_type = util::getCopyOperationType(last_operator.getDeviceSpecification().getProcessingDeviceID(),hype::PD0); //assume that PD0 is CPU!
                        assert(!copy_type.empty());
                        DeviceSpecification dev_spec = getDeviceSpecificationforCopyType(copy_type);
                        AlgorithmPtr copy_alg_ptr = this->getAlgorithm(util::toInternalAlgName(copy_type,dev_spec ));
                        assert(copy_alg_ptr!=NULL);
                ...
                void Scheduler::insertCopyOperationAtEnd(InternalPhysicalPlan& result_plan, const InternalPhysicalOperator& last_operator){
                ...
                void Scheduler::insertCopyOperationInsidePlan(SchedulingDecisionVectorPtr result_plan, const SchedulingDecision& current_operator){
                    if(!util::isCopyOperation(result_plan->back())){
                        if(current_operator.getDeviceSpecification().getMemoryID()!=result_plan->back().getDeviceSpecification().getMemoryID()){

                            std::string copy_type = util::getCopyOperationType(result_plan->back().getDeviceSpecification().getProcessingDeviceID(),current_operator.getDeviceSpecification().getProcessingDeviceID());
                            assert(!copy_type.empty());
                            DeviceSpecification dev_spec = getDeviceSpecificationforCopyType(copy_type);
                            AlgorithmPtr copy_alg_ptr = this->getAlgorithm(util::toInternalAlgName(copy_type,dev_spec ));
                            assert(copy_alg_ptr!=NULL);
                ...
                void Scheduler::insertCopyOperationInsidePlan(InternalPhysicalPlan& result_plan, const InternalPhysicalOperator& current_operator){
                    if(!util::isCopyOperation(result_plan.back().alg_ptr->getName())){
                        if(current_operator.alg_ptr->getDeviceSpecification().getMemoryID()!=result_plan.back().alg_ptr->getDeviceSpecification().getMemoryID()){

                            std::string copy_type = util::getCopyOperationType(result_plan.back().alg_ptr->getDeviceSpecification().getProcessingDeviceID(),current_operator.alg_ptr->getDeviceSpecification().getProcessingDeviceID());
                            assert(!copy_type.empty());
                            DeviceSpecification dev_spec = getDeviceSpecificationforCopyType(copy_type);
                            AlgorithmPtr copy_alg_ptr = this->getAlgorithm(util::toInternalAlgName(copy_type,dev_spec ));
                            assert(copy_alg_ptr!=NULL);
...
Scheduler::ProcessingDevices::ProcessingDevices() : virt_comp_devs_(), processing_device_mutex_(){}
...
//        Devices virt_comp_devs_ = core::Scheduler::instance().getProcessingDevices().getDevices();
        Devices::iterator it;
        ...
        typedef std::pair<queryprocessing::VirtualProcessingDevicePtr,PhysicalDevices> LogicalDevice;
...
ProcessingDeviceID Scheduler::ProcessingDevices::getProcessingDeviceID(ProcessingDeviceMemoryID mem_id) const{
    ...
    HYPE_FATAL_ERROR("Found no Processor with memory ID " << (int) mem_id << "!", std::cerr);
    return hype::PD0;
}

const std::list<std::pair<std::string,double> > Scheduler::ProcessingDevices::getTotalProcessorTimes() const{
    std::list<std::pair<std::string,double> > result;
	...
	Devices::const_iterator it;
        ...
        for(it=virt_comp_devs_.begin();it!=virt_comp_devs_.end();++it){
            ...
            for(phy_it=it->second.second.begin();phy_it!=it->second.second.end();++phy_it){
//                (*phy_it)->stop();
                std::string processor_name = util::getName(it->second.first->getDeviceSpecification().getDeviceType())+boost::lexical_cast<std::string>((int)(*phy_it)->getProcessingDeviceID());
                result.push_back(std::make_pair(processor_name,(*phy_it)->getTotalProcessingTime()));
            }
        }  
...
//        VirtualProcessingDevicePtr virt_dev = cit->second; //this->getVirtualProcessingDevice(it->second->getProcessingDeviceID());
//        assert(virt_dev!=NULL);
//        std::string processor_name = util::getName(virt_dev->getDeviceSpecification().getDeviceType())+boost::lexical_cast<std::string>((int)it->second->getProcessingDeviceID());
//        result.push_back(std::make_pair(processor_name,it->second->getTotalProcessingTime()));
//    }
    return result;
}

VirtualProcessingDevicePtr Scheduler::ProcessingDevices::getVirtualProcessingDevice(ProcessingDeviceID dev_id)
{
			Devices::iterator it;
...
queryprocessing::ProcessingDevicePtr Scheduler::ProcessingDevices::getProcessingDevice(ProcessingDeviceID dev_id){
    
                    core::Scheduler::ProcessingDevices::Devices virt_comp_devs_ = core::Scheduler::instance().getProcessingDevices().getDevices();
...
//				std::cout << "FATAL ERROR: Processing Device with ID " << dev_id <<  " does not exist!!!" << std::endl;
//				return queryprocessing::ProcessingDevicePtr(); //return NULL Pointer //exit(-1);
//			}
//			
...
bool Scheduler::ProcessingDevices::addSchedulingDecision(const SchedulingDecision& sched_dec)
{
				 ...
				 VirtualProcessingDevicePtr vir_proc_dev = this->getVirtualProcessingDevice(dev_id);
				 if(!vir_proc_dev){
				 ...
				 return vir_proc_dev->removeFinishedOperation(sched_dec);
					...
					void Scheduler::ProcessingDevices::print() const throw(){
						...
						for(cit= virt_comp_devs_.begin();cit!= virt_comp_devs_.end();++cit){
                                                    ...
                                                    PhysicalDevices::const_iterator phy_it;
                                                    for(phy_it=cit->second.second.begin();phy_it!=cit->second.second.end();++phy_it){
                                                        std::cout << "\t" << "Total Processing Time: " << (*phy_it)->getTotalProcessingTime()/(1000*1000*1000) << "s" << std::endl;
                                                    }
						}
                                                
//						for(cit= virt_comp_devs_.begin(), cit_phy=phy_comp_devs_.begin();cit!= virt_comp_devs_.end();++cit,++cit_phy){
//							cit->second.first->print();	
//                                                        std::cout << "\t" << "Total Processing Time: " << cit_phy->second->getTotalProcessingTime()/(1000*1000*1000) << "s" << std::endl;
//						}
					}		
