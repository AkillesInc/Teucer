...
namespace hype
{
	namespace core
	{
		...
		ResponseTime::ResponseTime(const std::string& name_of_operation) : OptimizationCriterion_Internal(std::string("Response Time"),name_of_operation)
		{
				...
				if( util::satisfiesDeviceConstraint(alg_ptrs[i]->getDeviceSpecification(), dev_constr) )
					if(t_est<min_time) {
                        ...
                        if(optimal_algorithm==NULL){
                            ...
                            HYPE_WARNING("Unsatisfyable Device Constrained! Falling back to CPU operator for operation" 
                                    << op.getName(), std::cout);
                            min_time=std::numeric_limits<double>::max();
                            for(unsigned int i=0; i<alg_ptrs.size(); ++i) {
                                    double t_est = alg_ptrs[i]->getEstimatedExecutionTime(input_values).getTimeinNanoseconds();
                                    if( util::satisfiesDeviceConstraint(alg_ptrs[i]->getDeviceSpecification(), hype::CPU_ONLY) )
                                            if(t_est<min_time) {
                                                    min_time=t_est;
                                                    optimal_algorithm=alg_ptrs[i];
                                            }
...
//					}
                                        ...
                                        if(!quiet && verbose && debug){
                                            std::cout << "SRT: Algorithm " << alg_ptrs[i]->getName() 
                        ...
                        if(optimal_algorithm && optimal_algorithm->getDeviceSpecification().getDeviceType()==hype::GPU){
                            if(Runtime_Configuration::instance().getTrackMemoryUsage()){
                                ...
                                DeviceMemoryPtr dev_mem = DeviceMemories::instance().getDeviceMemory(optimal_algorithm->getDeviceSpecification().getMemoryID());
                                mem_chunk = dev_mem->allocateMemory(optimal_algorithm->getEstimatedRequiredMemoryCapacity(input_values));
			...
			return SchedulingDecision(*optimal_algorithm,EstimatedTime(min_time),input_values,mem_chunk);
