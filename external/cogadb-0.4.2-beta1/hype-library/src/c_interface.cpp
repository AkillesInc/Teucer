...
extern "C" {
    ...
    int hype_setStatisticalMethod(const char* name_of_algorithm,
         ...
         if(!dev_spec) return int(false);
         DeviceSpecification* dev_spec_ptr = static_cast<DeviceSpecification*> (dev_spec->ptr);
         return int(Scheduler::instance().setStatisticalMethod(name_of_algorithm,*dev_spec_ptr,name_of_statistical_method));
    }
    ...
    int hype_setRecomputationHeuristic(const char* name_of_algorithm,
         ...
         if(!dev_spec) return int(false);
         DeviceSpecification* dev_spec_ptr = static_cast<DeviceSpecification*> (dev_spec->ptr);
         return int(Scheduler::instance().setRecomputationHeuristic(name_of_algorithm,*dev_spec_ptr,name_of_recomputation_strategy));
    ...
    C_SchedulingDecision* hype_getOptimalAlgorithm(const C_OperatorSpecification* op_spec, const C_DeviceConstraint* dev_constr) {
        ...
        SchedulingDecision sched_dec = Scheduler::instance().getOptimalAlgorithm(*op_ptr,*dev_ptr); //,*dev_ptr);  
    ...
    double hype_getEstimatedExecutionTime(const C_OperatorSpecification* op_spec, const char* alg_name, C_DeviceSpecification* dev_spec) {
        ...
        DeviceSpecification* dev_spec_ptr = static_cast<DeviceSpecification*> (dev_spec->ptr);
        OperatorSpecification* ptr = static_cast<OperatorSpecification*> (op_spec->ptr);
        return Scheduler::instance().getEstimatedExecutionTime(*ptr,alg_name,*dev_spec_ptr).getTimeinNanoseconds();
    }
