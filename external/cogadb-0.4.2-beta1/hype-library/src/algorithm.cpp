...
//void serialize(Archive & ar, boost::circular_buffer<BufferType> & circ_buf, const unsigned int version)
//{
//    ar & circ_buf.size();
//    for(unsigned )
//    ar & g.minutes;
...
//}
			...
			Algorithm::Algorithm(const std::string& name_of_algorithm, 
                        ...
                        statistics_(AlgorithmStatisticsManager::instance().getAlgorithmStatistics(comp_dev, name_of_algorithm)),
                        ...
                        mem_cost_model_func_ptr_(NULL)
			{
				...
				//getNewStatisticalMethodbyName(name_of_statistical_method);
                                ...
                                ss << Runtime_Configuration::instance().getOutputDirectory() 
                                   << "/" << this->operation_.getName() << "/" 
                                   << this->name_ << ".performance_model";
                                ...
                                if(Runtime_Configuration::instance().getStorePerformanceModels()){
                                    ...
                                    if(this->statistics_->loadFeatureVectors(path)){
                                        //ok, found obsevations of prior executions, compute initial approximation function!
			...
			bool Algorithm::setStatisticalMethod(boost::shared_ptr<StatisticalMethod_Internal> ptr_statistical_method){
			...
			bool Algorithm::setRecomputationHeuristic(boost::shared_ptr<RecomputationHeuristic_Internal> ptr_recomputation_heristic){
			...
			bool Algorithm::addMeasurementPair(const MeasurementPair& mp){
				...
				this->statistics_->total_execution_time_+=mp.getMeasuredTime().getTimeinNanoseconds();
                                ...
                                //estimation error is typically very large, but this means no harm at all, we rather
                                //have accurate estimates over a large interval!
                                if(mp.getEstimatedTime().getTimeinNanoseconds()!=-1 && mp.getMeasuredTime().getTimeinNanoseconds() > 1000*1000){
                                     relative_error=(mp.getMeasuredTime().getTimeinNanoseconds()-mp.getEstimatedTime().getTimeinNanoseconds())/mp.getEstimatedTime().getTimeinNanoseconds();
                                }
...
//				(measured_time.getTimeinNanoseconds()-scheduling_decision_.getEstimatedExecutionTimeforAlgorithm().getTimeinNanoseconds())/scheduling_decision_.getEstimatedExecutionTimeforAlgorithm().getTimeinNanoseconds() << "%" << endl; 
				...
				if(is_in_retraining_phase_){
					...
					if(Runtime_Configuration::instance().getRetrainingLength()>=retraining_length_++ || !this->ptr_statistical_method_->inTrainingPhase()){
						is_in_retraining_phase_=false;
				...
				return statistics_->executionHistory_.addMeasurementPair(mp);
                        ...
                        std::string  Algorithm::toString(unsigned int indent) const{
                            ...
                            ss << indent_str << "Algorithm: " << this->name_ << " (Operation: " << this->operation_.getName() << ")" << std::endl;
                            ss << indent_str << "StatisticalMethod: " << this->ptr_statistical_method_->getName() << std::endl;
                            ss << indent_str << "Recomputation Heuristic: " << this->ptr_recomputation_heristic_->getName() << std::endl;
...
//                            if(this->name_=="CPU_Groupby_Algorithm" || this->name_=="GPU_Groupby_Algorithm"){
//                                std::cout << "GET ESTIMATION FROM KNN REGRESSOR!" << std::endl;
//                                return ptr_statistical_method_->computeEstimation(input_values);
//                            }
                                if(ptr_statistical_method_->getName()=="KNN_Regression"){
                                    boost::shared_ptr<KNN_Regression> ptr = boost::dynamic_pointer_cast<KNN_Regression>(ptr_statistical_method_);
                                    if(ptr){
				...
				return ptr_statistical_method_->computeEstimation(input_values);
                        ...
                        double Algorithm::getEstimatedRequiredMemoryCapacity(const Tuple& input_values){
		...
		const DeviceSpecification Algorithm::getDeviceSpecification() const throw(){
			return comp_dev_;
                ...
                void Algorithm::setMemoryCostModel(MemoryCostModelFuncPtr mem_cost_model_func_ptr){
