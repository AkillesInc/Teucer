...
namespace hype
{
	namespace core{
			...
			double Report::getRelativeEstimationError(const std::string& algorithm_name,
                                                                     const DeviceSpecification& dev_spec) 
                                                                     const throw(std::invalid_argument){	

                        
                                std::string internal_algorithm_name=hype::util::toInternalAlgName(algorithm_name,dev_spec);
                        
				AlgorithmPtr ptr = Scheduler::instance().getAlgorithm(internal_algorithm_name);
				if(!ptr){
				...
				for(unsigned int i=length_of_initial_training_phase+1;i<ptr->getAlgorithmStatistics().relative_errors_.size();i++){
					sum+=std::abs(ptr->getAlgorithmStatistics().relative_errors_[i]);
					//std::cout << "sum " << sum << " " << ptr->getAlgorithmStatistics().relative_errors_[i] << std::endl;
				}
