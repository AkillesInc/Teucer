	...
	namespace queryprocessing
	{
		...
		bool Operator::operator() ()
		{
			...
			if(!hype::core::quiet && hype::core::verbose && hype::core::debug)
                        {
				std::cout << "[Operator] Executing "
                                          << this->getAlgorithmName();
                                if(this->logical_operator_){
                                    std::cout << " for logical operator: " << this->logical_operator_->toString(1);
                                }
                                std::cout << std::endl;
#ifdef HYPE_ENABLE_INTERACTION_WITH_COGADB
                            double used_memory_cpu_in_gb = double(CoGaDB::getUsedMainMemoryInBytes())/(1024*1024*1024);
                            double free_memory_gpu_in_gb=double(CoGaDB::HardwareDetector::instance().getFreeMemorySizeInByte(hype::PD_Memory_1))/(1024*1024*1024);                          
                            double total_memory_gpu_in_gb=double(CoGaDB::HardwareDetector::instance().getTotalMemorySizeInByte(hype::PD_Memory_1))/(1024*1024*1024);
                            std::cout << "\t Current Memory CPU: " 
                                << used_memory_cpu_in_gb << "GiB )" << std::endl;
                            std::cout << "\t Current Memory GPU: " 
                                << (total_memory_gpu_in_gb-free_memory_gpu_in_gb) << "GiB )" << std::endl;
#endif
                            std::cout << std::endl;
                        }
                        ...
                        if(!util::isCoprocessor(this->sched_dec_.getDeviceSpecification().getProcessingDeviceID())){
                            ...
                            try{
                                ...
                                std::cout << "Catched bad alloc for operator " << this->logical_operator_->toString(true) << std::endl;
                                ret = false;
                            }
                            if(ret){
                                alg_measure.afterAlgorithmExecution();
                            }else{
                                HYPE_FATAL_ERROR("A Non Co-Processor Operator Aborted, which is forbidden!" << std::endl
                                              << "Operator: " << this->logical_operator_->toString(true) << std::endl
                                              << "Physical Operator: " << this->getAlgorithmName(),std::cerr);
                            }                            
                        }else{
                            ...
                            assert(util::isCoprocessor(this->sched_dec_.getDeviceSpecification().getProcessingDeviceID())==true);
                            ...
                            if(ret){
                                ...
                                OperatorSpecification op_spec(this->logical_operator_->getOperationName(), this->logical_operator_->getFeatureVector(), hype::PD_Memory_0, hype::PD_Memory_0);

                                HYPE_WARNING("Operator '" << this->logical_operator_->toString(true) << "' running on processor " << (int)sched_dec_.getDeviceSpecification().getProcessingDeviceID() << " aborted: Starting Fallback operator on CPU..." 
#ifdef HYPE_ENABLE_INTERACTION_WITH_COGADB
                                        << " (Current Memory: " << double(CoGaDB::getUsedMainMemoryInBytes())/(1024*1024*1024) << "GiB )"
#endif
                                        ,std::cout);
                                SchedulingDecision cpu_sched_dec=core::Scheduler::instance().getOptimalAlgorithm(op_spec, hype::CPU_ONLY);
...
//                        }
...
#ifdef HYPE_ENABLE_INTERACTION_WITH_COGADB
				    std::cout << " (Current Memory: " << double(CoGaDB::getUsedMainMemoryInBytes())/(1024*1024*1024) << "GiB )" << std::endl;
#endif                                 
		...
		const hype::core::SchedulingDecision& Operator::getSchedulingDecision() const throw(){
		...
		Operator::Operator(const hype::SchedulingDecision& sched_dec) : sched_dec_(sched_dec), logical_operator_(), timeNeeded(0), timeEstimated(0), result_size_(0), has_aborted_(false), start_timestamp_(0), end_timestamp_(0)
{}
               
 
                Operator::Operator(const hype::core::SchedulingDecision& sched_dec, NodePtr logical_operator) : sched_dec_(sched_dec), logical_operator_(logical_operator), timeNeeded(0), timeEstimated(0), result_size_(0), has_aborted_(false), start_timestamp_(0), end_timestamp_(0)
{
...
//		}	
		...
		const core::DeviceSpecification Operator::getDeviceSpecification() const throw(){
			return sched_dec_.getDeviceSpecification();
                ...
                bool Operator::isInputDataCachedInGPU(){
                    return false;
                }
        ...
        void Operator::setResultSize(double result_size) {
            result_size_ = result_size;
        }

        double Operator::getTimeNeeded() const{
            return this->timeNeeded;
        }

        double Operator::getTimeEstimated() const{
            return this->timeEstimated;
        }

        double Operator::getResultSize() const {
            return result_size_;
        }


        uint64_t Operator::getBeginTimestamp() const{
             return this->start_timestamp_; 
	}

        uint64_t Operator::getEndTimestamp() const{
            return this->end_timestamp_;
	}

        bool Operator::hasAborted() const{
            return this->has_aborted_;
        }
