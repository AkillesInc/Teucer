...
//			StatisticalMethodMap::iterator it = map_statisticalmethodname_to_statisticalmethod_.find(name_of_operation);
		...
		const SchedulingDecision Scheduler::getOptimalAlgorithm(const OperatorSpecification& op_spec, const DeviceConstraint& dev_constr){
			...
			if(it==map_operationname_to_operation_.end()){ //operation does not exist
                ...
                typedef std::vector<std::pair<const OperatorSpecification&, const DeviceConstraint&> >  OperatorSequence;
                
                enum QueryOptimizationHeuristic {GREEDY_HEURISTIC,BACKTRACKING,TWO_COPY_HEURISTIC};
		...
		const SchedulingDecisionVectorPtr Scheduler::getOptimalAlgorithm(const OperatorSequence& op_seq, QueryOptimizationHeuristic heuristic){
                    
                    SchedulingDecisionVectorPtr result_plan (op_seq.size());
                    
                    if(heuristic==GREEDY_HEURISTIC){
                        for(unsigned int i=0;i<op_seq.size();++i){
                            result_plan[i]=this->getOptimalAlgorithm(op_seq[i].first, op_seq[i].second);
                        }
                        return result_plan;
...
Scheduler::ProcessingDevices::ProcessingDevices() : virt_comp_devs_(), phy_comp_devs_() {}
...
bool Scheduler::ProcessingDevices::addDevice(const DeviceSpecification& dev_spec)
{
	...
	this->virt_comp_devs_.insert(std::make_pair(dev_spec.getProcessingDeviceID(),
        ...
        this->phy_comp_devs_.insert(std::make_pair(dev_spec.getProcessingDeviceID(),
                                     ProcessingDevicePtr( new ProcessingDevice(dev_spec.getProcessingDeviceID())))
				    );
	return true;
}
