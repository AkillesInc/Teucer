...
namespace hype{
	namespace util{
            ...
            bool satisfiesDeviceConstraint(const DeviceSpecification& dev_spec, const DeviceConstraint& dev_constr){
                ...
                || (dev_constr==hype::FPGA_ONLY && dev_spec.getDeviceType()==hype::FPGA)    
                || (dev_constr==hype::XEON_PHI_ONLY && dev_spec.getDeviceType()==hype::XEON_PHI)
            ...
            DeviceTypeConstraint getDeviceConstraintForProcessingDeviceType(const ProcessingDeviceType& pt_type){
            ...
            bool isChainBreaker(const std::string& operator_name){
                ...
                || operator_name=="COPY_CP_CP"
                || operator_name=="CPU_COLUMN_SCAN"
                || operator_name=="INVISIBLE_JOIN"
                || operator_name=="PROJECTION"
                || operator_name=="ColumnComparatorOperation"
//                || operator_name=="COMPLEX_SELECTION"
                || operator_name=="COPY_CP_CPU"
                || operator_name=="CROSS_JOIN"
//                || operator_name=="JOIN"
                || operator_name=="RENAME"
                || operator_name=="UDF"                     
//                || operator_name=="ColumnAlgebraOperator"
                || operator_name=="COPY_CPU_CP"
                || operator_name=="SCAN"
...
//                || operator_name=="PositionList_Operator"
            ...
            bool isCPU(ProcessingDeviceID id){
                ProcessingDeviceType pd_type = core::Scheduler::instance().getProcessingDevices().getVirtualProcessingDevice(id)->getDeviceSpecification().getDeviceType();
                if(pd_type==CPU){
                    return true;
                }else{
                    return false;
            ...
            hype::ProcessingDeviceMemoryID getMemoryID(ProcessingDeviceID id){
                return core::Scheduler::instance().getProcessingDevices().getVirtualProcessingDevice(id)->getDeviceSpecification().getMemoryID();
            }
            
            ProcessingDeviceID getProcessingDeviceID(hype::ProcessingDeviceMemoryID mem_id){
                return core::Scheduler::instance().getProcessingDevices().getProcessingDeviceID(mem_id);
            }
            
            std::string getCopyOperationType(ProcessingDeviceID id1, ProcessingDeviceID id2){
                assert(id1!=id2);  
                if(isCPU(id1) && isCPU(id2)){
                    ...
                    HYPE_FATAL_ERROR("INVALID COMBINATION of PROCESSING DEVICE TYPES! MAYBE BUG IN hype::util::isCPU() OR hype::util::isCoprocessor() ?",std::cout);
                }
            }
