...
if [ $# -lt 1 ]; then
	...
	echo "Usage: $0 [AGGREGATION|SELECTION|SORT|SIMULATION]"
	exit -1
fi
...
if [[ "$OPERATION_NAME" != "SORT" && "$OPERATION_NAME" != "SELECTION" && "$OPERATION_NAME" != "AGGREGATION" && "$OPERATION_NAME" != "SIMULATION" ]]; then
	echo "First parameter has to be a valid Operation: [AGGREGATION|SELECTION|SORT|SIMULATION]"
...
#new for simulator
./eval.sh "Results/$OPERATION_NAME/$HOSTNAME-varying_number_of_processing_devices_benchmark_results.data" $OPERATION_NAME-varying_number_of_processing_devices
./eval.sh "Results/$OPERATION_NAME/$HOSTNAME-varying_relative_speed_of_processing_devices_benchmark_results.data" $OPERATION_NAME-varying_relative_speed_of_processing_devices
./eval.sh "Results/$OPERATION_NAME/$HOSTNAME-varying_number_and_relative_speed_of_processing_devices_benchmark_results.data" $OPERATION_NAME-varying_number_and_relative_speed_of_processing_devices
...
DIR=$(pwd)
for i in $OPERATION_NAME-varying_dataset_size $OPERATION_NAME-varying_number_of_operations $OPERATION_NAME-varying_number_of_datasets $OPERATION_NAME-varying_operator_queue_length $OPERATION_NAME-varying_number_of_processing_devices $OPERATION_NAME-varying_relative_speed_of_processing_devices $OPERATION_NAME-varying_number_and_relative_speed_of_processing_devices; do
    cd script_templates
 #   for j in *.plt; do
#	 
  #  done
...
#Extension for simulator start here	
#elif [ "$i" = "$OPERATION_NAME-varying_number_of_processing_devices" ]; then
#    cat execution_times_varying_number_of_processing_devices.plt  | sed 's/%OPERATION_NAME%/'"$OPERATION_NAME"'/g' > ../$i/execution_times_varying_number_of_processing_devices.plt
#    cat device_utilization_varying_number_of_processing_devices.plt  | sed 's/%OPERATION_NAME%/'"$OPERATION_NAME"'/g' > ../$i/device_utilization_varying_number_of_processing_devices.plt
#    cat average_relative_estimation_errors_varying_number_of_processing_devices.plt  | sed 's/%OPERATION_NAME%/'"$OPERATION_NAME"'/g' > ../$i/average_relative_estimation_errors_varying_number_of_processing_devices.plt

#elif [ "$i" = "$OPERATION_NAME-varying_relative_speed_of_processing_devices" ]; then
#    cat execution_times_varying_relative_speed_of_processing_devices.plt  | sed 's/%OPERATION_NAME%/'"$OPERATION_NAME"'/g' > ../$i/execution_times_varying_relative_speed_of_processing_devices.plt
#    cat device_utilization_varying_relative_speed_of_processing_devices.plt  | sed 's/%OPERATION_NAME%/'"$OPERATION_NAME"'/g' > ../$i/device_utilization_varying_relative_speed_of_processing_devices.plt
#    cat average_relative_estimation_errors_varying_relative_speed_of_processing_devices.plt  | sed 's/%OPERATION_NAME%/'"$OPERATION_NAME"'/g' > ../$i/average_relative_estimation_errors_varying_relative_speed_of_processing_devices.plt

#elif [ "$i" = "$OPERATION_NAME-varying_number_and_relative_speed_of_processing_devices" ]; then
#    cat execution_times_varying_number_and_relative_speed_of_processing_devices.plt  | sed 's/%OPERATION_NAME%/'"$OPERATION_NAME"'/g' > ../$i/execution_times_varying_number_and_relative_speed_of_processing_devices.plt
#    cat device_utilization_varying_number_and_relative_speed_of_processing_devices.plt  | sed 's/%OPERATION_NAME%/'"$OPERATION_NAME"'/g' > ../$i/device_utilization_varying_number_and_relative_speed_of_processing_devices.plt
#    cat average_relative_estimation_errors_varying_number_and_relative_speed_of_processing_devices.plt  | sed 's/%OPERATION_NAME%/'"$OPERATION_NAME"'/g' > ../$i/average_relative_estimation_errors_varying_number_and_relative_speed_of_processing_devices.plt
####################################################################################################
elif [ "$i" = "$OPERATION_NAME-varying_number_of_processing_devices" ]; then
    cat execution_times_varying_dataset_size.plt  | sed 's/%OPERATION_NAME%/'"$OPERATION_NAME"'/g' > ../$i/execution_times_varying_dataset_size.plt
    cat device_utilization_varying_dataset_size.plt  | sed 's/%OPERATION_NAME%/'"$OPERATION_NAME"'/g' > ../$i/device_utilization_varying_dataset_size.plt
...
for i in $OPERATION_NAME-varying_dataset_size $OPERATION_NAME-varying_number_of_operations $OPERATION_NAME-varying_number_of_datasets $OPERATION_NAME-varying_operator_queue_length $OPERATION_NAME-varying_number_of_processing_devices $OPERATION_NAME-varying_relative_speed_of_processing_devices $OPERATION_NAME-varying_number_and_relative_speed_of_processing_devices $OPERATION_NAME-summary; do
	cd $i
