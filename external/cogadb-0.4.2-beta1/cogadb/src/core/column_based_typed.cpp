...
//            ss.precision(3);       
    ...
    bool ColumnBaseTyped<T>::is_equal(ColumnPtr column) {
        ...
        if (column->type() != typeid(T)) {
            return false;
            //std::cout << "Fatal Error!!! Typemismatch for columns " << this->name_  << " and " << column->getName() << std::endl;
        ...
        if (column->size() != this->size()) {
            ...
            if (!quiet && debug) {
                std::cout << "Columns have different size." <<
                std::endl << "Candidate: " << column->size() <<
                " This: " << this->size() << std::endl;
            }
        ...
        shared_pointer_namespace::shared_ptr <ColumnBaseTyped<T> > typed_column = shared_pointer_namespace::static_pointer_cast<ColumnBaseTyped<T> >(
                column);

        ProcessorSpecification proc_spec(hype::PD0);
        DenseValueColumnPtr this_col = this->copyIntoDenseValueColumn(proc_spec);
        DenseValueColumnPtr that_col = typed_column->copyIntoDenseValueColumn(proc_spec);
        ...
        for (unsigned int i = 0; i < this_col->size(); i++) {
            if (!CoGaDB::is_equal(this_col_data[i], that_col_data[i])) {
                if (!quiet && debug) {
                    ...
                    std::cout << "Columnvalues in row " << i <<
                    " differ." << std::endl <<
    ...
    int ColumnBaseTyped<T>::compareValuesAtIndexes(TID id1, TID id2) {
        if ((*this)[id1] == (*this)[id2]) return 0;
        else if ((*this)[id1] < (*this)[id2]) return 1;
        else return -1;
        /*
     if(column->type()!=typeid(T)) {
        return false;
    }
    assert(id_column<column->size() && id_this_col < this->size());
//			return false;
    shared_pointer_namespace::shared_ptr<ColumnBaseTyped<T> > typed_column = shared_pointer_namespace::static_pointer_cast<ColumnBaseTyped<T> >(column); //static_cast<IntTypedColumnPtr>(column1);
    return (*this)[id_this_col]<typed_column[id_column]); */
    }

    template<class T>
    typename ColumnBaseTyped<T>::TypedVectorPtr ColumnBaseTyped<T>::copyIntoPlainVector() {
...
//            }else{
//                std::stringstream ss;
//                ss << "Cannot set Primary Key constraint: Column '" << this->getName() << "' contains duplicate keys!";
//                COGADB_ERROR(ss.str(),"");
//                return false;
//            }
        if (checkUniqueness()) {
    ...
    template<class T>
    const ExtendedColumnStatistics<T> &ColumnBaseTyped<T>::getExtendedColumnStatistics() const {
        return statistics_;
    }
    ...
    bool ColumnBaseTyped<T>::computeColumnStatistics() {
        ...
        //only CPU
        ProcessorSpecification proc_spec(hype::PD0);
        DenseValueColumnPtr dense_value_column = this->copyIntoDenseValueColumn(proc_spec);

        if (dense_value_column) {
            T *array = dense_value_column->data();
...
//                std::sort(array, array+array_size);
            ...
            if (ret) {
                if (!statistics_.store(this->path_to_column_)) {
                    COGADB_ERROR("Could not store statistics of column '"
                                 << this->name_ << "'!", "");
                }
            }
        }
    ...
    template<class T>
    bool ColumnBaseTyped<T>::setForeignKeyConstraint(const ForeignKeyConstraint &foreign_key_constraint) {
    ...
    bool ColumnBaseTyped<T>::checkReferentialIntegrity(ColumnPtr primary_key_col) {
        ...
        if (primary_key_col->type() != typeid(T)) {
            COGADB_ERROR(std::string("Check Referential Integrity Failed! Typemismatch for columns ") + this->name_ +
                         std::string(" and ") + primary_key_col->getName(), "");
            return false;
        }
//            if(primary_key_col->){
//                COGADB_ERROR(std::string("Check Referential Integrity Failed! Typemismatch for columns ") + this->name_  + std::string(" and ") + primary_key_col->getName(),"");
//                return false;
//            }
        shared_pointer_namespace::shared_ptr <ColumnBaseTyped<T> > pk_column = shared_pointer_namespace::dynamic_pointer_cast<ColumnBaseTyped<T> >(
                primary_key_col); //static_cast<IntTypedColumnPtr>(column1);
        assert(pk_column != NULL);
        if (!pk_column->hasPrimaryKeyConstraint()) {
            COGADB_ERROR(std::string("Cannot create ForeignKey constrained: referenced column '") +
                         primary_key_col->getName() + std::string("' is not a primary key! "), "");
            return false;
        }
        if (!pk_column->checkUniqueness()) {
            COGADB_ERROR(std::string("Column marked as PrimaryKey contains duplicates!: Column Name: '") +
        ...
        for (unsigned int i = 0; i < join_column_size; i++) {
            ...
            if (number_of_matches == 0) {
                std::stringstream ss;
                ss << "Cannot create ForeignKey constrained: broken reference: for value '" << (*this)[i] <<
                "'! "; // << endl;
                COGADB_ERROR(ss.str(), "");
                return false;
            }
            if (number_of_matches > 1) {
                std::stringstream ss;
                ss << "Cannot create ForeignKey constrained: referenced column '" << primary_key_col->getName() <<
                "' has duplicate value: '" << (*this)[i] << "'!";
                COGADB_ERROR(ss.str(), "");
                return false;
            }
        }
    ...
    template<class T>
    const ColumnPtr ColumnBaseTyped<T>::changeCompression(const ColumnType &col_type) const {
        ...
        if (col_type == this->getColumnType()) {
            ...
            ProcessorSpecification proc_spec(hype::PD0);
            DenseValueColumnPtr decompressed_column = this->copyIntoDenseValueColumn(proc_spec);
            ColumnPtr new_col = createColumn(this->getType(), this->getName(), this->getColumnType());
            if (!new_col) return ColumnPtr();
            shared_pointer_namespace::shared_ptr <ColumnBaseTyped<T> > new_col_typed = shared_pointer_namespace::dynamic_pointer_cast<ColumnBaseTyped<T> >(
                    new_col);
    ...
    template<class T>
    const ColumnPtr ColumnBaseTyped<T>::copy(const hype::ProcessingDeviceMemoryID &mem_id) const {
    ...
    template<class T>
    const DoubleDenseValueColumnPtr ColumnBaseTyped<T>::convertToDenseValueDoubleColumn(
        ...
        return backend->convertToDoubleDenseValueColumn(this->getName(), decompressed->data(), decompressed->size(),
                                                        proc_spec);
    }

    template<class T>
    const StringDenseValueColumnPtr ColumnBaseTyped<T>::convertToDenseValueStringColumn() const {
        ProcessorSpecification proc_spec(hype::PD0);
    ...
    template<class T>
    const ColumnPtr ColumnBaseTyped<T>::decompress(const ProcessorSpecification& proc_spec) const{
    ...
    const ColumnGroupingKeysPtr ColumnBaseTyped<T>::createColumnGroupingKeys(
            const ProcessorSpecification &proc_spec) const {
        COGADB_FATAL_ERROR(
                "Called createColumnGroupingKeys() in ColumnBaseTyped<T> for Type: " << typeid(T).name() << " !", "");
...
//    dense_column->print();
        ...
        //sort using dense value sort, and do notcopy column but instead sort in place
        return dense_column->sort(param); //, true); //true);
//    
//    
//    
...
//		return ids;
    ...
    public:
        TBB_Body_PrefixSum(std::vector<int> *y_, const std::vector<int> *x_) : sum(0), x(x_), y(y_) { }
        ...
        TBB_Body_PrefixSum(TBB_Body_PrefixSum &b, tbb::split) : x(b.x), y(b.y), sum(0) { }
    ...
    inline int TBB_Prefix_Sum(std::vector<int> &y, const std::vector<int> &x, unsigned int number_of_threads) {
        assert(y.size() == x.size());
        unsigned int chunk_size = x.size() / number_of_threads;
        TBB_Body_PrefixSum body(&y, &x);
        tbb::parallel_scan(tbb::blocked_range<int>(0, x.size(), chunk_size), body);
        return body.get_sum();
    }


    template<typename T>
    void selection_thread_set_flag_array(ColumnBaseTyped<T> *col, unsigned int thread_id, FlagsPtr flags,
                                         unsigned int number_of_threads, const T &value_for_comparison,
                                         const ValueComparator comp) {
        assert(flags->size() == col->size());
        if (!quiet) std::cout << "Using CPU for Selection (parallel mode)..." << std::endl;
        unsigned int array_size = col->size();
        unsigned int chunk_size = col->size() / number_of_threads;
        ...
        if (comp == EQUAL) {
            ...
            std::cerr << "FATAL Error! In CoGaDB::selection_thread_set_flag_array(): Invalid Value Comparator! " <<
            comp << std::endl;
        }
    ...
    template<typename T>
    void selection_thread_write_result_to_output_array(ColumnBaseTyped<T> *col, unsigned int thread_id, FlagsPtr flags,
        ...
        for (TID i = start_id; i < end_id; i++) {
            if ((*flags)[i] == 1) {
                unsigned int write_id = (*prefix_sum_array)[i] - 1;
        ...
        //if(!quiet) std::cout << "Using CPU for Selection (parallel mode)..." << std::endl;
//		unsigned int array_size=col->size();
//		for(TID i=thread_id;i<array_size;i+=number_of_threads){
//                    if((*flags)[i]==1){
//                        unsigned int write_id=(*prefix_sum_array)[i]-1;
//                        (*result_tids)[write_id]=i; //write matching TID to output buffer
//                    }
//                }
    ...
    template<class T>
    const PositionListPtr ColumnBaseTyped<T>::lock_free_parallel_selection(const boost::any &value_for_comparison,
                                                                           const ValueComparator comp,
                                                                           unsigned int number_of_threads) {
        ...
        for (unsigned int i = 0; i < number_of_threads; i++) {
            threads.add_thread(new boost::thread(
                    boost::bind(&CoGaDB::selection_thread_set_flag_array<T>, this, i, flags, number_of_threads, value,
                                comp)));
        }
        threads.join_all();
        ...
        unsigned int resul_size = prefix_sum.back();
        PositionListPtr result_tids(createPositionList(resul_size));

        for (unsigned int i = 0; i < number_of_threads; i++) {
            threads.add_thread(new boost::thread(
                    boost::bind(&CoGaDB::selection_thread_write_result_to_output_array<T>, this, i, flags, &prefix_sum,
                                result_tids, number_of_threads)));
        }
        threads.join_all();
    ...
    template<class T>
    const BitmapPtr ColumnBaseTyped<T>::bitmap_selection(const SelectionParam& param){
    ...
    template<class T>
    const PositionListPtr ColumnBaseTyped<T>::selection(const SelectionParam &param) {
    ...
    const PositionListPairPtr ColumnBaseTyped<T>::hash_join(ColumnPtr join_column_) {
        ...
        //create hash table
        HashTable hashtable;
        unsigned int hash_table_size = this->size();
        unsigned int join_column_size = join_column->size();

        assert(join_column_->size()>=this->size());                        
        
          T* join_tids_table1 = (T*) malloc(join_column_size*sizeof(T));   
          T* join_tids_table2 = (T*) malloc(join_column_size*sizeof(T));
    ...
    template<>
    const PositionListPairPtr ColumnBaseTyped<char*>::hash_join(ColumnPtr join_column_){
    ...
    const PositionListPairPtr ColumnBaseTyped<Type>::sort_merge_join(ColumnPtr join_column_) {
        ...
        shared_pointer_namespace::shared_ptr <ColumnBaseTyped<Type> > join_column = shared_pointer_namespace::static_pointer_cast<ColumnBaseTyped<Type> >(
                join_column_); //static_cast<IntTypedColumnPtr>(column1);
...
//
////				if(comp_dev==GPU){
////					std::cout << "No GPU Sort Merge Join implemented!" << std::endl;
////					exit(-1);
////				}
//
//				PositionListPtr tids_sorted_join_column1 = this->sort(ASCENDING); //doesnt change column itself, only creates lookup table
//				PositionListPtr tids_sorted_join_column2 = join_column->sort(ASCENDING);
//
//				assert(tids_sorted_join_column1!=NULL);
////				assert(tids_sorted_join_column1->first!=NULL);
////				assert(tids_sorted_join_column1->second!=NULL);
//
//				assert(tids_sorted_join_column2!=NULL);
//
//				assert(tids_sorted_join_column1->size()==this->size());
//				assert(tids_sorted_join_column2->size()==join_column->size());
//
//				TID i=0;
//				TID j=0;
//				bool end=false;
//				while(!end){
//						if(!quiet && verbose && debug){
//							std::cout << "index i: " << i << "(max: " << tids_sorted_join_column1->size() 
//							     << ") index j:" << j << "(max: " << tids_sorted_join_column2->size() << ")" << std::endl;
//						}
//						Type val = (*this)[(*tids_sorted_join_column1)[i]];
//						Type val2 = (*(join_column))[(*tids_sorted_join_column2)[j]]; //j);	
//						if(val>val2){
//							j++;
//							if(j>=join_column->size()-1){
//							  end=true;
//								
//							}
//						}else if (val<val2){
...
//								while(tmp_val==val){
//									if(i+index==this->size()-1){ index++; break;} 
//									index++;
//									tmp_val = (*this)[(*tids_sorted_join_column1)[i+index]];
//									//tmp_val = any_cast<T>(tmp_value);
//								}
//								tmp_val=val2;
//								unsigned int index2=0;
//								while(tmp_val==val2){
//				
//									if(j+index2==join_column->size()-1){ index2++; break;} 
//									index2++;
//									tmp_val = (*join_column)[(*tids_sorted_join_column2)[j+index2]];
//									//tmp_val = any_cast<T>(tmp_value);
//								}								
//								for(unsigned int k=i;k<i+index;k++){
//									for(unsigned int l=j;l<j+index2;l++){
////										join_tids.push_back(TID_Pair(tids_sorted_join_column1[k],tids_sorted_join_column2[l]));
//										join_tids->first->push_back((*tids_sorted_join_column1)[k]);
//										join_tids->second->push_back((*tids_sorted_join_column2)[l]);
//									}
//								}
//
//								i=i+index;
//								j=j+index2;
//
//								if(i>=this->size()-1 || j>=join_column->size()-1){
//									end=true;
//								}
//
//						}
//				}
    ...
    const PositionListPairPtr ColumnBaseTyped<Type>::nested_loop_join(ColumnPtr join_column_) {
        ...
        unsigned int join_column1_size = this->size();
        unsigned int join_column2_size = join_column->size();
    ...
    template<class Type>
    const PositionListPairPtr ColumnBaseTyped<Type>::radix_join(ColumnPtr join_column) {
    ...
    bool ColumnBaseTyped<T>::operator==(ColumnBase &column) {
        ColumnBaseTyped<T> *typed_column = dynamic_cast<ColumnBaseTyped<T> * >(&column);
        if (!typed_column) return false;
        return ((*this) == *typed_column);
    }

    template<class T>
    const AggregationResult ColumnBaseTyped<T>::aggregate(const AggregationParam &param) {
        typedef typename ColumnBaseTyped<T>::DenseValueColumn DenseValueColumn;
        typedef typename ColumnBaseTyped<T>::DenseValueColumnPtr DenseValueColumnPtr;
    ...
    template<class T>
    const PositionListPairPtr ColumnBaseTyped<T>::join(ColumnPtr join_column, const JoinParam &param) {
    ...
    template<class T>
    const PositionListPtr ColumnBaseTyped<T>::tid_semi_join(ColumnPtr join_column, const JoinParam &param) {
        typedef typename ColumnBaseTyped<T>::DenseValueColumn DenseValueColumn;
        typedef typename ColumnBaseTyped<T>::DenseValueColumnPtr DenseValueColumnPtr;
    ...
    template<class T>
    const BitmapPtr ColumnBaseTyped<T>::bitmap_semi_join(ColumnPtr join_column, const JoinParam &param) {
        typedef typename ColumnBaseTyped<T>::DenseValueColumn DenseValueColumn;
        typedef typename ColumnBaseTyped<T>::DenseValueColumnPtr DenseValueColumnPtr;
    ...
    template<class T>
    const ColumnPtr ColumnBaseTyped<T>::column_algebra_operation(ColumnPtr source_column,
        ...
        /* \todo: Here, we could save overhead by copy only once, because dense value columns also
         perform the computation on a copy of the "this" column */
        return dense_column->column_algebra_operation(source_column, param);
    }

    template<class T>
    const ColumnPtr ColumnBaseTyped<T>::column_algebra_operation(const boost::any &value,
                                                                 const AlgebraOperationParam &param) {
        typedef typename ColumnBaseTyped<T>::DenseValueColumn DenseValueColumn;
        typedef typename ColumnBaseTyped<T>::DenseValueColumnPtr DenseValueColumnPtr;
    ...
    template<class T>
    const AggregationResult ColumnBaseTyped<T>::aggregateByGroupingKeys(ColumnGroupingKeysPtr grouping_keys,
                                                                        const AggregationParam &param) {


        DenseValueColumnPtr column = this->copyIntoDenseValueColumn(param.proc_spec);

        //typename ColumnBaseTyped<T>::TypedVectorPtr values = this->copyIntoPlainVector();
    ...
    bool ColumnBaseTyped<Type>::store(const std::string &path_to_table_dir) {
        ...
        if (!quiet && verbose && debug)
            std::cout << "Writing Column " << this->getName() << " to File " << path_to_column << std::endl;
        std::ofstream outfile(path_to_column.c_str(), std::ios_base::binary | std::ios_base::out);
        if (!outfile.good()) {
    ...
    bool ColumnBaseTyped<Type>::load(const std::string &path_to_table_dir) {
        std::string path_to_column(path_to_table_dir);
        if (!quiet && verbose && debug)
            std::cout << "Loading column '" << this->name_ << "' from path '" << path_to_column << "'..." << std::endl;
        path_to_column += "/";
        path_to_column += this->name_;

        std::ifstream infile(path_to_column.c_str(), std::ios_base::binary | std::ios_base::in);
        boost::archive::binary_iarchive ia(infile);
        ia >> this->has_primary_key_constraint_;
        ia >> this->has_foreign_key_constraint_;
        ia >> this->fk_constr_;

        this->path_to_table_dir_ = path_to_table_dir;
        this->path_to_column_ = path_to_column;

        if (!this->statistics_.load(path_to_column)) {
//                COGADB_WARNING("Could not find statistics for column '"
//                        << path_to_column << "'!" << std::endl 
//                        << "Try the command 'analyze_table' on the" 
//                        << " table containing the column!","");
        }

        bool ret = this->load_impl(path_to_table_dir, ia);
        infile.close();
        return ret;
    }
