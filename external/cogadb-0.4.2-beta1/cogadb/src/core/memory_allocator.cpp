...
namespace CoGaDB{
	...
	class CPUMemoryAllocator_C : public MemoryAllocator<T>{
		public:
		CPUMemoryAllocator_C(hype::ProcessingDeviceMemoryID mem_id) : MemoryAllocator<T>(mem_id, "CPU Plain C Allocator") {}
	...
	class CPUMemoryAllocator_CPP : public MemoryAllocator<T>{
		public:
		CPUMemoryAllocator_CPP(hype::ProcessingDeviceMemoryID mem_id) : MemoryAllocator<T>(mem_id, "CPU C++ Allocator"), alloc_map_(){}
		...
		T* reallocate(T* memory_block, size_t new_number_of_elements){  
                        ...
                        //std::cout << "Called Realloc: " << (void*) memory_block << ": " << new_number_of_elements << std::endl;
	...
	class GPUMemoryAllocator : public MemoryAllocator<T>{
		public:
		GPUMemoryAllocator(hype::ProcessingDeviceMemoryID mem_id, int gpu_id) : MemoryAllocator<T>(mem_id, "GPU Allocator"), alloc_map_(), gpu_id_(gpu_id){}
		...
		T* reallocate(T* memory_block, size_t new_number_of_elements){  
                        ...
                        //std::cout << "Called CUDA Realloc: " << (void*) memory_block << ": " << new_number_of_elements << std::endl;
                        ret = allocate(new_number_of_elements);
                        //in case memory block is not NULL, copy data to new buffer
                        ...
                        if(memory_block && ret){
                            ...
                            CopyFunctionPtr func = CopyFunctionFactory<T>::getCopyFunction(this->getMemoryID(), this->getMemoryID());
                            assert(func!=NULL);
	...
	class DummyMemoryAllocator : public MemoryAllocator<T>{
		public:
		DummyMemoryAllocator(hype::ProcessingDeviceMemoryID mem_id) : MemoryAllocator<T>(mem_id, "Dummy Allocator") {}
	...
	template <typename T>
	const typename MemoryAllocator<T>::MemoryAllocatorPtr MemoryAllocator<T>::getMemoryAllocator(hype::ProcessingDeviceMemoryID mem_id){
		typedef MemoryAllocator<T>::MemoryAllocatorPtr MemoryAllocatorPtr;
		MemoryAllocatorPtr allocator;
                if(mem_id==hype::PD_Memory_0){
                    //create CPU allocators, plain C allocators for elemtary types, and a c++ allocator for object types 
                    ...
                    //create GPU allocator for elemtary types, and a dummy allocator for object types 
                    if(typeid(T)==typeid(std::string)){
...
#ifdef ENABLE_GPU_ACCELERATION
        template <typename T>
        typename MemoryAllocator<T>::MemoryAllocatorPtr getGPUMemoryAllocator_Cuda(hype::ProcessingDeviceMemoryID& mem_id){
...
#endif
        template <typename T>
        typename MemoryAllocator<T>::MemoryAllocatorPtr getCPUMemoryAllocator_CPP(hype::ProcessingDeviceMemoryID& mem_id){
              static typename MemoryAllocator<T>::MemoryAllocatorPtr ptr;
              if(!ptr){
        ...
        template <typename T>
        typename MemoryAllocator<T>::MemoryAllocatorPtr getCPUMemoryAllocator_C(hype::ProcessingDeviceMemoryID& mem_id){
              static typename MemoryAllocator<T>::MemoryAllocatorPtr ptr;
              if(!ptr){
        ...
        template <typename T>
        T* AllocationManager<T>::malloc(hype::ProcessingDeviceMemoryID mem_id, size_t number_of_elements){
        ...
        template <typename T>
        void AllocationManager<T>::free(hype::ProcessingDeviceMemoryID mem_id, T*& data){
		typedef typename MemoryAllocator<T>::MemoryAllocatorPtr MemoryAllocatorPtr;
		MemoryAllocatorPtr allocator;
                if(mem_id==hype::PD_Memory_0){
                    //create CPU allocators, plain C allocators for elemtary types, and a c++ allocator for object types 
        ...
        template <typename T>
        T* AllocationManager<T>::realloc(hype::ProcessingDeviceMemoryID mem_id, T* data, size_t number_of_elements){
		typedef typename MemoryAllocator<T>::MemoryAllocatorPtr MemoryAllocatorPtr;
		MemoryAllocatorPtr allocator;
                if(mem_id==hype::PD_Memory_0){
                    //create CPU allocators, plain C allocators for elemtary types, and a c++ allocator for object types 
        ...
        COGADB_INSTANTIATE_CLASS_TEMPLATE_FOR_SUPPORTED_TYPES(AllocationManager);
