...
const TablePtr udf_hand_compiled_tpch_q1(TablePtr table,
    ...
    UserDefinedFunctions::UserDefinedFunctions() : map_() {
        ...
        map_.insert(std::make_pair("UDF.EXPERIMENTAL.HAND_COMPILED_TPCH_Q1",&udf_hand_compiled_tpch_q1));
    }
    ...
    UserDefinedFunctionPtr UserDefinedFunctions::get(const std::string& function_name) {
    ...
    /* Build in UDFs */
    const TablePtr limit(TablePtr table,
                const std::string& function_name,
                const std::vector<boost::any>& function_parameters,
                const ProcessorSpecification& proc_spec){
        ...
        bool ret = backend->generateAscendingSequence(tids->data(), 
                num_result_rows, 
                TID(0), 
                proc_spec);
        assert(ret==true);
        TablePtr result = BaseTable::createResultTable(table, tids, LOOKUP, "LIMIT", proc_spec);
        
        return result;
    }
    
    const TablePtr top_k_sort(TablePtr table,
                const std::string& function_name,
                const std::vector<boost::any>& function_parameters,
                const ProcessorSpecification& proc_spec){
        return TablePtr();
    ...
    const TablePtr extract_year(TablePtr table,
                const std::string& function_name,
                const std::vector<boost::any>& function_parameters,
                const ProcessorSpecification& proc_spec){
            ...
            std::string column_name=boost::any_cast<std::string>(function_parameters.front());
            std::string new_column_name=boost::any_cast<std::string>(function_parameters.back());
            
            ColumnPtr col = table->getColumnbyName(column_name);
            if(!col){
                COGADB_ERROR("Column '" << column_name 
                        << "' not found in table '" 
                        << table->getName() << "'","");
                return TablePtr();
            }
...
//            if(!placed_col2) return TablePtr();
            ...
            DateDenseValueColumnPtr divisor = boost::make_shared<DateDenseValueColumn>("10000",UINT32, getMemoryID(proc_spec));
//            DateDenseValueColumnPtr divisor = boost::make_shared<DateDenseValueColumn>("10000",DATE, getMemoryID(proc_spec));
            if(!divisor) return TablePtr();
            
            try{
                divisor->resize(placed_col->size());
            }catch(std::bad_alloc& e){
                COGADB_ERROR("Out of memory for processor memory: " << (int)divisor->getMemoryID(),"");
                return TablePtr();
            }
            
            ProcessorBackend<uint32_t>* backend = ProcessorBackend<uint32_t>::get(proc_spec.proc_id);
        
            bool ret = backend->generateConstantSequence(divisor->data(), 
                    divisor->size(), 
                    uint32_t(10000), 
                    proc_spec);            
            ...
            boost::shared_ptr<ColumnBaseTyped<uint32_t> > typed_source_column = boost::dynamic_pointer_cast<ColumnBaseTyped<uint32_t> >(placed_col);
            assert(typed_source_column!=NULL);            
            DateDenseValueColumnPtr dense_placed_column = typed_source_column->copyIntoDenseValueColumn(param.proc_spec);          
//            if(PLAIN_MATERIALIZED==placed_col->getColumnType()){
//                DateDenseValueColumnPtr dense_placed_column = boost::dynamic_pointer_cast<DateDenseValueColumn>(placed_col);
...
//                assert(typed_source_column!=NULL);
//                dense_placed_column = typed_source_column->copyIntoDenseValueColumn(param.proc_spec);
//            }            
            if(!dense_placed_column) return TablePtr();            
            
            if(!backend->column_algebra_operation(dense_placed_column->data(), divisor->data(), dense_placed_column->size(), param)){
                COGADB_ERROR("Column algebra operation failed!","");
            }
            
            dense_placed_column->setType(UINT32);
            ColumnPtr result_col = dense_placed_column; // placed_col->column_algebra_operation(ColumnPtr(divisor), param);
            ...
            if(table->isMaterialized()){
                ...
                PositionListPtr tids = createPositionList(table->getNumberofRows(), proc_spec);
                if(!tids) return TablePtr();
                bool ret = backend->generateAscendingSequence(tids->data(), 
...
//                }
                ...
                LookupTablePtr result_lookup_table = createLookupTableforUnaryOperation(std::string("lookup(")+table->getName()+")",table, tids, param.proc_spec);
    ...
    const TablePtr hand_compiled_tpch_q1(const size_t& num_elements, 
        ...
        double* ht_sum_quantity = (double*) calloc(num_hash_table_entries, sizeof(double));
        double* ht_sum_base_price = (double*) calloc(num_hash_table_entries, sizeof(double));
        double* ht_sum_disc_price = (double*) calloc(num_hash_table_entries, sizeof(double));
        double* ht_sum_disc = (double*) calloc(num_hash_table_entries, sizeof(double));        
        double* ht_sum_charge = (double*) calloc(num_hash_table_entries, sizeof(double));
        double* ht_count_order = (double*) calloc(num_hash_table_entries, sizeof(double));
...
//         l_returnflag,
//        l_linestatus
        boost::shared_ptr<Column<char> > result_col_returnflag = boost::make_shared<Column<char> >("L_RETURNFLAG", CHAR); 
        boost::shared_ptr<Column<char> > result_col_linestatus = boost::make_shared<Column<char> >("L_LINESTATUS", CHAR);     
        
        boost::shared_ptr<Column<double> > result_col_sum_quantity = boost::make_shared<Column<double> >("SUM_QUANTITY", DOUBLE);
        boost::shared_ptr<Column<double> > result_col_sum_base_price = boost::make_shared<Column<double> >("SUM_BASE_PRICE", DOUBLE);        
        boost::shared_ptr<Column<double> > result_col_sum_disc_price = boost::make_shared<Column<double> >("SUM_DISC_PRICE", DOUBLE);
        boost::shared_ptr<Column<double> > result_col_sum_charge = boost::make_shared<Column<double> >("SUM_CHARGE", DOUBLE);        
        boost::shared_ptr<Column<int32_t> > result_col_count_order = boost::make_shared<Column<int32_t> >("COUNT_ORDER", INT);     
         
        boost::shared_ptr<Column<double> > result_col_avg_qty = boost::make_shared<Column<double> >("AVG_QTY", DOUBLE);
        boost::shared_ptr<Column<double> > result_col_avg_price = boost::make_shared<Column<double> >("AVG_PRICE", DOUBLE);
        boost::shared_ptr<Column<double> > result_col_avg_disc = boost::make_shared<Column<double> >("AVG_DISC", DOUBLE);
    ...
    const TablePtr udf_hand_compiled_tpch_q1(TablePtr table,
            ...
            ColumnPtr col_lineitem_extended_price = table->getColumnbyName("L_EXTENDEDPRICE");
            ...
            boost::shared_ptr<Column<float> > typed_col_lineitem_extended_price = boost::dynamic_pointer_cast<Column<float> >(col_lineitem_extended_price);
            boost::shared_ptr<Column<int> > typed_col_lineitem_quantity = boost::dynamic_pointer_cast<Column<int> >(col_lineitem_quantity);
            boost::shared_ptr<Column<float> > typed_col_lineitem_discount = boost::dynamic_pointer_cast<Column<float> >(col_lineitem_discount);
            boost::shared_ptr<Column<float> > typed_col_lineitem_tax = boost::dynamic_pointer_cast<Column<float> >(col_lineitem_tax);         
//            boost::shared_ptr<Column<float> > typed_ = boost::dynamic_pointer_cast<Column<float> >();        
//            boost::shared_ptr<Column<float> > typed_ = boost::dynamic_pointer_cast<Column<float> >();         
//            boost::shared_ptr<Column<float> > typed_ = boost::dynamic_pointer_cast<Column<float> >();
            
            boost::shared_ptr<Column<char> > typed_col_lineitem_returnflag = boost::dynamic_pointer_cast<Column<char> >(col_lineitem_returnflag);            
            boost::shared_ptr<Column<char> > typed_col_lineitem_linestatus = boost::dynamic_pointer_cast<Column<char> >(col_lineitem_linestatus);    

            boost::shared_ptr<Column<uint32_t> > typed_col_lineitem_shipdate = boost::dynamic_pointer_cast<Column<uint32_t> >(col_lineitem_shipdate);
            
            
            return hand_compiled_tpch_q1(table->getNumberofRows(),
                    typed_col_lineitem_extended_price->data(), 
                    typed_col_lineitem_quantity->data(),
                    typed_col_lineitem_discount->data(), 
                    typed_col_lineitem_tax->data(), 
                    typed_col_lineitem_returnflag->data(), 
