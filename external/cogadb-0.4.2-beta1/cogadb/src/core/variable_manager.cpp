...
namespace CoGaDB{
        ...
        VariableState::VariableState(const std::string& value, 
                       AttributeType type,
        ...
        AttributeType VariableState::getType() const{
            return type_;
        }
    ...
    bool VariableManager::addVariable(const std::string& variable_name, const VariableState& var_stat){
        VariableMap::iterator it=variables_.find(variable_name);
        if(it!=variables_.end()){
            //var already exists!
            COGADB_WARNING("Variable Manager: Variable '" << variable_name << "' already exists!","");
            return false;
        }else{
    ...
    bool VariableManager::setVariableValue(const std::string& variable_name, const std::string& value){
        VariableMap::iterator it=variables_.find(variable_name);
        if(it!=variables_.end()){
    ...
    std::string VariableManager::getVariableValueString(const std::string& variable_name) const{
       VariableMap::const_iterator it=variables_.find(variable_name);
        if(it!=variables_.end()){
            return it->second.getValue();
        }
        COGADB_WARNING("Variable Manager: Variable '" << variable_name << "' does not exist!","");
//        std::stringstream ss;
//        ss << "Variable Manager: Variable '" << variable_name << "' does not exist!";
        return "INVALID";//ss.str();
    }
    int VariableManager::getVariableValueInteger(const std::string& variable_name) const{
        return getVariableValue<int>(variable_name);
    }
    float VariableManager::getVariableValueFloat(const std::string& variable_name) const{
        return getVariableValue<float>(variable_name);
    }
    bool VariableManager::getVariableValueBoolean(const std::string& variable_name) const{
       VariableMap::const_iterator it=variables_.find(variable_name);
        if(it!=variables_.end()){
            std::string val = it->second.getValue();
            if(val=="true") return true;
            if(val=="false") return false;
            COGADB_FATAL_ERROR("Variable Manager: Variable '" << variable_name << "' has invalid value '" << val << "'!",""); 
        }
        COGADB_WARNING("Variable Manager: Variable '" << variable_name << "' does not exist!","");
        return false;
    }
