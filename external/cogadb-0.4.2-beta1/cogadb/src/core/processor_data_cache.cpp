...
namespace CoGaDB {
    ...
    DataCache::DataCache(size_t max_buffer_size_in_byte, const hype::ProcessingDeviceMemoryID& mem_id) :
    map_(),
    join_index_map_(),
    ...
    mem_id_(mem_id),
    cache_mutex_(),
    data_placement_thread_(new boost::thread(boost::bind(&DataCache::data_placement_thread, this))){
    
    }
    
    DataCache::~DataCache(){
        stop_data_placement_thread();
    }

    //		DataCache& DataCache::instance(){
    //			static DataCache cache(HardwareDetector::instance().getTotalMemorySizeInByte(mem_id_)*0.5); //use half of the total device memory as buffer for columns 
    //                        //static DataCache cache(HardwareDetector::instance().getTotalMemorySizeInByte(mem_id_)*0.4); //use half of the total device memory as buffer for columns 
    //			return cache;
    //		}
    ...
    void DataCache::printStatus(std::ostream& out) const throw () {
        ...
        out << " Buffer Size: " << max_buffer_size_in_byte_ / (1024 * 1024) << "MB" << endl;
        out << "Available  Buffer: " << this->getAvailableBufferSize_internal() / (1024 * 1024) << "MB" << endl;
        out << "Total  Memory: " << HardwareDetector::instance().getTotalMemorySizeInByte(mem_id_) / (1024 * 1024) << "MB" << endl;
        out << "Free  Memory: " << HardwareDetector::instance().getFreeMemorySizeInByte(mem_id_) / (1024 * 1024) << "MB" << endl;
        out << "Cached Columns:" << endl;
        Map::const_iterator cit;
        for (cit = map_.begin(); cit != map_.end(); ++cit) {
            out << cit->second->getName() << " of Size " << double(cit->first->getSizeinBytes()) / (1024 * 1024) << "MB (" << cit->second->size() << " elements), currently in use: " << !cit->second.unique() << endl;
            if (this->column_accesses_.find(cit->first) != this->column_accesses_.end())
                out << "\tReferenced: " << this->column_accesses_.find(cit->first)->second;
            if (this->column_least_recently_accessed_.find(cit->first) != this->column_least_recently_accessed_.end())
                out << "\tLast Used Timestamp: " << this->column_least_recently_accessed_.find(cit->first)->second << std::endl;
        }
        ...
        for (cit_indx = join_index_map_.begin(); cit_indx != join_index_map_.end(); ++cit_indx) {
            out << toString(cit_indx->second) << ": " << getSizeInBytes(cit_indx->second) / (1024 * 1024) << "MB" << std::endl;
            if (this->join_index_accesses_.find(cit_indx->second) != this->join_index_accesses_.end())
                out << "\tReferenced: " << this->join_index_accesses_.find(cit_indx->second)->second;
            if (this->join_index_least_recently_accessed_.find(cit_indx->second) != this->join_index_least_recently_accessed_.end())
                out << "\tLast Used Timestamp: " << this->join_index_least_recently_accessed_.find(cit_indx->second)->second << std::endl;
        }
    ...
    bool isPersistentColumn(ColumnPtr column_ptr) {
        ...
        //know because we never cache intermediate results for longer than the queries life time
        //but we do for indeces
        if(JoinIndexes::instance().isReverseJoinIndex(boost::dynamic_pointer_cast<PositionList>(column_ptr))) return true;
       
         
        std::list<std::pair<ColumnPtr, TablePtr> > columns = DataDictionary::instance().getColumnsforColumnName(column_ptr->getName());
        //we assume unique column names
        assert(columns.size() <= 1);
        if (columns.size() == 1) {
            if (columns.front().first == column_ptr) {
//                std::cout << "ColumnPtr " << column_ptr.get() <<  " to " << column_ptr->getName() << " is Persistent Column!" << std::endl;
                return true;
            } else {
//                std::cout << "ColumnPtr " << column_ptr.get() <<  " to " << column_ptr->getName() << " is Intermediate Column!" << std::endl;
                return false;
            }
        } else {
//            std::cout << "ColumnPtr " << column_ptr.get() <<  " to " << column_ptr->getName() << " is Intermediate Column!" << std::endl;
    ...
    bool DataCache::isCachingEnabled() {
        return this->caching_enabled_;
    }
    ...
    bool DataCache::setBufferSizeInByte(size_t size_in_bytes) {
        boost::lock_guard<boost::mutex> lock(cache_mutex_);
        if (size_in_bytes < HardwareDetector::instance().getTotalMemorySizeInByte(mem_id_)) {
    ...
    bool DataCache::cleanupMemory(size_t number_of_bytes_to_cleanup) {
        ...
        size_t initial_used_memory_in_bytes = HardwareDetector::instance().getTotalMemorySizeInByte(mem_id_) - HardwareDetector::instance().getFreeMemorySizeInByte(mem_id_);

        if (number_of_bytes_to_cleanup > HardwareDetector::instance().getTotalMemorySizeInByte(mem_id_)) {
            COGADB_ERROR("number_of_bytes_to_cleanup is greater than total memory capacity of !", "");
            //return true;
        }
        ...
        if (number_of_bytes_to_cleanup > initial_used_memory_in_bytes) {
            //ok, we have not yet a central allocation/deallocation mechanism for device memory
            //that means, during the time we computed the number_of_bytes to cleanup
            //a  operator might terminate, leaving initial_used_memory_in_bytes smaller than
            //the number_of_bytes_to_cleanup. 
            //for now, we will return here, because this is a very rare corner case
            COGADB_WARNING("number_of_bytes_to_cleanup is greater than initial_used_memory_in_bytes!", "");
            return true;
        }

        if (!this->pin_columns_) {
            BufferManagementStrategy buffer_strategy = RuntimeConfiguration::instance().getBufferManagementStrategy();
            if (buffer_strategy == LEAST_FREQUENTLY_USED) {
                while (!map_.empty() && (number_of_bytes_to_cleanup > this->getAvailableBufferSize_internal()) ) {
                    Map::iterator cit;
                    ...
                    std::cout << "Remove Column '" << cit_frequently_used_column->first->getName() << "' from  Cache" << std::endl;
                ...
                while (!map_.empty() && (number_of_bytes_to_cleanup > this->getAvailableBufferSize_internal()) ) {
                    ...
                    //JoinIndexAccesses::iterator jia_it = join_index_accesses_.begin();
                    for (cit = map_.begin(); cit != map_.end(); ++cit) {
                        if (this->column_least_recently_accessed_[cit->first] < least_recent_access) {
                            cit_least_recent_accessed_column = cit;
                            least_recent_access = this->column_least_recently_accessed_[cit->first];
                        }
                    }
                    //delete largest join index
                    std::cout << "Remove Column '" << cit_least_recent_accessed_column->first->getName() << "' from  Cache" << std::endl;
        ...
        if (!this->pin_join_indexes_) {
            ...
            BufferManagementStrategy buffer_strategy = RuntimeConfiguration::instance().getBufferManagementStrategy();
            if (buffer_strategy == LEAST_FREQUENTLY_USED) {
                while (!join_index_map_.empty()  && (number_of_bytes_to_cleanup > this->getAvailableBufferSize_internal())) {
                    JoinIndexMap::iterator cit_indx;
                    size_t least_number_of_accesses = std::numeric_limits<size_t>::max();
                    JoinIndexMap::iterator cit_frequently_used_join_index = join_index_map_.begin();
                    //JoinIndexAccesses::iterator jia_it = join_index_accesses_.begin();
                    for (cit_indx = join_index_map_.begin(); cit_indx != join_index_map_.end(); ++cit_indx) {
                        if (this->join_index_accesses_[cit_indx->first] < least_number_of_accesses) {
                            cit_frequently_used_join_index = cit_indx;
                            least_number_of_accesses = this->join_index_accesses_[cit_indx->first];
                        }
                    }
                    //delete largest join index
                    std::cout << "Remove Join Index '" << toString(cit_frequently_used_join_index->first) << "' from  Cache" << std::endl;
                    join_index_map_.erase(cit_frequently_used_join_index);
                    StatisticsManager::instance().addToValue("TOTAL_JOIN_INDEX_EVICTIONS", 1);
                }
            } else if (buffer_strategy == LEAST_RECENTLY_USED) {
                while (!join_index_map_.empty()  && (number_of_bytes_to_cleanup > this->getAvailableBufferSize_internal())) {
                    JoinIndexMap::iterator cit_indx;
                    uint64_t least_recent_access = std::numeric_limits<uint64_t>::max();
                    JoinIndexMap::iterator cit_least_recent_accessed_join_index = join_index_map_.begin();
                    //JoinIndexAccesses::iterator jia_it = join_index_accesses_.begin();
                    for (cit_indx = join_index_map_.begin(); cit_indx != join_index_map_.end(); ++cit_indx) {
                        if (this->join_index_least_recently_accessed_[cit_indx->first] < least_recent_access) {
                            cit_least_recent_accessed_join_index = cit_indx;
                            least_recent_access = this->join_index_least_recently_accessed_[cit_indx->first];
                        }
                    }
                    //delete largest join index
                    std::cout << "Remove Join Index '" << toString(cit_least_recent_accessed_join_index->first) << "' from  Cache" << std::endl;
        ...
        //if (initial_used_memory_in_bytes - (HardwareDetector::instance().getTotalMemorySizeInByte(mem_id_) - HardwareDetector::instance().getFreeMemorySizeInByte(mem_id_)) <= number_of_bytes_to_cleanup) {
        if(number_of_bytes_to_cleanup < this->getAvailableBufferSize_internal()){    
            std::cout << "Successfully cleaned up " << double(number_of_bytes_to_cleanup) / (1024 * 1024) << "MB from  Cache" << std::endl;
            return true;
        } else {
            std::cout << "Failed to cleaned up " << double(number_of_bytes_to_cleanup) / (1024 * 1024) << "MB from  Cache!" << std::endl;
            return false;
        }
    }
...
//            cache_mutex_.unlock();
        ...
        if (it == map_.end()) {
            if (!quiet)
                cout << "Cache Miss: " << column_ptr->getName() << endl;
            StatisticsManager::instance().addToValue("COLUMN_CACHE_MISS", 1);
            //unsigned int used_memory_in_bytes=HardwareDetector::instance().getTotalMemorySizeInByte(mem_id_)-HardwareDetector::instance().getFreeMemorySizeInByte(mem_id_);
            ...
            if (column_ptr->getSizeinBytes() + used_memory_in_bytes > max_buffer_size_in_byte_) {
                cout << "Warning: DataCache::getColumn(): Unsufficient Device Memory" << endl;
                size_t number_of_bytes_to_cleanup = (column_ptr->getSizeinBytes() + used_memory_in_bytes) - max_buffer_size_in_byte_;
                //try to cleanup the Cache so we have enough available memory on 
                //if it does not work, return a null pointer, otherwise, continue with the function
            ...
            //to avoid a deadlock here, we unlock the mutex for the duration of the copy
            //operation
            lock.unlock();
            result_ptr = copy(column_ptr, mem_id_);
            lock.lock();
            ...
            if (!quiet)
                cout << "Cache Hit: " << column_ptr->getName() << endl;
    ...
    const JoinIndexPtr DataCache::getJoinIndex_internal(JoinIndexPtr join_index) {
        ...
        if (it == join_index_map_.end()) {
            ...
            size_t size_of_joinindex = getSizeInBytes(join_index); //(join_index->first->getPositionList()->size()+join_index->second->getPositionList()->size())*sizeof(PositionList::value_type);
            if (size_of_joinindex + used_memory_in_bytes > max_buffer_size_in_byte_) {
                COGADB_WARNING("DataCache::getJoinIndex(): Unsufficient Device Memory"
                        << std::endl << "\tSizeof JoinIndex: " << size_of_joinindex / (1024 * 1024) << "MB"
                        << std::endl << "\tMemory in Use: " << used_memory_in_bytes / (1024 * 1024) << "MB"
                        << std::endl << "\t Buffer Size: " << max_buffer_size_in_byte_ / (1024 * 1024) << "MB"
                        , "");
                size_t number_of_bytes_to_cleanup = (size_of_joinindex + used_memory_in_bytes) - max_buffer_size_in_byte_;
                //try to cleanup the cache so we have enough available memory on 
                //if it does not work, return a null pointer, otherwise, continue with the function
                if (!cleanupMemory(number_of_bytes_to_cleanup)) {
                    return JoinIndexPtr();
    ...
    void OnExitCloseDataPlacementThread(){
        ...
        if(!quiet) 
            std::cout << "Calling exit handler OnExitCloseDataPlacementThread!" << std::endl;
    }
...
//        std::cout << "Stopped data placement thread" << std::endl;
    ...
    bool DataCache::placeMostFrequentlyUsedColumns(std::ostream& out){
            ...
            boost::lock_guard<boost::mutex> lock(global_mutex_place_frequently_used_columns);
            ...
            for(size_t i=0;i<tables.size();++i){
                boost::shared_ptr<Table> tab = boost::dynamic_pointer_cast<Table>(tables[i]);
                assert(tab!=NULL);
                ColumnAccessStatisticsVector stats = tab->getColumnAccessStatistics();
                for(size_t j=0;j<stats.size();++j){
                    access_frequency_map.insert(std::make_pair(stats[j].statistics.number_of_accesses, FullColumnName(stats[j].table_name, stats[j].column_name))); 
                }
            }
            ...
            for(cit=access_frequency_map.rbegin();cit!=access_frequency_map.rend();++cit){
                ...
                assert(col!=NULL);
                if(used_buffer_space_in_byte+col->getSizeinBytes() < current_buffer_size
                  && cit->first>0){
            ...
            
            {
                ...
                for(size_t i=0;i<columns_to_evict.size();++i){
                    out << "[data placement thread] evict column " << columns_to_evict[i]->getName() << std::endl;
            ...
            //put columns in buffer
            {
                ...
                for(cit=columns_to_place.begin();cit!=columns_to_place.end();++cit){
                    out << "[data placement thread] Cache Column: " << (*cit)->getName() << std::endl;
    ...
    void DataCache::data_placement_thread(){
        ...
        string dir_path(RuntimeConfiguration::instance().getPathToDatabase()+"/logfiles");
        ...
        if(RuntimeConfiguration::instance().getPathToDatabase()!="" 
            ...
            try{
               ...
               ofstream logfile(logfile_path.c_str(), ios::trunc);
               logfile.close();
        ...
        while(true){
            ...
            if(!VariableManager::instance().getVariableValueBoolean("enable_automatic_data_placement")){
                continue;
            }           
            ofstream logfile(logfile_path.c_str(), ios::app);
            ...
            if(logfile.good()){ 
                ...
                COGADB_ERROR("Could not write log message to file: '" << logfile_path << "'","");
            }
            logfile.close();
    ...
    DataCacheManager::DataCacheManager() : caches_() {
        ...
        const CoGaDB::DeviceSpecifications& dev_specs = CoGaDB::HardwareDetector::instance().getDeviceSpecifications();
        ...
        for (unsigned int i = 0; i < dev_specs.size(); ++i) {
            ...
            if (dev_specs[i].getDeviceType() != hype::CPU) {
                ...
                if (caches_.find(dev_specs[i].getMemoryID()) == caches_.end()) {
                    ...
                    DataCachePtr cache(new DataCache(dev_specs[i].getTotalMemoryCapacity()*0.5, dev_specs[i].getMemoryID()));
                    assert(cache != NULL);
        ...
        if(caches_.empty()){
            //add default device so calls to DataCacheManager::getDataCache() always return a data cache
    ...
    DataCache& DataCacheManager::getDataCache(const hype::ProcessingDeviceMemoryID mem_id) const {
    ...
    DataCache& DataCacheManager::getDataCache(const ProcessorSpecification& proc_spec) const {
