...
namespace CoGaDB {
    ...
    : BaseTable(name, schema), columns_(), 
    ...
    Table::Table(const std::string& name, const std::string& path_to_database, bool& error_occured) 
            ...
            path_to_database_(path_to_database)
    {
        ...
        ifstream infile(dir_path.c_str(), std::ios_base::binary | std::ios_base::in);
    ...
    bool Table::store(const std::string& path_to_table_dir) {
        ...
        CompressionSpecifications compress_spec = this->getCompressionSpecifications();
        //cout << "Storing Table schema of Table '" << this->name_ << "' in File '" << table_schema_filename << "' ..." << endl;
        ofstream outfile(table_schema_filename.c_str(), std::ios_base::binary | std::ios_base::out);
    ...
    bool Table::load(TableLoaderMode loader_mode) {
        ...
        if (loader_mode == LOAD_ALL_COLUMNS) {
            ...
            for (unsigned int i = 0; i < columns_.size(); i++) {
                //columns_[i]->load(dir_path);
                //boost::thread* thr = new boost::thread(boost::bind(&Foo::some_function, &f));
                threads.add_thread(new boost::thread(boost::bind(&ColumnBase::load, columns_[i], dir_path)));
            ...
            //this loads the foreign key constrained columns due to its internal use of getColumnbyName()
            //std::vector<const ForeignKeyConstraint*> constraints = this->getForeignKeyConstraints();
            //this->hasPrimaryKeyConstraint("");
            COGADB_FATAL_ERROR("Loading elementary columns only is currently not supported!", "");
    ...
    bool Table::loadDatafromFile(std::string filepath, bool quiet) {
        ...
        while (fin.good()) {
            ...
            for (tokenizer::iterator tok_iter = tokens.begin(); tok_iter != tokens.end(); ++tok_iter) {
                ...
                tmp_tok_it++;
                if (tmp_tok_it == tokens.end() && (*tok_iter == "" || *tok_iter == "\r")) break; //if reached last token and token is null value, then process next line

                if (tokencounter >= columns_.size()) {
                    COGADB_FATAL_ERROR("Fatal Error! While parsing file '" << filepath << "' by processing line: " << samplecounter << " Token: " << tokencounter << endl 
                            << "More Tokens found than columns in Table '" << name_ << "'" << endl,"");
                    return false;
                }
                ...
                if (type == INT) {
                    ...
                    COGADB_FATAL_ERROR("Error Processing row " << samplecounter 
                              << " column " << columns_[tokencounter]->getName() 
            ...
            if (str != "") //if processed line is empty, the tuple is empty as well, so to ignore empty lines, we just do not insert tuples coming from empty lines 
    ...
    /***************** status report *****************/
    size_t Table::getNumberofRows() const throw () {
    ...
    bool Table::isMaterialized() const throw () {
        return true;
    }
    ...
    bool Table::insert(const Tuple& t) {
        if (t.size() != columns_.size()) {
            COGADB_FATAL_ERROR("Fatal Error! Tuple has invalid number of values!" << endl << "Mumber of Columns in Table '" << name_ << "': " << columns_.size() << endl 
                    << "Number of values in Tuple: " << t.size() << endl << t << endl,"");
        ...
        for (it = t.begin(); it != t.end(); it++, i++) {
            ...
            if (ret_val == false) {
                COGADB_FATAL_ERROR("Fatal Error! could not insert value in Column " << columns_[i]->getName() << endl, "");
                return false;
            }
        }
        number_of_rows_++;
    ...
    bool Table::update(const std::string& attribute_name, const boost::any&) { //value){
    ...
    bool Table::remove(const std::string& attribute_name, const boost::any&) { //value){
        ColumnPtr col = this->getColumnbyName(attribute_name);
        if (col) {
            //return col->update();
        }
    ...
    bool Table::replaceColumn(const std::string& column_name, const ColumnPtr new_column){
...
//        }
    ...
    const ColumnPtr Table::getColumnbyName(const std::string& column_name) const throw () {
        ...
        if (result_it == schema_.end()) {
            //cout << "Error: could not find column " << column_name << " in Table '" << name_ << "'!" << endl;
            return ColumnPtr();
        }
        {   /* Protect for concurrent column loads from different threads. */
            boost::lock_guard<boost::mutex> lock(*column_mutexes_[counter]);
            if (!columns_[counter]->isLoadedInMainMemory()) {
    //            return columns_[counter];
    //        } else {
                //                        boost::lock_guard<boost::mutex> lock(table_mutex_);
                using namespace boost::filesystem;
                string dir_path(path_to_database_);
                if (!exists(dir_path)) {
                    COGADB_ERROR("Cannot find database in '" << dir_path << "': No such directory!", "");
                    return ColumnPtr();
                }
                dir_path += "/tables/";
                dir_path += name_;
                ...
                columns_[counter]->load(dir_path);
                columns_[counter]->setStatusLoadedInMainMemory(true);
            }
        }
        
        {
            boost::lock_guard<boost::mutex> lock(access_statistics_map_mutex_);
            access_statistics_map_[column_name].number_of_accesses++;
            access_statistics_map_[column_name].last_access_timestamp=getTimestamp();
        }
        return columns_[counter];
    }
        ...
        for(size_t i=0;i<columns_.size();++i){
            ...
            if(columns_[i]->isLoadedInMainMemory()){
                ...
                if(tmp){
                    ...
                    COGADB_FATAL_ERROR("Failed to transfer column back " << columns_[i]->getName() << " (" << util::getName(columns_[i]->getColumnType()) << ") to main memory!","");
                }
            }
        }
        return true;
    ...
    ColumnAccessStatisticsVector Table::getColumnAccessStatistics() const{
        ...
        AccessStatisticsMap::const_iterator cit;
        boost::lock_guard<boost::mutex> lock(access_statistics_map_mutex_);
        for(cit=access_statistics_map_.begin();cit!=access_statistics_map_.end();++cit){
             statistics.push_back(ColumnAccessStatistics(this->getName(), cit->first, cit->second));
        }
