...
namespace CoGaDB {
    ...
    StringHeap::StringHeap(const StringHeapType heap_type, const size_t max_string_length, 
                const hype::ProcessingDeviceMemoryID mem_id)
    : heap_type_(heap_type), max_string_length_(max_string_length), 
    mem_id_(mem_id), num_elements_(0)
    {
            
    }
    
    bool StringHeap::load(const std::string& base_path, const std::string& column_name, boost::archive::binary_iarchive& ia){
        ia >> num_elements_;
        ia >> heap_type_;
    ...
    bool StringHeap::store(const std::string& base_path, const std::string& column_name, boost::archive::binary_oarchive& oa){
        oa << num_elements_;
        oa << heap_type_;
    ...
    StringHeapType StringHeap::getStringHeapType () const{
        return heap_type_;
    }    
    
    size_t StringHeap::getMaxStringLength() const{
    ...
    size_t StringHeap::size() const{
        return num_elements_;
    }
    
    FixedLengthStringHeap::FixedLengthStringHeap(const size_t max_string_length, 
            const hype::ProcessingDeviceMemoryID mem_id)
    ...
    char* FixedLengthStringHeap::getPointerArray(){
        return data_->data();
    }
    
    bool FixedLengthStringHeap::initStringArray(char** c_string_ptr_array, 
            size_t num_elements, 
            const hype::ProcessingDeviceMemoryID& mem_id){
    ...
    char* FixedLengthStringHeap::push_back(const char* const val, bool& heap_memory_moved){
    ...
    void FixedLengthStringHeap::clear(){
        data_->clear();
    }
    ...
    const StringHeapPtr FixedLengthStringHeap::copy(const hype::ProcessingDeviceMemoryID& mem_id) const{
        boost::shared_ptr<FixedLengthStringHeap> copy (new FixedLengthStringHeap(this->getMaxStringLength(), 
            this->size(), this->getMemoryID()));
    ...
    void FixedLengthStringHeap::reserve(size_t new_capacity, bool& heap_memory_moved){
        char* original_array = data_->data();
    ...
    bool FixedLengthStringHeap::load_impl(const std::string& base_path, 
            const std::string& column_name, boost::archive::binary_iarchive& ia){
        std::stringstream name;
        name << column_name << ".fixed_length_string_heap";
        data_->setName(name.str());          
        return data_->load(base_path);
    }
    
    bool FixedLengthStringHeap::store_impl(const std::string& base_path, 
            const std::string& column_name, boost::archive::binary_oarchive& oa){
    ...
    const StringHeapPtr createStringHeap(StringHeapType type, 
        ...
        CStringColumn::CStringColumn(const std::string& name, size_t maximal_string_length, hype::ProcessingDeviceMemoryID mem_id)
        : Column<char*>(name, CHAR, mem_id), 
        heap_(createStringHeap(FIXED_LENGTH_STRING_HEAP, maximal_string_length, mem_id)){
...
//                assert(this->getMemoryID()==other.getMemoryID()); 
        ...
        bool CStringColumn::insert(const boost::any& new_value){
            ...
            this->push_back(const_value);
            return true;   
        }
        bool CStringColumn::insert(const char*& new_value){
        ...
        size_t CStringColumn::size() const throw (){
            return Column<char*>::size();
        }
        ...
        const ColumnPtr CStringColumn::copy(const hype::ProcessingDeviceMemoryID& mem_id) const {
            boost::shared_ptr<CStringColumn> col;
            try {
                col = boost::shared_ptr<CStringColumn>(
                        new CStringColumn(this->getName(),
                        this->heap_->getMaxStringLength(),
                        mem_id));
                col->resize(this->size());
            } catch (std::bad_alloc& e) {
                COGADB_ERROR("Out of memory on device memory " << (int) mem_id
                        << " for column '" << this->getName()
                        << "' requesting " << double(this->getSizeinBytes()) / (1024 * 1024 * 2014) << " GB memory" << std::endl
...
//                CopyFunctionPtr func = CopyFunctionFactory<char*>::getCopyFunction(mem_id, this->getMemoryID());
//                assert(func != NULL);
//                char** original_array = this->data();
//                if (!(*func)(col->data(), original_array, this->size() * sizeof (char*))) return ColumnPtr();
            }
            return col;
        }
        const Column<char*>::DenseValueColumnPtr CStringColumn::copyIntoDenseValueColumn(const ProcessorSpecification& proc_spec) const{
            COGADB_FATAL_ERROR("Called Unimplemented Function!","");
            return Column<char*>::DenseValueColumnPtr();
        }
        const DoubleDenseValueColumnPtr CStringColumn::convertToDenseValueDoubleColumn(const ProcessorSpecification& proc_spec) const{
            return DoubleDenseValueColumnPtr();
        }
        const ColumnPtr CStringColumn::materialize() throw (){
            return copy();
        }
        ...
        const ColumnPtr CStringColumn::gather(PositionListPtr tid_list, const GatherParam& param){
            ...
            ProcessorBackend<char*>* backend = ProcessorBackend<char*>::get(param.proc_spec.proc_id);
        ...
        const ColumnGroupingKeysPtr CStringColumn::createColumnGroupingKeys(const ProcessorSpecification& proc_spec) const{
        ...
        const AggregationResult CStringColumn::aggregateByGroupingKeys(ColumnGroupingKeysPtr grouping_keys, const AggregationParam&){
            COGADB_FATAL_ERROR("Called Unimplemented Function!","");
	...
	const PositionListPairPtr CStringColumn::join(ColumnPtr join_column, const JoinParam&){
            COGADB_FATAL_ERROR("Called Unimplemented Function!","");
            return PositionListPairPtr ();
        }
        const PositionListPairPtr CStringColumn::join(Column<char*>& join_column, const JoinParam& param){
            COGADB_FATAL_ERROR("Called Unimplemented Function!","");
            return PositionListPairPtr();
        }
        
        const PositionListPtr CStringColumn::tid_semi_join(ColumnPtr join_column, const JoinParam&){
            COGADB_FATAL_ERROR("Called Unimplemented Function!","");
            return PositionListPtr();
        }
        const PositionListPtr CStringColumn::tid_semi_join(CStringColumn& join_column, const JoinParam&){
            COGADB_FATAL_ERROR("Called Unimplemented Function!","");
            return PositionListPtr();
        }
        
        const BitmapPtr CStringColumn::bitmap_semi_join(ColumnPtr join_column, const JoinParam&){
            COGADB_FATAL_ERROR("Called Unimplemented Function!","");
            return BitmapPtr();
        }
        const BitmapPtr CStringColumn::bitmap_semi_join(CStringColumn& join_column, const JoinParam&){
            COGADB_FATAL_ERROR("Called Unimplemented Function!","");
            return BitmapPtr();
        }  

        const ColumnPtr CStringColumn::column_algebra_operation(ColumnPtr source_column, const AlgebraOperationParam&){
            COGADB_FATAL_ERROR("Called Unimplemented Function!","");
            return ColumnPtr();
        }
        const ColumnPtr CStringColumn::column_algebra_operation(CStringColumn& source_column, const AlgebraOperationParam&){
            COGADB_FATAL_ERROR("Called Unimplemented Function!","");
            return ColumnPtr();
        }
        const ColumnPtr CStringColumn::column_algebra_operation(const boost::any& value, const AlgebraOperationParam&){
            COGADB_FATAL_ERROR("Called Unimplemented Function!","");
            return ColumnPtr();
        }
        ...
        const PositionListPtr CStringColumn::sort(const SortParam& param, bool no_copy){
            COGADB_FATAL_ERROR("Called Unimplemented Function!","");
            return PositionListPtr();
        }
        ...
        void CStringColumn::reserve(size_t new_capacity){
             ...
             */
            if(heap_moved){
                if(!heap_->initStringArray(this->data(),heap_->size(), this->getMemoryID())){
        ...
        void CStringColumn::push_back(const char*& val){
            bool heap_moved;
...
//                    COGADB_WARNING("Heap expanded!","");
        ...
        void CStringColumn::clear(){
            ...
            heap_=StringHeapPtr(createStringHeap(FIXED_LENGTH_STRING_HEAP, 
                    heap_->getMaxStringLength(), this->getMemoryID()));
        }
            
        /* END CONTAINER OPERATIONS*/
        
        void CStringColumn::printHeap(){
            heap_->print();
        }

	bool CStringColumn::load_impl(const std::string& path, boost::archive::binary_iarchive& ia){
            ...
            heap_ = createStringHeap(str_heap_type, max_string_length, hype::PD_Memory_0); //load heap to CPU main memory
	...
	bool CStringColumn::store_impl(const std::string& path, boost::archive::binary_oarchive& oa){
