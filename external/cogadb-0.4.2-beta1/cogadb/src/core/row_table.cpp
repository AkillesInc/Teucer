...
RowTable::RowTable(const string& name, const TableSchema& schema) : BaseTable(name,schema), _rtp(), _row_pages(), _row_pages_prefix(), _row_page_mapping(), _cols(), _row_count(0), _isMaterialized(true)
{		
...
bool RowTable::store(const std::string& path_to_table_dir)
{	
	return false;
}
...
{			
	...
	cout << "DEBUG: (RowTable) Creating table '" << name_ << "' with " << nmbrOfCols << " column(s)" << endl;
	
	/* Iterate throw all columns of the schema and get the relative pages from BufferManager */
	for(it=schema.begin();it!=schema.end();it++)
	{		
		col = it->second;
		cout << "DEBUG: (RowTable) Adding column '" << col << "' to table '" << name_ << "'" << endl;
		...
		{
		    _cols.push_back(ColumnPtr(new RowValueColumn<bool>(_rtp , col, it->first)));
        }
		else if(type == FLOAT)
		{
			_cols.push_back(ColumnPtr(new RowValueColumn<float>(_rtp , col, it->first)));
		}
		else if(type == VARCHAR)
		{
			_cols.push_back(ColumnPtr(new RowValueColumn<string>(_rtp , col, it->first)));
		}				
	}
	
	cout << "DEBUG: (RowTable) Materialize table '" << name_ << "' ... " << endl;
	
	/* Materialize - fill RowPage's until no more data are available <fillPage == false> (materialize all data [early materialization]) */
	...
	/* fillPage result with false (no more data are available) but this row page was not stored */
	if(rp->count() > 0)
	{
		 _row_count += rp->count();
	     _row_pages.push_back(RowPagePtr(rp));		
	...
	cout << "(MEASUREMENT) Materialize: " << ((double)end - (double)start)/1000000 << " ms" << endl;
...
size_t RowTable::getNumberofRows() const throw()
{
	return _row_count;
}	
...
{
	/* Not implemented */
	cout << "DEBUG: (RowTable) Insert tuple with " << t.size() << " values" << endl;
	return false;
}
...
{
	/* Not implemented */
	cout << "DEBUG: (RowTable) Update " << attribute_name << " (is empty: " << value.empty() << ")" << endl;
	return false;
}
...
{
	/* Not implemented */
	cout << "DEBUG: (RowTable) Remove " << attribute_name << " (is empty: " << value.empty() << ")" << endl;
	return false;
}
	
const ColumnPtr RowTable::getColumnbyName(const string& column_name) const throw()
{	
	...
	{			
		/* if name match to column_name - return column */
	     if(_cols[index]->getName() == column_name)
			 return _cols[index];	
	}
...
const vector<ColumnPtr>& RowTable::getColumns() const
{		
	return _cols;
}
...
{		
	...
	{	
	    for(it=schema.begin();it!=schema.end();it++)
	    {
		    ...
		    if(type == INT)
		    {
				shared_pointer_namespace::shared_ptr<ColumnBaseTyped<int> > column = shared_pointer_namespace::static_pointer_cast<ColumnBaseTyped<int> >(cPtr);
				cout << (*column)[rowIndex] << "\t";				
			}			
			
			if(type == FLOAT)
		    {
				shared_pointer_namespace::shared_ptr<ColumnBaseTyped<float> > column = shared_pointer_namespace::static_pointer_cast<ColumnBaseTyped<float> >(cPtr);
				cout << (*column)[rowIndex] << "\t";				
			}
			
			if(type == BOOLEAN)
		    {
				shared_pointer_namespace::shared_ptr<ColumnBaseTyped<bool> > column = shared_pointer_namespace::static_pointer_cast<ColumnBaseTyped<bool> >(cPtr);
				cout << (*column)[rowIndex] << "\t";				
			}
			
			if(type == VARCHAR)
		    {
				shared_pointer_namespace::shared_ptr<ColumnBaseTyped<string> > column = shared_pointer_namespace::static_pointer_cast<ColumnBaseTyped<string> >(cPtr);
				cout << (*column)[rowIndex] << "\t";				
			}
...
bool RowTable::update(PositionListPtr tids, const std::string& attribute_name, const boost::any& value)
{
...
RowPagePtr RowTable::getPageByIndex(unsigned int index, unsigned int& prefix)
{
	...
	*/
	...
	prefix = _row_pages_prefix[index] - _row_pages[_row_page_mapping[index]]->count();
