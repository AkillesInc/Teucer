...
namespace CoGaDB{
    ...
    RuntimeConfiguration::RuntimeConfiguration() : path_to_database_("./data"), optimizer_name_("no_join_order_optimizer"), print_query_plan_(false), 
            enable_profiling_(false), global_device_constraint_(hype::ANY_DEVICE),two_phase_physical_optimization_parallelization_mode(SERIAL), 
            hybrid_optimizer_heuristic(hype::GREEDY_HEURISTIC), enable_query_chopping_(false), gpu_buffer_management_strategy_(LEAST_RECENTLY_USED),
            table_loader_mode_(LOAD_ALL_COLUMNS) //(LOAD_NO_COLUMNS)
    {
		...
		const std::string& RuntimeConfiguration::getOptimizer(){
                ...
                void RuntimeConfiguration::setQueryOptimizationHeuristic(hype::QueryOptimizationHeuristic opt_heu){
                    this->hybrid_optimizer_heuristic = opt_heu;
                }
                hype::QueryOptimizationHeuristic RuntimeConfiguration::getQueryOptimizationHeuristic() const{
                    return this->hybrid_optimizer_heuristic;
                }
                
                void RuntimeConfiguration::setParallelizationModeForTwoPhasePhysicalOptimizationQueryPlans(ParallelizationMode value){
                    this->two_phase_physical_optimization_parallelization_mode=value;
                }
                ParallelizationMode RuntimeConfiguration::getParallelizationModeForTwoPhasePhysicalOptimizationQueryPlans() const{
                    return this->two_phase_physical_optimization_parallelization_mode;
                }
                
                GPUBufferManagementStrategy RuntimeConfiguration::getGPUBufferManagementStrategy() const{
                    return this->gpu_buffer_management_strategy_;
                }
                void RuntimeConfiguration::setGPUBufferManagementStrategy(GPUBufferManagementStrategy value){
                    this->gpu_buffer_management_strategy_=value;
                }
                
                BufferManagementStrategy RuntimeConfiguration::getBufferManagementStrategy() const{
                    return getGPUBufferManagementStrategy();
                }
                void RuntimeConfiguration::setBufferManagementStrategy(BufferManagementStrategy value){
			...
			return this->print_query_plan_;
                ...
                bool RuntimeConfiguration::setGlobalDeviceConstraint(const hype::DeviceConstraint& value){
