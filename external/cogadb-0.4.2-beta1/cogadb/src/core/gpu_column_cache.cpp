...
using namespace std;
		...
		GPU_Column_Cache& GPU_Column_Cache::instance(){
			static GPU_Column_Cache cache(getTotalGPUMemorySizeInByte()*0.5); //use half of the total device memory as buffer for columns 
                        //static GPU_Column_Cache cache(getTotalGPUMemorySizeInByte()*0.4); //use half of the total device memory as buffer for columns 
			return cache;
		}
		...
		void GPU_Column_Cache::printStatus(std::ostream& out) const throw(){
			...
			out << "GPU Buffer Size: " << max_gpu_buffer_size_in_byte_/(1024*1024) << "MB" << endl; 
                        out << "Available GPU Buffer: " << this->getAvailableGPUBufferSize_internal()/(1024*1024) << "MB" << endl; 
			out << "Total GPU Memory: " << getTotalGPUMemorySizeInByte()/(1024*1024) << "MB" << endl; 
			out << "Free GPU Memory: " << getFreeGPUMemorySizeInByte()/(1024*1024) << "MB" << endl; 
			...
			out << "Cached Join Indexes:" << endl;
				...
				out << toString(cit_indx->second->cpu_join_index) << ": " <<  getSizeInBytes(cit_indx->second->cpu_join_index)/(1024*1024) << "MB";
                                if(this->join_index_accesses_.find(cit_indx->second->cpu_join_index)!=this->join_index_accesses_.end())
                                     out << "\tReferenced: " << this->join_index_accesses_.find(cit_indx->second->cpu_join_index)->second;
                                if(this->join_index_least_recently_accessed_.find(cit_indx->second->cpu_join_index)!=this->join_index_least_recently_accessed_.end())
                                     out << "\tLast Used Timestamp: " << this->join_index_least_recently_accessed_.find(cit_indx->second->cpu_join_index)->second << std::endl;
                        }
...
//                }
		...
		const gpu::GPU_JoinIndexPtr GPU_Column_Cache::getGPUJoinIndex(JoinIndexPtr join_index){
                ...
                bool GPU_Column_Cache::isCachingEnabled(){
...
//                    if(this->max_gpu_buffer_size_in_byte_<gpu_used_bytes) return 0;
//                    
//                    return this->max_gpu_buffer_size_in_byte_-gpu_used_bytes;
                ...
                bool GPU_Column_Cache::cleanupGPUMemory(size_t number_of_bytes_to_cleanup){
                    ...
                    size_t initial_used_gpu_memory_in_bytes=getTotalGPUMemorySizeInByte()-getFreeGPUMemorySizeInByte();
                    
                    if(number_of_bytes_to_cleanup>getTotalGPUMemorySizeInByte()){
                        COGADB_ERROR("number_of_bytes_to_cleanup is greater than total memory capacity of GPU!","");
                        //return true;
                    }
                    ...
                    if(number_of_bytes_to_cleanup>initial_used_gpu_memory_in_bytes){
                        ...
                        //a GPU operator might terminate, leaving initial_used_gpu_memory_in_bytes smaller than
                        //the number_of_bytes_to_cleanup. 
                        //for now, we will return here, because this is a very rare corner case
                        COGADB_WARNING("number_of_bytes_to_cleanup is greater than initial_used_gpu_memory_in_bytes!","");
                        return true;
                    }
                    
                    if(!this->pin_columns_){
//                        while(!map_.empty() && (double(initial_used_gpu_memory_in_bytes)-(getTotalGPUMemorySizeInByte()-getFreeGPUMemorySizeInByte())) <= number_of_bytes_to_cleanup){
//                            Map::iterator cit;
//                            Map::iterator cit_col_with_largest_memory=map_.begin();
//
//                            for(cit=map_.begin();cit!=map_.end();++cit){
...
//                            std::cout << "Remove Column '" <<  cit_col_with_largest_memory->first->getName() << "' from GPU Cache" << std::endl;
//                            map_.erase(cit_col_with_largest_memory);
//
//                        }
                        GPUBufferManagementStrategy buffer_strategy = RuntimeConfiguration::instance().getGPUBufferManagementStrategy();
                        if(buffer_strategy==LEAST_FREQUENTLY_USED){
                            while(!map_.empty() && initial_used_gpu_memory_in_bytes-(getTotalGPUMemorySizeInByte()-getFreeGPUMemorySizeInByte()) >= number_of_bytes_to_cleanup){
                                Map::iterator cit;
                                ...
                                std::cout << "Remove Column '" <<  cit_frequently_used_column->first->getName() << "' from GPU Cache" << std::endl;
                            ...
                            while(!map_.empty() && initial_used_gpu_memory_in_bytes-(getTotalGPUMemorySizeInByte()-getFreeGPUMemorySizeInByte()) >= number_of_bytes_to_cleanup){
                                ...
                                //delete largest join index
                                std::cout << "Remove Column '" <<  cit_least_recent_accessed_column->first->getName() << "' from GPU Cache" << std::endl;
...
//                        while(!join_index_map_.empty() && initial_used_gpu_memory_in_bytes-(getTotalGPUMemorySizeInByte()-getFreeGPUMemorySizeInByte()) >= number_of_bytes_to_cleanup){
//                            JoinIndexMap::iterator cit_indx;
//                            JoinIndexMap::iterator cit_join_index_with_largest_memory=join_index_map_.begin();
//                            for(cit_indx=join_index_map_.begin();cit_indx!=join_index_map_.end();++cit_indx){
//                                    if(getSizeInBytes(cit_indx->second->cpu_join_index)> getSizeInBytes(cit_join_index_with_largest_memory->second->cpu_join_index)){
//                                        cit_join_index_with_largest_memory=cit_indx;
//                                    }
//                            }
//                            //delete largest join index
//                            std::cout << "Remove Join Index '" <<  toString(cit_join_index_with_largest_memory->first) << "' from GPU Cache" << std::endl;
...
//                        }
                        ...
                        if(buffer_strategy==LEAST_FREQUENTLY_USED){
                            while(!join_index_map_.empty() && initial_used_gpu_memory_in_bytes-(getTotalGPUMemorySizeInByte()-getFreeGPUMemorySizeInByte()) >= number_of_bytes_to_cleanup){
                                ...
                                //delete largest join index
                                std::cout << "Remove Join Index '" <<  toString(cit_frequently_used_join_index->first) << "' from GPU Cache" << std::endl;
                            ...
                            while(!join_index_map_.empty() && initial_used_gpu_memory_in_bytes-(getTotalGPUMemorySizeInByte()-getFreeGPUMemorySizeInByte()) >= number_of_bytes_to_cleanup){
                                ...
                                //delete largest join index
                                std::cout << "Remove Join Index '" <<  toString(cit_least_recent_accessed_join_index->first) << "' from GPU Cache" << std::endl;
...
//                    }
                    ...
                    if(initial_used_gpu_memory_in_bytes-(getTotalGPUMemorySizeInByte()-getFreeGPUMemorySizeInByte()) <= number_of_bytes_to_cleanup){
                         std::cout << "Successfully cleaned up " << double(number_of_bytes_to_cleanup)/(1024*1024) << "MB from GPU Cache" << std::endl;
                        return true;
                    }else{
                         std::cout << "Failed to cleaned up " << double(number_of_bytes_to_cleanup)/(1024*1024) << "MB from GPU Cache!" << std::endl;
                        return false;     
                    }
                }
                
		const gpu::GPU_Base_ColumnPtr GPU_Column_Cache::getGPUColumn_internal(ColumnPtr column_ptr){
			boost::lock_guard<boost::mutex> lock(global_gpu_cache_mutex);
			...
			if(it==map_.end()){
				...
				cout << "Cache Miss: " << column_ptr->getName() << endl;
                                StatisticsManager::instance().addToValue("GPU_COLUMN_CACHE_MISS",1);
				//unsigned int used_memory_in_bytes=getTotalGPUMemorySizeInByte()-getFreeGPUMemorySizeInByte();
				...
				if(column_ptr->getSizeinBytes()+used_memory_in_bytes>max_gpu_buffer_size_in_byte_){
					cout << "Warning: GPU_Column_Cache::getGPUColumn(): Unsufficient Device Memory" << endl;
                                        size_t number_of_bytes_to_cleanup=(column_ptr->getSizeinBytes()+used_memory_in_bytes)-max_gpu_buffer_size_in_byte_;
...
//                                        //simple greedy strategy that deletes the largest column from the cache
//                                        Map::iterator cit;
//                                        Map::iterator cit_column_with_largest_size=map_.begin();
//					for(cit=map_.begin();cit!=map_.end();++cit){
//                                            //consider only unused columns for removal from cache
//                                            if(cit_column_with_largest_size->second->size()<cit->second->size() && cit_column_with_largest_size->second.unique()){
...
//					return gpu::GPU_Base_ColumnPtr();
                ...
                const gpu::GPU_JoinIndexPtr GPU_Column_Cache::getGPUJoinIndex_internal(JoinIndexPtr join_index){
			boost::lock_guard<boost::mutex> lock(global_gpu_cache_mutex); 
			...
			if(it==join_index_map_.end()){
				...
				if(size_of_joinindex+used_memory_in_bytes>max_gpu_buffer_size_in_byte_){
					COGADB_WARNING("GPU_Column_Cache::getGPUJoinIndex(): Unsufficient Device Memory" 
                                                << std::endl << "\tSizeof JoinIndex: " << size_of_joinindex/(1024*1024) << "MB" 
                                                << std::endl << "\tMemory in Use: " << used_memory_in_bytes/(1024*1024) << "MB"
                                                << std::endl << "\tGPU Buffer Size: " << max_gpu_buffer_size_in_byte_/(1024*1024) << "MB"
                                                ,"");
                                        size_t number_of_bytes_to_cleanup=(size_of_joinindex+used_memory_in_bytes)-max_gpu_buffer_size_in_byte_;
                                        //try to cleanup the cache so we have enough available memory on GPU
                                        //if it does not work, return a null pointer, otherwise, continue with the function
                                        if(!cleanupGPUMemory(number_of_bytes_to_cleanup)){
