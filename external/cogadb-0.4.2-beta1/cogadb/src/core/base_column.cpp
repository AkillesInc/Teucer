...
namespace CoGaDB {
    ...
    ColumnBase::ColumnBase(const std::string& name, AttributeType db_type, ColumnType column_type) : name_(name), db_type_(db_type), column_type_(column_type), is_loaded_(true) {
    ...
    void ColumnBase::setType(const AttributeType& type){
        this->db_type_= type;
    }    
    
    ColumnType ColumnBase::getColumnType() const throw () {
        return column_type_;
    }

    bool ColumnBase::isLoadedInMainMemory() const throw () {
        return is_loaded_;
    }
    ...
    const ColumnPtr createColumn(AttributeType type, const std::string& name) {
        ...
        //            //ptr = ColumnPtr(new DictionaryCompressedColumn<std::string>(name, VARCHAR));
        //            
        ...
        DefaultCompressionMethods::const_iterator cit = default_compression.find(type);
    ...
    //        template <typename AttributeType, int T>
    //        struct helper{
    ////            BOOST_STATIC_ASSERT();
    //        };
    //        
    //        //PLAIN_MATERIALIZED,LOOKUP_ARRAY,DICTIONARY_COMPRESSED,RUN_LENGTH_COMPRESSED,DELTA_COMPRESSED,BIT_VECTOR_COMPRESSED,BITPACKED_DICTIONARY_COMPRESSED
    //        
    //        template <typename AttributeType>
    //        struct helper<PLAIN_MATERIALIZED>{
    //            typedef Column<AttributeType> ColumnType;
    //        };
    //        
    //        template <typename AttributeType>
    //        struct helper<LOOKUP_ARRAY>{
    //            typedef Column<AttributeType> ColumnType;
    //        };
    //        
    //        template <typename AttributeType>
    ...
    //typedef const ColumnPtr (*CreateColumnFunc)(AttributeType type, const std::string& name, ColumnType column_type);

    //typedef std::map<ColumnType, CreateColumnFunc> TypedColumnFactory;

    //PLAIN_MATERIALIZED,LOOKUP_ARRAY,DICTIONARY_COMPRESSED,RUN_LENGTH_COMPRESSED,DELTA_COMPRESSED,BIT_VECTOR_COMPRESSED,BITPACKED_DICTIONARY_COMPRESSED

    template<typename T>
    const ColumnPtr CreateColumnPtr(AttributeType type, const std::string& name, ColumnType column_type) {
        switch (column_type) {
            ...
            case RUN_LENGTH_DELTA_ONE_COMPRESSED_NUMBER_PREFIX:
                return ColumnPtr(new RLEDeltaOneCompressedColumnNumberWithPrefixCounts<T>(name, type));
        }
    }
    ...
    const PositionListPtr createPositionList(size_t num_of_elements, const hype::ProcessingDeviceMemoryID& mem_id) {
    ...
    const PositionListPtr createPositionList(size_t num_of_elements, const ProcessorSpecification& proc_spec) {
    ...
    bool resize(PositionList& tids, size_t new_num_elements) {
        try {
            ...
            std::cerr << "Ran out of memory in memory " << (int) tids.getMemoryID() << "!" << std::endl;
            return false;
        }
        return true;
    }
    ...
    PositionListPairPtr copy(PositionListPairPtr pair_tids, const hype::ProcessingDeviceMemoryID& mem_id) {
        ...
        if (!new_pair_tids->first || !new_pair_tids->second) return PositionListPairPtr();
    ...
    ColumnPtr copy_if_required(ColumnPtr col, const hype::ProcessingDeviceMemoryID& mem_id) {
        ...
        if (col->getMemoryID() == mem_id) {
            ...
            if (!isCPUMemory(mem_id)) {
                ...
                ColumnPtr cached = DataCacheManager::instance().getDataCache(col->getMemoryID()).getHostColumn(col);
    ...
    std::vector<ColumnPtr> copy_if_required(const std::vector<ColumnPtr>& col_vec, const hype::ProcessingDeviceMemoryID& mem_id){
        std::vector<ColumnPtr> placed_col_vec;
        for(size_t i=0;i<col_vec.size();++i){
    ...
    PositionListPtr copy_if_required(PositionListPtr tids, const hype::ProcessingDeviceMemoryID& mem_id) {
        ...
        if (tids->getMemoryID() == mem_id) {
            ...
            if (!isCPUMemory(mem_id) && JoinIndexes::instance().isReverseJoinIndex(tids)) {
                ColumnPtr placed_tids = DataCacheManager::instance().getDataCache(mem_id).getColumn(tids);
    ...
    PositionListPairPtr copy_if_required(PositionListPairPtr pair_tids, const hype::ProcessingDeviceMemoryID& mem_id) {
        ...
        if (pair_tids->first->getMemoryID() == mem_id && pair_tids->second->getMemoryID() == mem_id) {
            return pair_tids;
        }
        //at least one positionlist needs to be copied
        PositionListPairPtr new_pair_tids(new PositionListPair());
    ...
    JoinIndexPtr copy_if_required(JoinIndexPtr join_index, const hype::ProcessingDeviceMemoryID& mem_id) {
        ...
        if (join_index->first->getMemoryID() == mem_id && join_index->second->getMemoryID() == mem_id) {
            return join_index;
        }

        if (!isCPUMemory(mem_id)) {
            return DataCacheManager::instance().getDataCache(mem_id).getJoinIndex(join_index);
        }
    ...
    std::vector<ColumnPtr> copy_if_required(const std::vector<ColumnPtr>& col_vec, const ProcessorSpecification& proc_spec){
    ...
    PositionListPairPtr copy_if_required(PositionListPairPtr pair_tids, const ProcessorSpecification& proc_spec) {
        return copy_if_required(pair_tids, hype::util::getMemoryID(proc_spec.proc_id));
    }

    LookupColumnPtr copy_if_required(LookupColumnPtr lookup_column, const ProcessorSpecification& proc_spec) {
        return copy_if_required(lookup_column, hype::util::getMemoryID(proc_spec.proc_id));
    }

    JoinIndexPtr copy_if_required(JoinIndexPtr join_index, const ProcessorSpecification& proc_spec) {
        return copy_if_required(join_index, hype::util::getMemoryID(proc_spec.proc_id));
    }
