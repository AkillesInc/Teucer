...
namespace CoGaDB{
    using namespace std;
        ...
        Predicate::Predicate(const std::string& column1_name, const std::string& column2_name,PredicateType pred_t, ValueComparator comp) 
	: pred_t_(pred_t), column1_name_(column1_name), column2_name_(column2_name), constant_(),comp_(comp){ assert(pred_t==ValueValuePredicate);}

	Predicate::Predicate(const std::string& column1_name, const boost::any& constant,PredicateType pred_t, ValueComparator comp) 
	: pred_t_(pred_t), column1_name_(column1_name), column2_name_(),constant_(constant),comp_(comp){assert(pred_t==ValueConstantPredicate || pred_t==ValueRegularExpressionPredicate);}

        PredicateType Predicate::getPredicateType() const{
            return this->pred_t_;
        }
        ...
        const boost::any& Predicate::getConstant() const{
            return this->constant_;
        }
        ValueComparator Predicate::getValueComparator() const{
            return this->comp_;
        }
