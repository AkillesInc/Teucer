...
#define COGADB_SELECTION_BODY_BRANCH(array,value,i,array_tids,pos,COMPARATOR) if(array[i] COMPARATOR value) array_tids[pos++]=i;
#define COGADB_SELECTION_BODY_NOBRANCH(array,value,i,array_tids,pos,COMPARATOR) array_tids[pos]=i; pos+=(array[i] COMPARATOR value);
...
namespace CoGaDB {
    ...
    template<class T>
    Column<T>::Column(const std::string &name, AttributeType db_type, hype::ProcessingDeviceMemoryID mem_id)
            : ColumnBaseTyped<T>(name, db_type, PLAIN_MATERIALIZED), type_tid_comparator(), values_(0),
              num_elements_(0), buffer_size_(0), mem_alloc(MemoryAllocator<T>::getMemoryAllocator(mem_id)) {
    ...
    //fill constructor
    template<class T>
    Column<T>::Column(const std::string &name, AttributeType db_type, size_t number_of_elements, const T &val,
                      hype::ProcessingDeviceMemoryID mem_id) : ColumnBaseTyped<T>(name, db_type, PLAIN_MATERIALIZED),
                                                               type_tid_comparator(), values_(0), num_elements_(0),
                                                               buffer_size_(0), mem_alloc(
                    MemoryAllocator<T>::getMemoryAllocator(mem_id)) {
        if (this->capacity() < number_of_elements) {
            this->reserve(number_of_elements);
        }
        assert(this->mem_alloc->getMemoryID() == hype::PD_Memory_0);
        //std::memset(values_,val,number_of_elements);
        std::fill_n(values_, number_of_elements, val);
        num_elements_ = number_of_elements;
    }

    template<class T>
    Column<T>::Column(const Column<T> &x) : ColumnBaseTyped<T>(x.getName(), x.getType(), PLAIN_MATERIALIZED),
                                            type_tid_comparator(), values_(0), num_elements_(0), buffer_size_(0),
                                            mem_alloc(MemoryAllocator<T>::getMemoryAllocator(
                                                    x.mem_alloc->getMemoryID())) {

        this->reserve(x.num_elements_);
        assert(values_!=NULL);

//            COGADB_WARNING("Called Column<T>::Column(const Column<T>& x) with allocator " << this->mem_alloc->getName() ,"");

        if(typeid(T)!=typeid(std::string)){
            if(!x.empty()){
                typedef typename CopyFunctionFactory<T>::CopyFunctionPtr CopyFunctionPtr;
                CopyFunctionPtr func = CopyFunctionFactory<T>::getCopyFunction(this->mem_alloc->getMemoryID(), x.mem_alloc->getMemoryID());
                assert(func!=NULL);
...
//            COGADB_WARNING("Called Column<T> & Column<T>::operator= (const Column<T> & other))","");

        if (this != &other) // protect against invalid self-assignment
        {
            assert(this->mem_alloc->getMemoryID() == other.mem_alloc->getMemoryID());

            this->mem_alloc = MemoryAllocator<T>::getMemoryAllocator(other.mem_alloc->getMemoryID());
            ...
            if (typeid(T) != typeid(std::string)) {
                if (!other.empty()) {
                    typedef typename CopyFunctionFactory<T>::CopyFunctionPtr CopyFunctionPtr;
                    CopyFunctionPtr func = CopyFunctionFactory<T>::getCopyFunction(this->mem_alloc->getMemoryID(),
                                                                                   other.mem_alloc->getMemoryID());
                    //                    CopyFunctionPtr func = getCopyFunction(this->mem_alloc->getMemoryID(), other.mem_alloc->getMemoryID());
                    assert(func != NULL);
                    bool ret = (*func)(values_, other.values_, other.num_elements_ * sizeof(T));
                    assert(ret == true);
                }
            } else {
                assert(this->mem_alloc->getMemoryID() == hype::PD_Memory_0);
            ...
            //std::memcpy(values_,other.values_,other.num_elements_*sizeof(T));
...
//    template<class T>
//    const PositionListPtr Column<T>::selection(ColumnPtr comparison_column, const ValueComparator comp) {
//        //same behaviour as in parent class
//        return CoGaDB::ColumnBaseTyped<T>::selection(comparison_column, comp);
//    }
    ...
    bool Column<T>::insert(const boost::any &new_value) {
        ...
        T value;
        bool ret_success = getValueFromAny(this->name_, new_value, value, this->db_type_);
...
//        return false;
    ...
    bool Column<T>::remove(PositionListPtr tids) {
        ...
        while (loop_counter > 0) {
            ...
            //values_.erase(values_.begin()+(*tids)[loop_counter]);
        }
    ...
    void Column<T>::print() const throw() {
        ...
        std::cout << "| Memory: " << (int) this->mem_alloc->getMemoryID() << " |" << std::endl;
        std::cout << "| Type: " << util::getName(this->getType()) << " C++ type: " << typeid(T).name() << " |" <<
        std::endl;
        std::cout << "| Compression: " << util::getName(this->getColumnType()) << " C++ type: " << typeid(T).name() <<
        " |" << std::endl;
...
//            COGADB_ERROR("Out of Memory On Device " << (int)this->mem_alloc->getMemoryID(),"");
            COGADB_ERROR("Out of memory on device memory " << (int)this->mem_alloc->getMemoryID()
                      << " for column '" << this->getName() 
                      << "' requesting " << double(this->size()*sizeof(T))/(1024*1024*2014) << " GB memory" << std::endl
                      << "Total free memory: " 
                      << double(HardwareDetector::instance().getFreeMemorySizeInByte(this->mem_alloc->getMemoryID()))/(1024*1024*2014),"");
            return ColumnPtr();
        }
        return col;
    }
    ...
    template<class T>
    const ColumnPtr Column<T>::copy(const hype::ProcessingDeviceMemoryID& mem_id) const{

        boost::shared_ptr<Column<T> > col;
        try {
            col = boost::shared_ptr<Column<T> >(new Column<T>(this->getName(), this->getType(), mem_id));
            col->resize(this->size());
        } catch (std::bad_alloc& e) {
            COGADB_ERROR("Out of memory on device memory " << (int) mem_id
                    << " for column '" << this->getName()
                    << "' requesting " << double(this->size() * sizeof (T)) / (1024 * 1024 * 2014) << " GB memory" << std::endl
                    << "Total free memory: " << double(HardwareDetector::instance().getFreeMemorySizeInByte(mem_id)) / (1024 * 1024 * 2014), "");
            return ColumnPtr();
        }

        if (col->size() > 0) {
            typedef typename CopyFunctionFactory<T>::CopyFunctionPtr CopyFunctionPtr;
            CopyFunctionPtr func = CopyFunctionFactory<T>::getCopyFunction(mem_id, this->mem_alloc->getMemoryID());
            assert(func != NULL);
            if (!(*func)(col->data(), values_, num_elements_ * sizeof (T))) return ColumnPtr();
        }
        return col;
    }

    template<class T>
    const typename ColumnBaseTyped<T>::DenseValueColumnPtr Column<T>::copyIntoDenseValueColumn(const ProcessorSpecification& proc_spec) const{
        ...
        ColumnPtr copied_column = this->copy(hype::util::getMemoryID(proc_spec.proc_id));
        if(!copied_column) return DenseValueColumnPtr();
        
        DenseValueColumnPtr typed_copied_column = boost::dynamic_pointer_cast<DenseValueColumn>(copied_column);
        assert(typed_copied_column!=NULL);
        return typed_copied_column;
    }
    
    template<class T>
    const DoubleDenseValueColumnPtr Column<T>::convertToDenseValueDoubleColumn(const ProcessorSpecification& proc_spec) const{
        ProcessorBackend<T>* backend = ProcessorBackend<T>::get(proc_spec.proc_id);
        return backend->convertToDoubleDenseValueColumn(this->getName(), this->values_, this->size(), proc_spec);
    }
    ...
    template<class T>
    const StringDenseValueColumnPtr Column<T>::convertToDenseValueStringColumn() const{
        assert(mem_alloc->getMemoryID()==hype::PD_Memory_0);
        StringDenseValueColumnPtr string_column (new StringDenseValueColumn(this->getName(),VARCHAR,hype::PD_Memory_0));
    ...
    template<>
    const StringDenseValueColumnPtr Column<std::string>::convertToDenseValueStringColumn() const{
        ColumnPtr col = this->copy();
        if(!col) return StringDenseValueColumnPtr();
        StringDenseValueColumnPtr result = boost::dynamic_pointer_cast<StringDenseValueColumn>(col);
        assert(result!=NULL);
        return result;
    }
    
    template<class T>
    const ColumnPtr Column<T>::gather(PositionListPtr tid_list, const GatherParam &param) {
        boost::shared_ptr<Column<T> > result(
                new Column<T>(this->name_, this->db_type_, this->mem_alloc->getMemoryID()));
        try {
        ...
        ProcessorBackend<T> *backend = ProcessorBackend<T>::get(param.proc_spec.proc_id);
    ...
    template<class T>
    const ColumnGroupingKeysPtr Column<T>::createColumnGroupingKeys(const ProcessorSpecification &proc_spec) const {

        ProcessorBackend<T> *backend = ProcessorBackend<T>::get(proc_spec.proc_id);
        return backend->createColumnGroupingKeys(this->values_, this->size(), proc_spec);
    ...
    const ColumnGroupingKeysPtr Column<std::string>::createColumnGroupingKeys(
        ...
        COGADB_ERROR("Called function createColumnGroupingKeys()m for unsupported data type: std::string", "");
        return ColumnGroupingKeysPtr(); 
    } 
    
    template<>
    const ColumnGroupingKeysPtr Column<char*>::createColumnGroupingKeys(const ProcessorSpecification& proc_spec) const{
        COGADB_ERROR("Called function createColumnGroupingKeys()m for unsupported data type: std::string", "");
        return ColumnGroupingKeysPtr(); 
    } 
    ...
    size_t Column<T>::getNumberOfRequiredBits() const {
        ...
        T max = *std::max_element(values_, values_ + num_elements_);
...
//
//         /* \fixme \todo check whether this column is and intermediate (query compiler) */
//        /* check whether statistics are available and compute them if neccesary */
//        if(!this->statistics_.statistics_up_to_date_){
//            if(!this->computeColumnStatistics()){
//                COGADB_FATAL_ERROR("Could not compute statistics for column " 
//                        << this->getName() << "!","");
//            }
//        }
//        /* fetch information from statistics */
//        T max = this->statistics_.max;
//        T min = this->statistics_.min;
//        /* check if we need all bits (in case negativ bit is set) */
//        if (min < 0) return sizeof(T) * 8;
//        return getGreaterPowerOfTwo(max);
    }
    ...
    template<class T>
    const AggregationResult Column<T>::aggregateByGroupingKeys(ColumnGroupingKeysPtr grouping_keys,
                                                               const AggregationParam &param) {
        ProcessorBackend<T> *backend = ProcessorBackend<T>::get(param.proc_spec.proc_id);
        AggregationResult result;
        result = backend->aggregateByGroupingKeys(grouping_keys, this->data(), this->size(), param);
    ...
    const AggregationResult Column<T>::aggregate(const AggregationParam& param){
        ProcessorBackend<T>* backend = ProcessorBackend<T>::get(param.proc_spec.proc_id);
        AggregationResult result = backend->aggregate(this->data(), this->size(), param);
        return result;
    }

    template<class T>
    const PositionListPairPtr Column<T>::join(ColumnPtr join_column, const JoinParam &param) {
        ...
        if (this->getColumnType() == join_column->getColumnType()) {
            DenseValueColumnPtr dense_join_column = boost::dynamic_pointer_cast<DenseValueColumn>(join_column);
            assert(dense_join_column != NULL);
            result = this->join(*dense_join_column.get(), param);
        } else  {
            boost::shared_ptr<ColumnBaseTyped<T> > typed_join_column = boost::dynamic_pointer_cast<ColumnBaseTyped<T> >(
                    join_column);
            assert(typed_join_column != NULL);
            DenseValueColumnPtr dense_join_column = typed_join_column->copyIntoDenseValueColumn(param.proc_spec);
    ...
    template<class T>
    const PositionListPairPtr Column<T>::join(Column<T> &join_column, const JoinParam &param) {

        ProcessorBackend<T> *backend = ProcessorBackend<T>::get(param.proc_spec.proc_id);
        return backend->join(this->data(), this->size(), join_column.data(), join_column.size(), param);

    }

    template<class T> 
    const PositionListPtr Column<T>::tid_semi_join(ColumnPtr join_column, const JoinParam& param){
        ...
        if(this->getColumnType()==join_column->getColumnType()){
            ...
            boost::shared_ptr<ColumnBaseTyped<T> > typed_join_column = boost::dynamic_pointer_cast<ColumnBaseTyped<T> >(join_column);
            assert(typed_join_column!=NULL);
            DenseValueColumnPtr dense_join_column = typed_join_column->copyIntoDenseValueColumn(param.proc_spec);
    ...
    template<class T> 
    const PositionListPtr Column<T>::tid_semi_join(Column<T>& join_column, const JoinParam& param){
        ProcessorBackend<T>* backend = ProcessorBackend<T>::get(param.proc_spec.proc_id);
        return backend->tid_semi_join(this->data(), this->size(), join_column.data(), join_column.size(), param);
    }
    
    template<class T>   
    const BitmapPtr Column<T>::bitmap_semi_join(ColumnPtr join_column, const JoinParam& param){
        typedef typename ColumnBaseTyped<T>::DenseValueColumnPtr DenseValueColumnPtr;
        typedef typename ColumnBaseTyped<T>::DenseValueColumn DenseValueColumn;
    ...
    template<class T> 
    const BitmapPtr Column<T>::bitmap_semi_join(Column<T>& join_column, const JoinParam& param){
        ProcessorBackend<T>* backend = ProcessorBackend<T>::get(param.proc_spec.proc_id);
        return backend->bitmap_semi_join(this->data(), this->size(), join_column.data(), join_column.size(), param);
    } 
    

    template<class T>
    const ColumnPtr Column<T>::column_algebra_operation(ColumnPtr source_column, const AlgebraOperationParam &param) {
        typedef typename ColumnBaseTyped<T>::DenseValueColumnPtr DenseValueColumnPtr;
        typedef typename ColumnBaseTyped<T>::DenseValueColumn DenseValueColumn;
...
//                //place column, so we can make the conversion on the specified processor
...
//                    decompressed_source_column = boost::dynamic_pointer_cast<DoubleDenseValueColumn>(placed_col);
//                    assert(decompressed_source_column!=NULL);
//                }else{
//                    //decompress column or materialize intermediate column
//                    
//                    DenseValueColumnPtr dense_value_col = placed_col->
//                }
//            }else{
                converted_column = source_column->convertToDenseValueDoubleColumn(param.proc_spec);
            }
            ...
            if(this->getType()==DOUBLE){
                ...
                DoubleDenseValueColumnPtr double_column = this->convertToDenseValueDoubleColumn(param.proc_spec);
        ...
        if (this->size() != source_column->size()) {
            COGADB_FATAL_ERROR("Cannot perform algebra operation on columns with different number of elements!", "");
            return ColumnPtr();
        }
        ...
        if(this->getColumnType()==source_column->getColumnType()){
            DenseValueColumnPtr dense_source_column = boost::dynamic_pointer_cast<DenseValueColumn>(placed_col);
            assert(dense_source_column!=NULL);
            return this->column_algebra_operation(*dense_source_column.get(), param);
        }else{
            boost::shared_ptr<ColumnBaseTyped<T> > typed_source_column = boost::dynamic_pointer_cast<ColumnBaseTyped<T> >(placed_col);
            assert(typed_source_column!=NULL);
            DenseValueColumnPtr dense_source_column = typed_source_column->copyIntoDenseValueColumn(param.proc_spec);
    ...
    template<class T>
    const ColumnPtr Column<T>::column_algebra_operation(Column<T> &source_column, const AlgebraOperationParam &param) {
        typedef typename ColumnBaseTyped<T>::DenseValueColumnPtr DenseValueColumnPtr;
        typedef typename ColumnBaseTyped<T>::DenseValueColumn DenseValueColumn;
        DenseValueColumnPtr dense_column = this->copyIntoDenseValueColumn(param.proc_spec);
        if (!dense_column) return ColumnPtr();
        ProcessorBackend<T> *backend = ProcessorBackend<T>::get(param.proc_spec.proc_id);
        if (backend->column_algebra_operation(dense_column->data(), source_column.data(), dense_column->size(),
                                              param)) {
            return dense_column;
        } else {
            return ColumnPtr();
    ...
    template<class T>
    const ColumnPtr Column<T>::column_algebra_operation(const boost::any &value, const AlgebraOperationParam &param) {
        typedef typename ColumnBaseTyped<T>::DenseValueColumnPtr DenseValueColumnPtr;
        typedef typename ColumnBaseTyped<T>::DenseValueColumn DenseValueColumn;
        ...
        if(value.type()==typeid(T)){
            ...
            if(!dense_column) return ColumnPtr();
            ProcessorBackend<T>* backend = ProcessorBackend<T>::get(param.proc_spec.proc_id);
            if(backend->column_algebra_operation(dense_column->data(), dense_column->size(), typed_value, param)){
                return dense_column;
            }else{
                return ColumnPtr();
            }
            ...
            bool ret_success = getValueFromAny(this->name_, value, double_value, this->db_type_);
            if(!ret_success) ColumnPtr();
            //convert this column in double column
            DoubleDenseValueColumnPtr double_column = this->convertToDenseValueDoubleColumn(param.proc_spec);
            if(!double_column) return ColumnPtr();
            return double_column->column_algebra_operation(boost::any(double_value), param);
        }
    }
    ...
    const PositionListPtr Column<std::string>::sort(const SortParam &param) {
        ...
        if (param.order == ASCENDING) {
            std::stable_sort(v.begin(), v.end(), std::less_equal<std::pair<std::string, TID> >());
        } else if (param.order == DESCENDING) {
            std::stable_sort(v.begin(), v.end(), std::greater_equal<std::pair<std::string, TID> >());

        } else {
            std::cout << "FATAL ERROR: Column<T>::sort(): Unknown Sorting Order!" << std::endl;
        }
    ...
    const PositionListPtr Column<char*>::sort(const SortParam& param)
    {
        ...
        if(param.order==ASCENDING){
                std::stable_sort(v.begin(),v.end(),std::less_equal<std::pair<char*,TID> >());
        }else if(param.order==DESCENDING){
                std::stable_sort(v.begin(),v.end(),std::greater_equal<std::pair<char*,TID> >()); 
    ...
    template<class T>
    bool Column<T>::store_impl(const std::string& path, boost::archive::binary_oarchive& oa) {
        ...
        size_t written_elements = fwrite (this->values_, sizeof(T), this->num_elements_, file);
        if(written_elements!=this->num_elements_){
            COGADB_FATAL_ERROR("Did not write as many elements as required during store operation of column '" << this->getName() << "'!","");
        }
        int err = fclose (file);
        assert(err==0);
        return true;
    ...
    template<>
    bool Column<std::string>::store_impl(const std::string& path, boost::archive::binary_oarchive& oa) {
        //write additional information to output archive
        oa << this->num_elements_;
    ...
    template<class T>
    bool Column<T>::load_impl(const std::string& path, boost::archive::binary_iarchive& ia){
        ...
        size_t read_elements = fread (this->values_, sizeof(T), this->num_elements_, file);
        if(read_elements!=this->num_elements_){
            COGADB_FATAL_ERROR("Did not read as many elements as required during load operation of column '" << this->getName() << "'!","");
        }
        int err = fclose (file);
        assert(err==0);
    ...
    template<>
    bool Column<std::string>::load_impl(const std::string& path, boost::archive::binary_iarchive& ia){
...
//    }
    ...
    /*
        template<>
        bool& Column<bool>::operator[](const int index) const{
		
                return false; // values_[index];
        }*/
    template<class T>
    ...
    const PositionListPairPtr Column<T>::hash_join(ColumnPtr join_column_) {
        ...
        if (join_column_->type() != typeid(T)) {
            std::cout << "Fatal Error!!! Typemismatch for columns " << this->name_ << " and " <<
            join_column_->getName() << std::endl;
        ...
        if (join_column_->isMaterialized()) {
            shared_pointer_namespace::shared_ptr <Column<T> > join_column = shared_pointer_namespace::dynamic_pointer_cast<Column<T> >(
                    join_column_);
            assert(join_column != NULL);
            input_column = join_column->data(); // hype::util::begin_ptr(join_column->values_);
            assert(input_column != NULL);

            T *this_array = this->data(); //hype::util::begin_ptr(this->values_);
            assert(this_array != NULL);
            return CDK::join::serial_hash_join(this_array, this->size(), input_column, join_column->size());


        } else if (!join_column_->isMaterialized() && !join_column_->isCompressed()) { //Lookup Array?
            shared_pointer_namespace::shared_ptr <LookupArray<T> > join_column = shared_pointer_namespace::dynamic_pointer_cast<LookupArray<T> >(
                    join_column_);
            assert(join_column != NULL);
            //PositionListPtr tids = join_column->getPositionList();
//                shared_pointer_namespace::shared_ptr<ColumnBaseTyped<T> > base_column_abstract = join_column->getIndexedColumn();
//                shared_pointer_namespace::shared_ptr<Column<T> > base_column = shared_pointer_namespace::dynamic_pointer_cast<Column<T> >(base_column_abstract);
//                assert(base_column!=NULL);
//                T* base_column_array = hype::util::begin_ptr(base_column->values_);
//                assert(base_column_array!=NULL);
//                //allocate temporary array
//                input_column = (T*) malloc(sizeof(T)*tids->size());
...
////                CDK::util::serial_gather(base_column_array, tids->data(), tids->size(), input_column);
//                if(tids->size()<300){
//                        CDK::util::serial_gather(base_column_array, tids->data(), tids->size(), input_column);
//                }else{
//                        CDK::util::parallel_gather(base_column_array, tids->data(), tids->size(), input_column, boost::thread::hardware_concurrency());
//                }
            T *input_column = join_column->materializeToArray();

            T *this_array = this->data();   //hype::util::begin_ptr(this->values_);
            assert(this_array != NULL);
            //PositionListPairPtr ret = CDK::join::serial_hash_join(this_array, this->size(), input_column, join_column->size());
            PositionListPairPtr ret = CDK::join::serial_hash_join(this_array, this->size(), input_column,
                                                                  join_column->size());
            ...
            COGADB_FATAL_ERROR(
                    "Detected Invalid Column Class! Not MAterialized, No LookupARray and no CompressedColumn!", "");
            return PositionListPairPtr();
        }
    }
    ...
    const PositionListPairPtr Column<T>::radix_join(ColumnPtr join_column_) {
        COGADB_FATAL_ERROR(
                "called non INT instantiation of Column<T>::radix_join, which only supports integer columns!", "");
        return PositionListPairPtr();
    }
...
//                COGADB_FATAL_ERROR("Radix Join works only for integer columns!","");
//            }
        ...
        if (join_column_->isMaterialized()) {
            shared_pointer_namespace::shared_ptr <Column<int> > join_column = shared_pointer_namespace::dynamic_pointer_cast<Column<int> >(
                    join_column_);
            assert(join_column != NULL);
            input_column = join_column->data();  //hype::util::begin_ptr(join_column->values_);
            assert(input_column != NULL);
            ...
            shared_pointer_namespace::shared_ptr <LookupArray<int> > join_column = shared_pointer_namespace::dynamic_pointer_cast<LookupArray<int> >(
                    join_column_);
            assert(join_column != NULL);
            //PositionListPtr tids = join_column->getPositionList();
//                shared_pointer_namespace::shared_ptr<ColumnBaseTyped<T> > base_column_abstract = join_column->getIndexedColumn();
...
//                }
            ...
            int *this_array = this->data(); //hype::util::begin_ptr(this->values_);
            assert(this_array != NULL);
            PositionListPairPtr ret = CDK::join::radix_join(this_array, this->size(), input_column,
                                                            join_column->size()); // serial_hash_join(this_array, this->size(), input_column, join_column->size());
            //ok, job done, cleanup temporary array
            free(input_column);
            return ret;
        } else if (join_column_->isCompressed()) {
...
//                assert(values_!=NULL);
    ...
    void Column<T>::resize(size_t new_size) {
        ...
        if (new_size > buffer_size_) {
            //values_=(T*) realloc(values_,new_size*sizeof(T));
...
//        template<class T>        
//        void Column<T>::push_back(const T& val) __attribute__((always_inline)){
//            //std::cout << "#elements: " << num_elements_ << " buffer_size: " << buffer_size_ << std::endl;
...
//        }
    ...
    template<class T>
    void selection_thread(unsigned int thread_id, unsigned int number_of_threads, const T &value,
                          const ValueComparator comp, ColumnBaseTyped<T> *col, TID *result_tids, size_t *result_size) {
    ...
    template<>
    void selection_thread<int>(unsigned int thread_id, unsigned int number_of_threads, const int &value,
                               const ValueComparator comp, ColumnBaseTyped<int> *col, TID *result_tids,
                               size_t *result_size) {
        //std::cout << "Hi I'm thread" << thread_id << std::endl;
        if (!quiet) std::cout << "Using CPU for Selection (parallel mode)..." << std::endl;
        ...
        size_t pos = 0;
            ...
            //if column is materialized, we can use our SIMD SCAN; ensure we have enough elements, otherwise SIMD SCAN might crash
            if(col->isMaterialized() && col->size()>=300){
                ...
                simd_selection_int_thread(&array[begin_index], array_size,  value, comp, &result_tids[begin_index], &pos, begin_index);
                //write result size to array
                *result_size=pos;
                return;
            }
    ...
    void write_selection_result_thread(unsigned int thread_id, unsigned int number_of_threads, size_t column_size,
        ...
        std::memcpy(&position_list_output_array[result_begin_index], &result_tids_array[begin_index],
...
//            std::memcpy(&join_tids_result_table1[begin_index_result],&join_tids_table1[begin_index],result_size*sizeof(T));
//            std::memcpy(&join_tids_result_table2[begin_index_result],&join_tids_table2[begin_index],result_size*sizeof(T));
//            
//            //assert(begin_index+result_size<join_column->size());
//            for(unsigned int i=0;i<result_size;++i){
//                assert(i+begin_index<end_index);
//                assert(i+begin_index_result<end_index_result);
//                assert(end_index<=join_column->size());
//                assert(end_index_result<=join_column->size());
//                
//                
//                  unsigned int tmp1=join_tids_table1[i+begin_index];
//                  join_tids_result_table1[i+begin_index_result]=tmp1;
...
//template<class T>
//void Column_Base_Typed<T>::write_selection_result_thread(unsigned int thread_id, unsigned int number_of_threads, unsigned int total_result_size, ColumnBaseTyped<T>* col, unsigned int* result_tids, unsigned int* result_size){
//	//std::cout << "Hi I'm thread" << thread_id << std::endl;
//	if(!quiet) std::cout << "Using CPU for Selection (parallel mode)..." << std::endl;
//            ColumnBaseTyped<T>& column_ref = dynamic_cast< ColumnBaseTyped<T>& >(*col);
//            //number of elements per thread
...
//}    
    ...
    template<class T>
    const PositionListPtr ColumnBaseTyped<T>::parallel_selection(const boost::any &value_for_comparison,
        ...
        if (result_tids_array == NULL) {
            COGADB_FATAL_ERROR("Malloc failed! While trying to allocate " << this->size() * sizeof(TID) << " bytes",
                               "");
        }
...
//               result_tids->resize(this->size());
        ...
        threads.add_thread(
                new boost::thread(boost::bind(&CoGaDB::resize_PositionListPtr_thread, result_tids, this->size())));
        for (unsigned int i = 0; i < number_of_threads; i++) {
            //create a selection thread
            threads.add_thread(new boost::thread(
                    boost::bind(&CoGaDB::selection_thread<T>, i, number_of_threads, value, comp, this,
...
//                for(unsigned int i=0;i<number_of_threads;i++){
//                    cout << "Result size thread "<< i << ": " << result_sizes[i] << endl;
//                }

//                //copy result chunks in vector
//                size_t chunk_size = this->size()/number_of_threads;
//                for(unsigned int i=0;i<number_of_threads;i++){
//                    size_t begin_index=chunk_size*i;
//                    //cout << "thread " << i << " start index: " << begin_index << " end index: " << begin_index+result_sizes[i] << endl; 
//                    
...
//                }
                ...
                for(unsigned int i=0;i<number_of_threads;i++){
                    ...
                    threads.add_thread(new boost::thread(boost::bind(&write_selection_result_thread, i, number_of_threads, this->size(), result_tids_array, result_tids, result_sizes[i], prefix_sum[i], prefix_sum[i+1])));
        ...
        const PositionListPtr Column<T>::selection(const SelectionParam& param){
            ...
            if(param.pred_type==ValueConstantPredicate){
                T value;
                bool ret_success = getValueFromAny(this->name_, param.value, value, this->db_type_);
                ...
                if(this->getType()!=param.comparison_column->getType()){
                    ...
                    if(this->getType()!=VARCHAR && param.comparison_column->getType()!=VARCHAR){
                        SelectionParam new_param(param);
                        ...
                        if(param.comparison_column->getType()!=DOUBLE){
                              ...
                              //besides VARCHAR
//                              boost::shared_ptr<ColumnBaseTyped<T> > typed_column = boost::dynamic_pointer_cast<ColumnBaseTyped<T> >(param.comparison_column);
//                              assert(typed_column!=NULL);
                              //copy into double column
                              DoubleDenseValueColumnPtr dense_column = param.comparison_column->convertToDenseValueDoubleColumn(param.proc_spec);
                        ...
                        if(this->getType()==DOUBLE){
                              ...
                              DoubleDenseValueColumnPtr this_dense_column = this->convertToDenseValueDoubleColumn(param.proc_spec);
                              if(!this_dense_column) return PositionListPtr();
                              return this_dense_column->selection(new_param);
                        }                        
                        
                    }else{
                        COGADB_FATAL_ERROR("Cannot compare columns with different types, where one column is a VARCHAR Type and the other is not!",""); 
                        return PositionListPtr();
                    }
                }
                ...
                if(this->getColumnType()==param.comparison_column->getColumnType()){
                      DenseValueColumnPtr column = boost::dynamic_pointer_cast<DenseValueColumn>(param.comparison_column);
                      assert(column!=NULL);
                    //place column
                    ...
                    DenseValueColumnPtr typed_column = boost::dynamic_pointer_cast<DenseValueColumn>(placed_column);
                    SelectionParam new_param(param);
                    new_param.comparison_column=typed_column;
                      
                    assert(new_param.comparison_column->getMemoryID()==hype::util::getMemoryID(new_param.proc_spec.proc_id));
                    return backend->tid_selection(this->data(), this->size(), new_param);
                }else{
                    boost::shared_ptr<ColumnBaseTyped<T> > typed_column = boost::dynamic_pointer_cast<ColumnBaseTyped<T> >(param.comparison_column);
                    assert(typed_column!=NULL);
                    DenseValueColumnPtr dense_column = typed_column->copyIntoDenseValueColumn(param.proc_spec);
                    if(!dense_column) return PositionListPtr();
...
#ifdef ENABLE_SIMD_ACCELERATION
        template<>
        inline const PositionListPtr Column<int>::selection(const boost::any& value_for_comparison, const ValueComparator comp){
...
//                COGADB_SIMD_SCAN_INT(array,array_size,result_array,comparison_value+1,_mm_cmplt_epi32,<,result_size);
//            }else if(comp==GREATER){
//                COGADB_SIMD_SCAN_INT(array,array_size,result_array,comparison_value,_mm_cmpgt_epi32,>,result_size);
//            }else if(comp==GREATER_EQUAL){
//                //substract one of comparison value to compare for greater equal
//                COGADB_SIMD_SCAN_INT(array,array_size,result_array,comparison_value-1,_mm_cmpgt_epi32,>,result_size);
//            }else{
...
//        if (param.value.type() != typeid(T)) {
//            
//            //catch some special cases
//            if (typeid(T) == typeid(float) && param.value.type() == typeid(int)) {
//                value = boost::any_cast<int>(param.value);
//            } else {
//                std::cout << "Fatal Error!!! Typemismatch for column " << this->name_ << std::endl;
//                std::cout << "Column Type: " << typeid(T).name() << " filter value type: " <<
//                param.value.type().name() << std::endl;
//                std::cout << "File: " << __FILE__ << " Line: " << __LINE__ << std::endl;
//                exit(-1);
//            }
//        } else {
...
//            SelectionParam param(proc_spec, ValueConstantPredicate, boost::any(value), comp);

        ProcessorBackend<T> *backend = ProcessorBackend<T>::get(param.proc_spec.proc_id);
        return backend->bitmap_selection(this->data(), this->size(), new_param);

//            T* values=this->data();
//            return CDK::selection::bitmap_selection(values, this->size(), value_for_comparison, comp);
    }

    template<typename T>
    void ColumnBaseTyped<T>::hash_join_pruning_thread(ColumnBaseTyped<T> *join_column, HashTable *hashtable,
                                                      TID *join_tids_table1, TID *join_tids_table2, size_t thread_id,
                                                      size_t number_of_threads, size_t *result_size) {
        ColumnBaseTyped<T> &join_column_ref = dynamic_cast< ColumnBaseTyped<T> & >(*join_column);
        //number of elements per thread
        ...
        if (!quiet && verbose && debug)
            std::cout << "Parallel Hash Join for array of size: " << join_column_ref.size() << " thread id: " <<
            thread_id << " chunksize: " << chunk_size << " elements  begin index: " << begin_index << " end index: " <<
            end_index << std::endl;
        ...
        for (size_t i = begin_index; i < end_index; i++) {
            ...
            for (it = range.first; it != range.second; ++it) {
                if (it->first == join_column_ref[i]) { //(*join_column)[i]){
                    ...
                    if (!quiet && verbose && debug)
                        std::cout << "match! " << it->second << ", " << i << "	" << it->first << std::endl;
                }
            }
        }
        assert(pos1 == pos2);
    ...
    template<typename T>
    void ColumnBaseTyped<T>::join_write_result_chunk_thread(ColumnBaseTyped<T> *join_column, TID *join_tids_table1,
                                                            TID *join_tids_table2, TID *join_tids_result_table1,
                                                            TID *join_tids_result_table2, size_t thread_id,
                                                            size_t number_of_threads, TID begin_index_result,
                                                            TID end_index_result) {
        assert(join_column != NULL);
...
//            }
        ...
        assert(join_column != NULL);
        if (!quiet && verbose && debug)
            cout << "Maximal result tuples for parallel PK FK Join: " << join_column->size() << endl;
        //end_index=(chunk_size*thread_id)+result_size;
        if (!quiet && verbose && debug)
            std::cout << "Parallel Hash Join for array of size: " << join_column->size() << " thread id: " <<
            thread_id << " chunksize: " << chunk_size << " elements  begin index: " << begin_index << " end index: " <<
            end_index << std::endl;
        ...
        //copy memory chunk to result array
        std::memcpy(&join_tids_result_table1[begin_index_result], &join_tids_table1[begin_index],
                    result_size * sizeof(TID));
        std::memcpy(&join_tids_result_table2[begin_index_result], &join_tids_table2[begin_index],
                    result_size * sizeof(TID));
...
//                  unsigned int tmp1=join_tids_table1[i+begin_index];
//                  join_tids_result_table1[i+begin_index_result]=tmp1;
//                  unsigned int tmp2=join_tids_table2[i+begin_index];
//                  join_tids_result_table2[i+begin_index_result]=tmp2; 
//            }

    }

    template<class T>
    const PositionListPairPtr ColumnBaseTyped<T>::parallel_hash_join(ColumnPtr join_column_,
                                                                     unsigned int number_of_threads) {
        ...
        shared_pointer_namespace::shared_ptr <ColumnBaseTyped<T> > join_column = shared_pointer_namespace::static_pointer_cast<ColumnBaseTyped<T> >(
...
//               ColumnBaseTyped<T>& join_column_ref = dynamic_cast< ColumnBaseTyped<T>& >(*join_column);
        ...
        for (size_t i = 0; i < number_of_threads; i++) {
            //ColumnBaseTyped<T>::hash_join_pruning_thread(join_column.get(), &hashtable, join_tids_table1, join_tids_table2, i, number_of_threads);
            //CoGaDB::hash_join_pruning_thread<T,HashTable>(join_column_ref, &hashtable, join_tids_table1, join_tids_table2, i, number_of_threads);
            //threads.add_thread(new boost::thread((&hash_join_pruning_thread<T,HashTable>),join_column_ref, &hashtable, join_tids_table1, join_tids_table2, i, number_of_threads));
            //ColumnBaseTyped<T>& join_column_ref, HashTable* hashtable, unsigned int* join_tids_table1, unsigned int* join_tids_table2, unsigned int thread_id, unsigned int number_of_threads)
            threads.add_thread(new boost::thread(
                    boost::bind(&ColumnBaseTyped<T>::hash_join_pruning_thread, join_column.get(), &hashtable,
                                join_tids_table1, join_tids_table2, i, size_t(number_of_threads),
        ...
        TID *result_tids_table1 = join_tids->first->data(); //hype::util::begin_ptr(*join_tids->first);
        TID *result_tids_table2 = join_tids->second->data(); //hype::util::begin_ptr(*join_tids->second);
        ...
        for (size_t i = 0; i < number_of_threads; i++) {
            if (!quiet && verbose && debug)
                cout << "Begin Index Result: " << begin_index << " End Index Result: " << end_index << endl;
//(ColumnBaseTyped<T>* join_column, unsigned int* join_tids_table1, unsigned int* join_tids_table2, unsigned int* join_tids_result_table1, unsigned int* join_tids_result_table2, unsigned int thread_id, unsigned int number_of_threads, unsigned int result_size, size_t begin_index_result, size_t end_index_result)
            //threads.add_thread(new boost::thread(boost::bind(&ColumnBaseTyped<T>::join_write_result_chunk_thread, join_column.get(), join_tids_table1, join_tids_table2, result_tids_table1, result_tids_table2, i, number_of_threads, begin_index, end_index)));
            ColumnBaseTyped<T>::join_write_result_chunk_thread(join_column.get(), join_tids_table1, join_tids_table2,
                                                               result_tids_table1, result_tids_table2, i,
                                                               size_t(number_of_threads), begin_index, end_index);
            ...
            if (!quiet && verbose && debug)
                cout << "thread: " << i << " result size: " << thread_result_sizes[i] << " Total Result size: " <<
                total_result_size << endl;
        }



        //put results in position list
        //join_tids->first->insert(join_tids->first->end(),join_tids_table1,join_tids_table1+join_column_size); //assume PK FK Join!
        ...
        if (!quiet && verbose && debug)
            ...
            double(build_hashtable_end - build_hashtable_begin) / (1000 * 1000) << "ms"
            << "Pruning Phase: " << double(prune_hashtable_end - prune_hashtable_begin) / (1000 * 1000) << "ms" <<
            std::endl;
...
//                Column<int> col_int("",INT);
//                Column<float> col_float("",FLOAT);
//                Column<std::string> col_string("",VARCHAR);
//                
//                //ColumnBaseTyped<int>
//        }
