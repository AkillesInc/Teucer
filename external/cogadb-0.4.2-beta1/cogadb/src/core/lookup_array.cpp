...
{
//    assert(col!=NULL);
    if(!col || !tids_) return ColumnPtr();
    
    AttributeType type=col->getType();   
    std::string column_name=col->getName();
    
    if(type==INT){
            ColumnPtr lookup_array(new LookupArray<int>(column_name,INT,col,tids_));
            return lookup_array;
    }else if(type==FLOAT){
...
//		}
	...
	bool LookupArray<T>::clearContent(){
        ...
        const PositionListPtr LookupArray<T>::selection(const SelectionParam& param){
            ...
            ColumnPtr materialized_column = placed_column->gather(placed_tids, gather_param);
...
//            if(this->column_->isMaterialized() && !this->column_->isCompressed()){
//                shared_pointer_namespace::shared_ptr<Column<T> > base_column = shared_pointer_namespace::dynamic_pointer_cast<Column<T> >(this->column_);
//                assert(base_column!=NULL);
//            
//                T* base_array = base_column->data(); //hype::util::begin_ptr(base_column->getContent());
//                assert(base_array!=NULL);
//
//                TID* tids = this->tids_->data();
//                assert(tids!=NULL);
//                
//                T* input_column = (T*) malloc(sizeof(T)*this->tids_->size());
...
//                        CDK::util::serial_gather(base_array, tids, this->tids_->size(), input_column);
//                }else{
//                        CDK::util::parallel_gather(base_array, tids, this->tids_->size(), input_column, boost::thread::hardware_concurrency());
//                }
//                
//                //perform parallel selection on the generated input column
//                PositionListPtr result = CDK::selection::parallel_selection(input_column, this->tids_->size(), value_for_comparison, comp, boost::thread::hardware_concurrency());
//                free(input_column);
//                return result;
//            }else{
//                //for strings we can use the default method
//                return CoGaDB::ColumnBaseTyped<T>::selection(value_for_comparison, comp);
//            }
        }
        
//        template<class T>
//        const PositionListPtr LookupArray<T>::selection(ColumnPtr comparison_column, const ValueComparator comp){
...
//        }
	...
	const PositionListPairPtr LookupArray<T>::hash_join(ColumnPtr join_column_){
            ...
            if(join_column_->isMaterialized()){
                shared_pointer_namespace::shared_ptr<Column<T> > join_column = shared_pointer_namespace::dynamic_pointer_cast<Column<T> >(join_column_);
                assert(join_column!=NULL);
                input_column=join_column->data();
                assert(input_column!=NULL);
        
                ret = CDK::join::serial_hash_join(this_array, this->size(), input_column, join_column->size());
                //ret = CDK::main_memory_joins::serial_hash_join(this_array, this->size(), input_column, join_column->size());

            }else if (!join_column_->isMaterialized() && !join_column_->isCompressed()){ //Lookup Array?
                shared_pointer_namespace::shared_ptr<LookupArray<T> > join_column = shared_pointer_namespace::dynamic_pointer_cast<LookupArray<T> >(join_column_);
                assert(join_column!=NULL); 
                ...
                COGADB_FATAL_ERROR("Detected Invalid Column Class! Not MAterialized, No LookupARray and no CompressedColumn!","");
...
//        template<>
//	const PositionListPairPtr LookupArray<int>::hash_join(ColumnPtr join_column_){
//            assert(this->size()<=join_column_->size()); 
...
//            PositionListPairPtr ret;
//            if(join_column_->isMaterialized()){
//                shared_pointer_namespace::shared_ptr<Column<int> > join_column = shared_pointer_namespace::dynamic_pointer_cast<Column<int> >(join_column_);
//                assert(join_column!=NULL);
//                input_column=join_column->data();
//                assert(input_column!=NULL);
//        
...
//
//            }else if (!join_column_->isMaterialized() && !join_column_->isCompressed()){ //Lookup Array?
//                shared_pointer_namespace::shared_ptr<LookupArray<int> > join_column = shared_pointer_namespace::dynamic_pointer_cast<LookupArray<int> >(join_column_);
//                assert(join_column!=NULL); 
//
...
//        }    
	...
	const PositionListPairPtr LookupArray<T>::radix_join(ColumnPtr join_column_){
            COGADB_FATAL_ERROR("called non INT instantiation of LookupArray<T>::radix_join, which only supports integer columns!","");
            return PositionListPairPtr(); 
        }
        
        template<>
	inline const PositionListPairPtr LookupArray<int>::radix_join(ColumnPtr join_column_){
            assert(this->size()<=join_column_->size());
            if(join_column_->type()!=typeid(int)){
                    std::cout << "Fatal Error!!! Typemismatch for columns " << this->name_  << " and " << join_column_->getName() << std::endl;
                    std::cout << "File: " << __FILE__ << " Line: " << __LINE__ << std::endl;
            ...
            if(join_column_->isMaterialized()){
                ...
                ret = CDK::join::radix_join(this_array, this->size(), input_column, join_column->size());
	...
	template<class T>
        hype::ProcessingDeviceMemoryID LookupArray<T>::getMemoryID() const{
            //in case memory locations differ, always return the memory ID of the non CPU,
            ...
            if(column_->getMemoryID()==tids_->getMemoryID()){
                ...
                if(column_->getMemoryID()==hype::PD_Memory_0){
                    ...
                    COGADB_FATAL_ERROR("An Unhandled Error Condition Occurred, check the code for details!","");
                    return hype::PD_Memory_0;
                }
            }
        }
        
        /* \todo REMOVE this function, it is deprecated and should no longer be used! Use copyIntoDenseValueColumn Instead!*/
	template<class T>
        T* LookupArray<T>::materializeToArray() throw(){
                if(this->size()==0 || this->column_->size()==0) return NULL;
                shared_pointer_namespace::shared_ptr<Column<T> > base_column = shared_pointer_namespace::dynamic_pointer_cast<Column<T> >(this->column_);
                assert(base_column!=NULL);
                T* base_column_array = base_column->data(); //hype::util::begin_ptr(base_column->values_);
...
//                CDK::util::serial_gather(base_column_array, tids->data(), tids->size(), input_column);
                if(this->size()<300){
                        CDK::util::serial_gather(base_column_array, this->tids_->data(), this->size(), input_column);
                }else{
                        CDK::util::parallel_gather(base_column_array, this->tids_->data(), this->size(), input_column, boost::thread::hardware_concurrency());
                }
                return input_column;
        }
	...
	const ColumnPtr LookupArray<T>::copy() const{
		PositionListPtr new_tids = boost::dynamic_pointer_cast<PositionList>(tids_->copy()); //(createPositionList(*tids_));
		return ColumnPtr(new LookupArray<T>(this->name_,this->db_type_,this->column_,new_tids));
	}
        
        template<class T>
        const ColumnPtr LookupArray<T>::copy(const hype::ProcessingDeviceMemoryID& mem_id) const{
            ...
            if(column_->getMemoryID()!=mem_id && isCPUMemory(mem_id))
                    placed_column = DataCacheManager::instance().getDataCache(column_->getMemoryID()).getHostColumn(column_);
            if(!placed_column)
                    placed_column = copy_if_required(column_, mem_id);
            ...
            //return ColumnPtr(new LookupArray<T>(this->name_,this->db_type_,placed_column,placed_tids));
//            
...
//            DenseValueColumnPtr dense_column = this->copyIntoDenseValueColumn(ProcessorSpecification(hype::PD0));
...
//            return ColumnBaseTyped<T>::copy(mem_id);
        ...
        template<class T>
        const typename ColumnBaseTyped<T>::DenseValueColumnPtr LookupArray<T>::copyIntoDenseValueColumn(const ProcessorSpecification& proc_spec) const{
            typedef typename ColumnBaseTyped<T>::DenseValueColumn DenseValueColumn;
            typedef typename ColumnBaseTyped<T>::DenseValueColumnPtr DenseValueColumnPtr;
...
//            ColumnPtr col = this->column_->gather(tids_, gather_param);
            ...
            if(col->getColumnType()!=PLAIN_MATERIALIZED){
                boost::shared_ptr<ColumnBaseTyped<T> > typed_col = boost::dynamic_pointer_cast<ColumnBaseTyped<T> >(col);
        ...
        template<class T>
        const ColumnPtr LookupArray<T>::gather(PositionListPtr tid_list, const GatherParam& gather_param){
            if(!tid_list) return ColumnPtr();
            //return column_->gather();
            hype::ProcessingDeviceMemoryID mem_id = hype::util::getMemoryID(gather_param.proc_spec.proc_id);
//            Column<T>* result = new Column<T>(this->name_,this->db_type_, mem_id);
            ...
            PositionListPtr new_tids = boost::dynamic_pointer_cast<PositionList>(new_tid_list);
...
//            return ColumnPtr(result); 
        ...
        template<class T>
        const ColumnGroupingKeysPtr LookupArray<T>::createColumnGroupingKeys(const ProcessorSpecification& proc_spec) const{
            ColumnPtr placed_column = copy_if_required(column_, proc_spec);
...
//        template<class T>
//        const AggregationResult LookupArray<T>::aggregateByGroupingKeys(ColumnGroupingKeysPtr grouping_keys, AggregationMethod agg_meth, AggregationAlgorithm agg_alg){ //const AggregationParam& param){
//            AggregationParam param(ProcessorSpecification(hype::PD0), agg_meth, agg_alg);
//            
...
//            ColumnPtr materialized_column = copied_column->gather(copied_tids, gather_param);
////            ColumnPtr materialized_column = column_->gather(tids_, GatherParam(ProcessorSpecification(param.proc_spec)));
//            if(materialized_column){
//                //we assume that there is no Lookup_Array on a Lookup_Array, 
//                //so we will not get infinite recursion here
//                return materialized_column->aggregateByGroupingKeys(grouping_keys, param);
//            }
...
//        } 
        ...
        template<class T>
        const AggregationResult LookupArray<T>::aggregate(const AggregationParam& param){
            if( (this->column_->size()==0) || (this->tids_->size()==0)){
                return AggregationResult();
            }
            ColumnPtr copied_column = copy_if_required(column_,  param.proc_spec);
        ...
        template<class T> 
        const PositionListPairPtr LookupArray<T>::join(ColumnPtr join_column, const JoinParam& param){
            if(!join_column) return PositionListPairPtr();
            ColumnPtr copied_column = copy_if_required(column_,  param.proc_spec);
            PositionListPtr copied_tids = copy_if_required(tids_,  param.proc_spec);
            if(!copied_column || !copied_tids) return PositionListPairPtr();
            //Empty result
            if( (this->column_->size()==0) || (this->tids_->size()==0) || join_column->size()==0){
            PositionListPairPtr r(new PositionListPair());
            r->first=createPositionList(0, hype::util::getMemoryID(param.proc_spec.proc_id)); 
            r->second=createPositionList(0, hype::util::getMemoryID(param.proc_spec.proc_id)); 
                return r;
            }
            GatherParam gather_param(param.proc_spec);
            ColumnPtr materialized_column = copied_column->gather(copied_tids, gather_param);
        ...
        template<class T>
        const ColumnPtr LookupArray<T>::column_algebra_operation(ColumnPtr source_column, const AlgebraOperationParam& param){
//            COGADB_FATAL_ERROR("Called column_algebra_operation on Lookup array, which is not allowed!","");
            ColumnPtr copied_column = copy_if_required(column_,  param.proc_spec);
            PositionListPtr copied_tids = copy_if_required(tids_,  param.proc_spec);
            if(!copied_column || !copied_tids) return ColumnPtr();
            GatherParam gather_param(param.proc_spec);
        ...
        template<class T>
        const ColumnPtr LookupArray<T>::column_algebra_operation(const boost::any& value, const AlgebraOperationParam& param){
//            COGADB_FATAL_ERROR("Called column_algebra_operation on Lookup array, which is not allowed!","");
            ColumnPtr copied_column = copy_if_required(column_,  param.proc_spec);
            PositionListPtr copied_tids = copy_if_required(tids_,  param.proc_spec);
            if(!copied_column || !copied_tids) return ColumnPtr();
	...
	template<class T>
	bool LookupArray<T>::load_impl(const std::string& path, boost::archive::binary_iarchive& ia){
		return false;
	}
	template<class T>
	bool LookupArray<T>::store_impl(const std::string& path, boost::archive::binary_oarchive& oa){
		return false;
	}
	template<class T>
	...
	bool LookupArray<T>::load(const std::string& path){
		...
		return false;
                ...
                std::cout << "Loading Column " << column_->getName() << " (" << column_->type().name() << ")" << std::endl;
//                std::cout << "Column Type: " << column_->getColumnType() << std::endl;
//                std::cout << "Attribute Type: " << column_->getType() << std::endl; 
                column_->load(path);
	...
	PositionListPtr LookupArray<T>::getPositionList(){
        ...
        template<class T>
        shared_pointer_namespace::shared_ptr<ColumnBaseTyped<T> > LookupArray<T>::getIndexedColumn(){
            return this->column_;
        }
