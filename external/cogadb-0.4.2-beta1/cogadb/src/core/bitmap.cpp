...
namespace CoGaDB{
    ...
    Bitmap::Bitmap(char* bitmap, size_t num_of_bits, const hype::ProcessingDeviceMemoryID& mem_id) : bitmap_(bitmap), num_of_bits_(num_of_bits), mem_alloc(MemoryAllocator<char>::getMemoryAllocator(mem_id)){
        assert(bitmap_!=NULL);
    }
    Bitmap::Bitmap(size_t num_of_bits, bool init_value, bool initialize, const hype::ProcessingDeviceMemoryID& mem_id) : bitmap_(NULL), num_of_bits_(num_of_bits), mem_alloc(MemoryAllocator<char>::getMemoryAllocator(mem_id)){
        bitmap_ = mem_alloc->allocate((num_of_bits+7)/8);
        if(!bitmap_) throw std::bad_alloc();
        if(initialize){
            int value=255*init_value;
            MemsetFunctionPtr func = MemsetFunctionFactory::getMemsetFunction(mem_alloc->getMemoryID());
            assert(func!=NULL);
    ...
    BitmapPtr Bitmap::copy(const hype::ProcessingDeviceMemoryID& mem_id) const{
        ...
        CopyFunctionPtr func = CopyFunctionFactory<char>::getCopyFunction(bitmap->getMemoryID(), this->mem_alloc->getMemoryID());
        assert(func!=NULL);
...
bool operator!=(Bitmap& bitmap1, Bitmap& bitmap2){
    ...
    BitmapPtr createBitmap(size_t number_of_bits, bool init_value, bool initialize, const hype::ProcessingDeviceMemoryID& mem_id){
