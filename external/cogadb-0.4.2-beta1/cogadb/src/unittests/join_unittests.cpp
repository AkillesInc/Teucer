...
namespace unit_tests{
    ...
    void error_report(std::string error_message, JoinPairs jp_left, JoinPairs jp_right){
...
//                cout << "Correct: " << jp_left[i].first << ", " <<  jp_left[i].second << endl;
//                   if(jp_left[i]!=jp_right[i])
//                     cout << "\tIncorrect: " <<  jp_right[i].first << ", " <<  jp_right[i].second << endl;
//                //break;
            

//            if(jp_left[i]!=jp_right[i]){
//                cout << "Variant:" << endl 
//                     << "\tCorrect: " << jp_left[i].first << ", " <<  jp_left[i].second << endl
//                     << "\tIncorrect: " <<  jp_right[i].first << ", " <<  jp_right[i].second << endl;
//                //break;
//            }
        }
...
//        s_column->print();
        ...
        PositionListPairPtr hash2_tid_pair = CDK::main_memory_joins::serial_hash_join(r_column->data(),r_column->size(), s_column->data(), s_column->size());
        
        std::cout << "Performing Parallel Hash Join (C + OpenMP): " << std::endl;
        //PositionListPairPtr hash3_tid_pair = CDK::main_memory_joins::parallel_hash_join(r_column->data(),r_column->size(), s_column->data(), s_column->size());
        ...
        //if(nlj!=hash3){
        //    error_report("Wrong Result for Parallel Hash Join (C + OpenMP)!",nlj, hash3);
        //}
