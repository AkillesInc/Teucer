...
//            v.insert(501);
        ...
        SelectivityEstimatorPtr sel_est_(new EquiHeightHistogram<int>(v.data(),v.size()));
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            Predicate p("Test_Col", boost::any(int(value)), ValueConstantPredicate, comp);
            hype::ProcessingDeviceID id=hype::PD0;
            ProcessorSpecification proc_spec(id);
            SelectionParam param(proc_spec, ValueConstantPredicate, boost::any(int(value)), comp);
            PositionListPtr tids= v.selection(param);            
//            PositionListPtr tids = v.selection( boost::any(int(value)), comp);
        ...
        double sum = std::accumulate(estimation_error.begin(),estimation_error.end(),0);
        double avg_est_error = sum/estimation_error.size();
        
        cout << "Min Estimation Error: " << *std::min_element(estimation_error.begin(),estimation_error.end()) << endl;
        cout << "Max Estimation Error: " << *std::max_element(estimation_error.begin(),estimation_error.end()) << endl;
...
//            v.insert(501);
        ...
        for (int i = 0; i < 1000; i++) {
            ...
            //ValueComparator comp=ValueComparator(rand()%2); //only GREATER, LESSER predicates
            Predicate lower_pred("Test_Col", boost::any(int(lower_value)), ValueConstantPredicate, GREATER);
            Predicate upper_pred("Test_Col", boost::any(int(upper_value)), ValueConstantPredicate, LESSER);
            
            
            
            hype::ProcessingDeviceID id=hype::PD0;
            ProcessorSpecification proc_spec(id);
            SelectionParam param_lower(proc_spec, ValueConstantPredicate, boost::any(int(lower_value)), GREATER);
            SelectionParam param_upper(proc_spec, ValueConstantPredicate, boost::any(int(upper_value)), LESSER);
            
            PositionListPtr tids_lower = v.selection(param_lower);
            PositionListPtr tids_upper = v.selection(param_upper);
            PositionListPtr tids = computePositionListIntersection(tids_lower, tids_upper);             
 

            cout << "Range Query: " << lower_pred.toString() << " AND " << upper_pred.toString() << endl;
            if(tids){
                cout << "Real Selectivity: " <<  double(tids->size())/v.size() << endl;
                measured_rows.push_back(tids->size());
            }
