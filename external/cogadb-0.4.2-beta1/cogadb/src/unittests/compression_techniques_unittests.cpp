...
namespace CoGaDB {
    namespace unit_tests {
        ...
        bool delta() {
            boost::shared_ptr<CompressedColumn<int> > col(new DeltaCompressedColumn<int>("int column delta", INT));
            boost::shared_ptr<CompressedColumn<float> > col_float(new DeltaCompressedColumn<float>("float column delta", FLOAT));
            boost::shared_ptr<CompressedColumn<std::string> > col_string(new DeltaCompressedColumn<std::string>("string column delta", VARCHAR));

            boost::shared_ptr<DeltaCompressedColumn<int> > compressed_col(new DeltaCompressedColumn<int>("int column delta", INT));

            boost::shared_ptr<Column<int> > uncompressed_col(new Column<int>("int column", INT));
            std::vector<int> reference_data(100);

            std::cout << std::endl << "TESTING DELTA COMPRESSED COLUMN..." << std::endl << std::endl;
        ...
        bool bit_vector() {
            boost::shared_ptr<CompressedColumn<int> > col(new BitVectorCompressedColumn<int>("int column bit vector", INT));
            boost::shared_ptr<CompressedColumn<float> > col_float(new BitVectorCompressedColumn<float>("float column bit vector", FLOAT));
            boost::shared_ptr<CompressedColumn<std::string> > col_string(new BitVectorCompressedColumn<std::string>("string column bit vector", VARCHAR));

            boost::shared_ptr<BitVectorCompressedColumn<int> > compressed_col(new BitVectorCompressedColumn<int>("int column bit vector", INT));

            boost::shared_ptr<Column<int> > uncompressed_col(new Column<int>("int column", INT));
            std::vector<int> reference_data(100);

            std::cout << std::endl << "TESTING BITVECTOR COMPRESSED COLUMN..." << std::endl << std::endl;
        ...
        bool run_length() {
            boost::shared_ptr<CompressedColumn<int> > col(new RLECompressedColumn<int>("int column run length", INT));
            boost::shared_ptr<CompressedColumn<float> > col_float(new RLECompressedColumn<float>("float column run length", FLOAT));
            boost::shared_ptr<CompressedColumn<std::string> > col_string(new RLECompressedColumn<std::string>("string column run length", VARCHAR));

            boost::shared_ptr<RLECompressedColumn<int> > compressed_col(new RLECompressedColumn<int>("int column run length", INT));

            boost::shared_ptr<Column<int> > uncompressed_col(new Column<int>("int column", INT));
            std::vector<int> reference_data(100);

            std::cout << std::endl << "TESTING RUN LENGTH COMPRESSED COLUMN..." << std::endl << std::endl;
        ...
        bool dictionary_compression() {
            boost::shared_ptr<CompressedColumn<int> > col(new DictionaryCompressedColumn<int>("int column dict", INT));
            boost::shared_ptr<CompressedColumn<float> > col_float(new DictionaryCompressedColumn<float>("float column dict", FLOAT));
            boost::shared_ptr<CompressedColumn<std::string> > col_string(new DictionaryCompressedColumn<std::string>("string column dict", VARCHAR));

            boost::shared_ptr<DictionaryCompressedColumn<int> > compressed_col(new DictionaryCompressedColumn<int>("int column dictionary encoded", INT));

            boost::shared_ptr<Column<int> > uncompressed_col(new Column<int>("int column", INT));
            std::vector<int> reference_data(100);

            std::cout << std::endl << "TESTING DICTIONARY COMPRESSED COLUMN..." << std::endl << std::endl;
        ...
        bool bitpacked_dictionary_compression() {
            boost::shared_ptr<CompressedColumn<int> > col(new BitPackedDictionaryCompressedColumn<int>("int column bit packed dict", INT, 7));
            boost::shared_ptr<CompressedColumn<float> > col_float(new BitPackedDictionaryCompressedColumn<float>("float column bit packed dict", FLOAT, 7));
            boost::shared_ptr<CompressedColumn<std::string> > col_string(new BitPackedDictionaryCompressedColumn<std::string>("string column bit packed dict", VARCHAR, 7));

            boost::shared_ptr<BitPackedDictionaryCompressedColumn<int> > compressed_col(new BitPackedDictionaryCompressedColumn<int>("int column bit packed dictionary encoded", INT, 7));

            boost::shared_ptr<Column<int> > uncompressed_col(new Column<int>("int column", INT));
            std::vector<int> reference_data(100);

            std::cout << std::endl << "TESTING BIT PACKED DICTIONARY COMPRESSED COLUMN..." << std::endl << std::endl;
        ...
        bool run_length_delta_one_number() {
            boost::shared_ptr<CompressedColumn<int> > col(new RLEDeltaOneCompressedColumnNumber<int>("int column run length", INT));
            // TODO better modularization of unit tests, for now we use the normal RLE column to not break the tests
            boost::shared_ptr<CompressedColumn<float> > col_float(new RLECompressedColumn<float>("float column run length", FLOAT));
            boost::shared_ptr<CompressedColumn<std::string> > col_string(new RLECompressedColumn<std::string>("string column run length", VARCHAR));
            ...
            std::cout << std::endl << "TESTING RUN LENGTH DELTA ONE COMPRESSED NUMBER COLUMN..." << std::endl << std::endl;
...
//                {
//                    Tuple t;
//                    t.push_back(reference_data[rand() % reference_data.size()]);
//                    tab->insert(t);
//                }
//            }
//
//
//            int selection_value;
//            ValueComparator selection_comparison_value = EQUAL; //0 EQUAL, 1 LESSER, 2 LARGER
//
//            boost::mt19937 rng;
//            boost::uniform_int<> selection_values(0, reference_data.size() - 1);
//            //boost::uniform_int<> filter_condition(0,2);
//
//            hype::DeviceConstraint default_dev_constr = CoGaDB::RuntimeConfiguration::instance().getGlobalDeviceConstraint();
//            for (unsigned int i = 0; i < 1000; i++) {
...
//                {
//                    Disjunction d;
//                    d.push_back(Predicate("values", boost::any(reference_data[selection_value]), ValueConstantPredicate, EQUAL));
//                    knf_expr.disjunctions.push_back(d);
//                }
//
//                TablePtr cpu_result = BaseTable::selection(tab, "values", boost::any(reference_data[selection_value]), EQUAL, LOOKUP, SERIAL, CPU);
//                TablePtr gpu_result = BaseTable::selection(tab, "values", boost::any(reference_data[selection_value]), EQUAL, LOOKUP, PARALLEL, GPU);
//                //enforce CPU Use for Plan Generation
//                CoGaDB::RuntimeConfiguration::instance().setGlobalDeviceConstraint(hype::CPU_ONLY);
//                TablePtr cpu_complex_selection_result = BaseTable::selection(tab, knf_expr, LOOKUP);
//                //enforce GPU Use for Plan Generation
//                CoGaDB::RuntimeConfiguration::instance().setGlobalDeviceConstraint(hype::GPU_ONLY);
//                TablePtr gpu_complex_selection_result = BaseTable::selection(tab, knf_expr, LOOKUP);
//
//                if (!cpu_result->getColumnbyName("values")->is_equal(gpu_result->getColumnbyName("values"))) {
//                    cerr << "Error in Simple GPU Selection! values Column not correct!" << endl;
//                    cout << "CPU Selection rows: " << cpu_result->getColumnbyName("values")->size() << " GPU Selection rows: " << gpu_result->getColumnbyName("values")->size() << endl;
...
//                }
//
//                if (!cpu_result->getColumnbyName("values")->is_equal(cpu_complex_selection_result->getColumnbyName("values"))) {
//                    cerr << "Error in Complex CPU Selection! values Column not correct!" << endl;
//                    cout << "CPU Selection rows: " << cpu_result->getColumnbyName("values")->size() << " CPU Complex Selection rows: " << cpu_complex_selection_result->getColumnbyName("values")->size() << endl;
//                    return false;
//                }
//
//                if (!cpu_result->getColumnbyName("values")->is_equal(gpu_complex_selection_result->getColumnbyName("values"))) {
//                    cerr << "Error in Complex GPU Selection! values Column not correct!" << endl;
//                    cout << "CPU Selection rows: " << cpu_result->getColumnbyName("values")->size() << " GPU Selection rows: " << gpu_complex_selection_result->getColumnbyName("values")->size() << endl;
//                    return false;
//                }
//
