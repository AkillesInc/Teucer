...
//        bool isJoinable(const std::string& left_table, const std::string& right_table, const Predicate& pred);
        ...
        void printJoinConditions(const JoinConditions& conditions){
            ...
            for(cit_cond=conditions.begin();cit_cond!=conditions.end();++cit_cond){
...
//            std::cout << join_tables.size() << std::endl;
            ...
            for(cit_tab1=join_tables.begin();cit_tab1!=join_tables.end();++cit_tab1){
                for(cit_tab2=join_tables.begin();cit_tab2!=join_tables.end();++cit_tab2){
...
//                        JoinSpecification spec(cit_tab1->second, PartialJoinSpecification(cit_tab2->second, *cit_cond));
//                        joinable_tables.insert(spec);
//                        std::cout << "Is Joinable: " <<  cit_tab1->second << " JOIN " << cit_tab2->second  << " USING " << cit_cond->toString() << std::endl;
//                        std::cout << "Is Joinable: " <<  cit_tab1->second << " JOIN " << cit_tab2->second  << " USING " << invertPredicate(*cit_cond).toString() << std::endl;
                        if(isJoinable(cit_tab1->second,cit_tab2->second, *cit_cond)){
                           joinable_tables.insert(JoinSpecification(cit_tab1->second, PartialJoinSpecification(cit_tab2->second, *cit_cond))); 
                           joinable_tables.insert(JoinSpecification(cit_tab2->second, PartialJoinSpecification(cit_tab1->second, invertPredicate(*cit_cond))));
                           if(!quiet) std::cout << "Joinable Tables: " << cit_tab1->second << " JOIN " << cit_tab2->second << " ON " << cit_cond->toString() << std::endl; 
                           break;
                        }
                        
//                        if(isJoinable(cit_tab1->second,cit_tab2->second, invertPredicate(*cit_cond))){
//                           joinable_tables.insert(JoinSpecification(cit_tab1->second, PartialJoinSpecification(cit_tab2->second, invertPredicate(*cit_cond)))); 
//                           joinable_tables.insert(JoinSpecification(cit_tab2->second, PartialJoinSpecification(cit_tab1->second, *cit_cond)));
//                           std::cout << "Joinable Tables: " << cit_tab1->second << " JOIN " << cit_tab2->second << " ON " << invertPredicate(*cit_cond).toString() << std::endl; 
//                           break;
//                        }
                    }
                }                
        ...
        std::list<PartialJoinSpecification> createJoinPath(const std::string first_table, 
                            const JoinConditions& conditions,
            ...
            typedef std::pair<JoinableTables::const_iterator,JoinableTables::const_iterator> SuccessorTables;
...
//            while(join_path.size()+1<conditions.size()){
            while(join_path.size()<conditions.size()){
                ...
                for(current_table_it=tables_in_join_path.begin();current_table_it!=tables_in_join_path.end();++current_table_it){
                    
                
                    SuccessorTables successor_tables = joinable_tables.equal_range(*current_table_it);
                    if(!quiet) std::cout << "Successor for table " << *current_table_it << ": " << std::endl;
                    for(cit=successor_tables.first;cit!=successor_tables.second;++cit){
...
//                            *current_table_it=cit->second.first;
        ...
        std::pair<std::string, std::list<PartialJoinSpecification> > getJoinPath(const JoinTables& join_tables, 
                         const JoinConditions& conditions){

            if(join_tables.empty()) return std::pair<std::string, std::list<PartialJoinSpecification> >("", std::list<PartialJoinSpecification>());
            JoinableTables joinable_tables = createJoinableTables(join_tables, conditions);
            if(!quiet) {   
            JoinableTables::iterator it;
            for(it=joinable_tables.begin();it!=joinable_tables.end();++it){
                std::cout << it->first << " JOINABLE WITH " << it->second.first << " ON PREDICATE " << it->second.second.toString() << std::endl; 
            }
            }
            
            std::list<PartialJoinSpecification> join_path = createJoinPath(join_tables.begin()->second, 
                                                                            conditions,
                                                                            joinable_tables);
            if(!quiet){
            ...
            return std::pair<std::string, std::list<PartialJoinSpecification> >(join_tables.begin()->second, join_path);
...
//						JOIN (LO_ORDERDATE=D_DATEKEY)
            ...
            conditions.insert(Predicate(std::string("LO_PARTKEY"), std::string("P_PARTKEY"), ValueValuePredicate, EQUAL));
            conditions.insert(Predicate(std::string("LO_SUPPKEY"),std::string("S_SUPPKEY"), ValueValuePredicate, EQUAL));
            conditions.insert(Predicate(std::string("LO_CUSTKEY"), std::string("C_CUSTKEY"), ValueValuePredicate, EQUAL));
            conditions.insert(Predicate(std::string("LO_ORDERDATE"), std::string("D_DATEKEY"), ValueValuePredicate, EQUAL));
