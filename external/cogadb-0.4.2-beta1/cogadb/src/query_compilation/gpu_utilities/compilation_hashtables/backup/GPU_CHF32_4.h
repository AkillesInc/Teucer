...
namespace GPU_CHF32_4 {
	...
	typedef struct __align__(4) Entry  {
	...
	void printHashTable(HashTable ht, int numEntriesToPrint = 0) {
		...
		std::cout << "-------- hash table status --------" << std::endl;
                std::cout << "Hash table size: " << ht.tableSizeBytes / (1000 * 1000) << "MB" << std::endl;
	...
	*/
	__device__ void insertEntry(HashTable& ht, HtUnsigned key, int* iterationCount = NULL) {
	  ...
	  for (int its = 1; its <= ht.maxInsertIterations; its++) {
	    ...
	    *(unsigned int*)&entry = atomicExch((unsigned int*)&ht.table[location], *(unsigned int*)&entry);
	  ...
	  if (entry.key != kEmpty) {
	    ...
	    *(unsigned int*)&replaced_entry = atomicCAS((unsigned int*)(stash + slot), kEntryEmpty, *(unsigned int*)&entry);
