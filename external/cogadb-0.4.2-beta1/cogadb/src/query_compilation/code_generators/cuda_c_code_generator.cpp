...
namespace CoGaDB {
    ...
    const std::string getIndexVarType() {
      return "uint32_t";
    }

    const std::string getDeviceInputArrayVarName(const AttributeReference& attr_ref) {
        std::stringstream ss;
        ss << "d_" << getInputArrayVarName(attr_ref);
        return ss.str();
    }

    const std::string getDeviceResultArrayVarName(const AttributeReference& attr_ref) {
      std::stringstream ss;
    ...
    CUDA_C_CodeGenerator::CUDA_C_CodeGenerator(
    ...
    void CUDA_C_CodeGenerator::init() {
        ...
        cuda_includes << "#include <query_compilation/gpu_utilities/util.cuh>" << std::endl;
        cuda_includes << "#include <thrust/scan.h>" << std::endl;
        cuda_includes << "#include <thrust/device_ptr.h>" << std::endl;
        cuda_includes << "#include <query_compilation/gpu_utilities/compilation_hashtables/GPU_CHT32_2.cuh>";

        blockSize = 128;
        constant_declarations << "const int kBlockSize = "
                              << blockSize << ";" << std::endl;
    }
    ...
    void CUDA_C_CodeGenerator::produceAllocArray(
        ...
        this->produceAllocArray(datatype,
                                varname,
    ...
    void CUDA_C_CodeGenerator::produceAllocArray(
        ...
        if(location == DEVICE) {
            allocCode << "cudaMalloc((void**)&" << varname
                      << ", sizeof(" << datatype << ") * "
                      << num_elements_varname << ");";
            if(doFree)
                freeCode << "cudaFree(" << varname << ");";
        } else if(location == HOST) {
            allocCode << varname << " = (" << datatype << "*)"
                      << "malloc(" << num_elements_varname
                      << "* sizeof(" << datatype << "));";
            if(doFree)
                freeCode << "free(" << varname << ");";
        }
        switch(code_pos) {
          case PRE_COMPUTE:
    ...
    void CUDA_C_CodeGenerator::produceDataTransfer(
         ...
         this->produceDataTransfer(datatype,
                                 name_dest,
                                 name_src,
                                 boost::lexical_cast<std::string>(num_elements),
                                 direction,
    ...
    void CUDA_C_CodeGenerator::produceDataTransfer(
        ...
        transferCode << "cudaMemcpy(" << name_dest << ", "
                     << name_src << ", " << num_elements_varname
                     << "* sizeof(" << datatype << "), ";
    ...
    void CUDA_C_CodeGenerator::generateCodeComputeStructure() {
        ...
        initComputeStructureVars
            << "int numKernelBlocks;" << std::endl
            << "int numKernelThreads;" << std::endl
            << "size_t current_result_size;" << std::endl;

        // compute number of blocks
        initComputeStructureVars
            <<  "dim3 grid( (" << input_table->getNumberofRows() << " + kBlockSize-1) / "
                << "kBlockSize);" << std::endl
            <<  "if (grid.x > 16384) {" << std::endl
            <<  "grid.y = (grid.x + 16384 - 1) / 16384;"  << std::endl
            <<  "grid.x = 16384;" << std::endl
            <<  "}" << std::endl
            <<  "if(grid.x == 0) grid.x = 1;" << std::endl
            <<  "numKernelBlocks = grid.x * grid.y;" << std::endl
            <<  "numKernelThreads = numKernelBlocks * kBlockSize;" << std::endl;

        // output compute structure info
        initComputeStructureVars
            << "std::cout << \"numKernelThreads: \" << numKernelThreads << std::endl;" << std::endl
            << "std::cout << \"numKernelBlocks: \" << numKernelBlocks << std::endl;" << std::endl
            << "std::cout << \"grid.x: \" << grid.x << std::endl;" << std::endl
            << "std::cout << \"grid.y: \" << grid.y << std::endl;" << std::endl
            << "std::cout << \"kBlockSize: \" << kBlockSize << std::endl;" << std::endl;

        compute_structure_code = initComputeStructureVars.str();

        produceAllocArray(getIndexVarType(),
                          "d_writePositions",
                          "numKernelThreads",
                          DEVICE,
                          PRE_COMPUTE);

        produceAllocArray("uint32_t",
                          "d_flagArray",
                          "numKernelThreads",
                          DEVICE,
                          PRE_COMPUTE);

        produceAllocArray("size_t",
                          "d_blockCounts",
                          "numKernelBlocks",
                          DEVICE,
                          PRE_COMPUTE);
    ...
    bool CUDA_C_CodeGenerator::produceTuples(const ScanParam& param) {
        ...
        for (cit = param.begin(); cit != param.end(); ++cit) {
            ...
            produceAllocArray(toCPPType(attr),
                              getDeviceInputArrayVarName(*cit),
                              cit->getTable()->getNumberofRows(),
                              DEVICE,
                              PRE_COMPUTE);

            produceDataTransfer(toCPPType(attr),
                              getDeviceInputArrayVarName(*cit),
    ...
    void CUDA_C_CodeGenerator::produceAllocateDeviceResult(std::string numElementsVarName) {
    ...
    bool CUDA_C_CodeGenerator::createForLoop_impl(
        ...
        for(int i = 0; i < scanned_attributes.size(); i++) {
        ...
        // compute structure
        project_pipeline_kernel.addParameter(getIndexVarType() + "*", "writePositions");
    ...
    bool CUDA_C_CodeGenerator::consumeSelection_impl(
        ...
        predicateCode << "selected = selected && ("
                      << pred_expr->getCPPExpression()
                      << ");" << std::endl;
    ...
    bool CUDA_C_CodeGenerator::consumeBuildHashTable_impl(
        ...
        codeInsertKey
            << "GPU_CHT32_2::insertEntry("
            << getHashTableVarName(attr) << ", "
            << "(uint32_t)"
            << getInputArrayVarName(attr) << "[thread_index], "
        ...
        this->create_result_table_code_block
            << "printHashTable(*" << getHashTableVarName(attr) << ", 20);" << std::endl;
     ...
     bool CUDA_C_CodeGenerator::consumeProbeHashTable_impl(
        ...
        codeGetHt << "GPU_CHT32_2::HashTable* " << getHashTableVarName(hash_table_attr)
                  << " = (GPU_CHT32_2::HashTable*) getHashTableFromSystemHashTable(generic_hashtable_"
                  << hash_table_attr.getVersionedAttributeName() << ");"
                  << std::endl;
        ...
        codeProbeHt << "uint32_t " << getTupleIDVarName(hash_table_attr) << ";" << std::endl;

        codeProbeHt << getTupleIDVarName(hash_table_attr)
                    << " = GPU_CHT32_2::probeKey("
                    << getHashTableVarName(hash_table_attr) << ", "
                    << getVarName(probe_attr) << "["
    ...
    // produces a projection of all specified attributes after selection using prefix sum
    ...
    void CUDA_C_CodeGenerator::produceScanFlagArray() {
      ...
      //   << "cub::DeviceScan::ExclusiveSum(d_temp_storage, temp_storage_bytes, d_flagArray, d_writePositions, numKernelThreads);" << std::endl
      ...
      scanStructureCode
          << "uint32_t h_flagN;" << std::endl
          << "uint32_t h_writePosN;" << std::endl
          << "cudaMemcpy(&h_flagN, &(d_flagArray[numKernelThreads-1]), sizeof(uint32_t), cudaMemcpyDeviceToHost);" << std::endl
          << "cudaMemcpy(&h_writePosN, &(d_writePositions[numKernelThreads-1]), sizeof(uint32_t), cudaMemcpyDeviceToHost);" << std::endl
          << "current_result_size = h_flagN + h_writePosN;" << std::endl;
      scanStructureCode << "std::cout << \"Final result size is:\" << current_result_size << std::endl;";
    ...
    void CUDA_C_CodeGenerator::produceBlockScanSelected(KernelExecution& kernel) {
      ...
      blockScanInit
          << "typedef cub::BlockScan<int, kBlockSize> BlockScan;" << std::endl
          << "__shared__ typename BlockScan::TempStorage temp_storage;" << std::endl
          << "int flag = 0;" << std::endl
      ...
      blockScanCode << "writePositions[thread_index] = prefixSum;"
                    << std::endl
    ...
    void CUDA_C_CodeGenerator::produceScanBlockResultSizes() {
      ...
      scanStructureCode
          << "void     *d_temp_storage = NULL;" << std::endl
          << "size_t   temp_storage_bytes = 0;" << std::endl
          << "cub::DeviceScan::ExclusiveSum(d_temp_storage, temp_storage_bytes, d_blockCounts, d_scanBlockCounts, numKernelBlocks);" << std::endl
          << "cudaMalloc(&d_temp_storage, temp_storage_bytes);" << std::endl
          << "cub::DeviceScan::ExclusiveSum(d_temp_storage, temp_storage_bytes, d_blockCounts, d_scanBlockCounts, numKernelBlocks);" << std::endl
          << "cudaFree(d_temp_storage);" << std::endl;
      ...
      scanStructureCode
          ...
          << "cudaMemcpy(&scannedBlockCounts[0], d_scanBlockCounts, sizeof(size_t) * numKernelBlocks, cudaMemcpyDeviceToHost);"  << std::endl
          << "for(int i=0;i<numKernelBlocks;i++) std::cout << scannedBlockCounts[i] << \", \";" << std::endl;
      scan_compute_structure = scanStructureCode.str();
    }
    ...
    void CUDA_C_CodeGenerator::produceCode() {
      ...
      // ----- changed mode from getcode to produce
          if(!codeIsProduced) {
              ...
              this->pre_compute_allocation_code
                  << "gpuErrchk(cudaGetLastError());" << std::endl;
              this->pre_compute_transfer_code
                  << "gpuErrchk(cudaGetLastError());" << std::endl;

              if(this->post_scan_allocation_code.str().size() > 0) {
                this->post_scan_allocation_code
                    << "gpuErrchk(cudaGetLastError());" << std::endl;
              }
              if(this->post_scan_transfer_code.str().size() > 0) {
                this->post_scan_transfer_code
                    << "gpuErrchk(cudaGetLastError());" << std::endl;
              }

              this->post_projection_allocation_code
                  << "gpuErrchk(cudaGetLastError());" << std::endl;
              this->post_projection_transfer_code
                  << "gpuErrchk(cudaGetLastError());" << std::endl;
              this->free_memory_code
                  << "gpuErrchk(cudaGetLastError());" << std::endl;
  ...
  // ----------------------------- OUTPUT ---------------------------------
    ...
    const PipelinePtr CUDA_C_CodeGenerator::compile() {
        ...
        std::string copy_last_query_command = std::string("cp '") + filename + std::string("' last_generated_query.cu");
        ...
        compile_command << "nvcc -g -O3 -I "
                        << PATH_TO_COGADB_SOURCE_CODE << "/include/ -I "
                        << PATH_TO_COGADB_SOURCE_CODE << "/../external_libraries/cub-1.4.1/ -I "
                        << PATH_TO_COGADB_SOURCE_CODE << "/../hype-library/include/ -c --compiler-options -fpic ";
