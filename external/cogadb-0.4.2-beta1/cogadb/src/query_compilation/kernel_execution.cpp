...
void KernelExecution::init(std::string name,
                           int block_size,
...
void KernelExecution::addParameter(std::string type,
...
void KernelExecution::produceFrameCode() {
  ...
  functionHeader << parameter_types[0] << " "
                 << parameter_names[0];

  for(int i = 1; i < parameter_types.size(); i++) {
      functionHeader << ", " << parameter_types[i]
                     << " "  << parameter_names[i];
  }
  ...
  if(use_selected)
      functionHeader << "bool selected = false;" << std::endl;
...
int KernelExecution::getBlockSize() {
  return block_size;
}
...
std::string KernelExecution::getInvocationCode() {
  ...
  for(int i = 1; i < parameter_names.size(); i++) {
...
std::string KernelExecution::getKernelCode() {
  ...
  for (cit = this->kernel_code_upper.begin();
          cit != this->kernel_code_upper.end();
          ++cit) {
  ...
  for (cit = this->kernel_code_lower.begin();
          cit != this->kernel_code_lower.end();
          ++cit) {
      out_kernel << *cit << std::endl;
  }
