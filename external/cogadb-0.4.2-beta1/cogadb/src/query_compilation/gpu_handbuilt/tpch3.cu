...
namespace CoGaDB {
    ...
    TablePtr tpch3() {
      ...
      boost::shared_ptr<Column<int> > tcol_lineitem_orderkey = boost::dynamic_pointer_cast<Column<int> >(col_lineitem_orderkey);
      boost::shared_ptr<Column<float> > tcol_lineitem_discount = boost::dynamic_pointer_cast<Column<float> >(col_lineitem_discount);
      boost::shared_ptr<Column<float> > tcol_lineitem_extended_price = boost::dynamic_pointer_cast<Column<float> >(col_lineitem_extended_price);
      boost::shared_ptr<Column<uint32_t> > tcol_lineitem_shipdate = boost::dynamic_pointer_cast<Column<uint32_t> >(col_lineitem_shipdate);
      boost::shared_ptr<DictionaryCompressedColumn<std::string> > dict_customer_mktsegment =
              boost::dynamic_pointer_cast<DictionaryCompressedColumn<std::string > > (col_customer_mktsegment);
      boost::shared_ptr<Column<int> > tcol_customer_custkey = boost::dynamic_pointer_cast<Column<int> >(col_customer_custkey);
      boost::shared_ptr<Column<int> > tcol_orders_shippriority = boost::dynamic_pointer_cast<Column<int> >(col_orders_shippriority);
      boost::shared_ptr<Column<uint32_t> > tcol_orders_orderdate = boost::dynamic_pointer_cast<Column<uint32_t> >(col_orders_orderdate);
      boost::shared_ptr<Column<int> > tcol_orders_orderkey = boost::dynamic_pointer_cast<Column<int> >(col_orders_orderkey);
      boost::shared_ptr<Column<int> > tcol_orders_custkey = boost::dynamic_pointer_cast<Column<int> >(col_orders_custkey);
      ...
      uint32_t* d_lineitem_shipdate = customMalloc<uint32_t>(DEVICE_MEMORY, lineitem_num_elements);
      uint32_t* d_customer_mktsegment = customMalloc<uint32_t>(DEVICE_MEMORY, customer_num_elements);
      int* d_customer_custkey = customMalloc<int>(DEVICE_MEMORY, customer_num_elements);
      int* d_orders_shippriority = customMalloc<int>(DEVICE_MEMORY, orders_num_elements);
      ...
      cudaMemcpy(d_lineitem_orderkey, tcol_lineitem_orderkey->data(), lineitem_num_elements * sizeof(int), cudaMemcpyHostToDevice);
      cudaMemcpy(d_lineitem_discount, tcol_lineitem_discount->data(), lineitem_num_elements * sizeof(float), cudaMemcpyHostToDevice);
      cudaMemcpy(d_lineitem_extended_price, tcol_lineitem_extended_price->data(), lineitem_num_elements * sizeof(float), cudaMemcpyHostToDevice);
      cudaMemcpy(d_lineitem_shipdate, tcol_lineitem_shipdate->data(), lineitem_num_elements * sizeof(uint32_t), cudaMemcpyHostToDevice);
      cudaMemcpy(d_customer_mktsegment, dict_customer_mktsegment->getIdData(), customer_num_elements * sizeof(uint32_t), cudaMemcpyHostToDevice);
      cudaMemcpy(d_customer_custkey, tcol_customer_custkey->data(), customer_num_elements * sizeof(int), cudaMemcpyHostToDevice);
      cudaMemcpy(d_orders_shippriority, tcol_orders_shippriority->data(), orders_num_elements * sizeof(int), cudaMemcpyHostToDevice);
      cudaMemcpy(d_orders_orderdate, tcol_orders_orderdate->data(), orders_num_elements * sizeof(uint32_t), cudaMemcpyHostToDevice);
      cudaMemcpy(d_orders_orderkey, tcol_orders_orderkey->data(), orders_num_elements * sizeof(int), cudaMemcpyHostToDevice);
      cudaMemcpy(d_orders_custkey, tcol_orders_custkey->data(), orders_num_elements * sizeof(int), cudaMemcpyHostToDevice);
      CUDA_CHECK_ERROR_RETURN("Failure while allocating gpu columns");
      ...
      uint32_t* d_ResultOrderDate = customMalloc<uint32_t>(DEVICE_MEMORY, cardinality);
      ...
      dim3 gridDim = computeGridDim(orders_num_elements, kGridSize, kBuildBlockSize);
      ...
      cub::DeviceSelect::Flagged(d_temp_storage, temp_storage_bytes, d_indices, d_flags, d_selectedIndices, d_numSelectedIndices, cardinality);
      ...
      cudaMemcpy(&h_aggregateRevenue[0], d_aggregateRevenue, cardinality * sizeof(float), cudaMemcpyDeviceToHost);
      cudaMemcpy(&numSelectedIndices, d_numSelectedIndices, sizeof(int), cudaMemcpyDeviceToHost);
      vector<uint32_t> h_selectedIndices(numSelectedIndices);
      cudaMemcpy(&h_selectedIndices[0], d_selectedIndices, numSelectedIndices * sizeof(uint32_t), cudaMemcpyDeviceToHost);
      CUDA_CHECK_ERROR_RETURN("reduce failed");
      ...
      for(size_t i=0; i < numSelectedIndices; i++) {
          ...
          if(h_aggregateRevenue[orderIndex] > minInsertRevenue) {
              ...
              int insert = 0;
              while(insert < topTuples.size() && topTuples[insert].revenue > tuple.revenue)
                  insert++;
      ...
      double reduce_time_gpu = double(reduce_gpu_end - end_probe_start_reduce) / (1000 * 1000);
      ...
      for(int i=0; i < 10; i++) {
          cout << topTuples[i].orderkey << ", "
               << topTuples[i].revenue << ", "
               << topTuples[i].orderdate << ", "
               << topTuples[i].shippriority << endl;
      }
