...
namespace CoGaDB {
    ...
    const TablePtr inner_tpch_q6_cpu(size_t num_elements,
                    float*  l_extended_price,
                    int*  l_quantity,
                    float*  l_discount,
                    uint32_t*  l_shipdate);
...
//        if(gpu) {
//        col_lineitem_shipdate = copy_if_required(col_lineitem_shipdate, hype::PD_Memory_1);
//        col_lineitem_discount = copy_if_required(col_lineitem_discount, hype::PD_Memory_1);
//        col_lineitem_quantity = copy_if_required(col_lineitem_quantity, hype::PD_Memory_1);
//        col_lineitem_extended_price = copy_if_required(col_lineitem_extended_price, hype::PD_Memory_1);
//        if(!col_lineitem_shipdate | !col_lineitem_discount | !col_lineitem_quantity | !col_lineitem_extended_price){
...
//        }
        ...
        if(gpu) {
            ...
            cudaMemcpy(d_extended_price, typed_col_lineitem_extended_price->data(), num_elements * sizeof(float), cudaMemcpyHostToDevice);
            cudaMemcpy(d_quantity, typed_col_lineitem_quantity->data(), num_elements * sizeof(int), cudaMemcpyHostToDevice);
            cudaMemcpy(d_discount, typed_col_lineitem_discount->data(), num_elements * sizeof(float), cudaMemcpyHostToDevice);
            cudaMemcpy(d_shipdate, typed_col_lineitem_shipdate->data(), num_elements * sizeof(uint32_t), cudaMemcpyHostToDevice);
        ...
        if(gpu) {
//        cudaMemset((void *)d_extended_price, 0, sizeof(float) * num_elements);
//        cudaDeviceSynchronize();
...
//        cudaMemset((void *)d_shipdate, 0, sizeof(uint32_t) * num_elements);
//        cudaDeviceSynchronize();
//        CUDA_CHECK_ERROR_RETURN("memset 1d failed");
            if(holistic) {
                inner_tpch_q6_holistic(num_elements,
                                       d_extended_price,
                                       d_quantity,
                                       d_discount,
                                       d_shipdate);
            } else {
                inner_tpch_q6(num_elements,
                              d_extended_price,
                              d_quantity,
                              d_discount,
                              d_shipdate);
                ...
                inner_tpch_q6_cpu(num_elements,
                                  d_extended_price,
                                  d_quantity,
                                  d_discount,
                                  d_shipdate);
...
#include "tpch6.cuh"
    ...
    const TablePtr inner_tpch_q6(const size_t& num_elements,
        ...
        CUDA_CHECK_ERROR_RETURN("Error in device memory allocation for intermediate computation data");

        int gridSize = 128;

        Timestamp begin_query = getTimestamp();

        kernel_select_tpch_q6<<<gridSize,blockSize>>>(num_elements, l_extended_price, l_quantity, l_discount, l_shipdate, d_flags);
        cudaDeviceSynchronize();
        CUDA_CHECK_ERROR_RETURN("kernel_select_tpch_q6");

        kernel_arithmetic_tpch_q6<<<gridSize,blockSize>>>(num_elements, l_extended_price, l_discount, d_flags, d_products);
        cudaDeviceSynchronize();
        CUDA_CHECK_ERROR_RETURN("kernel_arithmetic_tpch_q6");

        float sum = thrust::reduce( thrust::device_pointer_cast(d_products),  thrust::device_pointer_cast(d_products + num_elements));
        ...
        cudaMemcpy(h_flags, d_flags, num_elements * sizeof(char), cudaMemcpyDeviceToHost);
        size_t resultSize = 0;
    ...
    const TablePtr inner_tpch_q6_holistic(const size_t& num_elements,
        ...
        kernel_tpch_q6_holistic<<<computeGridDim(num_elements),blockSize>>>(num_elements, l_extended_price, l_quantity, l_discount, l_shipdate, d_blockSums);
        cudaDeviceSynchronize();
        CUDA_CHECK_ERROR_RETURN("kernel_tpch_q6_holistic failed");

        float result = thrust::reduce( thrust::device_pointer_cast(d_blockSums), thrust::device_pointer_cast(d_blockSums + gridSize) );
