...
//#include <query_compilation/gpu_utilities/compilation-hashtables/compilation_hashtables.cuh>
...
TablePtr ssb4_q3() {
    ...
    ColumnPtr col_lo_supplycost = table_lineorder->getColumnbyName("LO_SUPPLYCOST");
    ...
    boost::shared_ptr<Column<int> > typed_col_lo_custkey = boost::dynamic_pointer_cast<Column<int> >(col_lo_custkey);
    typed_col_lo_custkey->data();

    boost::shared_ptr<Column<int> > typed_col_lo_suppkey = boost::dynamic_pointer_cast<Column<int> >(col_lo_suppkey);
    typed_col_lo_suppkey->data();

    boost::shared_ptr<Column<int> > typed_col_lo_partkey = boost::dynamic_pointer_cast<Column<int> >(col_lo_partkey);
    typed_col_lo_partkey->data();

    boost::shared_ptr<Column<int> > typed_col_lo_orderdate = boost::dynamic_pointer_cast<Column<int> >(col_lo_orderdate);
    typed_col_lo_orderdate->data();

    boost::shared_ptr<Column<float> > typed_col_lo_revenue = boost::dynamic_pointer_cast<Column<float> >(col_lo_revenue);
    typed_col_lo_revenue->data();

    boost::shared_ptr<Column<float> > typed_col_lo_supplycost = boost::dynamic_pointer_cast<Column<float> >(col_lo_supplycost);
    typed_col_lo_supplycost->data();

    boost::shared_ptr<Column<int> > typed_col_c_custkey = boost::dynamic_pointer_cast<Column<int> >(col_c_custkey);
    typed_col_c_custkey->data();

    boost::shared_ptr<DictionaryCompressedColumn<std::string> > dict_customer_region =
            boost::dynamic_pointer_cast<DictionaryCompressedColumn<std::string > > (col_c_region);

    boost::shared_ptr<DictionaryCompressedColumn<std::string> > dict_customer_nation =
            boost::dynamic_pointer_cast<DictionaryCompressedColumn<std::string > > (col_c_nation);

    boost::shared_ptr<DictionaryCompressedColumn<std::string> > dict_supplier_region =
            boost::dynamic_pointer_cast<DictionaryCompressedColumn<std::string > > (col_s_region);

    boost::shared_ptr<Column<int> > typed_col_s_suppkey = boost::dynamic_pointer_cast<Column<int> >(col_s_suppkey);
    typed_col_s_suppkey->data();

    boost::shared_ptr<DictionaryCompressedColumn<std::string> > dict_part_mfgr =
            boost::dynamic_pointer_cast<DictionaryCompressedColumn<std::string > > (col_p_mfgr);

    boost::shared_ptr<Column<int> > typed_col_p_partkey = boost::dynamic_pointer_cast<Column<int> >(col_p_partkey);
    typed_col_p_partkey->data();

    boost::shared_ptr<Column<int> > typed_col_d_datekey = boost::dynamic_pointer_cast<Column<int> >(col_d_datekey);
    typed_col_d_datekey->data();

    boost::shared_ptr<Column<int> > typed_col_d_year = boost::dynamic_pointer_cast<Column<int> >(col_d_year);
    ...
    for(int i=0; i < 20; i++) {
       ...
       p_col_lo_supplycost = customMalloc<float>(DEVICE_MEMORY, lineorder_num_elements);
       ...
       p_col_d_year = customMalloc<int>(DEVICE_MEMORY, dates_num_elements);
        ...
        cudaMemcpy(p_col_lo_custkey, typed_col_lo_custkey->data(), lineorder_num_elements * sizeof(int), cudaMemcpyHostToDevice);
        cudaMemcpy(p_col_lo_suppkey, typed_col_lo_suppkey->data(), lineorder_num_elements * sizeof(int), cudaMemcpyHostToDevice);
        cudaMemcpy(p_col_lo_partkey, typed_col_lo_partkey->data(), lineorder_num_elements * sizeof(int), cudaMemcpyHostToDevice);
        cudaMemcpy(p_col_lo_orderdate, typed_col_lo_orderdate->data(), lineorder_num_elements * sizeof(int), cudaMemcpyHostToDevice);
        cudaMemcpy(p_col_lo_revenue, typed_col_lo_revenue->data(), lineorder_num_elements * sizeof(float), cudaMemcpyHostToDevice);
       cudaMemcpy(p_col_lo_supplycost, typed_col_lo_supplycost->data(), lineorder_num_elements * sizeof(float), cudaMemcpyHostToDevice);
       cudaMemcpy(p_col_c_custkey, typed_col_c_custkey->data(), customer_num_elements * sizeof(int), cudaMemcpyHostToDevice);
       cudaMemcpy(p_col_c_region, dict_customer_region->getIdData(), customer_num_elements * sizeof(uint32_t), cudaMemcpyHostToDevice);
       cudaMemcpy(p_col_c_nation, dict_customer_nation->getIdData(), customer_num_elements * sizeof(uint32_t), cudaMemcpyHostToDevice);
       cudaMemcpy(p_col_s_region, dict_supplier_region->getIdData(), supplier_num_elements * sizeof(uint32_t), cudaMemcpyHostToDevice);
       cudaMemcpy(p_col_s_suppkey, typed_col_s_suppkey->data(), supplier_num_elements * sizeof(int), cudaMemcpyHostToDevice);
       cudaMemcpy(p_col_p_mfgr, dict_part_mfgr->getIdData(), part_num_elements * sizeof(uint32_t), cudaMemcpyHostToDevice);
       cudaMemcpy(p_col_p_partkey, typed_col_p_partkey->data(), part_num_elements * sizeof(int), cudaMemcpyHostToDevice);
       cudaMemcpy(p_col_d_datekey, typed_col_d_datekey->data(), dates_num_elements * sizeof(int), cudaMemcpyHostToDevice);
       cudaMemcpy(p_col_d_year, typed_col_d_year->data(), dates_num_elements * sizeof(int), cudaMemcpyHostToDevice);
             ...
             cudaMallocManaged((void**)&result_d_year, sizeof(int) * numBlocks * maxResultsPerBlock);
             cudaMallocManaged((void**)&result_c_nation, sizeof(uint32_t) * numBlocks * maxResultsPerBlock);
             cudaMallocManaged((void**)&result_profit, sizeof(float) * numBlocks * maxResultsPerBlock);
      ...
      GPU_CHT32_2::HashTable supplierHashTable = GPU_CHT32_2::createHashTable(supplier_selected_num_elements, 1.25);
      dim3 gridDim = computeGridDim(supplier_num_elements, supplierHashTable.gridSize, supplierHashTable.blockSize);
      supplierBuildKernel<<<gridDim, supplierHashTable.blockSize>>>(supplierHashTable, p_col_s_region, p_col_s_suppkey, supplier_num_elements);
      ...
      GPU_CHT32_2::HashTable datesHashTable = GPU_CHT32_2::createHashTable(dates_selected_num_elements, 1.25);
      gridDim = computeGridDim(dates_num_elements, datesHashTable.gridSize, datesHashTable.blockSize);
      datesBuildKernel<<<gridDim, datesHashTable.blockSize>>>(datesHashTable, p_col_d_year, p_col_d_datekey, dates_num_elements);
      ...
      GPU_CHT32_2::HashTable customerHashTable = GPU_CHT32_2::createHashTable(customer_selected_num_elements, 1.25);
      gridDim = computeGridDim(customer_num_elements, customerHashTable.gridSize, customerHashTable.blockSize);
      customerBuildKernel<<<gridDim, customerHashTable.blockSize>>>(customerHashTable, p_col_c_region, p_col_c_custkey, customer_num_elements);
      ...
      GPU_CHT32_2::HashTable partHashTable = GPU_CHT32_2::createHashTable(part_selected_num_elements, 1.25);
      gridDim = computeGridDim(part_num_elements, partHashTable.gridSize, partHashTable.blockSize);
      partBuildKernel<<<gridDim, partHashTable.blockSize>>>(partHashTable, p_col_p_mfgr, p_col_p_partkey, part_num_elements);
      ...
      cout << "build: " << build_time << " ms, probe: " << probe_time << " ms" << endl;
