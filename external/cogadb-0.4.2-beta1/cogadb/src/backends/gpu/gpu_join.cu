...
namespace CoGaDB {
    ...
    template <typename T>
    typename GPU_Join<T>::JoinFunctionPtr GPU_Join<T>::get(const JoinType& join_type) {
        ...
        if (!initialized) {
            ...
            map.insert(std::make_pair(RIGHT_OUTER_JOIN, &GPU_Join<T>::right_outer_join));
    ...
    const PositionListPairPtr GPU_Join<T>::inner_join(T* join_column1,
        ...
        if(left_num_elements > right_num_elements) {
            result = binary_search_join(join_column1, join_column2, left_num_elements, right_num_elements, param);
        } else {
            result = binary_search_join(join_column2, join_column1, right_num_elements, left_num_elements, param);
    ...
    __global__ void join_and_count_kernel(T* column_left, size_t count_column_left, 
        ...
        while(tid<count_column_left){
            ...
            TID index = binary_search_first_occurence(column_right, count_column_right, column_left[tid]);
            TID count = 0;
    ...
    template<typename T>
    __global__ void join_and_write_kernel(T* fact_table, size_t count_fact_table, T* dimension_table, size_t count_dimension_table, TID* scan_times_found, TID* result_fact, TID* result_dimension){
        //threads align to join index entries   
        TID tid = threadIdx.x + blockIdx.x * blockDim.x;

        while(tid<count_fact_table){
            //try to find first occurence of key
            TID index = binary_search_first_occurence(dimension_table, count_dimension_table, fact_table[tid]);
    ...
    template <typename T>
    const PositionListPairPtr GPU_Join<T>::binary_search_join(T* join_column1, 
            T* join_column2,
            size_t left_num_elements, 
            size_t right_num_elements,
            const JoinParam& param) {
        ...
        gpu::GPUContextPtr contextptr = gpu::StreamManager::instance().getCudaContext(0);
        const hype::ProcessingDeviceMemoryID mem_id = hype::util::getMemoryID(param.proc_spec.proc_id);
        ...
        if(left_num_elements == 0 || right_num_elements == 0) {
            cout << "WARNING!! Join input with size zero (" << left_num_elements << ", " << right_num_elements<< ")" << endl;
            return emptyPair;
        }
        ...
        if(!right_column_is_sorted) {
            try {
                dev_vec_column2_sorted = thrust::device_vector<T>(thrust::device_pointer_cast(join_column2), 
            ...
            unsort_lookup = GPU_Util<T>::sort(join_column2, right_num_elements, sort_param);
        ...
        try {
            ...
            COGADB_ERROR("Ran out of memory during GPU_Join<T>::binary_search_join!" << std::endl
                    << "Attempted to allocate " << left_num_elements << " elements "
                    << "(" << double(left_num_elements*sizeof(TID))/(1024*1024*1024) << "GB)", "");
            return result;
        }
        ...
        join_and_count_kernel<<<number_of_blocks, number_of_threads,0,*stream>>>(
                join_column1, left_num_elements, 
                join_column2, right_num_elements,
                devptr_times_found);    
        cudaError_t err=cudaStreamSynchronize(*stream);
        if(err!=cudaSuccess){
            COGADB_ERROR("Error executing kernel join_and_count_kernel<T>!" 
                    << std::endl << "\tSize Left Table: " << left_num_elements << "rows"
                    << std::endl << "\tSize Right Table: " << right_num_elements << "rows\n", "");
            gpuErrchk(err);
        ...
        thrust::exclusive_scan(dev_vec_times_found.begin(), dev_vec_times_found.end(), dev_vec_scan.begin());
        size_t count_join_output = dev_vec_scan[left_num_elements-1] + dev_vec_times_found[left_num_elements-1];
        if(count_join_output == 0)
            return emptyPair;
        ...
        if(!right_column_is_sorted) {
            ...
            GPU_Util<TID>::gather(getPointer(*second), getPointer(*unsort_lookup), second, gather_param);
        }
    ...
    template <>
    const PositionListPairPtr GPU_Join<std::string>::binary_search_join(std::string* join_column1, 
            std::string* join_column2,
            size_t left_num_elements, 
            size_t right_num_elements,
            const JoinParam& param) {
    ...
    template <>
    const PositionListPairPtr GPU_Join<C_String>::binary_search_join(C_String* join_column1, 
            C_String* join_column2,
            size_t left_num_elements, 
            size_t right_num_elements,
            const JoinParam& param) {
        COGADB_FATAL_ERROR("Called Unimplemented function!", "");
    ...
    template <typename T>
    const PositionListPairPtr GPU_Join<T>::left_outer_join(T* join_column1,
            size_t left_num_elements,
            T* join_column2,
            size_t right_num_elements,
            const JoinParam& param) {
    ...
    template <typename T>
    const PositionListPairPtr GPU_Join<T>::right_outer_join(T* join_column1,
            size_t left_num_elements,
            T* join_column2,
            size_t right_num_elements,
            const JoinParam& param) {
    ...
    template <typename T>
    const PositionListPairPtr GPU_Join<T>::full_outer_join(T* join_column1,
            size_t left_num_elements,
            T* join_column2,
            size_t right_num_elements,
            const JoinParam& param) {
    ...
    template <typename T>
    typename GPU_Semi_Join<T>::TIDSemiJoinFunctionPtr GPU_Semi_Join<T>::getTIDSemiJoin(const JoinType& join_type) {
        ...
        static bool initialized = false;
        if (!initialized) {
            map.insert(std::make_pair(LEFT_SEMI_JOIN, &GPU_Semi_Join<T>::tid_left_semi_join));
            map.insert(std::make_pair(RIGHT_SEMI_JOIN, &GPU_Semi_Join<T>::tid_right_semi_join));
            map.insert(std::make_pair(LEFT_ANTI_SEMI_JOIN, &GPU_Semi_Join<T>::tid_left_anti_semi_join));
            map.insert(std::make_pair(RIGHT_ANTI_SEMI_JOIN, &GPU_Semi_Join<T>::tid_right_anti_semi_join));
            initialized = true;
        }
        if (map.find(join_type) != map.end()) {
            return map.find(join_type)->second;
        } else {
            COGADB_FATAL_ERROR("Detected use of unsupported Join Type: " << util::getName(join_type), "");
            return TIDSemiJoinFunctionPtr();
        }
    }

    template <typename T>
    typename GPU_Semi_Join<T>::BitmapSemiJoinFunctionPtr GPU_Semi_Join<T>::getBitmapSemiJoin(const JoinType& join_type) {
    ...
    template <typename T>
    const PositionListPtr GPU_Semi_Join<T>::tid_left_semi_join(T* join_column1,
            size_t left_num_elements,
            T* join_column2,
            size_t right_num_elements,
            const JoinParam& param) {
        
        PositionListPtr result = d_SemiJoin(join_column2, right_num_elements, join_column1, left_num_elements, false, param);
    ...
    template <typename T>
    const PositionListPtr GPU_Semi_Join<T>::tid_right_semi_join(T* join_column1,
            size_t left_num_elements,
            T* join_column2,
            size_t right_num_elements,
            const JoinParam& param) {

        return d_SemiJoin(join_column1, left_num_elements, join_column2, right_num_elements, false, param);
    }

    template <typename T>
    const PositionListPtr GPU_Semi_Join<T>::tid_left_anti_semi_join(T* join_column1,
            size_t left_num_elements,
            T* join_column2,
            size_t right_num_elements,
            const JoinParam& param) {

        return d_SemiJoin(join_column2, right_num_elements, join_column1, left_num_elements, true, param);
    }

    template <typename T>
    const PositionListPtr GPU_Semi_Join<T>::tid_right_anti_semi_join(T* join_column1,
            size_t left_num_elements,
            T* join_column2,
            size_t right_num_elements,
            const JoinParam& param) {

        return d_SemiJoin(join_column1, left_num_elements, join_column2, right_num_elements, true, param);
    }

    template <typename T>
    const BitmapPtr GPU_Semi_Join<T>::bitmap_left_semi_join(T* join_column1,
            size_t left_num_elements,
            T* join_column2,
            size_t right_num_elements,
            const JoinParam& param) {

        return d_BitmapSemiJoin(join_column2, right_num_elements, join_column1, left_num_elements, false, param);
    }

    template <typename T>
    const BitmapPtr GPU_Semi_Join<T>::bitmap_right_semi_join(T* join_column1,
            size_t left_num_elements,
            T* join_column2,
            size_t right_num_elements,
            const JoinParam& param) {

        return d_BitmapSemiJoin(join_column1, left_num_elements, join_column2, right_num_elements, false, param);
    }

    template <typename T>
    const BitmapPtr GPU_Semi_Join<T>::bitmap_left_anti_semi_join(T* join_column1,
            size_t left_num_elements,
            T* join_column2,
            size_t right_num_elements,
            const JoinParam& param) {
        
        return d_BitmapSemiJoin(join_column2, right_num_elements, join_column1, left_num_elements, true, param);
    }

    template <typename T>
    const BitmapPtr GPU_Semi_Join<T>::bitmap_right_anti_semi_join(T* join_column1,
            size_t left_num_elements,
            T* join_column2,
            size_t right_num_elements,
            const JoinParam& param) {
        
        return d_BitmapSemiJoin(join_column1, left_num_elements, join_column2, right_num_elements, true, param);
    }


    
    template<typename T>
    const PositionListPtr GPU_Semi_Join<T>::d_SemiJoin(T* d_build, size_t numBuild, T* d_probe, size_t numProbe, bool anti, const JoinParam& param) {

    std::clock_t start;
    double duration;
    start = std::clock();
    cout << "--- semi join input sizes - probe: " 
         << ((numProbe * sizeof(T)) / (double)SIZE_1MB) << "mb, build: " 
         << ((numBuild * sizeof(T)) / (double)SIZE_1MB) << "mb" << endl;
        ...
        // -------- hashjoin build --------
        if(!ht.Build(numBuild, (unsigned*)d_build, NULL))
            return PositionListPtr();

        // -------- hashjoin probe --------
        TID* d_semiJoin;
        if(!(gpuAssertCheck(cudaMalloc((void**)&d_semiJoin, sizeof(TID) * numProbe))))
            return PositionListPtr();
        
        unsigned int resultSize = 0;
        bool success = ht.RetrieveSemiJoin(numProbe, (unsigned*)d_probe, resultSize, (unsigned*)d_semiJoin, anti);
        if(!success) {
            cudaFree(d_semiJoin);
            return PositionListPtr();
        }
        ...
        if(!(gpuAssertCheck(cudaMemcpy(result->data(), d_semiJoin, resultSize * sizeof(TID), cudaMemcpyDeviceToDevice)))) {
            cudaFree(d_semiJoin);
            return PositionListPtr();
        }
        cudaFree(d_semiJoin);
    ...
    template<typename T>
    const BitmapPtr GPU_Semi_Join<T>::d_BitmapSemiJoin(T* d_build, size_t numBuild, T* d_probe, size_t numProbe, bool anti, const JoinParam& param) {
        ...
        bool success = ht.RetrieveBitmapSemiJoin(numProbe, (unsigned*)d_probe, d_bitmap, anti);
        if(!success) {
            cudaFree(d_bitmap);
            return BitmapPtr();
        }
        ...
        pack_flagarray_kernel<<<128,128,0,stream>>>(d_bitmap, numProbe, result->data());
