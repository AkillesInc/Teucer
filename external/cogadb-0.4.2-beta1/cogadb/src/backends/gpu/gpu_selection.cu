...
namespace CoGaDB {
    namespace gpu {
        ...
        template <typename T>
        __global__ void selection_set_flag_array_kernel(T* array, size_t array_size, T comparison_value, const ValueComparator comp, char* flags) {
        ...
        template <typename T>
        __global__ void column_comparison_set_flag_array_kernel(T* left_array, T* right_array, size_t array_size, const ValueComparator comp, char* flags) {
        ...
        __global__ void selection_fetch_tids_in_output_buffer_kernel(size_t array_size, char* flags, TID* write_positions_array, TID* result_buffer) {
        ...
        template <typename T>
        const PositionListPtr selection(T* input_array, size_t input_array_size, T value, ValueComparator comp, const hype::ProcessingDeviceMemoryID& mem_id) {

            if (input_array_size == 0) {
                return createPositionList(0, mem_id);
            }

            if(VariableManager::instance().getVariableValueBoolean("unsafe_feature.enable_immediate_selection_abort")) return PositionListPtr();
            ...
            if (!quiet) std::cout << "flag array: " << (void*) flag_array.get() << std::endl;
            ...
            selection_set_flag_array_kernel<T> << <number_of_blocks, number_of_threads_per_block, 0, *stream>>>(input_array, input_array_size, value, comp, flag_array.get());
            ...
            ret=GPU_Prefix_Sum(flag_array, write_positions_array, input_array_size, stream);
            ...
            if (!quiet) std::cout << "Result size GPU Selection:" << result_size << std::endl;
            //save second kernel call, in case result is empty
            if (result_size == 0) {
                thrust::device_free(write_positions_array);
                thrust::device_free(flag_array);
                return createPositionList(0, mem_id); //GPU_PositionlistPtr(new Impl_GPU_Positionlist());
            }
            ...
            selection_fetch_tids_in_output_buffer_kernel <<<number_of_blocks, number_of_threads_per_block, 0, *stream>>>(input_array_size, flag_array.get(), write_positions_array.get(), getPointer(*result));
            err=cudaStreamSynchronize(*stream);
            if(err!=cudaSuccess){
                //free allocated memory
                thrust::device_free(flag_array);
                thrust::device_free(write_positions_array);
                COGADB_ERROR("Error executing kernel selection_fetch_tids_in_output_buffer_kernel<T>!", "");
                gpuErrchk(err);
                return PositionListPtr();
            }
        ...
        const PositionListPtr selection(T* column, size_t num_elements,
            ...
            column_comparison_set_flag_array_kernel<T> <<<number_of_blocks, number_of_threads_per_block, 0, *stream>>>(column, comparison_column, num_elements, comp, flag_array.get());
            ...
            if(err!=cudaSuccess){
                ...
                thrust::device_free(flag_array);
                thrust::device_free(write_positions_array);
                COGADB_ERROR("Error executing kernel column_comparison_set_flag_array_kernel<T>!", "");
                gpuErrchk(err);
                return PositionListPtr();
            }
...
//            try {
//                result_size=GPU_Prefix_Sum(flag_array, write_positions_array, num_elements, stream);
//            } catch (std::bad_alloc &e) {
//                //free allocated memory
//                thrust::device_free(flag_array);
//                thrust::device_free(write_positions_array);
...
//            }
            ...
            //launch kernel that writes the matching TIDs into the output buffer
            selection_fetch_tids_in_output_buffer_kernel <<<number_of_blocks, number_of_threads_per_block, 0, *stream>>>(num_elements, flag_array.get(), write_positions_array.get(), getPointer(*result));
            err=cudaStreamSynchronize(*stream);
            if(err!=cudaSuccess){
                thrust::device_free(flag_array);
                thrust::device_free(write_positions_array);
    ...
    template <typename T>
    const PositionListPtr GPU_Selection<T>::tid_selection(T* column, size_t num_elements,
            T comparison_value,
    ...
    template <typename T>
    const BitmapPtr GPU_Selection<T>::bitmap_selection(T* column, size_t num_elements,
            T comparison_value,
            const ValueComparator& comp, const hype::ProcessingDeviceMemoryID& mem_id) {
    ...
    template <>
    const PositionListPtr GPU_Selection<std::string>::tid_selection(std::string* column, size_t num_elements,
    ...
    template <>
    const BitmapPtr GPU_Selection<std::string>::bitmap_selection(std::string* column, size_t num_elements,
            std::string comparison_value,
            const ValueComparator& comp, const hype::ProcessingDeviceMemoryID& mem_id) {
        COGADB_FATAL_ERROR("Called Unimplemented function!", "");
        return BitmapPtr();
