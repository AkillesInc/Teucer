...
namespace CoGaDB {
    ...
    template <typename T>
    bool GPU_Util<T>::generateConstantSequence(T* dest_column,
            size_t num_elements,
            T value,
    ...
    template <typename T>
    bool GPU_Util<T>::generateAscendingSequence(T* dest_column,
            size_t num_elements,
            T begin_value,
            const ProcessorSpecification& proc_spec) {
        try {
    ...
    template <typename T>
    __global__ void gather_kernel(T* dest_column, T* source_column, TID* tids, size_t number_of_tids) {
    ...
    template <typename T>
    bool GPU_Util<T>::gather(T* dest_column, T* source_column,
        ...
        gather_kernel << <number_of_blocks,
                number_of_threads_per_block, 0, *stream>>>(dest_column,
                source_column,
                getPointer(*tids),
                getSize(*tids));
    ...
    const PositionListPtr GPU_Util<T>::sort(T* column, size_t num_elements,
        ...
        try {
            ...
            thrust::sequence(thrust::device_pointer_cast(getPointer(*tids)),
                    thrust::device_pointer_cast(getPointer(*tids) + num_elements), 0); //   tids->begin(), tids->end(), 0);
            ...
            if (param.stable) {
                thrust::stable_sort_by_key(thrust::device_pointer_cast(column),
                        thrust::device_pointer_cast(column + num_elements),
                        thrust::device_pointer_cast(getPointer(*tids)));
            } else {
                thrust::sort_by_key(thrust::device_pointer_cast(column),
                        thrust::device_pointer_cast(column + num_elements),
            ...
            if (param.order == DESCENDING) {
                thrust::reverse(thrust::device_pointer_cast(getPointer(*tids)),
                        thrust::device_pointer_cast(getPointer(*tids) + num_elements)); //tids->begin(), tids->end());
            }
        } catch (std::bad_alloc &e) {
    ...
    template <>
    void GPU_Util<std::string>::print_device_array(std::string* pointer, size_t length) {

    }

    template <>
    bool GPU_Util<std::string>::generateConstantSequence(std::string* dest_column,
            size_t num_elements,
            std::string value,
            const ProcessorSpecification& proc_spec) {
        return false;
    ...
    template <>
    bool GPU_Util<std::string>::generateAscendingSequence(std::string* dest_column,
            size_t num_elements,
    ...
    template <>
    bool GPU_Util<std::string>::gather(std::string* dest_column, std::string* source_column,
            PositionListPtr tids, const GatherParam& param) {
        return false;
    }

    template <>
    const PositionListPtr GPU_Util<std::string>::sort(std::string* column, size_t num_elements,
            const SortParam& param) {
        return PositionListPtr();
    }
    ...
    template <>
    bool GPU_Util<C_String>::generateConstantSequence(C_String* dest_column,
            size_t num_elements,
            C_String value,
            const ProcessorSpecification& proc_spec) {
        return false;
    ...
    template <>
    bool GPU_Util<C_String>::generateAscendingSequence(C_String* dest_column,
            size_t num_elements,
            C_String begin_value,
            const ProcessorSpecification& proc_spec) {
        return false;
    }
    ...
    template <>
    const PositionListPtr GPU_Util<C_String>::sort(C_String* column, size_t num_elements,
            const SortParam& param) {
        return PositionListPtr();
    }
    ...
    std::pair<cudaError_t, size_t> GPU_Prefix_Sum(const thrust::device_ptr<char> flag_array,
        ...
        if(!quiet){
            ...
            std::cout << "Can use CUB prefix scan: " << can_use_cub_prefix_scan << std::endl;
        }
        ...
        if (can_use_cub_prefix_scan) {
            ...
            PositionListPtr converted_flags = createPositionList(input_array_size, hype::PD_Memory_1);
            if (!converted_flags) {
                double free_memory_in_gb = double(HardwareDetector::instance().getFreeMemorySizeInByte(hype::PD_Memory_1))/(1024*1024*1024);
                COGADB_ERROR("Ran out of memory during GPU_Prefix_Sum!" << std::endl
                        << "Attempted to allocate " << input_array_size << " elements "
                        << "(" << double(input_array_size*sizeof(TID))/(1024*1024*1024) << "GB)" << std::endl
                        << "Free Memory: " << free_memory_in_gb << "GB)", "");
                return std::make_pair(cudaErrorMemoryAllocation, 0);
            }
            type_cast_gpu << <128, 128, 0, *stream>>>(thrust::raw_pointer_cast(flag_array), converted_flags->data(), input_array_size);
            ...
            err = DeviceScan::ExclusiveSum(d_temp_storage, temp_storage_bytes, converted_flags->data(), thrust::raw_pointer_cast(write_positions_array), input_array_size, *stream);
            ...
            if (err != cudaSuccess) {
                if(err == cudaErrorMemoryAllocation){
                    COGADB_ERROR("Ran out of memory during GPU_Prefix_Sum!" << std::endl
                            << "Attempted to allocate " << input_array_size << " elements "
                            << "(" << double(input_array_size*sizeof(TID))/(1024*1024*1024) << "GB)", "");
                }else{
                    gpuErrchk(err);
                }
                return std::make_pair(err, 0);
            ...
            try {
                thrust::exclusive_scan(thrust::device_ptr<TID>(converted_flags->data()), thrust::device_ptr<TID>(converted_flags->data() + input_array_size), write_positions_array);
            } catch (std::bad_alloc &e) {
            ...
            try {
                thrust::device_ptr<TID> end_write_positions_array = thrust::exclusive_scan(flag_array, flag_array + input_array_size, write_positions_array);
                assert(end_write_positions_array == write_positions_array + input_array_size);
            } catch (std::bad_alloc &e) {
                COGADB_ERROR("Ran out of memory during GPU_Prefix_Sum!", "");
                return std::make_pair(err, 0);
            }
        ...
        thrust::copy(write_positions_array, write_positions_array + input_array_size, cpu_write_positions.begin());
        ...
        for (size_t i = 0; i < input_array_size; ++i) {
            ...
            if (sum != cpu_write_positions[i]) {
                ...
                std::cout << "Error: " << sum << "!=" << cpu_write_positions[i] << std::endl;
            }
            if (cpu_flags[i] > 1) {
                std::cout << "Error: Invalid flag at position " << i << ": " << cpu_flags[i] << std::endl;
            }
            sum += cpu_flags[i];
        }
#endif
        ...
        err = cudaMemcpyAsync(&result_size,
                (void*) &tmp[input_array_size - 1],
                sizeof (TID),
        ...
        err = cudaMemcpyAsync(&last_flag,
                (void*) &tmp2[input_array_size - 1],
                sizeof (char),
                cudaMemcpyDeviceToHost,
                *stream);
        err = cudaStreamSynchronize(*stream);
