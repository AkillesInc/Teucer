...
namespace CUDAWrapper {
    ...
    cudaError_t CallCuckooHash(const unsigned      n,
    ...
    bool CallHashRetrieve(const unsigned      n_queries,
        ...
        CUDA_SAFE_MALLOC(cudaMalloc((void**)&d_retrieval_probes, sizeof(unsigned) * n_queries));
    #endif
...
bool CallRetrieveHashJoin(const unsigned      n_queries,
                          const unsigned      num_hash_functions,
                          const unsigned     *d_keys,
                          const unsigned      table_size,
                          const Entry        *d_contents,
        ...
        try{
            thrust::exclusive_scan(d_block_result_sizes.begin(), d_block_result_sizes.end(), d_block_scans.begin());
        ...
        unsigned count = d_block_scans[gridDim.x * gridDim.y - 1] + d_block_result_sizes[gridDim.x * gridDim.y - 1];
        block_compaction<<<gridDim, kBlockSize, 0, computeStream>>>(n_queries, rawPtr(d_block_result_sizes), rawPtr(d_block_scans), d_temp_query, d_temp_table, d_out_query, d_out_table);
        err=cudaStreamSynchronize(computeStream);
        if(err!=cudaSuccess){
            gpuErrchk(err);
            COGADB_ERROR("Could not retrieve HashJoin result from hash table! " 
...
bool CallBitmapSemiJoin(  const unsigned      n_queries,
                          const unsigned      num_hash_functions,
                          const unsigned     *d_keys,
                          const unsigned      table_size,
                          const Entry        *d_contents,
