...
namespace CudaHT
{
    namespace CuckooHashing
    {
                                      ...
                                      const unsigned num_functions)
        {
            ...
            unsigned max_iterations = (unsigned) (4.0
                                          * ceil(-1.0 / (0.028255 + 1.1594772 * ln_load_factor) * lg_input_size));
#endif

            // Debugging: Print out how many iterations are allowed.

#ifdef _DEBUG
            sprintf(buffer, "Maximum iterations: %u (%f lg (N))", max_iterations, max_iterations / lg_input_size);
                                   ...
                                   const unsigned     num_functions)
        {
            ...
            {
                char message[256] = "Number of hash functions must be from 2 to 5; " "others are unimplemented.";
            ...
            {
                sprintf(buffer, "Minimum possible space usage for %u functions is %f.", num_functions,
                        minimum_space_usage);
                PrintMessage(buffer);
                              ...
                              const unsigned * d_values)
        {
            ...
            CUDA_SAFE_MALLOC(cudaMalloc((void **) &d_iterations_taken, sizeof(unsigned) * n));
#endif
            ...
            while (num_failures && (++num_attempts < kMaxRestartAttempts))
            {
                ...
                CUDAWrapper::ClearTable(slots_in_table, kEntryEmpty, d_contents_, computeStream);

                num_failures = 0;

                cudaError_t err = CUDAWrapper::CallCuckooHash(n, num_hash_functions_, d_keys, d_values, table_size_, constants_2_,
...
//            CUDA_CHECK_ERROR("Error occurred during hash table build.\n");
        ...
        void HashTable::Retrieve(const unsigned   n_queries,
                                 ...
                                 unsigned *       d_values)
        {
            CUDAWrapper::CallHashRetrieve(n_queries, num_hash_functions_, d_keys, table_size_, d_contents_,
                                          constants_2_, constants_3_, constants_4_, constants_5_, stash_constants_,
                                          stash_count_, d_values, computeStream);
        }

        bool HashTable::RetrieveJoin(const unsigned   n_queries,
                                         const unsigned * d_keys,
                                         ...
                                         unsigned *       d_out_table)
        {
            if (d_out_table != NULL)
            {
                CUDA_SAFE_MALLOC(cudaMalloc((void **) &d_table_temp, sizeof(unsigned) * n_queries));
                table_temp_size = n_queries;
            }
            CUDA_SAFE_MALLOC(cudaMalloc((void **) &d_query_temp, sizeof(unsigned) * n_queries));
            query_temp_size = n_queries;

            bool success = CUDAWrapper::CallRetrieveHashJoin(n_queries, num_hash_functions_, d_keys, table_size_,
                                 d_contents_, constants_2_, constants_3_, constants_4_, constants_5_, stash_constants_,
                                 stash_count_, result_size,  d_query_temp, d_table_temp, d_out_query, d_out_table, false,
                                 computeStream);
                ...
                bool                                        anti)
        {
            ...
            bool success;
            success = CUDAWrapper::CallRetrieveHashJoin(n_queries, num_hash_functions_, d_keys, table_size_,
                                 d_contents_, constants_2_, constants_3_, constants_4_, constants_5_, stash_constants_,
                                 stash_count_, result_size, d_query_temp, NULL, d_out_query, NULL, anti, computeStream);
        ...
        bool HashTable::RetrieveBitmapSemiJoin(const unsigned n_queries,
                ...
                bool                                              anti)
        {
            CUDAWrapper::CallBitmapSemiJoin(n_queries, num_hash_functions_, d_query_keys, table_size_, d_contents_,
                                            constants_2_, constants_3_, constants_4_, constants_5_, stash_constants_,
