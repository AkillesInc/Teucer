...
//        }
    ...
    template <typename T>
    __global__ void bit_shift_left_kernel(T* array, size_t array_size, size_t num_bits_to_shift) {
    ...
    template <typename T>
    __global__ void bit_shift_right_kernel(T* array, size_t array_size, size_t num_bits_to_shift) {
    ...
    template <typename T>
    __global__ void bitwise_and_kernel(T* target_array, T* source_array, size_t array_size) {
    ...
    template <typename T>
    __global__ void bitwise_or_kernel(T* target_array, T* source_array, size_t array_size) {
    ...
    bool GPU_BitOperation::bit_shift(ColumnGroupingKeysPtr keys,
        ...
        if (param.op == SHIFT_BITS_LEFT) {
            bit_shift_left_kernel << <number_of_blocks,
                    number_of_threads_per_block, 0, *stream>>>(
                    keys->keys->data(),
                     keys->keys->size(),
...
//                    getSize(*(keys->keys)),
            ...
            bit_shift_right_kernel << <number_of_blocks,
                    number_of_threads_per_block, 0, *stream>>>(
                     keys->keys->data(),
                     keys->keys->size(),
//                    getPointer(*(keys->keys)),
//                    getSize(*(keys->keys)),
    ...
    bool GPU_BitOperation::bitwise_combination(ColumnGroupingKeysPtr target_keys,
            ColumnGroupingKeysPtr source_keys, const BitwiseCombinationParam& param) {
        const int number_of_blocks = 512;
        const int number_of_threads_per_block = 512;

        cudaStream_t* stream = gpu::StreamManager::instance().getStream();
        if (param.op == BITWISE_AND) {
            bitwise_and_kernel << <number_of_blocks,
                    number_of_threads_per_block, 0, *stream>>>(
                     target_keys->keys->data(),
...
//                    );
            ...
            bitwise_or_kernel << <number_of_blocks,
                    number_of_threads_per_block, 0, *stream>>>(
                     target_keys->keys->data(),
                     source_keys->keys->data(),
                     target_keys->keys->size());
