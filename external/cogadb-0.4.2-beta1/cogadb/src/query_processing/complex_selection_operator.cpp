...
//Map_Init_Function init_function_ComplexSelection_operator=physical_operator::map_init_function_ComplexSelection_operator;
	...
	namespace physical_operator{
                ...
                {                    
                    ...
                    if(knf_expr_.disjunctions.size()>1){ 
                        this->result_ = query_processing::two_phase_physical_optimization_selection(this->getInputData(), knf_expr_, dev_constr_, mat_stat_, CoGaDB::RuntimeConfiguration::instance().getParallelizationModeForTwoPhasePhysicalOptimizationQueryPlans(), this->out);
                    //if we have just one disjunction, it is possible that we split up the query in selections for single disjunctions, which we execute serially
                    //the other case is that the user specified a condition that can be represented by a single disjunction
                    //in both cases, it is correct to create a column based plan for the disjunction and construct a Lookup Table from the result    
                        ...
                        //SelectionParam param(proc_spec, ValueConstantPredicate, knf_expr_.disjunctions[0], comp);   
                
                        this->result_= BaseTable::selection(this->getInputData(), knf_expr_.disjunctions[0], proc_spec, dev_constr_);
                    }else{
	...
	TypedOperatorPtr create_CPU_ComplexSelection_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr right_child)
	{
		logical_operator::Logical_ComplexSelection& log_selection_ref = static_cast<logical_operator::Logical_ComplexSelection&>(logical_node);
	...
	Physical_Operator_Map_Ptr map_init_function_complex_selection_operator()
	{
		...
		if(!quiet) std::cout << "calling map init function for SELECTION operator!" << std::endl;
		//hype::Scheduler::instance().addAlgorithm("SELECTION","CPU_ComplexSelection_Algorithm",hype::CPU,"Least Squares 1D","Periodic Recomputation");
		//hype::Scheduler::instance().addAlgorithm("SELECTION","GPU_ComplexSelection_Algorithm",hype::GPU,"Least Squares 1D","Periodic Recomputation");

#ifdef COGADB_USE_KNN_REGRESSION_LEARNER
		hype::AlgorithmSpecification selection_alg_spec_cpu("CPU_ComplexSelection_Algorithm",
                                                                    "COMPLEX_SELECTION",
                                                                    hype::KNN_Regression,hype::Periodic);
                
		hype::AlgorithmSpecification selection_alg_spec_gpu("GPU_ComplexSelection_Algorithm",
                                                                    "COMPLEX_SELECTION",
                                                                    hype::KNN_Regression,hype::Periodic);
#else
		hype::AlgorithmSpecification selection_alg_spec_cpu("CPU_ComplexSelection_Algorithm",
...
//            const hype::Tuple Logical_ComplexSelection::getFeatureVector() const{
//                hype::Tuple t;
...
//                    t.push_back(const_cast<Logical_ComplexSelection*>(this)->isInputDataCachedInGPU());
//#endif                   
//                }else{
...
//            }
            ...
            bool Logical_ComplexSelection::isInputDataCachedInGPU(){
                ...
                for(unsigned int i=0;i<knf.disjunctions.size();++i){
                    for(unsigned int j=0;j<knf.disjunctions[i].size();++j){
                        std::list<std::pair<ColumnPtr, TablePtr> > columns = DataDictionary::instance().getColumnsforColumnName(knf.disjunctions[i][j].getColumn1Name());
                        if(columns.size() != 1){
                            COGADB_WARNING("Could not find Column '" << knf.disjunctions[i][j].getColumn1Name() <<"' in data dictionary! I weill assume the columns is not cached!","");
                            is_everything_cached=false;
                            continue;
                        }
                        ...
                        is_everything_cached = is_everything_cached && DataCacheManager::instance().getDataCache(mem_id).isCached(col);      
                    }
                }
            ...
            const std::list<std::string> Logical_ComplexSelection::getNamesOfReferencedColumns() const{
        ...
        double estimateSelectivity(const Predicate& p){
            ...
            std::list<std::pair<ColumnPtr,TablePtr> > columns = DataDictionary::instance().getColumnsforColumnName(p.getColumn1Name());
            assert(columns.size()<=1);
            if(columns.empty()) return 0.1;
            
            if(columns.front().first){
                if(columns.front().first->getColumnStatistics().sel_est_){
                    est_sel=columns.front().first->getColumnStatistics().sel_est_->getEstimatedSelectivity(p);
                }
            }
        ...
        double estimateSelectivity(const Disjunction& disjunction){
        ...
        namespace logical_operator {
            ...
            void Logical_ComplexSelection::produce_impl(CodeGeneratorPtr code_gen, QueryContextPtr context){
                left_->produce(code_gen, context);
            }
            
            void Logical_ComplexSelection::consume_impl(CodeGeneratorPtr code_gen, QueryContextPtr context){
                ...
                for(unsigned int i=0;i<knf_expr_.disjunctions.size();++i){
                    ...
                    for(unsigned int j=0;j<knf_expr_.disjunctions[i].size();++j){
                        if(knf_expr_.disjunctions[i][j].getPredicateType()==ValueConstantPredicate){
                            ...
                            AttributeReferencePtr attr = createInputAttribute(
                                ...
                                PredicateExpressionPtr pred_expr 
                                    = createColumnConstantComparisonPredicateExpression(
                                        attr, 
                                        knf_expr_.disjunctions[i][j].getConstant(),
                                        knf_expr_.disjunctions[i][j].getValueComparator());
                                disjunctions.push_back(pred_expr); 
                        }else if(knf_expr_.disjunctions[i][j].getPredicateType()==ValueRegularExpressionPredicate){
                            COGADB_FATAL_ERROR("Currently Unsupported","");
                        }else if(knf_expr_.disjunctions[i][j].getPredicateType()==ValueValuePredicate){
                            COGADB_FATAL_ERROR("Currently Unsupported","");
                        }else{
                ...
                code_gen->consumeSelection(createPredicateExpression(conjunctions, LOGICAL_AND));
