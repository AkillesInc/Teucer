                ...
                if (this->left_) { //if left child is valid (has to be by convention!), add input data size
                    ...
                    if (this->right_) { //if right child is valid (not null), add input data size for it as well
                        if(this->right_->getPhysicalOperator()){
                            t.push_back(this->right_->getPhysicalOperator()->getResultSize()); // ->result_size_;
                        }else{
...
//                t.push_back(this->isInputDataCachedInGPU_internal().second);
            ...
            typedef hype::queryprocessing::UnaryOperator<TablePtr,TablePtr> PhysicalTableUnaryOperator;
            typedef boost::shared_ptr<PhysicalTableUnaryOperator> PhysicalTableUnaryOperatorPtr;
            
            typedef hype::queryprocessing::BinaryOperator<TablePtr,TablePtr,TablePtr> PhysicalTableBinaryOperator;
            typedef boost::shared_ptr<PhysicalTableBinaryOperator> PhysicalTableBinaryOperatorPtr;
            ...
            std::pair<bool, double> Logical_Join::isInputDataCachedInGPU_internal() const {
                ...
                bool col1_cached = false, col2_cached = false;
                {
                    if (this->left_ && this->right_
                        ...
                        PhysicalTableOperatorPtr left_phy_op = boost::dynamic_pointer_cast<PhysicalTableOperator>(this->getLeft()->getPhysicalOperator());
                        PhysicalTableOperatorPtr right_phy_op = boost::dynamic_pointer_cast<PhysicalTableOperator>(this->getRight()->getPhysicalOperator());
                        ...
                        if (result_table_left && result_table_right) {
                            ColumnPtr col1 = result_table_left->getColumnbyName(this->join_column1_name_);
                            ColumnPtr col2 = result_table_right->getColumnbyName(this->join_column2_name_);
                            if (col1 && col2) {
                                DataCache& gpu_cache = DataCacheManager::instance().getDataCache(mem_id);
                                ...
                                (*this->out) << "Column '" << col1->getName()
                                        << "' from table '" << result_table_left->getName()
                                        << "' is cached: " << col1_cached
                                        << std::endl;
                                (*this->out) << "Column '" << col2->getName()
                                        << "' from table '" << result_table_right->getName()
                                        << "' is cached: " << col2_cached
                                        << std::endl;
                    ...
                    std::list<std::pair<ColumnPtr, TablePtr> > columns = DataDictionary::instance().getColumnsforColumnName(this->join_column1_name_);
                    assert(columns.size() <= 1);
                    if (columns.empty()) {
                        COGADB_WARNING("Join Column " << this->join_column1_name_ << " not found! I will assume it is not cached!", "");
                        return std::make_pair(false, double(0));
                    }
                    ColumnPtr col = columns.front().first;
                    assert(col != NULL);
                    col1_cached = DataCacheManager::instance().getDataCache(mem_id).isCached(col);
                }
                {
                    std::list<std::pair<ColumnPtr, TablePtr> > columns = DataDictionary::instance().getColumnsforColumnName(this->join_column2_name_);
                    assert(columns.size() <= 1);
                    if (columns.empty()) {
                        COGADB_WARNING("Join Column " << this->join_column1_name_ << " not found! I will assume it is not cached!", "");
                        return std::make_pair(false, double(0));
                    ...
                    ColumnPtr col = columns.front().first;
                    assert(col != NULL);
                    col2_cached = DataCacheManager::instance().getDataCache(mem_id).isCached(col);
                }
                ...
                if (col1_cached && col2_cached) {
                    if (this->getLeft()->getOperationName() != "SCAN"
                        ...
                        if (this->getLeft()->getPhysicalOperator() && this->getLeft()->getPhysicalOperator()->getDeviceSpecification().getDeviceType() == hype::GPU
                                && this->getRight()->getPhysicalOperator() && this->getRight()->getPhysicalOperator()->getDeviceSpecification().getDeviceType() == hype::GPU) {
                        ...
                        if (left_->getDeviceConstraint().getDeviceTypeConstraint() == hype::GPU_ONLY) {
                            ...
                            && this->getRight()->getOperationName() != "SCAN") {
                        if (this->getRight()->getPhysicalOperator() && this->getRight()->getPhysicalOperator()->getDeviceSpecification().getDeviceType() == hype::GPU) {
                            return std::make_pair(true, number_of_cached_access_strutures + 1);
                        }
                        if (right_->getDeviceConstraint().getDeviceTypeConstraint() == hype::GPU_ONLY) {
                            return std::make_pair(true, number_of_cached_access_strutures + 1);
                        }
                    }
            ...
            void Logical_Join::produce_impl(CodeGeneratorPtr code_gen, QueryContextPtr context){
                ...
                CodeGeneratorPtr code_gen_build_phase = createCodeGenerator(CPP_CODE_GENERATOR, 
                        param);
                QueryContextPtr context_build_phase = createQueryContext(BUILD_HASH_TABLE_PIPELINE);
                ...
                TablePtr result = build_pipeline->getResult();
                if(!result){
                    COGADB_FATAL_ERROR("Execution of compiled code for hash table build failed!","");
                }
...
//                result->print();
                ...
                /* add attribute references of result table to probe pipelines 
            ...
            void Logical_Join::consume_impl(CodeGeneratorPtr code_gen, QueryContextPtr context){
...
//                            = context->getAccessedColumns(this->join_column1_name_);
                    if(!left_join_attr){
                        COGADB_FATAL_ERROR("Could not retrieve Attribute Reference for build side of hash join!","");
                    }
                    ...
                    if(!left_join_attr){
                        COGADB_FATAL_ERROR("Could not retrieve Attribute Reference to column " << this->join_column1_name_ << " for build side of hash join!","");
                    }
                    ...
                    if(!right_join_attr){
                        COGADB_FATAL_ERROR("Could not retrieve Attribute " 
                                << "Reference to column " <<  this->join_column2_name_ 
        ...
        namespace physical_operator {
            ...
            TypedOperatorPtr create_Join_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr right_child, const JoinAlgorithm& join_alg) {
                logical_operator::Logical_Join& log_join_ref = static_cast<logical_operator::Logical_Join&> (logical_node);
                if (!quiet && verbose && debug) std::cout << "create CPU_NestedLoopJoin_Operator!" << std::endl;
                if (!left_child) {
                    std::cout << "Error! File: " << __FILE__ << " Line: " << __LINE__ << std::endl;
                    exit(-1);
                }

                hype::ProcessingDeviceID id=sched_dec.getDeviceSpecification().getProcessingDeviceID();
                ...
                return TypedOperatorPtr(new Join_Operator(sched_dec,
                        left_child,
                        right_child,
                        log_join_ref.getLeftJoinColumnName(),
                        log_join_ref.getRightJoinColumnName(),
                        param));
            }
            
            TypedOperatorPtr create_NestedLoopJoin_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr right_child) {
                return create_Join_Operator(logical_node, sched_dec, left_child, right_child, NESTED_LOOP_JOIN);
            }

            TypedOperatorPtr create_SortMergeJoin_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr right_child) {
                return create_Join_Operator(logical_node, sched_dec, left_child, right_child, SORT_MERGE_JOIN);
            }

            TypedOperatorPtr create_HashJoin_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr right_child) {
                return create_Join_Operator(logical_node, sched_dec, left_child, right_child, HASH_JOIN);
            }
            
//            TypedOperatorPtr create_CPU_Parallel_HashJoin_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr right_child) {
//                return create_Join_Operator(logical_node, sched_dec, left_child, right_child, HASH_JOIN);
//            }

            TypedOperatorPtr create_RadixJoin_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr right_child) {
                return create_Join_Operator(logical_node, sched_dec, left_child, right_child, RADIX_JOIN);
            }
            
            TypedOperatorPtr create_IndexNestedLoop_Join_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr right_child) {
                return create_Join_Operator(logical_node, sched_dec, left_child, right_child, INDEX_NESTED_LOOP_JOIN);
            }
            ...
            Physical_Operator_Map_Ptr map_init_function_join_operator() {
                ...
                hype::AlgorithmSpecification join_alg_spec_gpu_sort_merge_join("GPU_SortMergeJoin_Algorithm",
                        "JOIN",
                        hype::KNN_Regression,
                        hype::Periodic);
                
                hype::AlgorithmSpecification join_alg_spec_gpu_inlj_join("GPU_IndexNestedLoopJoin_Algorithm",
                        "JOIN",
                        hype::KNN_Regression,
                        hype::Periodic);
                
               hype::AlgorithmSpecification join_alg_spec_cpu_left_semi_join("CPU_LEFT_SEMI_JOIN",
                        "LEFT_SEMI_JOIN",
                        hype::KNN_Regression,
                        hype::Periodic);
                
                hype::AlgorithmSpecification join_alg_spec_cpu_right_semi_join("CPU_RIGHT_SEMI_JOIN",
                        "RIGHT_SEMI_JOIN",
                        hype::KNN_Regression,
                        hype::Periodic);
                
               hype::AlgorithmSpecification join_alg_spec_gpu_left_semi_join("GPU_LEFT_SEMI_JOIN",
                        "LEFT_SEMI_JOIN",
                        hype::KNN_Regression,
                        hype::Periodic);
                
                hype::AlgorithmSpecification join_alg_spec_gpu_right_semi_join("GPU_RIGHT_SEMI_JOIN",
                        "RIGHT_SEMI_JOIN",
                        hype::KNN_Regression,
                        hype::Periodic);
                
                hype::AlgorithmSpecification join_alg_spec_cpu_left_anti_semi_join("CPU_LEFT_ANTI_SEMI_JOIN",
                        "LEFT_ANTI_SEMI_JOIN",
                        hype::KNN_Regression,
                        hype::Periodic);
                
                hype::AlgorithmSpecification join_alg_spec_cpu_right_anti_semi_join("CPU_RIGHT_ANTI_SEMI_JOIN",
                        "RIGHT_ANTI_SEMI_JOIN",
                        hype::KNN_Regression,
                        hype::Periodic);
                
                hype::AlgorithmSpecification join_alg_spec_cpu_left_outer_join("CPU_LEFT_OUTER_JOIN",
                        "LEFT_OUTER_JOIN",
                        hype::KNN_Regression,
                        hype::Periodic);
                
                hype::AlgorithmSpecification join_alg_spec_cpu_right_outer_join("CPU_RIGHT_OUTER_JOIN",
                        "RIGHT_OUTER_JOIN",
                        hype::KNN_Regression,
                        hype::Periodic);
                
                hype::AlgorithmSpecification join_alg_spec_cpu_full_outer_join("CPU_FULL_OUTER_JOIN",
                        "FULL_OUTER_JOIN",
                        hype::KNN_Regression,
                        hype::Periodic);
                
                hype::AlgorithmSpecification join_alg_spec_cpu_fetch_join("CPU_GATHER_JOIN",
                        "GATHER_JOIN",
                        hype::KNN_Regression,
                        hype::Periodic);                
               ...
               hype::AlgorithmSpecification join_alg_spec_cpu_parallel_hashjoin("CPU_Parallel_HashJoin_Algorithm",
                        "JOIN",
                        hype::Multilinear_Fitting_2D,
                        hype::Periodic);
               
               hype::AlgorithmSpecification join_alg_spec_cpu_radixjoin("CPU_RadixJoin_Algorithm",
                        "JOIN",
                        hype::Multilinear_Fitting_2D,
                        hype::Periodic);
                ...
                hype::AlgorithmSpecification join_alg_spec_cpu_fetch_join("CPU_FETCH_JOIN",
                        "FETCH_JOIN",
                        hype::Multilinear_Fitting_2D,
                        hype::Periodic);                 
#endif
                ...
                for (unsigned int i = 0; i < dev_specs.size(); ++i) {
                    if (dev_specs[i].getDeviceType() == hype::CPU) {
                        ...
                        hype::Scheduler::instance().addAlgorithm(join_alg_spec_cpu_left_semi_join, dev_specs[i]);
                        hype::Scheduler::instance().addAlgorithm(join_alg_spec_cpu_right_semi_join, dev_specs[i]);              
                        hype::Scheduler::instance().addAlgorithm(join_alg_spec_cpu_left_anti_semi_join, dev_specs[i]);
                        hype::Scheduler::instance().addAlgorithm(join_alg_spec_cpu_right_anti_semi_join, dev_specs[i]);
                        hype::Scheduler::instance().addAlgorithm(join_alg_spec_cpu_left_outer_join, dev_specs[i]);
                        hype::Scheduler::instance().addAlgorithm(join_alg_spec_cpu_right_outer_join, dev_specs[i]);
                        hype::Scheduler::instance().addAlgorithm(join_alg_spec_cpu_full_outer_join, dev_specs[i]);
                        hype::Scheduler::instance().addAlgorithm(join_alg_spec_cpu_fetch_join, dev_specs[i]);
                        
                        
                        //Radix Join has still some stability issues
                        //hype::Scheduler::instance().addAlgorithm(join_alg_spec_cpu_radixjoin, dev_specs[i]);
                        //hype::Scheduler::instance().addAlgorithm(join_alg_spec_cpu_parallel_hashjoin, dev_specs[i]);
...
//		       hype::Scheduler::instance().addAlgorithm(join_alg_spec_gpu_sort_merge_join,dev_specs[i]);
                       hype::Scheduler::instance().addAlgorithm(join_alg_spec_gpu_inlj_join,dev_specs[i]);
                       hype::Scheduler::instance().addAlgorithm(join_alg_spec_gpu_left_semi_join, dev_specs[i]);
                       hype::Scheduler::instance().addAlgorithm(join_alg_spec_gpu_right_semi_join, dev_specs[i]);
...
//                map["CPU_RadixJoin_Algorithm"] = create_RadixJoin_Operator;
                map["GPU_IndexNestedLoopJoin_Algorithm"] = create_IndexNestedLoop_Join_Operator;
...
//                map["CPU_Parallel_HashJoin_Algorithm"] = create_CPU_Parallel_HashJoin_Operator;
