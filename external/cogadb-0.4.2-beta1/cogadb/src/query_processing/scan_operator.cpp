...
namespace CoGaDB
{
	...
	{
		//Map_Init_Function init_function_scan_operator=physical_operator::map_init_function_scan_operator;

		namespace physical_operator
		{

			TypedOperatorPtr create_scan_operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr, TypedOperatorPtr)
			{
				logical_operator::Logical_Scan& log_sort_ref = static_cast<logical_operator::Logical_Scan&>(logical_node);
				...
				*/
				return TypedOperatorPtr( new scan_operator(sched_dec, log_sort_ref.getTablePtr()) );
            }
            ...
            Physical_Operator_Map_Ptr map_init_function_scan_operator() {
                ...
                if (!quiet) std::cout << "calling map init function! (SCAN OPERATION)" << std::endl;
			...
			scan_operator::scan_operator(const hype::SchedulingDecision& sched_dec, TablePtr table)
                        : UnaryOperator<TablePtr,TablePtr>(sched_dec, TypedOperatorPtr()),table_(table)
		...
		{
			Logical_Scan::Logical_Scan(std::string table_name) : TypedNode_Impl<TablePtr,physical_operator::map_init_function_scan_operator>(), table_(getTablebyName(table_name))
			{
                            if(table_==NULL){
                                COGADB_FATAL_ERROR(std::string("Table ") + table_name + " not found in global Table List!" ,"");
                            }
			}
                        Logical_Scan::Logical_Scan(TablePtr table) : TypedNode_Impl<TablePtr,physical_operator::map_init_function_scan_operator>(), table_(table)
			{
                            assert(table_!=NULL);
			}
            ...
            double Logical_Scan::getCalculatedSelectivity() const {
				return 1.0;
			}
			std::string Logical_Scan::getOperationName() const
			{
				return "SCAN";
			}
                        
                        std::string Logical_Scan::toString(bool verbose) const{
				return std::string("SCAN ")+table_->getName();
			}
                        
			const std::string& Logical_Scan::getTableName()
			{
				return table_->getName();
			}
                        
                        const TablePtr Logical_Scan::getTablePtr()
			{
				return table_;
			}

//                        //redefining virtual function -> workaround so that scan operator can be processed uniformely with other operators
//                        TypedOperatorPtr Logical_Scan::getOptimalOperator(TypedOperatorPtr left_child, TypedOperatorPtr right_child, hype::DeviceTypeConstraint dev_constr){
//                                hype::Tuple t;
//
...
//                        }
                        ...
                        void Logical_Scan::produce_impl(CodeGeneratorPtr code_gen, 
                            ...
                            std::vector<std::string> proj_cols = context->getProjectionList();
                            if(!proj_cols.empty()){
                                for(size_t i=0;i<proj_cols.size();++i){
                                    if(this->table_->hasColumn(proj_cols[i])){
                                        AttributeReference attr(this->table_, proj_cols[i]);
                                        code_gen->addAttributeProjection(attr);
                                        if(debug_code_generator){
                                            std::cout << "[SCAN]: Add projection attribute: "
                                 ...
                                 */
//                                std::vector<ColumnProperties> col_props = this->table_->getPropertiesOfColumns();
//                                for(size_t i=0;i<col_props.size();++i){
//                                    AttributeReference attr(this->table_, col_props[i].name);
//                                    code_gen->addAttributeProjection(attr);
//                                    
//                                }
                            }
                            ...
                            for(size_t i=0;i<accessed_columns.size();++i){
                                if(this->table_->hasColumn(accessed_columns[i])){
                                    AttributeReferencePtr attr = createInputAttribute(
                                            this->table_, accessed_columns[i]);
                                    assert(attr!=NULL);
                                    if(debug_code_generator){
                            ...
                            if(!code_gen->createForLoop(this->table_, 1)){
