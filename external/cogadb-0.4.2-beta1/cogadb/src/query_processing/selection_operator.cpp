...
//Map_Init_Function init_function_Selection_operator=physical_operator::map_init_function_Selection_operator;
	...
	TypedOperatorPtr create_CPU_Selection_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr right_child)
	{
		logical_operator::Logical_Selection& log_selection_ref = static_cast<logical_operator::Logical_Selection&>(logical_node);
		...
		return TypedOperatorPtr(new CPU_Selection_Operator(sched_dec,
		                        left_child,
		                        log_selection_ref.getPredicate(),
		                        log_selection_ref.getMaterializationStatus()) );
	}

        TypedOperatorPtr create_CPU_ParallelSelection_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr right_child){
 		logical_operator::Logical_Selection& log_selection_ref = static_cast<logical_operator::Logical_Selection&>(logical_node);
		//std::cout << "create CPU_Selection_Operator!" << std::endl;
		if(!left_child) {
                    std::cout << "Error! File: " << __FILE__ << " Line: " << __LINE__ << std::endl;
		...
		assert(right_child==NULL); //unary operator
		return TypedOperatorPtr(new CPU_ParallelSelection_Operator(sched_dec,
		                        left_child,
		                        log_selection_ref.getPredicate(),
		                        log_selection_ref.getMaterializationStatus()) );                       
	...
	TypedOperatorPtr create_GPU_Selection_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr right_child)
	{
		logical_operator::Logical_Selection& log_selection_ref = static_cast<logical_operator::Logical_Selection&>(logical_node);
		...
		assert(right_child==NULL); //unary operator
		return TypedOperatorPtr(new GPU_Selection_Operator(sched_dec,
		                        left_child,
		                        log_selection_ref.getPredicate(),
		                        log_selection_ref.getMaterializationStatus()) );
	}
	...
	Physical_Operator_Map_Ptr map_init_function_selection_operator()
	{
		...
		//hype::Scheduler::instance().addAlgorithm("SELECTION","CPU_Selection_Algorithm",hype::CPU,"Least Squares 1D","Periodic Recomputation");
		//hype::Scheduler::instance().addAlgorithm("SELECTION","GPU_Selection_Algorithm",hype::GPU,"Least Squares 1D","Periodic Recomputation");
		...
		map["CPU_Selection_Algorithm"]=create_CPU_Selection_Operator;
                map["CPU_ParallelSelection_Algorithm"]=create_CPU_ParallelSelection_Operator;
...
//                return CoGaDB::GPU_Column_Cache::instance().isCached(col);
            }
            }
