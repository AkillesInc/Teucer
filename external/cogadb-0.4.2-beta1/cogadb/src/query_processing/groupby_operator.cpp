...
namespace CoGaDB {
    ...
    namespace query_processing {
        ...
        //Map_Init_Function init_function_Groupby_operator=physical_operator::map_init_function_groupby_operator;
        ...
        namespace physical_operator {
            ...
            bool isHashableAggregationFunction(const AggregationFunction& agg_func) {
                if (agg_func == COUNT
                        ...
                        || agg_func == MAX
                        || agg_func == AVERAGE
                        || agg_func == AGG_GENOTYPE
                        || agg_func == AGG_CONCAT_BASES
                        || agg_func == AGG_IS_HOMOPOLYMER
            ...
            AggregationAlgorithm getAggregationAlgorithm(const std::list<ColumnAggregation>& aggregation_functions) {
            ...
            bool CPU_Groupby_Operator::execute() {
                ...
                //group by operation
                AggregationAlgorithm agg_alg = getAggregationAlgorithm(aggregation_functions_);
                ...
                if (this->getInputData()
                        && this->getInputData()->isMaterialized()
                        && grouping_columns_.size() == 1) {
                    ColumnPtr col = this->getInputData()->getColumnbyName(grouping_columns_.front());
                    if (col) {
                ...
                //                        agg_alg = SORT_BASED_AGGREGATION;
                for (cit = aggregation_functions_.begin(); cit != aggregation_functions_.end(); ++cit) {
                    aggregation_functions.push_back(std::make_pair(cit->first, AggregationParam(proc_spec, cit->second.first, agg_alg, cit->second.second)));
                }
            ...
            TypedOperatorPtr create_CPU_Groupby_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr right_child) {
                logical_operator::Logical_Groupby& log_Groupby_ref = static_cast<logical_operator::Logical_Groupby&> (logical_node);
                if (!quiet && verbose)
                ...
                return TypedOperatorPtr(new CPU_Groupby_Operator(sched_dec,
                        left_child,
                        log_Groupby_ref.getGroupingColumns(),
                        log_Groupby_ref.getColumnAggregationFunctions(),
                        log_Groupby_ref.getMaterializationStatus()));
            }

            TypedOperatorPtr create_GPU_Groupby_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr right_child) {
                logical_operator::Logical_Groupby& log_Groupby_ref = static_cast<logical_operator::Logical_Groupby&> (logical_node);
                if (!quiet && verbose)
                ...
                assert(right_child == NULL); //unary operator
                return TypedOperatorPtr(new GPU_Groupby_Operator(sched_dec,
                        left_child,
                        log_Groupby_ref.getGroupingColumns(),
                        log_Groupby_ref.getColumnAggregationFunctions(),
                        log_Groupby_ref.getMaterializationStatus()));
            ...
            Physical_Operator_Map_Ptr map_init_function_groupby_operator() {
                ...
                if (!quiet) std::cout << "calling map init function for GROUPBY operator!" << std::endl;
                //hype::Scheduler::instance().addAlgorithm("GROUPBY","CPU_Groupby_Algorithm",hype::CPU,"Least Squares 1D","Periodic Recomputation");
                //hype::Scheduler::instance().addAlgorithm("GROUPBY","GPU_Groupby_Algorithm",hype::GPU,"Least Squares 1D","Periodic Recomputation");


#ifdef COGADB_USE_KNN_REGRESSION_LEARNER
                hype::AlgorithmSpecification group_by_alg_spec_cpu("CPU_Groupby_Algorithm",
                        "GROUPBY",
                        hype::KNN_Regression, //hype::Multilinear_Fitting_2D, //hype::Least_Squares_1D,
                        hype::Periodic);

                hype::AlgorithmSpecification group_by_alg_spec_gpu("GPU_Groupby_Algorithm",
                        "GROUPBY",
                        hype::KNN_Regression, //hype::Multilinear_Fitting_2D, //hype::Least_Squares_1D,
                        hype::Periodic);
#else
                ...
                for (unsigned int i = 0; i < dev_specs.size(); ++i) {
                    if (dev_specs[i].getDeviceType() == hype::CPU) {
                        hype::Scheduler::instance().addAlgorithm(group_by_alg_spec_cpu, dev_specs[i]);
                ...
                //hype::Scheduler::instance().addAlgorithm("SELECTION","GPU_Groupby_Algorithm",hype::GPU,"Least Squares 1D","Periodic Recomputation");
                map["CPU_Groupby_Algorithm"] = create_CPU_Groupby_Operator;
                map["GPU_Groupby_Algorithm"] = create_CPU_Groupby_Operator; //create_GPU_Groupby_Operator;
        ...
        namespace logical_operator {
            ...
            void Logical_Groupby::produce_impl(CodeGeneratorPtr code_gen, 
                ...
                for (cit = referenced_colums.begin();
                        cit != referenced_colums.end(); ++cit) {
            ...
            void Logical_Groupby::consume_impl(CodeGeneratorPtr code_gen, 
                ...
                for (cit_groups = grouping_columns_.begin();
                    ...
                    AttributeReferencePtr attr = getAttributeReference(*cit_groups, code_gen, context);
                    if(!attr){
                        code_gen->print();
...
//                    AttributeReferencePtr scan_attr = code_gen->getScannedAttributeByName(cit_aggr->first);
                    AttributeReferencePtr scan_attr = getAttributeReference(cit_aggr->first, code_gen, context);
//                    assert(scan_attr != NULL);
                    if(!scan_attr){
                        COGADB_FATAL_ERROR("Could not retrieve scan attribute reference for column " << cit_aggr->first,"");
                    }
//                    AttributeType type = DOUBLE;
//                    if(cit_aggr->second.first==COUNT)
//                        type=INT;
                    /* create computed attribute */
//                    AttributeReferencePtr result_attr(new AttributeReference(scan_attr->getUnversionedAttributeName(),
//                            type,
//                            cit_aggr->second.second));
                    AggregateSpecification aggr_spec = createAggregateSpecification(*scan_attr, cit_aggr->second.first, cit_aggr->second.second);
