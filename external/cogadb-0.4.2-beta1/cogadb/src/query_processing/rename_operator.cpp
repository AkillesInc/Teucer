...
namespace CoGaDB {
    ...
    namespace query_processing {
        //Map_Init_Function init_function_rename_operator=physical_operator::map_init_function_rename_operator;

        namespace physical_operator {

            TypedOperatorPtr create_rename_operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr) {
                logical_operator::Logical_Rename& log_sort_ref = static_cast<logical_operator::Logical_Rename&> (logical_node);
                 ...
                 */
                return TypedOperatorPtr(new rename_operator(sched_dec, left_child, log_sort_ref.getRenameList()));
            }
            ...
            Physical_Operator_Map_Ptr map_init_function_rename_operator() {
                ...
                if (!quiet) std::cout << "calling map init function! (RENAME OPERATION)" << std::endl;
                //hype::Scheduler::instance().addAlgorithm("RENAME","RENAME",hype::CPU,"Least Squares 1D","Periodic Recomputation");
                
#ifdef COGADB_USE_KNN_REGRESSION_LEARNER
                hype::AlgorithmSpecification selection_alg_spec_cpu("CPU_RENAME",
                        "RENAME",
                        hype::KNN_Regression,
            ...
            rename_operator::rename_operator(const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, RenameList rename_list)
            : UnaryOperator<TablePtr, TablePtr>(sched_dec, left_child), rename_list_(rename_list)
 {
        ...
        namespace logical_operator {
            ...
            Logical_Rename::Logical_Rename(const RenameList& rename_list) : TypedNode_Impl<TablePtr, physical_operator::map_init_function_rename_operator>(), rename_list_(rename_list) {
            ...
            double Logical_Rename::getCalculatedSelectivity() const {
                return 1.0;
            }

            std::string Logical_Rename::getOperationName() const {
                return "RENAME";
            }
            ...
            const RenameList& Logical_Rename::getRenameList() {
                return this->rename_list_;
            }


            //	//redefining virtual function -> workaround so that scan operator can be processed uniformely with other operators
            //	TypedOperatorPtr Logical_Rename::getOptimalOperator(TypedOperatorPtr left_child, TypedOperatorPtr right_child, hype::DeviceTypeConstraint dev_constr){
            //		hype::Tuple t;
            //
