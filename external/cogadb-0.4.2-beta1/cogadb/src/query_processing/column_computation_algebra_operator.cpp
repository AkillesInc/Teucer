...
namespace CoGaDB {
    ...
    namespace query_processing {
        ...
        //Map_Init_Function init_function_sort_operator=physical_operator::map_init_function_column_algebra_operator;

        namespace physical_operator {

            TypedOperatorPtr create_CPU_ColumnAlgebra_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, TypedOperatorPtr left_child, TypedOperatorPtr right_child) {
                logical_operator::Logical_ColumnAlgebraOperator& log_sort_ref = static_cast<logical_operator::Logical_ColumnAlgebraOperator&> (logical_node);
                ...
                return TypedOperatorPtr(new CPU_ColumnAlgebraOperator(sched_dec,
                        left_child,
                        log_sort_ref.getColumn1Name(),
                        log_sort_ref.getColumn2Name(),
                        log_sort_ref.getResultColumnName(),
            ...
            Physical_Operator_Map_Ptr map_init_function_column_algebra_operator() {
                ...
                if (!quiet) std::cout << "calling map init function for ColumnAlgebra Operator!" << std::endl;
                //hype::Scheduler::instance().addAlgorithm("SORT","CPU_Sort_Algorithm",hype::CPU,"Least Squares 1D","Periodic Recomputation");
                //hype::Scheduler::instance().addAlgorithm("SORT","GPU_Sort_Algorithm",hype::GPU,"Least Squares 1D","Periodic Recomputation");

#ifdef COGADB_USE_KNN_REGRESSION_LEARNER
                hype::AlgorithmSpecification sort_alg_spec_cpu("CPU_ColumnAlgebra_Algorithm",
                        "ColumnAlgebraOperator",
                        hype::KNN_Regression,
                        hype::Periodic);

                hype::AlgorithmSpecification sort_alg_spec_gpu("GPU_ColumnAlgebra_Algorithm",
                        "ColumnAlgebraOperator",
                        hype::KNN_Regression,
                        hype::Periodic);
#else
        ...
        namespace logical_operator {
            ...
            Logical_ColumnAlgebraOperator::Logical_ColumnAlgebraOperator(const std::string& column1_name,
            ...
            : TypedNode_Impl<TablePtr, physical_operator::map_init_function_column_algebra_operator>(false, dev_constr),
            ...
            CoGaDB::ColumnAlgebraOperation Logical_ColumnAlgebraOperator::getColumnAlgebraOperation() {
                return operation_;
            }

            const std::list<std::string> Logical_ColumnAlgebraOperator::getNamesOfReferencedColumns() const {
            ...
            void Logical_ColumnAlgebraOperator::produce_impl(CodeGeneratorPtr code_gen, QueryContextPtr context) {
            ...
            void Logical_ColumnAlgebraOperator::consume_impl(CodeGeneratorPtr code_gen, QueryContextPtr context) {
                
                bool debug_code_generator = VariableManager::instance()
                    .getVariableValueBoolean("debug_code_generator");
                
                AttributeReferencePtr attr_1 = getAttributeReference(this->column1_name_, code_gen, context);
                AttributeReferencePtr attr_2 = getAttributeReference(this->column2_name_, code_gen, context);
                ...
                std::pair<bool, AttributeReference> ret = code_gen->consumeAlgebraComputation(*attr_1,
                        *attr_2,
                        operation_);
                ...
                if (!ret.first) {
                    ...
                    std::set<std::string> unresolved_projections = context->getUnresolvedProjectionAttributes();
                    std::set<std::string>::const_iterator cit=unresolved_projections.find(computed_column_name);
                    if(cit!=unresolved_projections.end()){
                        code_gen->addAttributeProjection(ret.second);
                        if(debug_code_generator){
                            std::cout << "[MAP]: resolved attribute '" 
                                      << computed_column_name << "' (" 
                                      << CoGaDB::toString(*comp_attr) << ")" 
                                      << std::endl; 
                        }
                    }
                    if(debug_code_generator){
                        ...
                        for(cit=unresolved_projections.begin();cit!=unresolved_projections.end();++cit){
