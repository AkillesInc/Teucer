...
namespace CoGaDB {
    ...
    namespace query_processing {
        ...
        namespace physical_operator {
            ...
            column_processing::cpu::TypedOperatorPtr create_CPU_PositionList_Operator(column_processing::cpu::TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, column_processing::cpu::TypedOperatorPtr left_child, column_processing::cpu::TypedOperatorPtr right_child) {
                logical_operator::Logical_PositionList_Operator& log_algebra_ref = static_cast<logical_operator::Logical_PositionList_Operator&> (logical_node);
                if (!quiet && debug && verbose) std::cout << "create CPU_Columnalgebra_Operator!" << std::endl;
                if (!left_child) {
                    std::cout << "[HyPE Library]: Fatal Error! in query_processing::physical_operator::create_CPU_PositionList_Union_Operator()" << std::endl;
                    std::cout << "Left Child of Node " << logical_node.getOperationName() << " is Invalid!" << std::endl;
                    exit(-1);
                }

                assert(right_child != NULL); //binary operator
                return column_processing::cpu::TypedOperatorPtr(new CPU_PositionList_Operator(sched_dec,
                        left_child,
                        right_child,
            ...
            column_processing::cpu::Physical_Operator_Map_Ptr map_init_function_cpu_positionlist_operator() {
                ...
                //std::cout << "calling map init function for CPU_PositionList_Union_Operator operator!" << std::endl;
                ...
                hype::AlgorithmSpecification col_algebra_alg_spec_cpu("CPU_PositionList_Operator",
                        "PositionList_Operator",
                        hype::KNN_Regression,
                        hype::Periodic);

                hype::AlgorithmSpecification col_algebra_alg_spec_gpu("GPU_PositionList_Operator",
                        "PositionList_Operator",
                        hype::KNN_Regression,
                        hype::Periodic);        
#else
                ...
                bool CPU_PositionList_Operator::execute() {
                    ...
                    input_tids_left = copy_if_required(pos_list_op_left->getResultPositionList(), mem_id);
                    if(!input_tids_left) return false;
                    
                    PositionListPtr input_tids_right;
                    input_tids_right = copy_if_required(pos_list_op_right->getResultPositionList(), mem_id);
                    if(!input_tids_right) return false;
                    

                     ProcessorBackend<TID>* backend = ProcessorBackend<TID>::get(id);
                     tids = backend->computePositionListSetOperation(input_tids_left, input_tids_right, param);
