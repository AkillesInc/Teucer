...
namespace CoGaDB {
    ...
    namespace query_processing {
        //Map_Init_Function init_function_column_constant_filter_operator=physical_operator::map_init_function_column_constant_filter_operator;

        namespace physical_operator {

            column_processing::cpu::TypedOperatorPtr create_CPU_column_constant_filter_operator(column_processing::cpu::TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, column_processing::cpu::TypedOperatorPtr left_child, column_processing::cpu::TypedOperatorPtr right_child) {
                logical_operator::Logical_Column_Constant_Filter& log_sort_ref = static_cast<logical_operator::Logical_Column_Constant_Filter&> (logical_node);
                ...
                return column_processing::cpu::TypedOperatorPtr(new CPU_column_constant_filter_operator(sched_dec, left_child, log_sort_ref.getPredicate()));
            }
            ...
            column_processing::cpu::Physical_Operator_Map_Ptr map_init_function_column_constant_filter_operator() {
                ...
                if (!quiet) std::cout << "calling map init function! (COLUMN_CONSTANT_FILTER OPERATION)" << std::endl;
                ...
                hype::AlgorithmSpecification selection_alg_spec_cpu("CPU_COLUMN_CONSTANT_FILTER",
                        "COLUMN_CONSTANT_FILTER",
                        hype::KNN_Regression,
                        hype::Periodic);

                hype::AlgorithmSpecification selection_alg_spec_gpu("GPU_COLUMN_CONSTANT_FILTER",
                        "COLUMN_CONSTANT_FILTER",
                        hype::KNN_Regression,
                        hype::Periodic);
#else
                ...
                for (unsigned int i = 0; i < dev_specs.size(); ++i) {
                    if (dev_specs[i].getDeviceType() == hype::CPU) {
                        ...
                        hype::Scheduler::instance().registerMemoryCostModel(selection_alg_spec_gpu, dev_specs[i], &CoGaDB::gpu::GPU_Operators_Memory_Cost_Models::columnSelection);
#endif
                    }
                }
                map["CPU_COLUMN_CONSTANT_FILTER"] = create_CPU_column_constant_filter_operator;
                map["GPU_COLUMN_CONSTANT_FILTER"] = create_CPU_column_constant_filter_operator; //create_GPU_column_constant_filter_operator;
                return column_processing::cpu::Physical_Operator_Map_Ptr(new column_processing::cpu::Physical_Operator_Map(map));
            }

            CPU_column_constant_filter_operator::CPU_column_constant_filter_operator(const hype::SchedulingDecision& sched_dec, TypedOperatorPtr child,
                    const Predicate& pred) : UnaryOperator<ColumnPtr, ColumnPtr>(sched_dec, child), PositionListOperator(), pred_(pred) {
        ...
        namespace logical_operator {
            ...
            Logical_Column_Constant_Filter::Logical_Column_Constant_Filter(const Predicate& pred, hype::DeviceConstraint dev_constr) : TypedNode_Impl<ColumnPtr, physical_operator::map_init_function_column_constant_filter_operator>(false,dev_constr), pred_(pred){
                if(pred_.getPredicateType()==ValueConstantPredicate){
                    ...
                    if(columns.size()!=1){
                   	if(columns.size()>1){
                    		COGADB_FATAL_ERROR("Column name not unique! Found Column " << pred_.getColumn1Name() << " more than once (" << columns.size() << ") in database!","");
                    	}else{
//                    		COGADB_FATAL_ERROR("Column " << pred_.getColumn1Name() << " not found in database!","");
...
//                    }
            ...
            bool Logical_Column_Constant_Filter::isInputDataCachedInGPU(){
                std::list<std::pair<ColumnPtr,TablePtr> > columns = DataDictionary::instance().getColumnsforColumnName( this->pred_.getColumn1Name());
                assert(columns.size()<=1);
