...
namespace CoGaDB {
    ...
    namespace query_processing {
        ...
        namespace physical_operator {
            ...
            column_processing::cpu::TypedOperatorPtr create_CPU_ColumnComparatorOperator(column_processing::cpu::TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, column_processing::cpu::TypedOperatorPtr left_child, column_processing::cpu::TypedOperatorPtr right_child) {
                logical_operator::Logical_ColumnComparatorOperation& log_algebra_ref = static_cast<logical_operator::Logical_ColumnComparatorOperation&> (logical_node);
                if (!quiet && debug && verbose) std::cout << "create CPU_Columnalgebra_Operator!" << std::endl;
                if (!left_child) {
                    std::cout << "[HyPE Library]: Fatal Error! in query_processing::physical_operator::create_CPU_ColumnAlgebraOperator()" << std::endl;
                    std::cout << "Left Child of Node " << logical_node.getOperationName() << " (" << log_algebra_ref.getPredicate().toString() << ") is Invalid!" << std::endl;
                    exit(-1);
                }
                ...
                return column_processing::cpu::TypedOperatorPtr(new ColumnComparatorOperation(sched_dec,
                        left_child,
                        right_child,
            ...
            column_processing::cpu::Physical_Operator_Map_Ptr map_init_function_cpu_column_comparison_operator() {
                ...
                //std::cout << "calling map init function for CPU_ColumnAlgebraOperator operator!" << std::endl;
                ...
                hype::AlgorithmSpecification col_algebra_alg_spec_cpu("CPU_ColumnComparator_Algorithm",
                        "ColumnComparatorOperation",
                        hype::KNN_Regression,
                        hype::Periodic); 
#else
                hype::AlgorithmSpecification col_algebra_alg_spec_cpu("CPU_ColumnComparator_Algorithm",
