...
namespace CoGaDB {
    ...
    namespace query_processing {
        //Map_Init_Function init_function_column_scan_operator=physical_operator::map_init_function_column_scan_operator;

        namespace physical_operator {

            column_processing::cpu::TypedOperatorPtr create_column_scan_operator(column_processing::cpu::TypedLogicalNode& logical_node, const hype::SchedulingDecision& sched_dec, column_processing::cpu::TypedOperatorPtr, column_processing::cpu::TypedOperatorPtr) {
                logical_operator::Logical_Column_Scan& log_sort_ref = static_cast<logical_operator::Logical_Column_Scan&> (logical_node);
                 ...
                 */
                //				return column_processing::cpu::TypedOperatorPtr( new column_scan_operator(sched_dec, log_sort_ref.getTableName(),log_sort_ref.getColumnName()) );
                return column_processing::cpu::TypedOperatorPtr(new column_scan_operator(sched_dec, log_sort_ref.getTablePtr(), log_sort_ref.getColumnName()));
            }
            ...
            column_processing::cpu::Physical_Operator_Map_Ptr map_init_function_column_scan_operator() {
                ...
                if (!quiet) std::cout << "calling map init function! (CPU_COLUMN_SCAN OPERATION)" << std::endl;
                ...
                hype::AlgorithmSpecification selection_alg_spec_cpu("CPU_COLUMN_SCAN",
                        "CPU_COLUMN_SCAN",
                        hype::KNN_Regression,
                        hype::Periodic);
#else
            ...
            column_scan_operator::column_scan_operator(const hype::SchedulingDecision& sched_dec,
                    const std::string& table_name, const std::string& column_name) : UnaryOperator<ColumnPtr, ColumnPtr>(sched_dec, column_processing::cpu::TypedOperatorPtr()),
            table_name_(table_name), column_name_(column_name), table_ptr_(getTablebyName(table_name)) {
                if (table_ptr_ == NULL) {
            ...
            column_scan_operator::column_scan_operator(const hype::SchedulingDecision& sched_dec, TablePtr table_ptr, const std::string& column_name) : UnaryOperator<ColumnPtr, ColumnPtr>(sched_dec, column_processing::cpu::TypedOperatorPtr()),
            table_name_(), column_name_(column_name), table_ptr_(table_ptr) {
                if (table_ptr_ == NULL) {
                    COGADB_FATAL_ERROR("TablePtr is zero, cannot scan column of unknown Table!", "");
                }
            ...
            bool column_scan_operator::execute() {
                ...
                //std::cout << "SCAN: Table " <<  table_ptr_->getName() << "(" << (void*) table_ptr_.get() << ") on Column " << column_name_ << std::endl;
                this->result_ = table_ptr_->getColumnbyName(column_name_);
                //std::cout << "Result: " << (void*) this->result_.get() << std::endl;
                if (this->result_ == NULL) {
                    COGADB_FATAL_ERROR(std::string("Could not find Column ") + column_name_ + std::string(" in table ") + table_ptr_->getName(), "");
                }
                if (this->result_) {
        ...
        namespace logical_operator {
            ...
            Logical_Column_Scan::Logical_Column_Scan(const std::string& table_name, const std::string& column_name) : TypedNode_Impl<ColumnPtr, physical_operator::map_init_function_column_scan_operator>(), table_name_(table_name), column_name_(column_name), table_ptr_(getTablebyName(table_name_)) {
                assert(table_ptr_ != NULL);
            }

            Logical_Column_Scan::Logical_Column_Scan(TablePtr table, const std::string& column_name) : table_name_(), column_name_(column_name), table_ptr_(table) {
                assert(table_ptr_ != NULL);
                table_name_ = table_ptr_->getName();
            }
            ...
            double Logical_Column_Scan::getCalculatedSelectivity() const {
                return 1.0;
            }
            ...
            const TablePtr Logical_Column_Scan::getTablePtr() {
                return table_ptr_;
            }
            ...
            //	column_processing::cpu::TypedOperatorPtr Logical_Column_Scan::getOptimalOperator(column_processing::cpu::TypedOperatorPtr left_child, column_processing::cpu::TypedOperatorPtr right_child, hype::DeviceTypeConstraint dev_constr){
            //		hype::Tuple t;
            //
            ...
            hype::query_optimization::QEP_Node* Logical_Column_Scan::toQEP_Node() {
                ...
                hype::OperatorSpecification op_spec(this->getOperationName(),
                        t,
                ...
                hype::query_optimization::QEP_Node* node = new hype::query_optimization::QEP_Node(op_spec, this->dev_constr_);
