  ...
  void start()
  {
	...
	while(true){
          ...
          catch (std::exception& e)
          {
            ...
            std::cout << "Close connection with IP '" << socket_.remote_endpoint().address().to_string() << "' from port '" << socket_.remote_endpoint().port() << "'..." << std::endl;
...
//		}
		...
		if(!cmd.execute(input, client)) client->getOutputStream() << "Error! Command '" << input << "' failed!" << endl;
...
//        acceptor_.accept(stream.rdbuf());
      ...
      const boost::system::error_code& error)
  {
    ...
    if (!error)
    { 
      std::cout << "Accepted new connection from IP '" << new_connection->socket().remote_endpoint().address().to_string() << "' from port '" << new_connection->socket().remote_endpoint().port() << "'..." << std::endl;
  ...
  tcp::acceptor acceptor_;
    ...
    std::string  LocalClient::getNextCommand(){
        return std::string();
    }
    std::ostream& LocalClient::getOutputStream(){
        return std::cout;
    } 
    ...
    std::ostream& EmulatedClient::getOutputStream(){
        return fout_;
    } 
    ...
    RemoteClient::RemoteClient(boost::asio::ip::tcp::socket* socket) : Client(), socket_(socket), stream_(){
...
//            boost::asio::streambuf::mutable_buffers_type bufs =receivedStreamBuffer.prepare( 1024 );
...
//        
    ...
    std::ostream& RemoteClient::getOutputStream(){
        return stream_;
    } 
...
//    }
    ...
    bool acceptNetworkConnections(const std::string& port_number, ClientPtr client){
