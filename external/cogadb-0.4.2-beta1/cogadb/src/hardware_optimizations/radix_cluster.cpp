...
RadixHashTablePtr createRadixHashTable(int* array, size_t array_size){
	...
	//template<typename T>
	void nested_loop_join(RadixPair* __restrict__ column1, const size_t& col1_array_size,RadixPair* __restrict__ column2, const size_t& col2_array_size){
                assert(column1!=NULL);
                assert(column2!=NULL);
...
//                                        join_tids->second->push_back(j);
	...
	void simple_nested_loop_join(int* __restrict__ column1, const size_t& col1_array_size,int* __restrict__ column2, const size_t& col2_array_size){
                assert(column1!=NULL);
                assert(column2!=NULL);
                ...
                for(unsigned int i=0;i<join_column1_size;i++){
                        for(unsigned int j=0;j<join_column2_size;j++){
                                if(column1[i]==column2[j]){
                                          std::cout << "MATCH: (" << column1[i] << "," << i << "; " << column2[j] << "," << j << ")" << std::endl;
//                                        join_tids->first->push_back(i);
//                                        join_tids->second->push_back(j);
                                }
                        }
...
void radix_join(int* __restrict__ column1, const size_t& col1_array_size, int* __restrict__ column2, const size_t& col2_array_size){

	RadixHashTablePtr hash_table_col1 = createRadixHashTable(column1,col1_array_size);
	RadixHashTablePtr hash_table_col2 = createRadixHashTable(column2,col2_array_size);
...
// exit(0);
 ...
 //Perform Radix Join:
 radix_join(&prim_keys[0],prim_keys.size(),&foreign_keys[0],foreign_keys.size());

	cout << "==================================================" << endl;
	cout << "Perform Nested Loop Join: " << endl;
 //Perform Nested Loop Join:
 simple_nested_loop_join(&prim_keys[0],prim_keys.size(),&foreign_keys[0],foreign_keys.size());
 exit(0);
 }
...
// radix_cluster(&v[0], v.size(), total_number_of_bits, bits_per_pass, 1, buckets, 1, 0, result_0, result_1);
// }
...
// unsigned int total_number_of_bits=6;
 ...
 radix_cluster(&v[0], v.size(), total_number_of_bits, bits_per_pass, number_of_passes, buckets, total_number_of_buckets, 0, 0, result_0, result_1);
 ...
 RadixPair* result;
	...
	for(unsigned int i=0;i<total_number_of_buckets;++i){
		cout << "Cluster " << i << " size: " << buckets[i] << " " << bucket_borders[i] << endl;
		for(unsigned int j=0;j<buckets[i];++j){
			cout << result[j+bucket_borders[i]].key << " Radix: " << (bitset<32>)(result[j+bucket_borders[i]].key & bitmask) << "  " << (void*) &result[j]<< endl;
		}
	}
