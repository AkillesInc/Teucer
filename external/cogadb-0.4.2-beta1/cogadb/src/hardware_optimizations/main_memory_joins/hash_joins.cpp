...
namespace CoGaDB {
    ...
    namespace CDK {
        ...
        namespace main_memory_joins {
            ...
            const PositionListPairPtr serial_hash_join(int* build_column, size_t br_size, int* probe_column, size_t pr_size) {
                PositionListPairPtr join_tids(new PositionListPair());
                join_tids->first = createPositionList(0, hype::PD_Memory_0); //PositionListPtr(new PositionList());
                join_tids->second = createPositionList(0, hype::PD_Memory_0); //PositionListPtr(new PositionList());
...
//            void hash_join_pruning_thread(hashtable_t *hashtable, int* probe_column, TID begin_index, TID end_index, PositionListPairPtr join_tids) {
//
//                /* Join phase */
...
//            }
            ...
            //[old_hash_table] void hash_join_pruning_thread(hashtable_t * hashtable, TID* probe_column, TID begin_index, TID end_index, PositionListPairPtr join_tids) {
            void hash_join_pruning_thread(HashTablePtr hashtable, TID* probe_column, TID begin_index, TID end_index, PositionListPairPtr join_tids) {

                /* Join phase */
                for (unsigned long s = begin_index; s < end_index; ++s) {
                    //[old_hash_table] unsigned long hash = HASH(probe_column[s]) & hashtable->mask;
                    TypedHashTable::hash_bucket_t *bucket = hashtable->getBucket(probe_column[s]);
                    //[old_hash_table] hash_bucket_t *bucket = &(hashtable->buckets[hash]);

                    while (bucket) {
                        for (unsigned int i = 0; i < bucket->count; i++) {
                            if (bucket->tuples[i].key == probe_column[s]) {
                                //[old_hash_table] join_tids->first->push_back(bucket->tuples[i].value);
            ...
            void write_partial_result_tids_to_output_thread(TID* join_tids_table1, TID* join_tids_table2, TID* join_tids_result_table1, TID* join_tids_result_table2, size_t thread_id, size_t number_of_threads, TID begin_index_result, TID end_index_result) {
                size_t result_size = end_index_result - begin_index_result;
                //copy memory chunk to result array
                std::memcpy(&join_tids_result_table1[begin_index_result], join_tids_table1, result_size * sizeof (TID));
                std::memcpy(&join_tids_result_table2[begin_index_result], join_tids_table2, result_size * sizeof (TID));

            }

            const PositionListPairPtr parallel_hash_join(TID* build_relation, size_t br_size, TID* probe_relation, size_t pr_size) {
                ...
                #pragma omp parallel default(shared) private(thread_id, num_threads, partition_size, begin_index, end_index)
                {
                    ...
                    //call pruning thread
                    hash_join_pruning_thread(hashtable, probe_relation, begin_index, end_index, partial_results[thread_id]);
                }
                ...
                PositionListPairPtr join_tids(new PositionListPair());
                join_tids->first = createPositionList(total_result_size, hype::PD_Memory_0); //PositionListPtr(new PositionList());
                join_tids->second = createPositionList(total_result_size, hype::PD_Memory_0); //PositionListPtr(new PositionList());
                ...
                for (size_t i = 0; i < number_of_threads; i++) {
                    if (thread_result_sizes[i] > 0) {
                        write_partial_result_tids_to_output_thread(
                                ...
                                join_tids->first->data(), join_tids->second->data(),
                                i, number_of_threads,
                                write_indexes[i],
                                write_indexes[i + 1]);
                    }
...
//            const PositionListPairPtr parallel_hash_join(int* build_relation, size_t br_size, int* probe_relation, size_t pr_size) {
//
//                Timestamp build_hashtable_begin = getTimestamp();
//
//                hashtable_t *hashtable;
