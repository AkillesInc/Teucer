 ...
 */
unsigned long
hashjoin_impl (const relation_t R, const relation_t S)
{
    ...
    for (unsigned long r = 0; r < R.count; r++)
        hash_put (hashtable, R.tuples[r]);
            ...
            for (unsigned int i = 0; i < bucket->count; i++)
                if (bucket->tuples[i].key == S.tuples[s].key)
                {
                    ...
                    printf (" %10u | %10u | %10u | %10u \n",
                            bucket->tuples[i].key, bucket->tuples[i].value,
                            S.tuples[s].key, S.tuples[s].value);
#endif
                    num_result++;
                }
...
/* For debugging only. */
static void
print_relation (relation_t rel, char *name) __attribute__ ((unused));
                        ...
                        const part_bits_t *part_bits)
{
    ...
    {
        fprintf (stderr,
                "Error allocating memory for partition start address.\n");
        exit (EXIT_FAILURE);
    }
