...
usage (void)
{
    ...
    printf ("  -r SIZE | --size-of-r SIZE      size of (smaller) input relation"
            " R\n");
    printf ("  -s SIZE | --size-of-s SIZE      size of (larger) input relation"
            " S\n");
    printf ("  -a SIZE | --alphabet-for-s SIZE alphabet size used for values"
            " in S\n");
    printf ("                                  (default: size of R)\n");
    printf ("  -z VAL | --zipf VAL             Zipf factor for value"
            " distribution in S\n");
    printf ("  -p PARTITIONSPEC | --partition PARTITIONSPEC\n");
    printf ("                                  partition before computing hash "
            " joins\n");
    printf ("                                  (this realizes partitioned hash "
            " join)\n");
    printf ("\n");
    printf ("Example:\n");
    printf ("\n");
    ...
    printf ("\n");
    printf ("(This will join two relations with 1M tuples each and perform "
            "two-pass\n");
    printf ("partitioning using bits 37-43 in the first pass and 44-50 in the "
            "second.)\n");
    printf ("\n");
    printf ("Have fun!\n");
}
...
static part_bits_t *
parse_part_bits (const char *part_str)
{
    ...
    while (*s)
    {
        ...
        if (*s != '-')
        {
            fprintf (stderr, "Illegal partitioning range; expected `-', "
                             "got `%c'\n", *s ? *s : ' ');
            exit (EXIT_FAILURE);
        }
        s++;  /* skip that `-' */
...
parse_commandline_args (int argc, char **argv)
{
    ...
    while ((ch = getopt_long (argc, argv, "Pa:hp:r:s:z:", longopts, NULL)) != -1)
    {
        switch (ch)
        {
            case 'P': ret.print_relations = true;
                      break;

            case 'a': ret.s_alphabet = strtol (optarg, NULL, 10);
                      break;
            ...
            case 'p': ret.part_str = strdup (optarg);
                      break;

            case 'r': ret.r_size = strtol (optarg, NULL, 10);
                      break;

            case 's': ret.s_size = strtol (optarg, NULL, 10);
                      break;

            case 'z': ret.s_size = strtod (optarg, NULL);
                      break;
    ...
    if (ret.s_alphabet == 0)
        ret.s_alphabet = ret.r_size;

    if (ret.part_str)
        ret.part_bits = parse_part_bits (ret.part_str);

    return ret;
}

static void
print_relation (relation_t rel, char *name)
{
    printf ("Relation %s:\n\n", name);
    printf ("      key    |   value\n");
    printf (" ------------+------------\n");
