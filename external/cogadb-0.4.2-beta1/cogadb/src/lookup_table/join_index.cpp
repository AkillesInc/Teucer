...
namespace CoGaDB {
        ...
        std::string JoinIndexes::joinExpressionToString(TablePtr pk_tab, std::string pk_tab_col_name, TablePtr fk_tab, std::string fk_tab_col_name){
            std::stringstream ss;
            ss << "JOIN_INDEX(" <<  pk_tab->getName() << "." << pk_tab_col_name << "," << fk_tab->getName() << "." << fk_tab_col_name << ")";
            return ss.str();
        }
        
        JoinIndexPtr JoinIndexes::getJoinIndex(TablePtr pk_tab, std::string pk_tab_col_name, TablePtr fk_tab, std::string fk_tab_col_name){
...
//            assert(fk_tab->hasForeignKeyConstraint(fk_tab_col_name)==true);
            std::string index_name = joinExpressionToString(pk_tab, pk_tab_col_name, fk_tab, fk_tab_col_name);
            ...
            if(it!=indeces_.end()){
                ...
                if(!join_index){
                    ...
                    join_index = createJoinIndex(pk_tab, pk_tab_col_name, fk_tab, fk_tab_col_name);
        ...
        const PositionListPtr JoinIndexes::getReverseJoinIndex(TablePtr pk_tab, std::string pk_tab_col_name, TablePtr fk_tab, std::string fk_tab_col_name){
            boost::lock_guard<boost::mutex> lock(global_join_index_mutex);
            std::string index_name = joinExpressionToString(pk_tab, pk_tab_col_name, fk_tab, fk_tab_col_name);
            ...
            if(it!=reverse_indeces_.end()){
                ...
                if(!reverse_join_index){
                    //ok, no join index found, compute it and store it on disk
                    reverse_join_index = createReverseJoinIndex(pk_tab, pk_tab_col_name, fk_tab, fk_tab_col_name);
                    storeReverseJoinIndex(reverse_join_index, index_name);  
                }
                this->reverse_indeces_.insert(std::make_pair(index_name,reverse_join_index));
        ...
        bool JoinIndexes::isReverseJoinIndex(const PositionListPtr tids) const{
            ...
            ReverseJoinIndexes::const_iterator it = reverse_indeces_.find(tids->getName());
            if(it!=reverse_indeces_.end()){
                return true;
            }else{
                return false;
        ...
        const JoinIndexPtr JoinIndexes::createJoinIndex(TablePtr pk_tab, std::string pk_tab_col_name, TablePtr fk_tab, std::string fk_tab_col_name){
            
            hype::ProcessingDeviceID id=hype::PD0;
            ProcessorSpecification proc_spec(id);    
            JoinParam param(proc_spec, HASH_JOIN);
            
            TablePtr tab = BaseTable::join(pk_tab, pk_tab_col_name, fk_tab, fk_tab_col_name, param);
...
//            tab=BaseTable::sort(tab,column_names, ASCENDING, LOOKUP, CPU);
            ...
            std::vector<LookupColumnPtr> lookup_columns = lookup_table->getLookupColumns();
            assert(lookup_columns.size()==2);
            JoinIndexPtr join_index(new JoinIndex(std::make_pair(lookup_columns[0],lookup_columns[1])));
...
//                sorted_pairs.push_back(TID_Pair(pk_column_tids[i], fk_column_tids[i]));
//            }
            ...
            PositionListPtr pk_column_tids_sorted(createPositionList(lookup_table->getNumberofRows()));
            PositionListPtr fk_column_tids_sorted(createPositionList(lookup_table->getNumberofRows()));
            ...
            LookupColumnPtr pk_lc_sorted(new LookupColumn(join_index->first->getTable(),pk_column_tids_sorted)); 
            LookupColumnPtr fk_lc_sorted(new LookupColumn(join_index->second->getTable(),fk_column_tids_sorted)); 
   ...
   const PositionListPtr JoinIndexes::createReverseJoinIndex(TablePtr pk_tab, std::string pk_tab_col_name, TablePtr fk_tab, std::string fk_tab_col_name){
            ...
            PositionListPtr reverse_join_index(createPositionList(lookup_table->getNumberofRows()));
            for(unsigned int i=0;i<join_pairs.size();++i){
...
//                (*fk_column_tids_sorted)[i]=join_pairs[i].second;
   ...
   size_t getSizeInBytes(const JoinIndexPtr join_index){
       ...
       size_t size_of_joinindex=(join_index->first->getPositionList()->size()+join_index->second->getPositionList()->size())*sizeof(PositionList::value_type);
       return size_of_joinindex;
   }

   bool storeJoinIndex(JoinIndexPtr join_index, const std::string& join_index_name){
		...
		join_index->second->getPositionList()->store(path);
       		...
       		std::ofstream outfile (path_meta_data.c_str(), std::ios_base::binary | std::ios_base::out);
   ...
   JoinIndexPtr loadJoinIndex(const std::string& join_index_name){
       		...
       		if ( !exists( path ) ){ 
                ...
                std::ifstream infile (path_meta_data.c_str(),std::ios_base::binary | std::ios_base::in);
   ...
   bool storeReverseJoinIndex(PositionListPtr reverse_join_index, const std::string& join_index_name){
       		using namespace boost::filesystem;
       		std::string path = RuntimeConfiguration::instance().getPathToDatabase();
       		if ( !exists( path ) ){ 
                    create_directory(path);
...
//            if (boost::filesystem::is_regular_file(dir_iter->status()) )
              if (boost::filesystem::is_directory(dir_iter->status()) )
            {
                ...
                JoinIndexPtr join_index = loadJoinIndex(index_path.filename().generic_string());
                if(!join_index) return false;
                this->indeces_.insert(std::make_pair(index_path.filename().generic_string(),join_index));
                PositionListPtr reverse_join_index = loadReverseJoinIndex(index_path.filename().generic_string());
                if(!reverse_join_index) return false;
                this->reverse_indeces_.insert(std::make_pair(index_path.filename().generic_string(),reverse_join_index));
            }
          }
          return true;
        }else{
    ...
    //matching tids comes from a filter operation on a primary key table, and we now seek the corresponding foreign key tids
    //Note: assumes the Join Index is sorted by the first PositionList (the Primary Key Table TID list)
    PositionListPtr fetchMatchingTIDsFromJoinIndex(JoinIndexPtr join_index, PositionListPtr matching_tids){
        ...
        while(left_pos < matching_tids->size() && right_pos < number_of_join_pairs){
            //std::cout << "Left: " << (*matching_tids)[left_pos] << "   Right: " << (*pk_column_tids)[right_pos] << std::endl;
    ...
    bool JoinIndexes::placeJoinIndexesOnGPU(const hype::ProcessingDeviceMemoryID& mem_id){
...
//                    gpu::GPU_JoinIndexPtr gpu_join_index = GPU_Column_Cache::instance().getGPUJoinIndex((*rit).second);
//                }else{
//                    break;
//                }
...
//            }
            ...
            //each query accesses several dimension tables, and based on the number of accesses per dimension table in all queries,
            //we determiend the order, where the Join Indexes are load in the GPU buffer
//              Q1.1	Q1.2	Q1.3	Q2.1	Q2.2	Q2.3	Q3.1	Q3.2	Q3.3	Q3.4	Q4.1	Q4.2	Q4.3	ALL without FILTER	ALL
//    DATES	x	x	x	x (no filter)	x (no filter)	x (no filter)	x	x	x	x	x (no filter)	x	x	9	13
//    SUPPLIER	-	-	-	x	x	x	x	x	x	x	x	x	x	10	10
//    CUSTOMER	-	-	-	-	-	-	x	x	x	x	x	x	-	6	6
//    PART	-	-	-	x	x	x	-	-	-	-	x	x	x	6	6
            
            for(it= indeces_.begin();it!=indeces_.end();++it){
                if((*it).second->first->getTable()->getName()=="DATES"){
                    if(DataCacheManager::instance().getDataCache(mem_id).getAvailableBufferSize()>getSizeInBytes((*it).second)){
                        JoinIndexPtr join_index = DataCacheManager::instance().getDataCache(mem_id).getJoinIndex((*it).second);
                    }else{
                        break;
                    }
                }
    ...
    bool placeColumnOnCoprocessor(ClientPtr client, const hype::ProcessingDeviceMemoryID& mem_id, const std::string& table_name, const std::string& column_name){
        ...
        if(!col) return false;
            ...
            if(DataCacheManager::instance().getDataCache(mem_id).getAvailableBufferSize()>col->getSizeinBytes()){
                ColumnPtr device_column = DataCacheManager::instance().getDataCache(mem_id).getColumn(col);
                if(!device_column){
                    return false;
                }else{
                    out << "Placed column " << col->getName() << " on coprocessor with mem_id " << (int) mem_id << std::endl;
                    return true;
                }
            }
...
//we add columns to the buffer starting form the most frequently used column, and
...
//Cached Join Indexes:
//JOIN_INDEX(SUPPLIER,LINEORDER): 45MB	Referenced: 75	Last Used Timestamp: 4102238750838514
//JOIN_INDEX(DATES,LINEORDER): 45MB	Referenced: 59	Last Used Timestamp: 4102238763715114
//JOIN_INDEX(CUSTOMER,LINEORDER): 45MB	Referenced: 45	Last Used Timestamp: 4102237991520227
//JOIN_INDEX(PART,LINEORDER): 45MB	Referenced: 43	Last Used Timestamp: 4102238751422509
...
//                                      "LO_QUANTITY"};
        ...
        const char* column_names[] = {"S_SUPPKEY",
                                      "S_CITY",
                                      "S_NATION",
                                      "S_REGION",
                                      "LO_PARTKEY",
        ...
        for(unsigned int i=0;i<number_of_columns;++i){
            std::list<std::pair<ColumnPtr,TablePtr> > columns = DataDictionary::instance().getColumnsforColumnName(column_names[i]);
            ...
            if(DataCacheManager::instance().getDataCache(mem_id).getAvailableBufferSize()>col->getSizeinBytes()){
                ...
                if(!device_column) 
                    ...
                    std::cout << "Placed column " << col->getName() << " on GPU" << std::endl;
            }
        }
   ...
   //Note: assumes the Join Index is sorted by the first PositionList (the Primary Key Table TID list)
    PositionListPtr fetchMatchingTIDsFromJoinIndexInParallel(JoinIndexPtr join_index, PositionListPtr matching_tids){
        assert(join_index!=NULL);
        assert(matching_tids!=NULL);
    ...
    PositionListPairPtr fetchJoinResultFromJoinIndex(JoinIndexPtr join_index, PositionListPtr matching_tids_pk_column){
        //join result tids
        PositionListPtr pk_column_matching_tids=createPositionList();
        PositionListPtr fk_column_matching_tids=createPositionList();
        PositionListPairPtr join_tids(new PositionListPair(std::make_pair(pk_column_matching_tids,fk_column_matching_tids)));
        ...
        size_t number_of_join_pairs=join_index->first->getPositionList()->size();
        //assume that matching tids contains no duplicates and is sorted
        while(left_pos < matching_tids_pk_column->size() && right_pos < number_of_join_pairs){
            if((*matching_tids_pk_column)[left_pos] == (*pk_column_tids)[right_pos]){
                ...
                right_pos++;
            }else if((*matching_tids_pk_column)[left_pos] < (*pk_column_tids)[right_pos]){
                left_pos++;
            }else if((*matching_tids_pk_column)[left_pos] > (*pk_column_tids)[right_pos]){
                right_pos++;
            }
...
//
//    
////    void setBitmapMatchingTIDsFromJoinIndex(JoinIndexPtr join_index, PositionListPtr matching_tids, char* fk_column_matching_rows_bitmap, size_t bitmap_num_bits){
//    BitmapPtr createBitmapOfMatchingTIDsFromJoinIndex(JoinIndexPtr join_index, PositionListPtr matching_tids){
//        assert(join_index!=NULL);
//        assert(matching_tids!=NULL);
//        //create bitmap, init every bit with 0
//        BitmapPtr result(new Bitmap(join_index->second->getPositionList()->size(), false, true));
//        std::cout << "Bitmap Fetch Join: #Rows in Bitmap: " << join_index->second->getPositionList()->size() << std::endl;
//        char* fk_column_matching_rows_bitmap = result->data();
//        char* byte_wise_bitmap = (char*) calloc(join_index->second->getPositionList()->size(),sizeof(char));
//        PositionListPtr pk_column_tids = join_index->first->getPositionList();
//        PositionListPtr fk_column_tids = join_index->second->getPositionList();
...
//        unsigned int number_of_join_pairs=join_index->first->getPositionList()->size();
//        //assume that matching tids contains no duplicates and is sorted
//        while(left_pos < matching_tids->size() && right_pos < number_of_join_pairs){
////            TID left_value = (*matching_tids)[left_pos];
////            TID right_value = (*pk_column_tids)[right_pos];
...
//            }
//        }
//        CoGaDB::CDK::selection::PositionListToBitmap_pack_flag_array_thread(byte_wise_bitmap, join_index->second->getPositionList()->size(), fk_column_matching_rows_bitmap);
//        return result;
//    }
    ...
    BitmapPtr createBitmapOfMatchingTIDsFromJoinIndex(JoinIndexPtr join_index, PositionListPtr matching_tids){
        ...
        while(left_pos < matching_tids->size() && right_pos < number_of_join_pairs){
            ...
            if((*matching_tids)[left_pos] == (*pk_column_tids)[right_pos]){
                //fk_column_matching_tids->push_back((*fk_column_tids)[right_pos]);
                bitmap[(*fk_column_tids)[right_pos]/8]|=(1 << ((*fk_column_tids)[right_pos]&7));
                //std::cout << "Set Bit: " << (*fk_column_tids)[right_pos] << "" << std::endl;
                count++;
                right_pos++;
            }else if((*matching_tids)[left_pos] < (*pk_column_tids)[right_pos]){
                left_pos++;
...
//    }
    ...
    void setFetchJoinFlagArray_thread(TID* matching_tids, size_t number_of_matching_tids, TID* pk_column_tids, TID* fk_column_tids, size_t right_pos_begin, size_t right_pos_end, char* flag_array){
    
        //intersect(pk_column_matching_tids, matching_tids) -> matching fks
        TID left_pos=0;
        //search for correct begin position to start the merging
        //unsigned int left_pos= binary_index_search(matching_tids, pk_column_tids[right_pos_begin],0, number_of_matching_tids);
        TID right_pos=right_pos_begin;  
        
        
//        while(left_pos>0 && matching_tids[left_pos] == pk_column_tids[right_pos_begin]){
//            left_pos--;
//        }
        //unsigned int number_of_join_pairs=join_index->first->getPositionList()->size();
        //assume that matching tids contains no duplicates and is sorted
    ...
    BitmapPtr createBitmapOfMatchingTIDsFromJoinIndexParallel(JoinIndexPtr join_index, PositionListPtr matching_tids){
        assert(join_index!=NULL);
        assert(matching_tids!=NULL);
        size_t count=0;
        //PositionListPtr fk_column_matching_tids=createPositionList();
...
//        unsigned int right_pos=0;  
	...
	//TODO: parallelize
//	setFetchJoinFlagArray_thread(matching_tids->data(), matching_tids->size(), pk_column_tids->data(), fk_column_tids->data(),
//	 			     0, number_of_join_pairs, flag_array);
        
        size_t number_of_threads=8;
        size_t chunk_size= (number_of_join_pairs+number_of_threads-1)/number_of_threads;
//        //std::cout << "rows in fact table: " << number_of_join_pairs << "" << std::endl; 
//        for(unsigned int thread_id=0;thread_id<number_of_threads;++thread_id){
//            TID begin=chunk_size*thread_id; //std::min(chunk_size*thread_id, number_of_join_pairs);
//            TID end = std::min(chunk_size*(thread_id+1), number_of_join_pairs);
//            //std::cout << "BFJ: begin: " << begin << " end: " << end << std::endl;
//            setFetchJoinFlagArray_thread(matching_tids->data(), matching_tids->size(), pk_column_tids->data(), fk_column_tids->data(),
...
//        }
        ...
        for(unsigned int thread_id=0;thread_id<number_of_threads;++thread_id){
            ...
            TID end = std::min(chunk_size*(thread_id+1), number_of_join_pairs);
            threads.add_thread(new boost::thread(setFetchJoinFlagArray_thread,matching_tids->data(), matching_tids->size(), pk_column_tids->data(), fk_column_tids->data(),
                                         begin, end, flag_array));
        }
        threads.join_all();
        
//        boost::thread* thr = new boost::thread(setFetchJoinFlagArray_thread,matching_tids->data(), matching_tids->size(), pk_column_tids->data(), fk_column_tids->data(),
//	 			     0, number_of_join_pairs, flag_array);
//        
//        thr->join();
//        delete thr;

        CDK::selection::PositionListToBitmap_pack_flag_array_thread(flag_array, number_of_join_pairs, bitmap);
        free(flag_array);
        return result;
    }
