...
namespace CoGaDB{
        ...
        ColumnGroupingKeys::ColumnGroupingKeys(const hype::ProcessingDeviceMemoryID& mem_id) : keys(new GroupingKeys(std::string("GROUPING_KEYS")+boost::lexical_cast<std::string>(getUniqueID()), OID, mem_id)),  required_number_of_bits(65){
            
        }

        ColumnGroupingKeys::ColumnGroupingKeys(const ColumnGroupingKeys& x) 
//        : keys(boost::dynamic_pointer_cast<GroupingKeys>(copy(x.keys))), 
        : keys(boost::dynamic_pointer_cast<GroupingKeys>(CoGaDB::copy(x.keys, x.keys->getMemoryID()))),
          required_number_of_bits(x.required_number_of_bits)
        {
            assert(keys!=NULL);
        }
        
        ColumnGroupingKeys& ColumnGroupingKeys::operator=(const ColumnGroupingKeys& other){
            if (this != &other) // protect against invalid self-assignment
            {
                this->keys=boost::dynamic_pointer_cast<GroupingKeys>(CoGaDB::copy(other.keys, other.keys->getMemoryID()));
                assert(this->keys!=NULL);
                this->required_number_of_bits=other.required_number_of_bits;
            }
        }
        
        const PositionListPtr ColumnGroupingKeys::sort(const ProcessorSpecification& proc_spec){  
        ...
        ColumnGroupingKeysPtr ColumnGroupingKeys::copy(const hype::ProcessingDeviceMemoryID& mem_id) const{
            ColumnGroupingKeysPtr col(new ColumnGroupingKeys(mem_id));
            col->keys=boost::dynamic_pointer_cast<GroupingKeys>(CoGaDB::copy(this->keys, mem_id));
            if(!col->keys){
        ...
        GroupingKeys::value_type ColumnGroupingKeys::getGreaterPowerOfTwo(GroupingKeys::value_type val){
