...
#define UINT64MAX 18446744073709551615UL
    ...
    bool BaseCentric_SimpleKey_Importer::_createDatabase() {
        ...
        if (this->compress_data) {
            ...
            // #########################
            // reference genome
            rg_id_column = OIDTypedColumnPtr(new VoidCompressedColumnNumber<TID>(RG_ID_COL_NAME, OID));
            rg_name_column = StringTypedColumnPtr(
                    new DictionaryCompressedColumn<std::string>(RG_NAME_COL_NAME, VARCHAR));
            // contig
            c_id_column = OIDTypedColumnPtr(new VoidCompressedColumnNumber<TID>(C_ID_COL_NAME, OID));
            c_name_column = StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(C_NAME_COL_NAME, VARCHAR));
            c_rg_id_column = OIDTypedColumnPtr(new RLECompressedColumn<TID>(C_REF_GENOME_ID_COL_NAME, OID));
            // reference base
            rb_id_column = OIDTypedColumnPtr(new VoidCompressedColumnNumber<TID>(RB_ID_COL_NAME, OID));
            ...
            rb_base_column = StringTypedColumnPtr(
                    new BitPackedDictionaryCompressedColumnForBaseValues<std::string>(RB_BASE_VALUE_COL_NAME,
                                                                                      VARCHAR,
                                                                                      dictionary,
                                                                                      reverse_look_up_vector, 3));
            //rb_pos_column = IntTypedColumnPtr(new RLEDeltaOneCompressedColumnNumber<int>(RB_POSITION_COL_NAME, INT));
            rb_pos_column = IntTypedColumnPtr(new RLEDeltaOneCompressedColumnNumberWithPrefixCounts<int>(RB_POSITION_COL_NAME, INT));
//            rb_pos_column = IntTypedColumnPtr(new Column<int>(RB_POSITION_COL_NAME, INT));
            rb_c_id_column = OIDTypedColumnPtr(new RLECompressedColumn<TID>(RB_CONTIG_ID_COL_NAME, OID));

            // #################
            // # sample genome #
            // #################
            // sample genome
            sg_id_column = OIDTypedColumnPtr(new VoidCompressedColumnNumber<TID>(SG_ID_COL_NAME, OID));
            sg_name_column = StringTypedColumnPtr(
                    new DictionaryCompressedColumn<std::string>(SG_NAME_COL_NAME, VARCHAR));
            // read
            r_id_column = OIDTypedColumnPtr(new VoidCompressedColumnNumber<TID>(R_ID_COL_NAME, OID));
            r_qname_column = StringTypedColumnPtr(
                    new DictionaryCompressedColumn<std::string>(R_QNAME_COL_NAME, VARCHAR));
            r_sg_id_column = OIDTypedColumnPtr(new RLECompressedColumn<TID>(R_SAMPLE_GENOME_ID_COL_NAME, OID));
            r_map_qual_column = IntTypedColumnPtr(new RLECompressedColumn<int>(R_MAPPING_QUALITY_COL_NAME, INT));
            ...
            // sample base
            sb_id_column = OIDTypedColumnPtr(new VoidCompressedColumnNumber<TID>(SB_ID_COL_NAME, OID));
            sb_rb_id_column = OIDTypedColumnPtr(new RLEDeltaOneCompressedColumnNumberWithPrefixCounts<TID>(SB_RB_ID_COL_NAME, OID));
//            sb_rb_id_column = OIDTypedColumnPtr(new Column<TID>(SB_RB_ID_COL_NAME, OID));
            sb_r_id_column = OIDTypedColumnPtr(new RLECompressedColumn<TID>(SB_READ_ID_COL_NAME, OID));
            //sb_base_column = StringTypedColumnPtr(
            //        new BitPackedDictionaryCompressedColumnForBaseValues<std::string>(SB_BASE_VALUE_COL_NAME, VARCHAR,
            //                                                                          dictionary,
            //                                                                          reverse_look_up_vector, 3));
            sb_base_column = StringTypedColumnPtr(
                    new ReferenceBasedCompressedColumn<std::string>(SB_BASE_VALUE_COL_NAME, VARCHAR, SB_TBL_NAME,
                                                                    SB_RB_ID_COL_NAME, RB_TBL_NAME,
                                                                    RB_BASE_VALUE_COL_NAME));
            //sb_insert_column = IntTypedColumnPtr(new RLECompressedColumn<int>(SB_INSERT_OFFSET_COL_NAME, INT));
            sb_insert_column = IntTypedColumnPtr(new RLECompressedColumnWithPrefixCounts<int>(SB_INSERT_OFFSET_COL_NAME, INT));
            sb_qual_column = IntTypedColumnPtr(
                    new BitPackedDictionaryCompressedColumn<int>(SB_BASE_CALL_QUALITY_COL_NAME, INT, 8));
        } else {
            // #########################
            // # reference genome data #
            // #########################
            ...
            //sb_base = ColumnPtr(new DictionaryCompressedColumn<std::string>(SB_BASE_VALUE_COL_NAME, VARCHAR));
            sb_base_column = StringTypedColumnPtr(
        ...
        if (!contigTable->setForeignKeyConstraint(C_REF_GENOME_ID_COL_NAME, RG_ID_COL_NAME, RG_TBL_NAME)) {
        ...
        if (!refBaseTable->setForeignKeyConstraint(RB_CONTIG_ID_COL_NAME, C_ID_COL_NAME, C_TBL_NAME)) {
            COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
            return false;
        }
        ...
        if (!readTable->setForeignKeyConstraint(R_SAMPLE_GENOME_ID_COL_NAME, SG_ID_COL_NAME, SG_TBL_NAME)) {
            COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
            return false;
        }
        // add read table before we can set a foreign key
        addToGlobalTableList(readTable);
        if (!readTable->setForeignKeyConstraint(R_MATE_ID_COL_NAME, R_ID_COL_NAME, R_TBL_NAME)) {
            COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
            return false;
        }
        // sample base
        ...
        if (!sampleBaseTable->setForeignKeyConstraint(SB_READ_ID_COL_NAME, R_ID_COL_NAME, R_TBL_NAME)) {
            COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
            return false;
        }
        if (!sampleBaseTable->setForeignKeyConstraint(SB_RB_ID_COL_NAME, RB_ID_COL_NAME, RB_TBL_NAME)) {
            COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
            return false;
        }
    ...
    BaseCentric_SimpleKey_Importer::BaseCentric_SimpleKey_Importer(ostream &_out, bool _compress_data, bool _verbose)
            : GenomeDataImporter(_out, _compress_data, _verbose) {
        this->_initDatabaseConnection();
    };

    bool BaseCentric_SimpleKey_Importer::_importReferenceGenomeData(const string reference_genome_name,
        ...
        out << "Time to IMPORT reference data: " << time.first << time.second << " (" << end_importing << "ns)" <<
        endl;
    ...
    bool BaseCentric_SimpleKey_Importer::importReferenceGenomeData(const string path_to_fasta_file,
        ...
        bool success = false;
        success = this->_importReferenceGenomeData(reference_genome_name, path_to_fasta_file);
        if (success) {
            out << " SUCCESS!" << endl;
 ...
 */
    bool BaseCentric_SimpleKey_Importer::_importAlignment(TID sample_genome_id,
                                                          TID id_of_first_reference_base_in_contig, bam1_t *alignment,
                                                          const bam1_core_t c, TID read_id, TID mate_id,
                                                          TID &sample_base_id,
        ...
        if (sb_base_column->getColumnType() == REFERENCE_BASED_COMPRESSED) {
            boost::shared_ptr<ReferenceBasedCompressedColumn<string> > sb_base_value_referenced =
                    boost::dynamic_pointer_cast<ReferenceBasedCompressedColumn<string> >(sb_base_column);
    ...
    bool BaseCentric_SimpleKey_Importer::_importSampleGenomeData(string sample_genome_name,
        ...
        map<string, contig_ids> contigName2IdMap;
        {
            ...
            ColumnPtr selected_contig_names = c_name_column->gather(contig_TIDs, GatherParam(processorSpecification));
            // contig join reference_base on c_id = rb_c_id - first TID list for (modified) contig, second for rb
            PositionListPairPtr join_pair = selected_contig_ids->join(rb_c_id_column,
                                                                      JoinParam(processorSpecification, HASH_JOIN));
            // contig_id_column stores TIDs of contig -> first join list contains the joined contig TIDs
            ColumnPtr joined_contig_ids = selected_contig_ids->gather(join_pair->first,
                                                                GatherParam(processorSpecification));
            // gather contig names
            ColumnPtr joined_contig_names = selected_contig_names->gather(join_pair->first,
                                                                  GatherParam(processorSpecification));
            // gather rb_ids - TODO check for void is not necessarily required as we know how the data is stored
            ...
            // we just have to use one column for grouping as the pairs are also distinct
            ...
            PositionListPtr agg_result_TIDs = boost::dynamic_pointer_cast<Column<TID> >(aggregation_result.first);
            assert(agg_result_TIDs != NULL);
            ColumnPtr grouped_contig_ids = joined_contig_ids->gather(agg_result_TIDs, GatherParam(processorSpecification));
            assert(grouped_contig_ids != NULL);
            ColumnPtr grouped_contig_names = joined_contig_names->gather(agg_result_TIDs,
                                                                         GatherParam(processorSpecification));
            ...
            if (queried_aggregation_results_typed->getColumnType() != PLAIN_MATERIALIZED) {
      ...
      * first read through the complete file and count the alignment */
        ...
        while (alignmentFile->getNextAlignment(loop_alignment) >= 0) {
            ...
            if (mate_iterator != mate_map.end()) {
                ...
                if ((mate_core.flag & BAM_FUNMAP) == 0 && mate_core.tid >= 0 &&
                    ...
                    contigNameMate = string(alignmentFile->getHeader()->target_name[chromosome_id_mate]);
                    map<string, contig_ids>::iterator it = contigName2IdMap.find(contigNameMate);
                ...
                if (import_mate_id < UINT64MAX && id_of_first_reference_base_in_contig_mate < UINT64MAX) {
                    ...
                    /* do CIGAR conversion */
                    this->_convertCIGAR(cigar, mate_core.n_cigar, mate_core.pos, &converted_cigar);
        ...
        for (mate_map_iterator_type read_iterator = mate_map.begin();
            ...
            if (import_id < UINT64MAX && id_of_first_reference_base_in_contig < UINT64MAX) {
                ...
                //if (it == cigar_conversion_cache.end()) {
                /* do CIGAR conversion */
                this->_convertCIGAR(cigar, nomate_core.n_cigar, nomate_core.pos, &converted_cigar);
                //    cigar_conversion_cache.insert(std::make_pair(cigar_lookup_key, converted_cigar));
                //} else {
                //    cigar_cache_hits++;
                //    converted_cigar = it->second;
    ...
    bool BaseCentric_SimpleKey_Importer::importSampleGenomeData(const string path_to_sam_bam_file,
                                                                const string reference_genome_name,
