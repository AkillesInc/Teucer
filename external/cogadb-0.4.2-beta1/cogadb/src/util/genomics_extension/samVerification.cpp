...
{   
   ...
   bool compareSamFiles(std::string originalSam, std::string exportedSam, std::ostream& clientOut){
       using std::endl;
	...
	if(line.at(0)=='@'){
           ...
           try {
               flag = boost::lexical_cast<unsigned int>(tmpSamline[1]);
                    ...
                    for(unsigned int i; i<tmpSamline.size();i++){
                        clientOut << tmpSamline[i];    
                    }
                    clientOut << "' does not match Sam-Format!" << endl;
                    return false;
                }
           //if 0x4 bit is set or cigar = *, the line is unmapped and therefore not in database
       ...
       //clientOut << "DEBUG: Original-Sam has " << samlinesOriginalSam.size() << " Lines at beginning!" << endl;
	...
	Samline tmpSamline;
       ...
       if(missMatchFound){
            clientOut << endl << endl << "______________________________________________________________________________________________"<<endl;
       }
       if(samlinesOriginalSam.size()>0){
//            clientOut << samlinesOriginalSam.size() << " lines from original Samfile are not exported." << endl;
              clientOut << "______________________________________________________________________________________________"<<endl <<endl;
              for(unsigned int i = 0; i<samlinesOriginalSam.size();i++){
                    clientOut << "Error: Line ";
       ...
       if(missMatchFound || samlinesOriginalSam.size()>0){
           clientOut << "====> Original-Sam and Exported-Sam are functionally different." << endl;
           return false;
       }
       else{
           clientOut << "====> Original-Sam and Exported-Sam are functionally equal." << endl;
           return true;
       }
   }
   
   bool _isSamlinesEqual(const Samline& originalSamline,const Samline& exportedSamline){
        ...
        * 10 = QUAL 
        */
       return ((originalSamline[0]==exportedSamline[0]) && (originalSamline[1]==exportedSamline[1]) 
               && (originalSamline[2]==exportedSamline[2]) &&
               (originalSamline[3]==exportedSamline[3]) && (originalSamline[4]==exportedSamline[4]) && 
               (originalSamline[5]==exportedSamline[5]) && (originalSamline[9]==exportedSamline[9]) && 
        ...
        for(unsigned int i=0; i<cigar.size();i++){
            ...
            if(boost::regex_match(currentChar, numberReg)){
                ...
                case 'N':
                   ...
                   break;
               case 'D':
                   //Do nothing. Keep in Cigar, SEQ and Qual but don't move position Counter
                   break;
                default:
                    return false;
                }
