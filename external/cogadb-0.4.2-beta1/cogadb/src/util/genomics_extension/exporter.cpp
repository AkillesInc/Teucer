...
namespace CoGaDB {
    ...
    bool _igvControl(const std::string ip, const std::string port, const std::string refGenomeId,
                     const std::string sampleGenomePath, const std::string locus);

    bool _igvControl(const std::string ip, const std::string port, const std::string refGenomeId,
            const std::string sampleGenomePath, const std::string locus, std::string snapshotDirectory, std::string snapshotName);
    std::string _calculateSamfile(TablePtr cigarBaseTable);
    
    /**
     * \brief Entry Point for the SamExporter. Exports a Sam-File based on the Parameter
    ...
    bool exportSampleGenome(const std::string &args_string, ClientPtr client) {
        ...
        std::string exporterInputType = VariableManager::instance().getVariableValueString("sam_exporter_input_api");
        if (exporterInputType == "simple") {
            samExportParser = boost::shared_ptr<SamExportParser>(new SimpleSamExportParser());
        }
        else {
            out << "Error: Input Api " << exporterInputType << " is not defined." << endl;
            return false;
        }
        ...
        if (timeKeepingEnabled) {
            out << "Parsing Arguments: " << (double(getTimestamp() - localTimeStart)) / 1000000000 << "s" << endl;
            localTimeStart = getTimestamp();
        }

        //Execute Query
        TablePtr samCalcBaseTable = CoGaDB::SQL::executeSQL(queryBuilder.getQuery(), client);

        if (timeKeepingEnabled) {
            out << "Executing SQL-Query: " << (double(getTimestamp() - localTimeStart)) / 1000000000 << "s" << endl;
            localTimeStart = getTimestamp();
        }
        ...
        //Generate Samfile
        //std::string tmpSamOutput = samCalcBaseTable->toString("sam"); //old version over result_output_format
        std::string tmpSamOutput = _calculateSamfile(samCalcBaseTable);
        if (timeKeepingEnabled) {
            out << "Calculating Samfile: " << (double(getTimestamp() - localTimeStart)) / 1000000000 << "s" << endl;
            localTimeStart = getTimestamp();
        }
        ...
        file.close();
        if (timeKeepingEnabled) {
            out << "Writing Data into File: " << (double(getTimestamp() - localTimeStart)) / 1000000000 << "s" << endl;
            out << "Total Time:  " << (double(getTimestamp() - totalTimeStart)) / 1000000000 << "s" << endl;
        }
        //Control IGV
        if (samExportParser->isIgvControlEnabled()) {
            ...
            try {
                ...
                out << "Error. Could not delete old index file. " << e.what() << std::endl;
            }
            ...
            system(indexCommand.c_str());

            if (timeKeepingEnabled)
                out << "Indexing Sam-File: " << (double(getTimestamp() - localTimeStart)) / 1000000000 << "s" << endl;
            ...
            igvLocusOut << samCalcBaseTable->getColumnbyName(CONTIG_NAME_COLUMN)->getStringValue(0);
            if (end >= 0) {
                igvLocusOut << ":" << boost::lexical_cast<std::string>(start) << "-" <<
                boost::lexical_cast<std::string>(end);
            }
            //TablePtr referenceNameTable = CoGaDB::SQL::executeSQL(REFERENCE_NAME_QUERY, client);
            if (samExportParser->isSnapshotEnabled()) {
                if (!_igvControl(IGV_HOST, VariableManager::instance().getVariableValueString("igv_port"),
                                 samCalcBaseTable->getColumnbyName(REFERENCE_BASE_NAME_COLUMN)->getStringValue(0),
                                 samSavePath, igvLocusStringStream.str(),
                                 VariableManager::instance().getVariableValueString("igv_snapshot_path"),
                                 VariableManager::instance().getVariableValueString("igv_snapshot_name"))) {
    ...
    inline bool _igvControl(const std::string ip, const std::string port, const std::string refGenomeId,
                            const std::string sampleGenomePath, const std::string locus) {
    ...
    inline bool _igvControl(const std::string ip, const std::string port, const std::string refGenomeId,
                            const std::string sampleGenomePath, std::string locus, std::string snapshotDirectory,
    ...
    bool verificateSamFiles(const std::string &args_string, ClientPtr client) {
        ...
        //return CoGaDB::compareSamFiles("/home/john/coga_samfiles_csvfiles/Harrington_contig1-30.sam","/home/john/coga_samfiles_csvfiles/default.sam", out);
    ...
    bool _exportDatabaseToCSV(ClientPtr client) {
        ...
        out << "DEBUG: This is a Debug function to export some Tables to csv." << endl;

        //DEBUG
        //Sample_base
        TablePtr cigarBaseTable = CoGaDB::SQL::executeSQL("select * from sample_base;", client);
        ...
        cigarBaseTable = CoGaDB::SQL::executeSQL("select * from reference_base order by rb_position;", client);
        //std::string samOutput = cigarBaseTable->toString("sam");
        ...
        //samplebase join reference base     order by sb_read_id, rb_position, sb_insert_offset;
        cigarBaseTable = CoGaDB::SQL::executeSQL(
                "select * from sample_base join reference_base on sb_rb_id = rb_id order by sb_read_id, rb_position, sb_insert_offset;",
                client);
        //std::string samOutput = cigarBaseTable->toString("sam");
        ...
        cigarBaseTable = CoGaDB::SQL::executeSQL(
                "select * from sample_base join reference_base on sb_rb_id = rb_id join contig on rb_c_id = c_id order by sb_read_id, rb_position, sb_insert_offset;",
                client);
        //std::string samOutput = cigarBaseTable->toString("sam");
        ...
        cigarBaseTable = CoGaDB::SQL::executeSQL(
                "select * from sample_base join reference_base on sb_rb_id = rb_id join contig on rb_c_id = c_id join read on sb_read_id = r_id order by sb_read_id, rb_position, sb_insert_offset;",
                client);
        //std::string samOutput = cigarBaseTable->toString("sam");
        std::ofstream file7("sampleJoinReferenceJoinContigJoinRead.csv", std::ios::trunc);
        if (!file7.good()) {
            return false;
        }
    ...
    std::string _calculateSamfile(TablePtr cigarBaseTable) {
        ...
        OIDTypedColumnPtr sb_read_id = boost::dynamic_pointer_cast<OIDTypedColumnType>(
                cigarBaseTable->getColumnbyName("SB_READ_ID")); //0
        assert(sb_read_id != NULL);
        StringTypedColumnPtr r_qname = boost::dynamic_pointer_cast<StringTypedColumnType>(
        ...
        assert(r_flag != NULL);
        StringTypedColumnPtr c_name = boost::dynamic_pointer_cast<StringTypedColumnType>(
                cigarBaseTable->getColumnbyName("C_NAME"));  //3
        assert(c_name != NULL);
        IntTypedColumnPtr rb_position = boost::dynamic_pointer_cast<IntTypedColumnType>(
        ...
        assert(r_mapq != NULL);
        StringTypedColumnPtr sb_base_value = boost::dynamic_pointer_cast<StringTypedColumnType>(
                cigarBaseTable->getColumnbyName("SB_BASE_VALUE"));//6
        assert(sb_base_value != NULL);
        IntTypedColumnPtr sb_insert_offset = boost::dynamic_pointer_cast<IntTypedColumnType>(
        ...
        //        boost::dynamic_pointer_cast<RLECompressedColumn<int> >(sb_insert_offset_lookup->getIndexedColumn());//7
        //PositionListPtr sb_insert_offset_tids = sb_insert_offset_lookup->getPositionList();
        //assert(sb_insert_offset_rle != NULL);

        IntTypedColumnPtr sb_base_call_quality = boost::dynamic_pointer_cast<IntTypedColumnType>(
        ...
        headerOutStream << "@HD\t" << "VN:1.0\t" << "SO:coordinate" << endl;

        TID _last_sb_insert_offset_tid = 0;
        ...
        for (unsigned int i = 0; i < sb_read_id->size(); i++) {
            ...
            if (tmpReadId != currentReadId) {
                //New Headerline if new contigName. ContigName cannot change if ReadId does not change
...
//                                std::string headerLengthQuery = "select count(rb_id) as length from "
//                                        "reference_base join contig on rb_c_id=c_id where c_name=";
//                                headerLengthQuery+=currentCname;
//                                headerLengthQuery+=";";
//                                TablePtr referenceBaseCount = CoGaDB::SQL::executeSQL(headerLengthQuery, client);
//                                std::string currentHeaderLength = referenceBaseCount->getColumnbyName("length")->getStringValue(0);                                
                    headerOutStream << "@SQ\t" << "SN:" << tmpCName << "\t" <<
...
//                            currentCigarString = cigarFSM.getCigar();
                    ...
                    currentSamLine =
                            currentQName + "\t" + boost::lexical_cast<std::string>(currentFlag) + "\t" + currentCname +
                            "\t" + boost::lexical_cast<std::string>(currentSmallestRbPosition + 1)
                            + "\t" + boost::lexical_cast<std::string>(currentMapQ) + "\t" + cigarFSM.getCigar() + "\t" +
                            "*" + "\t" + "0" + "\t" + "0" + "\t" + currentBaseValueSequence
                            + "\t" + currentQual;
...
//                        
        ...
        //Write last line FIXME - Better way please.. put one output at the end of the loop
        out << currentQName + "\t" + boost::lexical_cast<std::string>(currentFlag) + "\t" + currentCname + "\t" +
               boost::lexical_cast<std::string>(currentSmallestRbPosition + 1)
               + "\t" + boost::lexical_cast<std::string>(currentMapQ) + "\t" + cigarFSM.getCigar() + "\t" + "*" + "\t" +
               "0" + "\t" + "0" + "\t" + currentBaseValueSequence
               + "\t" + currentQual;
