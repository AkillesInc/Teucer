...
#define UINT64MAX 18446744073709551615UL
    ...
    bool SequenceCentric_SimpleKey_WithStash_Importer::_createDatabase() {
        ...
        if (this->compress_data) {
            ...
            c_seq_column = StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(C_SEQ_COL_NAME, VARCHAR));
            ...
            r_c_id_column = OIDTypedColumnPtr(new RLECompressedColumn<TID>(R_C_ID_COL_NAME, OID));
            ...
            r_seq_column = StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(R_SEQ_COL_NAME, VARCHAR));
            r_qual_column = StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(R_QUAL_COL_NAME, VARCHAR));
            ...
            // unmapped reads
            ur_id_column = OIDTypedColumnPtr(new VoidCompressedColumnNumber<TID>(UR_ID_COL_NAME, OID));
            ur_qname_column = StringTypedColumnPtr(
                    new DictionaryCompressedColumn<std::string>(UR_QNAME_COL_NAME, VARCHAR));
            ur_flag_column = IntTypedColumnPtr(
                    new BitPackedDictionaryCompressedColumn<int>(UR_FLAG_COL_NAME, INT, 16));
            ur_c_id_column = OIDTypedColumnPtr(new RLECompressedColumn<TID>(UR_C_ID_COL_NAME, OID));
            ur_pos_column = IntTypedColumnPtr(new Column<int>(UR_POS_COL_NAME, INT));
            ur_map_qual_column = IntTypedColumnPtr(new RLECompressedColumn<int>(UR_MAPPING_QUALITY_COL_NAME, INT));
            ur_cigar_column = StringTypedColumnPtr(
                    new DictionaryCompressedColumn<std::string>(UR_CIGAR_COL_NAME, VARCHAR));
            ur_seq_column = StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(UR_SEQ_COL_NAME, VARCHAR));
            ur_qual_column = StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(UR_QUAL_COL_NAME, VARCHAR));
            ur_sg_id_column = OIDTypedColumnPtr(new RLECompressedColumn<TID>(UR_SAMPLE_GENOME_ID_COL_NAME, OID));
            // FIXME mate information - no compression yet
            ur_mate_id_column = OIDTypedColumnPtr(new Column<TID>(UR_MATE_ID_COL_NAME, OID));

            read_aux_id_column = OIDTypedColumnPtr(new VoidCompressedColumnNumber<TID>(RAUX_ID_COL_NAME, OID));
            read_aux_read_id_column = OIDTypedColumnPtr(new RLECompressedColumn<TID>(RAUX_READ_ID_COL_NAME, OID));
            // 128 unique tags
            read_aux_tag_column = StringTypedColumnPtr(
                    new BitPackedDictionaryCompressedColumn<std::string>(RAUX_TAG_COL_NAME, VARCHAR, 8));
            // 16 possible types
            read_aux_type_column = StringTypedColumnPtr(
                    new BitPackedDictionaryCompressedColumn<std::string>(RAUX_TYPE_COL_NAME, VARCHAR, 4));
            read_aux_value_column = StringTypedColumnPtr(
                    new DictionaryCompressedColumn<std::string>(RAUX_VALUE_COL_NAME, VARCHAR));

            uread_aux_id_column = OIDTypedColumnPtr(new VoidCompressedColumnNumber<TID>(URAUX_ID_COL_NAME, OID));
            uread_aux_read_id_column = OIDTypedColumnPtr(new RLECompressedColumn<TID>(URAUX_READ_ID_COL_NAME, OID));
            // 128 unique tags
            uread_aux_tag_column = StringTypedColumnPtr(
                    new BitPackedDictionaryCompressedColumn<std::string>(URAUX_TAG_COL_NAME, VARCHAR, 8));
            // 16 possible types
            uread_aux_type_column = StringTypedColumnPtr(
                    new BitPackedDictionaryCompressedColumn<std::string>(URAUX_TYPE_COL_NAME, VARCHAR, 4));
            uread_aux_value_column = StringTypedColumnPtr(
                    new DictionaryCompressedColumn<std::string>(URAUX_VALUE_COL_NAME, VARCHAR));
        } else {
            // #########################
            // # reference genome data #
            // #########################
            ...
            ur_pos_column = IntTypedColumnPtr(new Column<int>(UR_POS_COL_NAME, INT));
            ur_map_qual_column = IntTypedColumnPtr(new Column<int>(UR_MAPPING_QUALITY_COL_NAME, INT));
            ur_cigar_column = StringTypedColumnPtr(
                    new DictionaryCompressedColumn<std::string>(UR_CIGAR_COL_NAME, VARCHAR));
            ur_seq_column = StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(UR_SEQ_COL_NAME, VARCHAR));
            ur_qual_column = StringTypedColumnPtr(new DictionaryCompressedColumn<std::string>(UR_QUAL_COL_NAME, VARCHAR));

            ur_sg_id_column = OIDTypedColumnPtr(new Column<TID>(UR_SAMPLE_GENOME_ID_COL_NAME, OID));
            // FIXME mate information - no compression yet
            ur_mate_id_column = OIDTypedColumnPtr(new Column<TID>(UR_MATE_ID_COL_NAME, OID));

            read_aux_id_column = OIDTypedColumnPtr(new Column<TID>(RAUX_ID_COL_NAME, OID));
            read_aux_read_id_column = OIDTypedColumnPtr(new Column<TID>(RAUX_READ_ID_COL_NAME, OID));
            read_aux_tag_column = StringTypedColumnPtr(
                    new DictionaryCompressedColumn<std::string>(RAUX_TAG_COL_NAME, VARCHAR));
            read_aux_type_column = StringTypedColumnPtr(
                    new DictionaryCompressedColumn<std::string>(RAUX_TYPE_COL_NAME, VARCHAR));
            read_aux_value_column = StringTypedColumnPtr(
                    new DictionaryCompressedColumn<std::string>(RAUX_VALUE_COL_NAME, VARCHAR));

            uread_aux_id_column = OIDTypedColumnPtr(new Column<TID>(URAUX_ID_COL_NAME, OID));
            uread_aux_read_id_column = OIDTypedColumnPtr(new Column<TID>(URAUX_READ_ID_COL_NAME, OID));
            uread_aux_tag_column = StringTypedColumnPtr(
                    new DictionaryCompressedColumn<std::string>(URAUX_TAG_COL_NAME, VARCHAR));
            uread_aux_type_column = StringTypedColumnPtr(
                    new DictionaryCompressedColumn<std::string>(URAUX_TYPE_COL_NAME, VARCHAR));
            uread_aux_value_column = StringTypedColumnPtr(
                    new DictionaryCompressedColumn<std::string>(URAUX_VALUE_COL_NAME, VARCHAR));
        }
        ...
        if (!readTable->setForeignKeyConstraint(R_C_ID_COL_NAME, C_ID_COL_NAME, C_TBL_NAME)) {
            COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
            return false;
        }
...
//        }
        ...
        if (!unmappedReadsTable->setForeignKeyConstraint(UR_SAMPLE_GENOME_ID_COL_NAME, SG_ID_COL_NAME, SG_TBL_NAME)) {
            COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
            return false;
        }
        if (!unmappedReadsTable->setForeignKeyConstraint(UR_C_ID_COL_NAME, C_ID_COL_NAME, C_TBL_NAME)) {
            COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
            return false;
        }
        // add read table before we can set a foreign key
        ...
        // FIXME disabled as mapped and unmapped reads can be in one pair
//        if (!unmappedReadsTable->setForeignKeyConstraint(UR_MATE_ID_COL_NAME, R_ID_COL_NAME, R_TBL_NAME)) {
//            COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
//            return false;
//        }
        ...
        if (!readAuxTable->setForeignKeyConstraint(RAUX_READ_ID_COL_NAME, R_ID_COL_NAME, R_TBL_NAME)) {
            COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
            return false;
        }
        ...
        if (!unmappedReadsAuxTable->setForeignKeyConstraint(URAUX_READ_ID_COL_NAME, UR_ID_COL_NAME, UR_TBL_NAME)) {
            COGADB_ERROR("Failed to set Foreign Key Constraint!", "");
            return false;
        }
    ...
    SequenceCentric_SimpleKey_WithStash_Importer::SequenceCentric_SimpleKey_WithStash_Importer(ostream &_out, bool _compress_data,
                                                                           bool _verbose)
            : GenomeDataImporter(_out, _compress_data, _verbose) {
        this->_initDatabaseConnection();
    };

    bool SequenceCentric_SimpleKey_WithStash_Importer::_importReferenceGenomeData(const string reference_genome_name,
    ...
    bool SequenceCentric_SimpleKey_WithStash_Importer::importReferenceGenomeData(const string path_to_fasta_file,
    ...
    std::vector<GenomeDataImporter::aux_info> SequenceCentric_SimpleKey_WithStash_Importer::getAuxiliaryStringVector(bam1_t *alignment) {
        ...
        for(int j = 0; j < alignment->l_aux; ) {
            ...
            switch(type) {
                ...
                case 'c':
                case 'A':
                    value_string << (char) auxiliary_info[j];
                    j += 1;
                    type_string = "A";
                    break;
                case 'S':
                    value_string << (uint16_t) auxiliary_info[j];
                    j += 1;
                    type_string = "S";
                    break;
                case 's':
                    value_string << (int16_t) auxiliary_info[j];
                    type_string = "s";
                    j += 2;
                    break;
                case 'I':
                    value_string << (uint32_t) auxiliary_info[j];
                    type_string = "I";
                    j += 4;
                    break;
                case 'i':
                    value_string << (int32_t) auxiliary_info[j];
                    type_string = "i";
                    j += 4;
                    break;
                case 'f':
                case 'F':
                    value_string << (float) auxiliary_info[j];
                    type_string = "f";
                    j += 4;
                    break;
                case 'Z':
            ...
            result.push_back(aux_info(tag_string.str(), type_string, value_string.str()));
        }
        return result;
    }
...
*/
    bool SequenceCentric_SimpleKey_WithStash_Importer::_importUnalignedAlignment(TID sample_genome_id,
                                                              TID id_of_contig,
                                                              bam1_t *alignment,
                                                              const bam1_core_t c, TID uread_id, TID mate_id,
                                                              TID &uraux_id,
        ...
        ur_flag_column->insert((int) c.flag);
        ur_c_id_column->insert(id_of_contig);
        ur_pos_column->insert(pos);
        ur_map_qual_column->insert((int) c.qual);
 ...
 * int* numberOfReads ... for statistics
 */
    bool SequenceCentric_SimpleKey_WithStash_Importer::_importAlignment(TID sample_genome_id,
                                                              TID id_of_contig,
                                                              bam1_t *alignment,
    ...
    string SequenceCentric_SimpleKey_WithStash_Importer::convertCIGAR2String(uint32_t *cigar, uint32_t cigarLength) {
        ...
        for (i = 0; i < cigarLength; ++i) {
            switch (bam_cigar_op(*cigar)) {
                ...
                case BAM_CREF_SKIP: /* N = skip on the reference (e.g. spliced alignment) */
                    /* according to the SAM Format reference, this CIGAR operation is only defined
                 * for mRNA-to-genome mapping! */
                    /* according to http://davetang.org/wiki/tiki-index.php?page=SAM#Spliced_alignment it can be
                ...
                case BAM_CHARD_CLIP: /* H = clip on the read with clipped sequence trimmed off */
                    /* hard clipping means there were bases that do not match the reference but they are not in the
                 * sequence anymore -> nothing to do just ignore this */
                    /* TODO just log it */
                    cigar_string << bam_cigar_oplen(*cigar) << "H";
                    break;
                case BAM_CPAD: /* P = padding */
                    /* we can ignore the padding operation as it tells us that there is an deletion in the read and
                 * also the reference in accordance to another read that inserts bases at this position,
    ...
    bool SequenceCentric_SimpleKey_WithStash_Importer::_importSampleGenomeData(string sample_genome_name,
...
//
//            OIDTypedColumnPtr selected_contig_ids_typed = boost::dynamic_pointer_cast<OIDTypedColumnType>(
//                    selected_contig_ids);
//            assert(selected_contig_ids_typed != NULL);
//            if (selected_contig_ids_typed->getColumnType() != PLAIN_MATERIALIZED) {
//                queried_contig_ids = selected_contig_ids_typed->copyIntoDenseValueColumn(processorSpecification);
//            } else {
//                queried_contig_ids = boost::dynamic_pointer_cast<Column<TID> >(selected_contig_ids);
//                assert(queried_contig_ids != NULL);
//            }
//
//            StringTypedColumnPtr selected_contig_names_typed = boost::dynamic_pointer_cast<StringTypedColumnType>(
//                    selected_contig_names);
//            assert(selected_contig_names_typed != NULL);
//            queried_contig_names = selected_contig_names_typed->copyIntoDenseValueColumn(processorSpecification);
//
        ...
        while (alignmentFile->getNextAlignment(loop_alignment) >= 0) {
            ...
            if (mate_iterator != mate_map.end()) {
                ...
                if ((mate_core.flag & BAM_FUNMAP) == 0) {
                    // TODO if chromosome id not set (< 0 or > n_targets) flag seems to be corrupted!
...
//                        }
                ...
                if (isMateAligned) {
                    _importAlignment(sample_genome_id, id_of_contig_mate, alignment_mate,
                                     mate_core, import_mate_id, import_read_id, raux_id, converted_cigar);
                } else {
                    _importUnalignedAlignment(sample_genome_id, id_of_contig_mate, alignment_mate,
                                     mate_core, import_mate_id, import_read_id, uraux_id, converted_cigar);
                ...
                if (isAligned) {
                    _importAlignment(sample_genome_id, id_of_contig, alignment,
                                     core, import_read_id, import_mate_id, raux_id, converted_cigar);
                } else {
                    // unmapped read
                    _importUnalignedAlignment(sample_genome_id, id_of_contig, alignment,
                                     core, import_read_id, import_mate_id, uraux_id, converted_cigar);
                    ++numberOfUnmappedReads;
                }
...
//                    }
            ...
            if (isAligned) {
                ...
                // unmapped read
                _importUnalignedAlignment(sample_genome_id, id_of_contig, alignment_nomate,
                                 nomate_core, import_id, -1, uraux_id, converted_cigar);
                ++numberOfUnmappedReads;
            }
            count++;
            if (this->verbose && count > printProgressCounter) {
    ...
    bool SequenceCentric_SimpleKey_WithStash_Importer::importSampleGenomeData(const string path_to_sam_bam_file,
                                                                    const string reference_genome_name,
