...
//SamQueryCreator::SamQueryCreator(const SamNameSelectionPtr selectionLinesByName){
...
bool SamQueryCreator::_isLineWithThreeStars(){
    for(unsigned int i = 0; i < selectionLinesWithContigID->size(); i++ ){
        if(boost::get<0>((*selectionLinesWithContigID)[i]) == -1 && boost::get<1>((*selectionLinesWithContigID)[i]) == -1 && boost::get<2>((*selectionLinesWithContigID)[i]) == -1)
            return true;
    }
    return false;
}
...
bool SamQueryCreator::_isRangeSame(){
    ...
    for(unsigned int i = 1; i < selectionLinesWithContigID->size(); i++ ){
        if((start != boost::get<0>((*selectionLinesWithContigID)[i])) || (end != boost::get<1>((*selectionLinesWithContigID)[i])))
            return false;
    }
    return true;    
}
