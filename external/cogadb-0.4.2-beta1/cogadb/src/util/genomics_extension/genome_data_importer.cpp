...
namespace CoGaDB {
    ...
    GenomeDataImporter::GenomeDataImporter(ostream &_out, bool _compress_data, bool _verbose) : out(_out),
                                                                                                compress_data(
                                                                                                        _compress_data),
                                                                                                verbose(_verbose) { }
...
*
* Output:
* uint32_t* positions ... corrected alignment positions for every base in a read according to the CIGAR string
* uint32_t* insert_offsets ... insert offsets for every base ion a read according to the CIGAR string
* uint32_t* deleted_positions ... positions where deleted bases 'X' have to be inserted
...
*/
    ...
    void GenomeDataImporter::_convertCIGAR(uint32_t *cigar, uint32_t cigarLength, int startPos,
                                           cigar_conversion *converted_cigar) {
        int hard_clip_not_present = 1;
        int cur_index = 0;
        int cur_del_index = 0;
        int i, j;
        int pos = 0;//startPos;
        for (i = 0; i < cigarLength; ++i) {
            switch (bam_cigar_op(*cigar)) {
                ...
                case BAM_CINS: /* I = insertion to the reference */
                    /* position stays the same as for the base before, but the insert_offset is increased */
                    for (j = 0; j < bam_cigar_oplen(*cigar); j++) {
                ...
                case BAM_CDEL: /* D = deletion from the reference */
                    /* length stores the number of deleted bases in the read according to the reference;
                 * to avoid biased analysis results add an 'X' base for every deleted base with averageQuality */
                    for (j = 0; j < bam_cigar_oplen(*cigar); j++) {
                 ...
                 * -> so if S occurs it must be at the first(0), second (1), (last - 1) or last position in the CIGAR string
                 * -> first and second influence the start offset, (last - 1) and last the end offset
                 *  */
                    if (i <= 1 - hard_clip_not_present) {
        ...
        converted_cigar->del_end_offset = cur_del_index;
    }


    // Returns human readable time and according unit string
    std::pair<double, std::string> GenomeDataImporter::_getTimeAsHumanReadableString(Timestamp time_in_nanoseconds) {
        int unit = 0;
