...
{   
    ...
    samExportParameter.insert(std::make_pair("-t", &SamExportParser::enableTimeKeeping));
    samExportParameter.insert(std::make_pair("-i", &SamExportParser::enableIgvControl));
    samExportParameter.insert(std::make_pair("-s", &SamExportParser::enableSnapshot));
    ...
    savePath = VariableManager::instance().getVariableValueString("sam_save_path") + "/" + DEFAULT_SAM_EXPORT_FILENAME + ".sam";
    
    //Initialisation 
    samSelection = SamQueryCreator::SamIdSelectionPtr(new SamQueryCreator::SamIdSelection());
}
...
bool SamExportParser::isTimeKeepingEnabled(){
    return timeKeepingEnabled;
}
...
bool SamExportParser::isIgvControlEnabled(){
    return igvControlEnabled;
}
...
bool SamExportParser::isSnapshotEnabled(){
...
void SamExportParser::setNewSaveName(const std::string newSaveName){
    savePath = VariableManager::instance().getVariableValueString("sam_save_path");
    savePath+="/";
    savePath+=newSaveName;
    savePath+=".sam";
}

std::string SamExportParser::getSavePath(){
    return savePath;
}

int SamExportParser::getStartValue(){
        return startValue; //Database starts position with 0 so we add 1 to match normal position
}

int SamExportParser::getEndValue(){
    return endValue; //Database starts position with 0 so we add 1 to match normal position
}
...
bool SimpleSamExportParser::addArguments(std::vector<std::string> newArg, std::ostream& clientOut){
    //SamExportParser();
	...
	if(endValue > 0){
	   ...
	   endTransform = true;
            ...
            clientOut << "Error: Values for Start or End cannot be cast to Integer!" << endl;
            return false;
        }
    ...
    for(unsigned int i=0; i<contigIdsForSamQueryBuilder.size();i++){
        try{
        int currentContigId = boost::lexical_cast<int>(contigIdsForSamQueryBuilder[i]);
        //ContigID has to be greater than or equal to 0
        if(currentContigId<0){
            clientOut << "Error: " << i+1 <<". ContigID is not greater than or equal to 0." << endl;
            return false;
        }
        samSelection->push_back(boost::make_tuple(startValue, endValue, currentContigId));
        }catch(const boost::bad_lexical_cast &){
            clientOut << "Error: " << i+1 <<". ContigID cannot be cast to Integer." << endl;
            return false;
        }
     }
   }   
...
//        boost::split(parameterSplit, newArg[i], boost::is_any_of("="));
        ...
        if(samExportIterator != samExportParameter.end()){
            ...
            if(!(this->*handlerFunction)()){
                clientOut << "Error: Executing Parameter " << newArg[i] << " failed!" << endl;
                return false;
            }
        }
        else{
            clientOut << "Error: Parameter " << newArg[i] << " is not defined!" << endl;
            return false; 
        }
    }
...
bool SimpleSamExportParser::isContigWithStar(const std::vector<std::string>& contigs){
