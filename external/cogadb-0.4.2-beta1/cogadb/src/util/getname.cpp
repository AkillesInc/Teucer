...
namespace CoGaDB {
    ...
    namespace util {
        ...
        const std::string getName(AttributeType x) {
            const char * const names[] = {"INT", "FLOAT", "VARCHAR", "BOOLEAN","UINT32", "OID", "DOUBLE", "CHAR", "DATE"};
        ...
        const std::string getName(ColumnType x) {
            const char * const names[] = {"PLAIN_MATERIALIZED", "LOOKUP_ARRAY",
                "DICTIONARY_COMPRESSED", "RUN_LENGTH_COMPRESSED", "DELTA_COMPRESSED",
                "BIT_VECTOR_COMPRESSED", "BITPACKED_DICTIONARY_COMPRESSED", 
        ...
        const std::string getName(AggregationMethod x) {
            const char * const names[] = {"COUNT", "SUM", "MIN", "MAX", "AVERAGE",
            "AGG_GENOTYPE", "AGG_CONCAT_BASES", "AGG_IS_HOMOPOLYMER", "AGG_GENOTYPE_STATISTICS"};
        ...
        const std::string getName(AggregationAlgorithm x) {
            const char * const names[] = {"SORT_BASED_AGGREGATION", "HASH_BASED_AGGREGATION"};
            return std::string(names[x]);
        }
        ...
        const std::string getName(Operation x) {
            const char * const names[] = {"SELECTION", "PROJECTION", "JOIN", "GROUPBY", "SORT", "COPY", "AGGREGATION", "FULL_SCAN", "INDEX_SCAN"};

            return std::string(names[x]);
        }

        const std::string getName(JoinAlgorithm x) {
            const char * const names[] = {"SORT_MERGE_JOIN", "NESTED_LOOP_JOIN", "HASH_JOIN"};

            return std::string(names[x]);
        }
        
        const std::string getName(JoinType x) {
            const char * const names[] = {"JOIN", "LEFT_SEMI_JOIN", 
            "RIGHT_SEMI_JOIN", "LEFT_ANTI_SEMI_JOIN", "RIGHT_ANTI_SEMI_JOIN", 
        ...
        const std::string getName(PositionListOperation x) {
            const char * const names[] = {"POSITIONLIST_INTERSECTION", "POSITIONLIST_UNION"};
            return std::string(names[x]);
        }
        ...
        const std::string getName(GPUBufferManagementStrategy x) {
            const char * const names[] = {"LEAST_RECENTLY_USED", "LEAST_FREQUENTLY_USED", "DISABLED_GPU_BUFFER"};
            return std::string(names[x]);
        }

        const std::string getName(TableLoaderMode x) {
            const char * const names[] = {"LOAD_ALL_COLUMNS", "LOAD_NO_COLUMNS", "LOAD_ELEMENTARY_COLUMNS"};
            return std::string(names[x]);
        }
    };
