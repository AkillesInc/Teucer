...
namespace CoGaDB{
    ...
    if(cudaSuccess!=cudaMemGetInfo(&available_memory_in_bytes, &total_memory_in_bytes)){
            //std::cerr << "Error while trying to retrieve memory size information! Aborting..." << std::endl;
            return 0;
     }
    ...
    void printGPUStatus(){
     std::cout << "Number of GPU Cores: " << getNumberofGPUMultiprocessors() <<  std::endl;
     std::cout << "Total GPU RAM: " << getTotalGPUMemorySizeInByte() <<  std::endl;
     std::cout << "Available GPU RAM: " << getFreeGPUMemorySizeInByte() <<  std::endl;
    }
    ...
    bool printGPUs(std::ostream& out){
        ...
        for(int i=0;i<device_count;++i){
            ...
            out << "\t" << "Global Memory Size (GB): " << double(prop.totalGlobalMem)/(1024*1024*1024) << std::endl;
            out << "\t" << "#Multiprocessors: " << prop.multiProcessorCount << std::endl;
            out << "\t" << "Clockrate (Mhz): " << double(prop.clockRate)/(1000) << std::endl;
            out << "\t" << "Supports Concurrent Kernels: " << prop.concurrentKernels << std::endl;
            out << "\t" << "asyncEngineCount: " << prop.asyncEngineCount << std::endl;
            out << "\t" << "Kernel Timeout Enabled: " << prop.kernelExecTimeoutEnabled << std::endl;
            out << "\t" << "Overlap Copy and Computation: " << prop.deviceOverlap << std::endl;
            out << "\t" << "ECC Enabled: " << prop.ECCEnabled << std::endl;
        }
  ...
  int  check_for_CUDA_devices(){
    ...
    void printGPUStatus(){
     std::cout << "Cannot determine status of GPUs! CoGaDB was compiled without GPU support!" <<  std::endl;
    }

    bool printGPUs(std::ostream& out){
        out << "Cannot list GPUs! CoGaDB was compiled without GPU support!" <<  std::endl;
        return true;
    }
...
HardwareDetector::HardwareDetector() : dev_specs_(), processing_device_id_(hype::PD0){
...
size_t HardwareDetector::getFreeMemorySizeInByte(const hype::ProcessingDeviceMemoryID& mem_id) const{
    for(size_t i=0;i<this->dev_specs_.size();++i){
        if(dev_specs_[i].getMemoryID()==mem_id) return dev_specs_[i].getAvailableMemoryCapacity();
    }
    COGADB_ERROR("Could not find Processing Device with memory ID " << (int) mem_id,"");
    return 0;
}
size_t HardwareDetector::getTotalMemorySizeInByte(const hype::ProcessingDeviceMemoryID& mem_id) const{
    for(size_t i=0;i<this->dev_specs_.size();++i){
        if(dev_specs_[i].getMemoryID()==mem_id) return dev_specs_[i].getTotalMemoryCapacity();
    }
    COGADB_ERROR("Could not find Processing Device with memory ID " << (int) mem_id,"");
    return 0;
}
...
bool HardwareDetector::detectGPUs(){
        ...
        if(nDevices>0)
            return createGPUDevices(1); //nDevices);
...
size_t HardwareDetector::getNumberOfProcessorsForDeviceType(hype::ProcessingDeviceType type) const{
...
bool HardwareDetector::createCPUDevices(unsigned int number_of_devices){
        ...
        for(unsigned int i=0;i<number_of_devices; ++i){
            ...
            dev_specs_.push_back(DeviceSpecification((hype::ProcessingDeviceID)processing_device_id_++,hype::CPU,hype::PD_Memory_0)); //one host CPU
        }
        return true;
...
//        }
        ...
        for(unsigned int i=0;i<number_of_devices;++i){
            assert(processing_device_id_<hype::PD_DMA0);
            dev_specs_.push_back(DeviceSpecification((hype::ProcessingDeviceID)processing_device_id_++,hype::GPU, hype::PD_Memory_1, &getFreeGPUMemorySizeInByte, getTotalGPUMemorySizeInByte())); //one dedicated GPU  
        }
        return true;
...
bool HardwareDetector::parseConfigFile(){
		...
		desc.add_options()
			 ("number_of_cpus", boost::program_options::value<unsigned int>(), "set the number of virtual cpu processing devices")
			 ("number_of_dedicated_gpus", boost::program_options::value<unsigned int>(), "set the number of virtual dedicated gpu processing devices")
		;
		...
		if(!config_file.good()){
			...
			//if (!quiet) std::cout << "[HyPE]: No Configuration File 'hype.conf' found! Parsing environment variables..." << std::endl;
			...
			if (!quiet) std::cout << "Parsing Configuration File 'hardware_specification.conf'..." << std::endl;
		...
		if (vm.count("number_of_cpus")) {
			if (!quiet) std::cout << "number_of_cpus: " 
		 << vm["number_of_cpus"].as<unsigned int>() << "\n";
			number_of_cpus=vm["number_of_cpus"].as<unsigned int>();
			if(!this->createCPUDevices(number_of_cpus)){
                            COGADB_FATAL_ERROR("Invalid value for variable number_of_cpus: " << number_of_cpus,"");
                        }
		} else {
			if (!quiet) std::cout << "number_of_cpus was not specified, perform automatic detection of available CPUs...\n";
                        this->detectCPUs();
		}
                
		if (vm.count("number_of_dedicated_gpus")) {
			if (!quiet) std::cout << "number_of_dedicated_gpus: " 
                         << vm["number_of_dedicated_gpus"].as<unsigned int>() << "\n";
			number_of_dedicated_gpus=vm["number_of_dedicated_gpus"].as<unsigned int>();    
			if(!this->createGPUDevices(number_of_dedicated_gpus)){
                            COGADB_FATAL_ERROR("Invalid value for variable number_of_dedicated_gpus: " << number_of_dedicated_gpus,"");
                        }
		} else {
			if (!quiet) std::cout << "number_of_dedicated_gpus was not specified, perform automatic detection of available GPUs\n";
                        this->detectGPUs();
		}
