...
void print_pcm_measurements(PCM * m,
    ...
    out << " FREQ  : relation to nominal CPU frequency='unhalted clock ticks'/'invariant timer ticks' (includes Intel Turbo Boost)" << "\n";
    if (cpu_model != PCM::ATOM) out << " AFREQ : relation to nominal CPU frequency while in active state (not in power-saving C state)='unhalted clock ticks'/'invariant timer ticks while in C0-state'  (includes Intel Turbo Boost)" << "\n";
    if (cpu_model != PCM::ATOM) out << " L3MISS: L3 cache misses " << "\n";
    if (cpu_model == PCM::ATOM)
        out << " L2MISS: L2 cache misses " << "\n";
    else
        out << " L2MISS: L2 cache misses (including other core's L2 cache *hits*) " << "\n";
    if (cpu_model != PCM::ATOM) out << " L3HIT : L3 cache hit ratio (0.00-1.00)" << "\n";
    out << " L2HIT : L2 cache hit ratio (0.00-1.00)" << "\n";
    if (cpu_model != PCM::ATOM) out << " L3CLK : ratio of CPU cycles lost due to L3 cache misses (0.00-1.00), in some cases could be >1.0 due to a higher memory latency" << "\n";
    if (cpu_model != PCM::ATOM) out << " L2CLK : ratio of CPU cycles lost due to missing L2 cache but still hitting L3 cache (0.00-1.00)" << "\n";
    if (cpu_model != PCM::ATOM) out << " READ  : bytes read from memory controller (in GBytes)" << "\n";
    if (cpu_model != PCM::ATOM) out << " WRITE : bytes written to memory controller (in GBytes)" << "\n";
    if (m->memoryIOTrafficMetricAvailable()) out << " IO    : bytes read/written due to IO requests to memory controller (in GBytes); this may be an over estimate due to same-cache-line partial requests" << "\n";
    out << " TEMP  : Temperature reading in 1 degree Celsius relative to the TjMax temperature (thermal headroom): 0 corresponds to the max temperature" << "\n";
    out << "\n";
    out << "\n";
    out.precision(2);
    out << std::fixed;
    ...
    else
    {
        if (m->memoryIOTrafficMetricAvailable())
            out << " Core (SKT) | EXEC | IPC  | FREQ  | AFREQ | L3MISS | L2MISS | L3HIT | L2HIT | L3CLK | L2CLK  | READ  | WRITE |  IO   | TEMP" << "\n" << "\n";
        else
            out << " Core (SKT) | EXEC | IPC  | FREQ  | AFREQ | L3MISS | L2MISS | L3HIT | L2HIT | L3CLK | L2CLK  | READ  | WRITE | TEMP" << "\n" << "\n";
    }
    ...
    if (show_core_output)
    {
        for (uint32 i = 0; i < m->getNumCores(); ++i)
        {
            ...
            if (cpu_model != PCM::ATOM)
            {
                out << " " << std::setw(3) << i << "   " << std::setw(2) << m->getSocketId(i) <<
        ...
        if (!(m->getNumSockets() == 1 && cpu_model == PCM::ATOM))
        {
            ...
            for (uint32 i = 0; i < m->getNumSockets(); ++i)
            {
                ...
                if (m->memoryTrafficMetricsAvailable())
                    out << "    " << getBytesReadFromMC(sktstate1[i], sktstate2[i]) / double(1024ULL * 1024ULL * 1024ULL) <<
                    "    " << getBytesWrittenToMC(sktstate1[i], sktstate2[i]) / double(1024ULL * 1024ULL * 1024ULL);
                else
                    out << "     N/A     N/A";

                if (m->memoryIOTrafficMetricAvailable())
                    out << "    " << getIORequestBytesFromMC(sktstate1[i], sktstate2[i]) / double(1024ULL * 1024ULL * 1024ULL);
    ...
    if (show_system_output)
    {
        if (cpu_model != PCM::ATOM)
        {
            ...
            if (m->memoryTrafficMetricsAvailable())
                out << "    " << getBytesReadFromMC(sstate1, sstate2) / double(1024ULL * 1024ULL * 1024ULL) <<
                "    " << getBytesWrittenToMC(sstate1, sstate2) / double(1024ULL * 1024ULL * 1024ULL);
            else
                out << "     N/A     N/A";

            if (m->memoryIOTrafficMetricAvailable())
                out << "    " << getIORequestBytesFromMC(sstate1, sstate2) / double(1024ULL * 1024ULL * 1024ULL);
    ...
    if (show_system_output)
    {
        ...
        if (m->isCoreCStateResidencySupported(s))
            out << " C" << s << " core residency: " << (getCoreCStateResidency(s, sstate1, sstate2)*100.) << " %;";
        out << "\n";
        for (int s = 0; s <= PCM::MAX_C_STATE; ++s)
        if (m->isPackageCStateResidencySupported(s))
            out << " C" << s << " package residency: " << (getPackageCStateResidency(s, sstate1, sstate2)*100.) << " %;";
        out << "\n";
        ...
        if (m->getNumSockets() > 1) // QPI info only for multi socket systems
        {
            ...
            for (uint32 i = 0; i < m->getNumSockets(); ++i)
            {
                ...
                if (m->qpiUtilizationMetricsAvailable())
                {
                    out << "|  ";
                    for (uint32 l = 0; l < qpiLinks; ++l)
                        out << std::setw(3) << std::dec << int(100. * getIncomingQPILinkUtilization(i, l, sstate1, sstate2)) << "%   ";
                }
    ...
    if (show_system_output)
    {
        ...
        if (m->getNumSockets() > 1) // QPI info only for multi socket systems
            out << "Total QPI incoming data traffic: " << unit_format(getAllIncomingQPILinkBytes(sstate1, sstate2)) << "     QPI data traffic/Memory controller traffic: " << getQPItoMCTrafficRatio(sstate1, sstate2) << "\n";
    }
    ...
    if (show_socket_output)
    {
        if (m->getNumSockets() > 1 && (m->outgoingQPITrafficMetricsAvailable())) // QPI info only for multi socket systems
        {
            ...
            for (uint32 i = 0; i < m->getNumSockets(); ++i)
            {
                ...
                out << "|  ";
                for (uint32 l = 0; l < qpiLinks; ++l)
                    out << std::setw(3) << std::dec << int(100. * getOutgoingQPILinkUtilization(i, l, sstate1, sstate2)) << "%   ";

                out << "\n";
            }

            out << "----------------------------------------------------------------------------------------------" << "\n";
            out << "Total QPI outgoing data and non-data traffic: " << unit_format(getAllOutgoingQPILinkBytes(sstate1, sstate2)) << "\n";
        }
    }
    if (show_socket_output)
    {
        if (m->packageEnergyMetricsAvailable())
        {
            ...
            for (uint32 i = 0; i < m->getNumSockets(); ++i)
            {
                out << " SKT   " << std::setw(2) << i << " package consumed " << getConsumedJoules(sktstate1[i], sktstate2[i]) << " Joules\n";
            }
            out << "----------------------------------------------------------------------------------------------" << "\n";
            out << " TOTAL:                    " << getConsumedJoules(sstate1, sstate2) << " Joules\n";
        }
        if (m->dramEnergyMetricsAvailable())
        {
            ...
            for (uint32 i = 0; i < m->getNumSockets(); ++i)
            {
                out << " SKT   " << std::setw(2) << i << " DIMMs consumed " << getDRAMConsumedJoules(sktstate1[i], sktstate2[i]) << " Joules\n";
            }
            out << "----------------------------------------------------------------------------------------------" << "\n";
            out << " TOTAL:                  " << getDRAMConsumedJoules(sstate1, sstate2) << " Joules\n";
        }
    }
}
