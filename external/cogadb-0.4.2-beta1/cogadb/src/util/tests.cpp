...
namespace CoGaDB{
    ...
    void SetupTestConfiguration(ClientPtr client){
        std::string path_to_config_file=getPathToHomeConfigDir()+"test_config.coga";            
        ...
        if(!iStartUpFile.good()) {
            ...
            if(oStartUpFile.is_open()) {
                 ...
                 std::cout << "Please enter the location of a star schema database with scale factor 1: " << 
                    std::endl;
                ...
                oStartUpFile << "set path_to_ssb_sf1_database=" << databaseLocation 
                 ...
                 std::cout << "Please enter the location of a TPC-H database with scale factor 1: " << 
                    std::endl;
            ...
            std::cout << "Successfully configured pathes to reference databases. "
                      << "Stored config in '" << path_to_config_file << "'..." << std::endl;
        }
    ...
    bool loadReferenceDatabaseStarSchemaScaleFactor1(ClientPtr client){
        ...
        SetupTestConfiguration(client);
            if(VariableManager::instance()
                    .getVariableValueString("path_to_ssb_sf1_database")==""){
                COGADB_FATAL_ERROR("Could not find reference star schema database of scale factor 1. "
                        << "Please specify the path to such a database in the variable 'path_to_ssb_sf1_database'"
                        << " in the file '" << path_to_config_file << "'","");
            }
            loadTablesFromDirectory(VariableManager::instance()
                    .getVariableValueString("path_to_ssb_sf1_database"),std::cout);
            if(!getTablebyName("LINEORDER")){
                COGADB_FATAL_ERROR("Did not find table 'LINEORDER', variable "
                        << "'path_to_ssb_sf1_database' does not seem to point to star schema database!"
            ...
            if(getTablebyName("LINEORDER")->getNumberofRows()!=6001171){
                COGADB_FATAL_ERROR("Star schema benchmark queries require a scale factor 1 reference database!","");
            }
            return true;
    }
    ...
    bool loadReferenceDatabaseTPCHScaleFactor1(ClientPtr client){
        ...
        SetupTestConfiguration(client);
            if(VariableManager::instance()
                    .getVariableValueString("path_to_tpch_sf1_database")==""){
                COGADB_FATAL_ERROR("Could not find reference TPC-H database of scale factor 1. "
                        << "Please specify the path to such a database in the variable 'path_to_tpch_sf1_database'"
                        << " in the file '" << path_to_config_file << "'","");
            }
            loadTablesFromDirectory(VariableManager::instance()
                    .getVariableValueString("path_to_tpch_sf1_database"),std::cout);
            if(!getTablebyName("LINEITEM")){
                COGADB_FATAL_ERROR("Did not find table 'LINEITEM', variable "
                        << "'path_to_tpch_sf1_database' does not seem to point to TPC-H database!"
                        << " Edit the file '" << path_to_config_file 
                        << "' and fix the path for variable 'path_to_tpch_sf1_database'","");
            }
            if(getTablebyName("LINEITEM")->getNumberofRows()!=6001215){
                COGADB_FATAL_ERROR("TPC-H benchmark queries require a scale factor 1 reference database!","");
            }
            return true;
    }    
