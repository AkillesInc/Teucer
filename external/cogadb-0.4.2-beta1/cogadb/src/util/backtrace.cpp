...
namespace CoGaDB{
    ...
    std::string demangleFunctionName(const std::string& line){
        ...
        if(strs.size()==4){
            ...
            char *demangledName = abi::__cxa_demangle(function_name.c_str(), NULL, NULL, &status );
    ...
    void printStackTrace(std::ostream& out){
           ...
           const size_t MAXIMAL_NUMBER_OF_STACKFRAMES = 50;
           
           void *buffer[MAXIMAL_NUMBER_OF_STACKFRAMES];
           char **strings;

           const int number_of_stack_frames = backtrace(buffer, MAXIMAL_NUMBER_OF_STACKFRAMES);
           assert(number_of_stack_frames<MAXIMAL_NUMBER_OF_STACKFRAMES);
           
           strings = backtrace_symbols(buffer, number_of_stack_frames);
           if (strings == NULL) {
               out << "Could not produce backtrace: backtrace_symbols failed!" << std::endl;
               return;
           }

           for (size_t i = 0; i < number_of_stack_frames; ++i){
