...
using namespace std;
        ...
        template <typename T>
        EquiHeightHistogram<T>::EquiHeightHistogram(ColumnBaseTyped<T>* col, unsigned int number_of_buckets) : buckets_(number_of_buckets), number_of_values_per_bucket_(){
            
//            if(!col) return;
//            typename ColumnBaseTyped<T>::TypedVectorPtr plain_value_column =  col->copyIntoPlainVector();
//            std::sort(plain_value_column->begin(), plain_value_column->end());
////            ColumnPtr copy = col->copy();
////            PositionListPtr tids = copy->sort();
////            ColumnPtr sorted = copy->gather(tids);
////            shared_pointer_namespace::shared_ptr<Column<T> > plain_value_column = shared_pointer_namespace::dynamic_pointer_cast<Column<T> >(sorted);
...
//            }
        ...
        template <typename T>
        EquiHeightHistogram<T>::EquiHeightHistogram(const T* array, size_t array_size, unsigned int number_of_buckets) 
        : buckets_(number_of_buckets), number_of_distinct_keys_per_bucket_(number_of_buckets), 
          number_of_values_per_bucket_(0), number_of_values_in_column_(0){
            ...
            if(array && array_size>0){
                ...
                for (unsigned int i = 0; i < array_size; ++i, ++value_counter) {
                    if(i<array_size-1 && array[i]<array[i+1]) distinct_value_counter++;
                    //are we at end of a bucket, or are we at the end of the last bucket?
                    if(value_counter==chunk_size || i+value_counter==array_size){
                        this->number_of_distinct_keys_per_bucket_[bucket_id]=distinct_value_counter;
...
//                            buckets_[bucket_id].first=buckets_[bucket_id-1].second;
        ...
        template <typename T>
        size_t EquiHeightHistogram<T>::countNumberOfExpectedMatches(T value, ValueComparator comp) const{
...
//            cout << "Apply Predicate " << pred.toString() << " to histogram..." << endl;
            cout << "Apply Predicate 'Column" << util::getName(comp) << value << "' to histogram..." << endl;
            cout << "start bucket id: " << start_bucket_id 
                 << " (" << buckets_[start_bucket_id].first << ", " << buckets_[start_bucket_id].second << ")" << endl;
            
            cout << "end bucket id: " << end_bucket_id 
                 << " (" << buckets_[end_bucket_id].first << ", " << buckets_[end_bucket_id].second << ")" << endl;
            
            double estimated_number_of_matches=0;
            
            if(comp==EQUAL){
                for (size_t i = start_bucket_id; i < end_bucket_id; ++i) {
                    double number_of_distinct_values_in_bucket=number_of_distinct_keys_per_bucket_[i];
                    
                    if(i==start_bucket_id){
                        //bucket with first occurence of value
                        //assume equal distribution of values in a single bucket
                         estimated_number_of_matches+=ceil(double(number_of_values_per_bucket_)/number_of_distinct_values_in_bucket);
            ...
            //sum up number of values per bucket for all buckets in range [bucket0,end_bucked_id]
            //this is for predicates such as x<10 or y<=15
            }else if(comp==LESSER || comp==LESSER_EQUAL){
                for (size_t i = 0; i < end_bucket_id; ++i) {
                    estimated_number_of_matches+=number_of_values_per_bucket_;
                }
            //sum up number of values per bucket for all buckets in range [start_bucket_id,last_bucket]
            //this is for predicates such as x>10 or y>=15
        ...
        template <typename T>
        double EquiHeightHistogram<T>::getEstimatedSelectivity(const Predicate& pred) const{
            
            if(pred.getPredicateType()==ValueValuePredicate){
                COGADB_ERROR("Cannot estimate selectivity of predicate '" << pred.toString() << "' using a single histogram!","");
                return 0.1;
            }
            ...
            cout << "Apply Predicate 'Column" << util::getName(comp) << value << "' to histogram..." << endl;
            cout << "start bucket id: " << start_bucket_id 
                 << "(" << buckets_[start_bucket_id].first << ", " << buckets_[end_bucket_id].second << ")" << endl;
 ...
 // = buckets_[i].second-buckets_[i].first;
        ...
        template <typename T>
        double EquiHeightHistogram<T>::getEstimatedSelectivity(const Predicate& lower_pred, const Predicate& upper_pred) const{
            ...
            if((lower_pred.getValueComparator()==LESSER || lower_pred.getValueComparator()==LESSER_EQUAL) 
            && (upper_pred.getValueComparator()==LESSER || upper_pred.getValueComparator()==LESSER_EQUAL)){
               
                estimated_matches_in_interval = countNumberOfExpectedMatches(lower_value, lower_pred.getValueComparator());

            }else if((lower_pred.getValueComparator()==GREATER || lower_pred.getValueComparator()==GREATER_EQUAL) 
            && (upper_pred.getValueComparator()==GREATER || upper_pred.getValueComparator()==GREATER_EQUAL)){
                
                estimated_matches_in_interval = countNumberOfExpectedMatches(upper_value, upper_pred.getValueComparator());
            ...
            && (upper_pred.getValueComparator()==LESSER || upper_pred.getValueComparator()==LESSER_EQUAL)){
                ...
                size_t estimated_matches_lower_pred = countNumberOfExpectedMatches(lower_value, comp);
                size_t estimated_matches_upper_pred = countNumberOfExpectedMatches(upper_value, upper_pred.getValueComparator());
                assert(estimated_matches_upper_pred>=estimated_matches_lower_pred);
                estimated_matches_in_interval = estimated_matches_upper_pred-estimated_matches_lower_pred;
                     
            }else{
                COGADB_FATAL_ERROR("Invalid value range: " << lower_pred.toString() << " AND " << upper_pred.toString(),"");
            }

            cout << "Estimated matches for range query '" << lower_pred.toString() << " AND " << upper_pred.toString() << "': " << estimated_matches_in_interval << endl;
            if(number_of_values_in_column_>0)
        ...
        template <typename T>
        SelectivityEstimationStrategy EquiHeightHistogram<T>::getSelectivityEstimationStrategy() const{
        ...
        std::string EquiHeightHistogram<T>::toString() const{
            ...
            for (size_t i = 0; i < buckets_.size(); i++) {
                ss << "\tBucket " << i << ": Begin " << buckets_[i].first 
                   ...
                   << " Distinct Values: " << number_of_distinct_keys_per_bucket_[i] << endl;
            }
            return ss.str();
        }
