...
using namespace std;
	boost::mutex global_statistics_manager_mutex;
        ...
        std::pair<bool, double> StatisticsManager::getValue(const std::string& metric) const{
	    ...
	    Statistics::iterator it=statistics_.find(metric);
            if(it==statistics_.end()){
	        statistics_.insert( std::make_pair(metric, value)); //assume initialization with 0    
                //if(!quiet) std::cout << "[StatisticsManager]: added variable: " << metric << std::endl;
            }else{
                it->second+=value;
                //if(!quiet) std::cout << "[StatisticsManager]: added value '" << value << "' to variable: " << metric << std::endl;
            }
        }
        ...
        std::string StatisticsManager::toString() const{
            ...
            //ss << "NUMBER_OF_EXECUTED_GPU_OPERATORS: " << getValue(NUMBER_OF_EXECUTED_GPU_OPERATORS).second << std::endl;
            //ss << "NUMBER_OF_ABORTED_GPU_OPERATORS: " << getValue(NUMBER_OF_ABORTED_GPU_OPERATORS).second << std::endl;
	    Statistics::const_iterator it;
            ...
            ss << "NOTE: The counters for join indexes are redundant, meaning " 
               ...
               << "However, it helps us to know"  << std::endl << "how much cost were caused by fetch joins " 
