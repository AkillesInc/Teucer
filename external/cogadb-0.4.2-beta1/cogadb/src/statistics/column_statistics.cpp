...
/* \brief checks for dense value array startig with 0, such as 0, 1, 2, 3, ..., N-1, N*/
template <typename T>
bool is_dense_value_array_starting_with_zero(T* array, size_t array_size){
    ...
    : is_sorted_ascending_(false), is_sorted_descending_(false), 
      is_dense_value_array_starting_with_zero_(false), statistics_up_to_date_(false), sel_est_(){
    ...
    template <typename T>
    bool ExtendedColumnStatistics<T>::computeStatistics(const boost::any& array, size_t array_size){
    ...
    template <typename T>
    bool ExtendedColumnStatistics<T>::computeStatistics(const T* array, size_t array_size){
        
        if(!array) return false;
        if(array_size==0) return false;
        
        is_sorted_ascending_=is_sorted(array,array+array_size, less<T>());
        is_sorted_descending_=is_sorted(array,array+array_size, greater<T>());
        is_dense_value_array_starting_with_zero_ = is_dense_value_array_starting_with_zero(array,array_size);
        ...
        sel_est_=SelectivityEstimator::createSelectivityEstimator(EQUI_HEIGHT_HISTOGRAM, array, array_size);
        return true;
    }
    ...
    const std::string ExtendedColumnStatistics<T>::toString() const{
        std::stringstream ss;
        ss << "Dense Value Column Starting with zero: " << is_dense_value_array_starting_with_zero_ << endl;
    ...
    template <>
    bool ExtendedColumnStatistics<char*>::computeStatistics(const C_String* array, size_t array_size){
    ...
    template <>
    bool ExtendedColumnStatistics<std::string>::computeStatistics(const std::string* array, size_t array_size){
        ...
        sel_est_=SelectivityEstimatorPtr(new EquiHeightHistogram<std::string>(array,array_size)); 
        return true;
    }
    ...
    bool ExtendedColumnStatistics<T>::load(const std::string& path_to_column){
            ...
            std::ifstream infile(path_to_statistics.c_str(), std::ios_base::binary | std::ios_base::in);
            if(!infile.good()) return false;
            boost::archive::binary_iarchive ia(infile);
            SelectivityEstimationStrategy sel_est_strat;
...
//            ia >> sel_est_strat;
    ...
    bool ExtendedColumnStatistics<T>::store(const std::string& path_to_column){
            ...
            std::ofstream outfile(path_to_statistics.c_str(), std::ios_base::binary | std::ios_base::out | std::ios_base::trunc);
...
//            if(this->sel_est_){
//                /* \todo Write Selectivity Estimator data to disk. */
//                oa << sel_est_->getSelectivityEstimationStrategy();
//            }else{
//                oa << NO_SELECTIVITY_ESTIMATION;
//            }
            outfile.flush();
            outfile.close();
            return true;
    ...
    bool ExtendedColumnStatistics<char*>::load(const std::string& path_to_column){
        COGADB_WARNING("Called unimplemented function! Will not load Column statistics for Column<char*>!","");
        return false;
    }
    
    template <>
    bool ExtendedColumnStatistics<char*>::store(const std::string& path_to_column){
        COGADB_WARNING("Called unimplemented function! Will not store Column statistics for Column<char*>","");
        return false;  
    }
