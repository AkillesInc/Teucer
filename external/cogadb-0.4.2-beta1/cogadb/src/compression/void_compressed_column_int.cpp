...
namespace CoGaDB {
    ...
    template<class T>
    VoidCompressedColumnNumber<T>::VoidCompressedColumnNumber(const std::string &name, AttributeType db_type)
            : CompressedColumn<T>(name, db_type, VOID_COMPRESSED_NUMBER), number_of_rows_(0) {
    ...
    bool VoidCompressedColumnNumber<T>::insert(InputIterator first, InputIterator last) {
    ...
    template<class T>
    const typename ColumnBaseTyped<T>::DenseValueColumnPtr VoidCompressedColumnNumber<T>::copyIntoDenseValueColumn(
    ...
    template<>
    const typename ColumnBaseTyped<int32_t>::DenseValueColumnPtr VoidCompressedColumnNumber<int32_t>::copyIntoDenseValueColumn(
            const ProcessorSpecification &proc_spec) const {

        assert(proc_spec.proc_id == hype::PD0);

        typedef typename ColumnBaseTyped<int32_t>::DenseValueColumnPtr DenseValueColumnPtr;
    ...
    template<>
    const typename ColumnBaseTyped<TID>::DenseValueColumnPtr VoidCompressedColumnNumber<TID>::copyIntoDenseValueColumn(
            const ProcessorSpecification &proc_spec) const {

        assert(proc_spec.proc_id == hype::PD0);

        typedef typename ColumnBaseTyped<TID>::DenseValueColumnPtr DenseValueColumnPtr;
...
//    const ColumnPtr VoidCompressedColumnNumber<T>::gather(PositionListPtr tid_list, const GatherParam&){
//        
//    }

    template<class T>
    const PositionListPtr VoidCompressedColumnNumber<T >::selection(const SelectionParam &param) {
        COGADB_FATAL_ERROR("Not implemented for any type but int.", "");
    }

    template<>
    const PositionListPtr VoidCompressedColumnNumber<TID >::selection(const SelectionParam &param) {
    ...
    template<class T>
    bool VoidCompressedColumnNumber<T>::update(TID tid, const boost::any &new_value) {
        return false;
    }

    template<class T>
    bool VoidCompressedColumnNumber<T>::update(PositionListPtr tids, const boost::any &new_value) {
        return false;
    }
    ...
    template<class T>
    bool VoidCompressedColumnNumber<T>::store_impl(const std::string& path, boost::archive::binary_oarchive& oa){
        oa << number_of_rows_;
        return true;
    }

    template<class T>
    bool VoidCompressedColumnNumber<T>::load_impl(const std::string& path, boost::archive::binary_iarchive& ia){
        ia >> number_of_rows_;
        return true;
    }
    ...
    COGADB_INSTANTIATE_CLASS_TEMPLATE_FOR_SUPPORTED_TYPES(VoidCompressedColumnNumber);
};
