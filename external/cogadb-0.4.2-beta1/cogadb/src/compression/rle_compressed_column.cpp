...
namespace CoGaDB {
    ...
    template<class T>
    RLECompressedColumn<T>::RLECompressedColumn(const std::string &name, AttributeType db_type) : CompressedColumn<T>(
            name, db_type, RUN_LENGTH_COMPRESSED), values_(), count_(), last_compressed_value(), _last_lookup_index(0),
    ...
    template<>
    inline bool RLECompressedColumn<std::string>::insert(const std::string &new_value) {
    ...
    template<class T>
    const typename ColumnBaseTyped<T>::DenseValueColumnPtr RLECompressedColumn<T>::copyIntoDenseValueColumn(
...
//        }
        ...
        for (size_t i = 0; i < number_of_threads; ++i) {
            partial_results.push_back(DenseValueColumnPtr(new DenseValueColumn(this->getName(), this->getType())));
        }
        ...
        DenseValueColumnPtr result(new DenseValueColumn(this->getName(), this->getType()));
        // TODO size can be determined faster by just looking up the very last count_ TID value
        ...
        for (size_t i = 0; i < number_of_threads; i++) {
            if (thread_result_sizes[i] > 0) {
                int j, k;
                for(k = 0, j = write_indexes[i]; k < thread_result_sizes[i], j < write_indexes[i] + thread_result_sizes[i]; ++j, ++k) {
    ...
    template<class T>
    const ColumnPtr RLECompressedColumn<T>::gather(PositionListPtr tid_list, const GatherParam &param) {
        ...
        DenseValueColumnPtr materialized_column = RLECompressedColumn<T>::copyIntoDenseValueColumn(param.proc_spec);
    ...
    template<class T>
    const PositionListPtr RLECompressedColumn<T>::selection(const SelectionParam &param) {
    ...
    template<class T>
    bool RLECompressedColumn<T>::store_impl(const std::string &path, boost::archive::binary_oarchive &oa) {
        oa << values_;
        oa << count_;
        return true;
    }
    ...
    template<class T>
    bool RLECompressedColumn<T>::load_impl(const std::string &path, boost::archive::binary_iarchive &ia) {
        ia >> values_;
        ia >> count_;
        return true;
    }
    ...
    template<>
    bool RLECompressedColumn<char*>::store_impl(const std::string& path, boost::archive::binary_oarchive& oa){
        return false;
    }

    template<>
    bool RLECompressedColumn<char*>::load_impl(const std::string& path, boost::archive::binary_iarchive& ia){
        return false;
    }
    ...
    template<>
    inline std::string &RLECompressedColumn<std::string>::operator[](const TID index) {
        //find value
