...
namespace CoGaDB {
    ...
    template<class T>
    const typename DictionaryCompressedColumn<T>::Dictionary& DictionaryCompressedColumn<T>::getDictionary() const throw () {
        return dictionary_;
    }

    template<class T>
    const std::vector<uint32_t>& DictionaryCompressedColumn<T>::getCompressedValues() const throw () {
    ...
    template<class T>
    std::pair<bool, uint32_t> DictionaryCompressedColumn<T>::getDictionaryID(const T& value) const {
    ...
    template<class T>
    DictionaryCompressedColumn<T>::DictionaryCompressedColumn(const std::string& name, AttributeType db_type, const hype::ProcessingDeviceMemoryID& mem_id) : CompressedColumn<T>(name, db_type, DICTIONARY_COMPRESSED), ids_(new IDColumn(name + "_ENCODING_IDS", UINT32, mem_id)), dictionary_(), reverse_lookup_vector_(), maximal_id_(0) {

    }

    template<class T>
    DictionaryCompressedColumn<T>::DictionaryCompressedColumn(const DictionaryCompressedColumn<T>& other)
    : CompressedColumn<T>(other.getName(), other.getType(), other.getColumnType()), ids_(boost::dynamic_pointer_cast<IDColumn>(other.ids_->copy())), dictionary_(other.dictionary_), reverse_lookup_vector_(other.reverse_lookup_vector_), maximal_id_(other.maximal_id_) {
        assert(ids_ != NULL);
    }

    template<class T>
    DictionaryCompressedColumn<T>& DictionaryCompressedColumn<T>::operator=(const DictionaryCompressedColumn<T>& other) {
    ...
    template<class T>
    const ColumnPtr DictionaryCompressedColumn<T>::copy(const hype::ProcessingDeviceMemoryID& mem_id) const {
        ...
        DictionaryCompressedColumn<T>* new_col = new DictionaryCompressedColumn<T>(this->getName(), this->getType(), mem_id);
    ...
    template<class T>
    const typename ColumnBaseTyped<T>::DenseValueColumnPtr DictionaryCompressedColumn<T>::copyIntoDenseValueColumn(const ProcessorSpecification& proc_spec) const {
...
//        assert(proc_spec.proc_id == hype::PD0);
        ...
        DictionaryCompressedColumn<T>* this_col = const_cast<DictionaryCompressedColumn<T>*> (this);
    ...
    template<class T>
    const ColumnPtr DictionaryCompressedColumn<T>::gather(PositionListPtr tid_list, const GatherParam& param) {
        DictionaryCompressedColumn<T>* result = new DictionaryCompressedColumn<T>(this->name_, this->db_type_);
    ...
    template<class T>
    const ColumnGroupingKeysPtr DictionaryCompressedColumn<T>::createColumnGroupingKeys(const ProcessorSpecification& proc_spec) const {
    ...
    template<class T>
    bool DictionaryCompressedColumn<T>::store_impl(const std::string& path_to_table_dir, boost::archive::binary_oarchive& oa){
        oa << dictionary_;
    ...
    template<>
    bool DictionaryCompressedColumn<char*>::store_impl(const std::string& path_to_table_dir, boost::archive::binary_oarchive& oa){
        COGADB_FATAL_ERROR("","");
        return false;
    }
    ...
    template<class T>
    bool DictionaryCompressedColumn<T>::load_impl(const std::string& path_to_table_dir, boost::archive::binary_iarchive& ia){
        ia >> dictionary_;
        ia >> reverse_lookup_vector_;
        ia >> this->maximal_id_;
        return ids_->load(path_to_table_dir);
    }

    template<>
    bool DictionaryCompressedColumn<char*>::load_impl(const std::string& path_to_table_dir, boost::archive::binary_iarchive& ia){
        COGADB_FATAL_ERROR("","");
        return false;
    }    
    ...
    template<class T>
    hype::ProcessingDeviceMemoryID DictionaryCompressedColumn<T>::getMemoryID() const {
        //column is dormant in memory where ever our ids_ column is
        return ids_->getMemoryID();
    }

    template<class T>
    const PositionListPtr DictionaryCompressedColumn<T>::selection(const SelectionParam& param) {

        if (param.pred_type == ValueConstantPredicate && (param.comp == EQUAL || param.comp == UNEQUAL)) {
            ...
            ColumnPtr matching_ids = getMatchingIDsFromDictionary<T>(boost::any_cast<std::string>(param.value), this->dictionary_);
    ...
    template<class T>
    const BitmapPtr DictionaryCompressedColumn<T>::bitmap_selection(const SelectionParam& param) {
    ...
    template<>
    inline size_t DictionaryCompressedColumn<std::string>::getSizeinBytes() const throw () {
...
//    template class DictionaryCompressedColumn<std::string>;
    COGADB_INSTANTIATE_CLASS_TEMPLATE_FOR_SUPPORTED_TYPES(DictionaryCompressedColumn);
