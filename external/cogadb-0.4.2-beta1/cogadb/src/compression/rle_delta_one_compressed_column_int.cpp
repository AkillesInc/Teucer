...
namespace CoGaDB {
    ...
    template<class T>
    RLEDeltaOneCompressedColumnNumber<T>::RLEDeltaOneCompressedColumnNumber(const std::string &name,
                                                                            AttributeType db_type)
            : CompressedColumn<T>(name, db_type, RUN_LENGTH_DELTA_ONE_COMPRESSED_NUMBER), values_(), count_(),
    ...
    template<>
    inline bool RLEDeltaOneCompressedColumnNumber<std::string>::insert(const std::string &new_value) {
    ...
    template<class T>
    const typename ColumnBaseTyped<T>::DenseValueColumnPtr RLEDeltaOneCompressedColumnNumber<T>::copyIntoDenseValueColumn(
    ...
    template<>
    const typename ColumnBaseTyped<std::string>::DenseValueColumnPtr RLEDeltaOneCompressedColumnNumber<std::string>::copyIntoDenseValueColumn(
            const ProcessorSpecification &proc_spec) const {
    ...
    template<class T>
    bool RLEDeltaOneCompressedColumnNumber<T>::update(TID tid, const boost::any &new_value) {
        if (new_value.empty()) {
    ...
    template<class T>
    bool RLEDeltaOneCompressedColumnNumber<T>::update(PositionListPtr tids, const boost::any &new_value) {
        if (!tids)
            return false;
        if (new_value.empty()) return false;
        TID tid;
    ...
    template<class T>
    bool RLEDeltaOneCompressedColumnNumber<T>::store_impl(const std::string& path, boost::archive::binary_oarchive& oa){
        oa << values_;
        oa << count_;
        return true;
    }
    ...
    template<class T>
    bool RLEDeltaOneCompressedColumnNumber<T>::load_impl(const std::string& path, boost::archive::binary_iarchive& ia){
        ia >> values_;
        ia >> count_;
        return true;
    }
    ...
    template<>
    bool RLEDeltaOneCompressedColumnNumber<char*>::store_impl(const std::string& path, boost::archive::binary_oarchive& oa){
        return false;
    }

    template<>
    bool RLEDeltaOneCompressedColumnNumber<char*>::load_impl(const std::string& path, boost::archive::binary_iarchive& ia){
        return false;
    }
    ...
    template<>
    inline std::string &RLEDeltaOneCompressedColumnNumber<std::string>::operator[](const TID index) {
    ...
    COGADB_INSTANTIATE_CLASS_TEMPLATE_FOR_SUPPORTED_TYPES(RLEDeltaOneCompressedColumnNumber);
};
