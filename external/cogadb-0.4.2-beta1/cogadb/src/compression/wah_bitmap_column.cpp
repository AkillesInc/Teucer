...
namespace CoGaDB {
    ...
    WAHBitmapColumn::WAHBitmapColumn(const std::string column_name)
            : words_(), counts_(), number_of_rows_(0), tailing_word_(0), column_name_(column_name),
              _last_index_position(0),
              _last_lookup_index(0) { };

    WAHBitmapColumn::~WAHBitmapColumn() { };

    void WAHBitmapColumn::setNextRow() {
        this->appendRow(1);
    };

    void WAHBitmapColumn::unsetNextRow() {
        this->appendRow(0);
    };
    ...
    TID WAHBitmapColumn::getPrefixSum(const TID index) {
        ...
        //TID containing_vector_index = binary_search_find_nearest_greater(counts_.data(), counts_.size(), index);
        ...
        //int fill_index = 0;
        /*for (; compressed_index < words_.size() && uncompressed_index < containing_vector_index;) {
            if (isFillWord(words_[compressed_index])) {
                // it is a fill word
                uint32_t fill_word_pop_count = getFillWordValue(words_[compressed_index]) * number_of_values_per_word_;
                result += fill_word_pop_count;
                fill_index++;
                uncompressed_index++;
                compressed_index += (getFillWordCount(words_[compressed_index]) == fill_index);
            } else {
                // it is a literal word
                uint32_t literal_word_pop_count = pop_count(getLiteralWordValues(words_[compressed_index]));
        ...
        if (counts_.size() == 0) {
            ...
            uint32_t bitmask = 0xFFFFFFFF << ((number_of_rows_ % number_of_values_per_word_) - index_in_word);
        ...
        while (counts_[i] <= index) {
            if (isFillWord(words_[i])) {
                ...
                uint32_t fill_word_pop_count =
                        getFillWordValue(words_[i]) * getFillWordCount(words_[i]) * number_of_values_per_word_;
                result += fill_word_pop_count;
            } else {
                // it is a literal word
                uint32_t literal_word_pop_count = pop_count(getLiteralWordValues(words_[i]));
                result += literal_word_pop_count;
            }
            i++;
        }
        if (index >= counts_.back()) {
            // the index is in the tailing word, this also means that the previous while loop already visited all words_
            ...
            if (isFillWord(words_[i])) {
                ...
                if (i == 0) {
                    ...
                    uint32_t fill_word_pop_count =
                            getFillWordValue(words_[i]) * (index - counts_[i - 1]) *
                            number_of_values_per_word_;
                    result += fill_word_pop_count;
                }
            } else {
                // it is a literal word
                uint32_t bitmask = 0xFFFFFFFF << (number_of_values_per_word_ - index_in_word);
        ...
        if (containing_vector_index < tailing_word_index) {
            ...
            if (isFillWord(words_[compressed_index])) {
                containing_word_pop_count = getFillWordValue(words_[compressed_index]) * index_in_word;
            } else {
                uint32_t bitmask = 0xFFFFFFFF << (number_of_values_per_word_ - index_in_word);
                containing_word_pop_count = pop_count(words_[compressed_index] & bitmask);
            }
    ...
    uint32_t WAHBitmapColumn::getRowValue(TID index) {
        ...
        if (index >= count) {
            ...
            int highest_offset_in_tailing_word = (int) number_of_rows_ % number_of_values_per_word_;
            // extract compressed value
            uint32_t y = containing_word >> ((highest_offset_in_tailing_word - 1) - offset) & 1;
            ...
            if (_last_lookup_index == 0 || index - 1 != _last_lookup_index) {
                word_index = binary_search_find_nearest_greater(counts_.data(), counts_.size(), index);
        ...
        if (!isFillWord(containing_word)) {
            ...
            // create bit mask to retrieve compressed value
            // extract compressed value
            uint32_t y = containing_word >> ((number_of_values_per_word_ - 1) - offset) & 1;
            // return uncompressed value
            return y;
        } else {
    ...
    void WAHBitmapColumn::appendRow(uint32_t set) {
        ...
        if (this->number_of_rows_ % number_of_values_per_word_ == 0) {
            // we require a new word, e.g., every 31st value requires a new 32 bit word when we have 31 values per word
            if (number_of_rows_ > 0) {
                // we already have a tailing word, thus, we have to check whether it fits to the previous fill word,
                // becomes part of a new fill word or becomes a literal word
                if (tailing_word_ == 0) {
                    if (words_.size() > 0 && isFillWord(words_.back()) && getFillWordValue(words_.back()) == 0) {
                        // check whether number of words is less than maximum, otherwise create a new fill
                        uint32_t number_of_words_in_fill = words_.back() & fill_word_count_bitmask;
                        ...
                        // it is the first fill word ever or after a literal word or fill one word
                        // create a new fill
                        words_.push_back(uint32_t(initial_zero_fill_word));
                        if (counts_.empty()) {
                    ...
                    // if all possible bits in the tailing word are one this is equal to the literal_word_values_bitmask
                    if (words_.size() > 0 && isFillWord(words_.back()) && getFillWordValue(words_.back()) == 1) {
                        // check whether number of words is less than maximum, otherwise create a new fill
                        uint32_t number_of_words_in_fill = words_.back() & fill_word_count_bitmask;
                        if (number_of_words_in_fill <= fill_word_count_bitmask) {
                            // add to previous fill
                    ...
                    // this tailing word is not suitable to use as a fill word, just add it as literal word to
    ...
    size_t WAHBitmapColumn::getSizeInBytes() {
        return 8 * sizeof(uint32_t) + sizeof(TID) + (words_.capacity() * sizeof(uint32_t)) +
               (counts_.capacity() * sizeof(TID));
    }

    TID WAHBitmapColumn::getNumberOfRows() {
    ...
    bool WAHBitmapColumn::store(const std::string &path_to_database) {
        ...
        if (!quiet && verbose && debug)
            std::cout << "Writing Column " << this->column_name_ << " to File " << path_to_column << std::endl;
    ...
    bool WAHBitmapColumn::load(const std::string &path_to_database) {
        std::string path_to_column(path_to_database);
        if (!quiet && verbose && debug)
            std::cout << "Loading column '" << this->column_name_ << "' from path '" << path_to_column << "'..." <<
            std::endl;
        path_to_column += "/";
        path_to_column += this->column_name_;
