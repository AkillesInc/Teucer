...
namespace CoGaDB {
    ...
    template<class T>
    RLECompressedColumnWithPrefixCounts<T>::RLECompressedColumnWithPrefixCounts(const std::string &name, AttributeType db_type) : CompressedColumn<T>(
            name, db_type, RUN_LENGTH_COMPRESSED_PREFIX), values_(), count_(), last_compressed_value(), _last_lookup_index(0),
    ...
    template<class T>
    bool RLECompressedColumnWithPrefixCounts<T>::insert(const boost::any &new_value) {
    ...
    template<class T>
    const typename ColumnBaseTyped<T>::DenseValueColumnPtr RLECompressedColumnWithPrefixCounts<T>::copyIntoDenseValueColumn(
...
/*
    ...
    template<class T>
    const ColumnPtr RLECompressedColumnWithPrefixCounts<T>::gather(PositionListPtr tid_list, const GatherParam &param) {
        ...
        DenseValueColumnPtr materialized_column = RLECompressedColumnWithPrefixCounts<T>::copyIntoDenseValueColumn(param.proc_spec);
    ...
    template<class T>
    bool RLECompressedColumnWithPrefixCounts<T>::update(TID tid, const boost::any &new_value) {
        if (new_value.empty()) {
    ...
    template<class T>
    bool RLECompressedColumnWithPrefixCounts<T>::update(PositionListPtr tids, const boost::any &new_value) {
        if (!tids)
            return false;
        if (new_value.empty()) return false;
        TID tid;
    ...
    template<class T>
    void RLECompressedColumnWithPrefixCounts<T>::reset_sequential_lookup_variables() {
    ...
    template<class T>
    const PositionListPtr RLECompressedColumnWithPrefixCounts<T>::selection(const SelectionParam &param) {
        ...
        if (comp == EQUAL) {
            TID i = 0;
            // special handling for i = 0 as the number of values in the run is computed differently
            ...
            for (; i < values_.size(); ++i) {
                number_of_matching_tids = (values_array[i] == value) * (count_[i] - count_[i - 1]);
                for (size_t j = 0; j < number_of_matching_tids; ++j) {
                    result_tids->push_back(current_pos + j);
                }
                current_pos = count_[i];
    ...
    template<class T>
    bool RLECompressedColumnWithPrefixCounts<T>::store_impl(const std::string &path, boost::archive::binary_oarchive &oa) {
        oa << values_;
        oa << count_;
        return true;
    }
    ...
    template<class T>
    bool RLECompressedColumnWithPrefixCounts<T>::load_impl(const std::string &path, boost::archive::binary_iarchive &ia) {
        ia >> values_;
        ia >> count_;
        return true;
    }
    ...
    template<>
    bool RLECompressedColumnWithPrefixCounts<char*>::store_impl(const std::string &path, boost::archive::binary_oarchive &oa) {
        COGADB_FATAL_ERROR("Called unimplemented method!","");
        return false;
    }

    template<>
    bool RLECompressedColumnWithPrefixCounts<char*>::load_impl(const std::string &path, boost::archive::binary_iarchive &ia) {
        COGADB_FATAL_ERROR("Called unimplemented method!","");
        return false;
    }
    ...
    T &RLECompressedColumnWithPrefixCounts<T>::operator[](const TID index) {
        ...
        if(_last_lookup_index == 0 || index - 1 != _last_lookup_index) {
            TID i = binary_search_find_nearest_greater(count_.data(), count_.size(), index);
    ...
    template<class T>
    T &RLECompressedColumnWithPrefixCounts<T>::fast_sequential_lookup(const TID index) {
    ...
    template<>
    inline size_t RLECompressedColumnWithPrefixCounts<std::string>::getSizeinBytes() const throw() {
    ...
    COGADB_INSTANTIATE_CLASS_TEMPLATE_FOR_SUPPORTED_TYPES(RLECompressedColumnWithPrefixCounts);
};