    ...
    void checkColumnExistence(const KNF_Selection_Expression& knf){
        for(unsigned int i=0;i<knf.disjunctions.size();++i){
            for(unsigned int j=0;j<knf.disjunctions[i].size();++j){
                ...
                }else if(knf.disjunctions[i][j].getPredicateType()==ValueValuePredicate){
...
TablePtr
CreateTable::execute(ClientPtr client)
{
...
void
AtomExpression::setColumnName(const String &name)
{
	/* set by AS clause */
	explicit_result_colname = name;
}

String AtomExpression::getColumnName() const{
...
AtomExpression::toString()
{
	...
	case INT:
		snprintf(buf, sizeof(buf), "%d",
			 boost::any_cast<int>(atom));
		return String(buf);

	case FLOAT:
		snprintf(buf, sizeof(buf), "%f",
			 double(boost::any_cast<float>(atom)));
		return String(buf);
	case DOUBLE:
		snprintf(buf, sizeof(buf), "%f",
			 boost::any_cast<double>(atom));
		return String(buf);
	...
	case BOOLEAN:
		return boost::any_cast<bool>(atom)
...
String
AtomExpression::getQueryPlan(TypedNodePtr &io_node)
{
	TypedNodePtr result;
	...
	result = boost::make_shared
		<logical_operator::Logical_AddConstantValueColumn>(result_col, 
                type, atom, 
...
String ColumnExpression::getColumnName() const{
    return column;
}

String
ColumnExpression::getQueryPlan(TypedNodePtr &io_node)
{
//        if(!this->rename_map_.empty()){
//            RenameMap::iterator it=rename_map_.find(column);
//            if(it!=rename_map_.end()){
//                //create rename entry: first comes the name of the column to 
//                //be renamed, second comes the resulting column name
//                RenameEntry ren_ent(it->second,it->first);
//                RenameList rename_list;
//                rename_list.push_back(ren_ent);
...
void
FunctionExpression::setColumnName(const String &name)
{
...
String FunctionExpression::getRenameColumnName(){
    return this->new_name;
}

String
FunctionExpression::getQueryPlan(TypedNodePtr &io_node)
{
        //RenameList rename_list;
        //rename_list.push_back(RenameEntry(this->param->getColumnName(), this->new_name));
        //TypedNodePtr result(new logical_operator::Logical_Rename(rename_list));
...
void
AlgebraExpression::setColumnName(const String &name)
{
	/* set by AS clause */
	explicit_result_colname = name;
}
...
String
AlgebraExpression::getQueryPlan(TypedNodePtr &io_node)
{
		...
		if (dynamic_cast<AtomExpression *>(lvalue.get())) {
			...
			if (result_col.empty())
				...
				(value->toString(), value->type, value->atom,
                        ...
                        result = boost::make_shared<logical_operator::Logical_ColumnAlgebraOperator>
			...
			if (result_col.empty())
				result_col = "("+col+op2name[op]+value->toString()+")";
                        result = boost::make_shared<logical_operator::Logical_ColumnConstantOperator>
...
AndCondition::getCNF(TypedNodePtr &io_node)
{
 ...
 */
KNF_Selection_Expression
OrCondition::getCNF(TypedNodePtr &io_node)
{
	KNF_Selection_Expression l_knf = lvalue->getCNF(io_node);
	KNF_Selection_Expression r_knf = rvalue->getCNF(io_node);
	...
	for (int n = 0; n < l_knf.disjunctions.size(); n++) {
		Disjunction &l_disj = l_knf.disjunctions[n];

		for (int m = 0; m < r_knf.disjunctions.size(); m++) {
			Disjunction &r_disj = r_knf.disjunctions[m];
			Disjunction result_disj;

			result_disj.reserve(l_disj.size() + r_disj.size());

			result_disj.insert(result_disj.end(),
					   l_disj.begin(), l_disj.end());
			result_disj.insert(result_disj.end(),
					   r_disj.begin(), r_disj.end());
 ...
 */
static CoGaDB::Predicate
make_pred(const std::string &column, const boost::any &value,
	  PredicateType pred, CoGaDB::ValueComparator comp)
{
	if (pred == ValueValuePredicate)
            return CoGaDB::Predicate(column, boost::any_cast<std::string>(value),
                                     pred, comp);
	else{
 ...
 */
KNF_Selection_Expression
ComparisonPredicate::getCNF(TypedNodePtr &io_node)
{
...
//        checkColumnExistence(cogadb_column);
	...
	switch (normalized_op) {
	case LESSER_EQUAL:
		disjunction.push_back(make_pred(cogadb_column, cogadb_value,
						cogadb_type, CoGaDB::LESSER_EQUAL));
                break;
	case LESSER:
		disjunction.push_back(make_pred(cogadb_column, cogadb_value,
						cogadb_type, CoGaDB::LESSER));
		break;

	case GREATER_EQUAL:
		disjunction.push_back(make_pred(cogadb_column, cogadb_value,
						cogadb_type, CoGaDB::GREATER_EQUAL));
                break;
	case GREATER:
		disjunction.push_back(make_pred(cogadb_column, cogadb_value,
						cogadb_type, CoGaDB::GREATER));
		break;

	case EQUAL:
		disjunction.push_back(make_pred(cogadb_column, cogadb_value,
						cogadb_type, CoGaDB::EQUAL));
		break;
...
{
	ComparisonPredicatePtr lpred = boost::make_shared<ComparisonPredicate>(_exp, GREATER_EQUAL, _lvalue);
	ComparisonPredicatePtr rpred = boost::make_shared<ComparisonPredicate>(_exp, LESSER_EQUAL, _rvalue);
 ...
 */
KNF_Selection_Expression
BetweenPredicate::getCNF(TypedNodePtr &io_node)
{
...
NotBetweenPredicate::NotBetweenPredicate(ScalarExpressionPtr _exp,
					 ...
					 ScalarExpressionPtr _rvalue)
{
	ComparisonPredicatePtr lpred = boost::make_shared<ComparisonPredicate>(_exp, LESSER, _lvalue);
	ComparisonPredicatePtr rpred = boost::make_shared<ComparisonPredicate>(_exp, GREATER, _rvalue);
 ...
 */
KNF_Selection_Expression
NotBetweenPredicate::getCNF(TypedNodePtr &io_node)
{
...
void warn_if_database_empty(std::ostream& out){
    ...
    if(path=="" || path =="./data"){
        ...
        out << "[INFO]: Your database is empty. Either you did not specify a "
            << "database or you have not loaded it." << std::endl << std::endl
        ...
        out << "If you do not have created a cogadb database, you can do so by "
            ...
            << "recommend the star schema benchmark (https://github.com/electrum/ssb-dbgen) "
            << "and the TPC-H benchmark (http://www.tpc.org/tpch/)." << std::endl
            << "\t2. Import the data. You can do so by using the command "
            << "'import_csv_file <TABLE_NAME> <PATH_TO_CSV_FILE>'. For convenience, "
            << "you can use 'create_ssb_database <PATH_TBL_FILES/>' and "
            << "'create_tpch_database <PATH_TBL_FILES/>' to create a schema and"
            << " import all tables of the star schema benchmark and the TPC-H benchmark, respectively." << std::endl
            << "\t3. Save your configuration in a config file 'startup.coga'. " << std::endl
        ...
        out << "You can find more information in the CoGaDB user guide "
            ...
            << "overview of commands, please type 'help'." 
            << std::endl << std::endl;
        out << "If you have questions, suggestions, or a bug to report, please "
            << "feel free to contact the CoGaDB developer team <cogadb@googlegroups.com>."
            << std::endl 
            << "===========================================================================";
    }
}
...
TypedNodePtr
TableName::getQueryPlan()
{
	if(!getTablebyName(this->table)){
		std::stringstream ss;
		ss << "Table '" << this->table << "' not found!";
                warn_if_database_empty(ss);
		//std::cout << "Error! Table " << this->table << " not found in database" << std::endl;
		throw Driver::ParseError(ss.str());
		//return TypedNodePtr();
	}
...
TypedNodePtr
InnerJoin::getQueryPlan()
{
	TypedNodePtr join;
	std::string join_col1, join_col2;

	ComparisonPredicatePtr predicate(boost::dynamic_pointer_cast<ComparisonPredicate>(condition));
	...
	if (predicate && predicate->op == EQUAL &&
		...
		ColumnExpressionPtr lvalue(boost::static_pointer_cast<ColumnExpression>(predicate->lvalue));
		ColumnExpressionPtr rvalue(boost::static_pointer_cast<ColumnExpression>(predicate->rvalue));
...
//        checkColumnExistence(join_col2);
	join = boost::make_shared<logical_operator::Logical_Join>(join_col1, join_col2, INNER_JOIN, RuntimeConfiguration::instance().getGlobalDeviceConstraint());
...
TablePtr
InsertInto::execute(ClientPtr client)
{
...
TableExpression::TableExpression(TableReferenceList &from, SearchConditionPtr where,
				 ColumnListPtr _group_by, OrderByPtr _order_by, SearchConditionPtr _having, LimitPtr _limit)
				: group_by(_group_by), order_by(_order_by), having(_having), limit(_limit)
{
	/*
...
//            }
	 ...
	 * cross joins
	 */
	for (TableReferenceList::iterator it = from.begin();
	     it != from.end(); it++) {
...
//                checkColumnExistence(knf);
		...
		table = boost::make_shared<logical_operator::Logical_ComplexSelection>(knf,LOOKUP,CoGaDB::RuntimeConfiguration::instance().getGlobalDeviceConstraint());
		table->setLeft(io_node);
	}
}

SelectFrom::SelectFrom(ScalarExpressionListPtr columns, TableExpression &table_exp)
{
	...
	if (columns) {
		...
		for (ScalarExpressionList::iterator it = columns->begin();
			...
			if (dynamic_cast<FunctionExpression *>(cur.get())) {
				FunctionExpressionPtr fnc(boost::static_pointer_cast<FunctionExpression>(cur));
				aggr_list.push_back(ColumnAggregation(cur_colname, Aggregate(fnc->function, fnc->getRenameColumnName())));
                                rename_list.push_back(RenameEntry(cur_colname, fnc->getRenameColumnName()));
...
//                            checkColumnExistence(cur_colname);
	...
	if (table_exp.group_by) {
		...
		if (aggr_list.empty()) {
                    std::stringstream ss;
                    ss << "Invalid GROUP BY! At least one column_name must be specified!" << std::endl;
                    throw Driver::ParseError(ss.str());  
		}

		/*
		 * presort table to guarantee that grouping works
		 */
//		sort = boost::make_shared<logical_operator::Logical_Sort>
//						(*table_exp.group_by, ASCENDING);
//		sort->setLeft(node);
//
//		node = boost::make_shared<logical_operator::Logical_Groupby>
//						(*table_exp.group_by, aggr_list);
//		node->setLeft(sort);
                
                //error checking: in SQL queries with group by statement, 
                //the groupby attributes have to appear in the selection clause and vice versa
                std::list<std::string> grouping_columns=*table_exp.group_by;
                grouping_columns.sort();
                
//                {
//                    std::cout << "Grouping Columns: " << std::endl;
//                    std::list<std::string>::iterator it;
//                    for(it=grouping_columns.begin();it!=grouping_columns.end();++it){
//                        std::cout << *it << std::endl;
//                    }
//                }
//                {
//                    std::cout << "Non Computed Columns: in selection clause: " << std::endl;
//                    std::set<std::string>::iterator it;
//                    for(it=non_computed_columns.begin();it!=non_computed_columns.end();++it){
//                        std::cout << *it << std::endl;
//                    }
//                }
                
                std::vector<std::string> intersect(std::max(grouping_columns.size(), non_computed_columns.size()));
                std::vector<std::string>::iterator it=std::set_intersection(grouping_columns.begin(), grouping_columns.end(), 
                                      non_computed_columns.begin(), non_computed_columns.end(),
                                      intersect.begin());
                intersect.resize(it-intersect.begin());
                if(grouping_columns.size()!=intersect.size() || non_computed_columns.size()!=intersect.size()){
                    //COGADB_ERROR("Grouping Columns do not match non computed columns in selection clause!","");
                    std::stringstream ss;
                    ss << "Grouping Columns '";
                    {
                        std::list<std::string>::iterator it;
                        for(it=grouping_columns.begin();it!=grouping_columns.end();++it){
                            ss << *it;
                            if(it!=--grouping_columns.end()) 
                                ss << ",";
                        }
                    }
                    ss << "' do not match non computed columns in selection clause '";
                    {
                        std::set<std::string>::iterator it;
                        for(it=non_computed_columns.begin();it!=non_computed_columns.end();++it){
                            ss << *it;
                            if(it!=--non_computed_columns.end()) 
                                ss << ",";
                        }
                    }       
                    ss << "'!";
		...
		TypedNodePtr groupby = boost::make_shared<logical_operator::Logical_Groupby>(std::list<std::string>(), aggr_list);
                groupby->setLeft(node);
                node=groupby;
	}

        if(table_exp.having){
            /* \todo TODO: Check that columns in having clause are computed by this query*/
            TypedNodePtr io_node = node;
            KNF_Selection_Expression knf = table_exp.having->getCNF(io_node);
            
            std::cout << "Having clause found! Here are the computed columns:" << std::endl;
            RenameList::const_iterator cit; 
            for(cit=rename_list.begin();cit!=rename_list.end();++cit){
//                checkColumnExistence(cit->first);
...
//                    bool found = false;
//                    for(cit=rename_list.begin();cit!=rename_list.end();++cit){
//                        std::cout << "Compare: Predicate Name: '" << column_name << "' Computed Column Name: '" << cit->second << "'" << std::endl;
//                        if(column_name == cit->second){
//                            found=true;
//                        }
//                    }    
//                    if(!found){
//                        std::stringstream ss;
//                        ss << "Column '" << column_name << "' is not computed by this query!" << std::endl;
//                        ss << "Correct example: select RID, sum(VALUES) as computed_result from t where computed_result>100;" << std::endl;
//                        ss << "Check whether you can filter this column in the where clause!" << std::endl;
//                        throw Driver::ParseError(ss.str());  
//                    }
//                }
//            }
            ...
            boost::shared_ptr<logical_operator::Logical_ComplexSelection> having_selection = boost::make_shared<logical_operator::Logical_ComplexSelection>(knf,LOOKUP,CoGaDB::RuntimeConfiguration::instance().getGlobalDeviceConstraint());
            //tell logical optimizer that it should not try to push down this selection down
		...
		TypedNodePtr sort;
		sort = boost::make_shared<logical_operator::Logical_Sort>
						(table_exp.order_by->order); //, table_exp.order_by->order);
		sort->setLeft(node);
		node = sort;
	}
		...
		limit = boost::make_shared<logical_operator::Logical_UDF>
						("LIMIT",parameters, CoGaDB::RuntimeConfiguration::instance().getGlobalDeviceConstraint()); //, table_exp.order_by->order);
		limit->setLeft(node);
		node = limit;
        }
        
	if (!proj_list.empty() && !table_exp.group_by && number_of_non_aggregation_columns>0) {
...
//        TypedNodePtr source = node;
//        node =  boost::make_shared<logical_operator::Logical_Rename>(rename_list);    
////	node = boost::make_shared<logical_operator::Logical_Rename>(RenameList());
//	node->setLeft(source);
        
}

TablePtr
SelectFrom::execute(ClientPtr client)
{
	...
	optimizer::Logical_Optimizer::instance().optimize(log_plan);
        ...
        const std::string query_exec_policy 
                = VariableManager::instance().getVariableValueString("query_execution_policy");
        if(query_exec_policy=="compiled"){
            ProjectionParam project_param;
            CodeGeneratorPtr code_gen = createCodeGenerator(CPP_CODE_GENERATOR,project_param);
            ...
            out << "Compile Time: " 
                ...
                << std::endl;
            out << "Exeuction Time: " 
                << double(end-begin)/(1000*1000*1000) + context->getExecutionTimeSec() 
                << "s" << std::endl;
            result = pipeline->getResult();
        }else if(query_exec_policy=="interpreted"){
            CoGaDB::query_processing::PhysicalQueryPlanPtr result_plan = CoGaDB::query_processing::optimize_and_execute("",*log_plan, client);
            assert(result_plan!=NULL);
            result = result_plan->getResult();
        }else{
            COGADB_FATAL_ERROR("Invalid query execution policy: '" << query_exec_policy << "'","");
        }
        if(result){
...
void
SelectFrom::explain(std::ostream &os)
{
	LogicalQueryPlanPtr log_plan(boost::make_shared<LogicalQueryPlan>(node));
...
CoGaDB::query_processing::LogicalQueryPlanPtr SelectFrom::getLogicalQueryPlan(){
    return CoGaDB::query_processing::LogicalQueryPlanPtr(boost::make_shared<LogicalQueryPlan>(node));
}

TypedNodePtr SelectFrom::getQueryPlan(){
    return node;
}

TablePtr
Sequence::execute(ClientPtr client)
{
	TablePtr result;
...
void
Sequence::explain(std::ostream &os)
{
	for (std::list<StatementPtr>::iterator it = statements.begin();
	     it != statements.end();
	     it++) {
...
std::list<CoGaDB::query_processing::LogicalQueryPlanPtr> Sequence::getLogicalQueryPlans()
{
