...
namespace CoGaDB {
    ...
    public:
        WAHBitmapColumnTest() {
            wah_column_zero_one = WAHBitmapColumnPtr(new WAHBitmapColumn("ZERO_ON_ALTERNATING"));
            for(int i = 0; i < 100; i++) {
                if (i % 2 == 0) {
    ...
    protected:
        ...
        uint32_t getSizeOfWordsVector(WAHBitmapColumnPtr bitmap_column) {
            return bitmap_column->words_.size();
        }

        uint32_t getVectorWord(WAHBitmapColumnPtr bitmap_column, int index) {
    ...
    TEST_F(WAHBitmapColumnTest, NUMBER_OF_ROWS_ZERO_ONE_ALTERNATING) {
        ASSERT_EQ(100, wah_column_zero_one->getNumberOfRows());
    };

    TEST_F(WAHBitmapColumnTest, NUMBER_OF_ROWS_100_ONES) {
        ASSERT_EQ(100, wah_column_ones->getNumberOfRows());
    };

    TEST_F(WAHBitmapColumnTest, NUMBER_OF_ROWS_100_ZEROES) {
        ASSERT_EQ(100, wah_column_zeroes->getNumberOfRows());
    };

    TEST_F(WAHBitmapColumnTest, NUMBER_OF_ROWS_64_ZEROES_60_ONES_3_ZEROES_3_ONES_70_ZEROES) {
        ASSERT_EQ(200, wah_column_mixed->getNumberOfRows());
    };

    TEST_F(WAHBitmapColumnTest, NUMBER_OF_ROWS_EMPTY) {
        ASSERT_EQ(0, wah_column_empty->getNumberOfRows());
    };
    ...
    TEST_F(WAHBitmapColumnTest, GET_FILL_WORD_VALUE) {
        ASSERT_EQ(0, getFillWordValue(wah_column_empty, 0x80000000));
        ASSERT_EQ(0, getFillWordValue(wah_column_empty, 0xA0000001));
        ASSERT_EQ(1, getFillWordValue(wah_column_empty, 0xC0000001));
        ASSERT_EQ(1, getFillWordValue(wah_column_empty, 0xF0005001));
    }

    TEST_F(WAHBitmapColumnTest, GET_FILL_WORD_COUNT) {
        ASSERT_EQ(0x00000001, getFillWordCount(wah_column_empty, 0x80000001));
        ASSERT_EQ(0x20000001, getFillWordCount(wah_column_empty, 0xA0000001));
        ASSERT_EQ(0x20000001, getFillWordCount(wah_column_empty, 0xE0000001));
        ASSERT_EQ(0x30005001, getFillWordCount(wah_column_empty, 0xF0005001));
    }

    TEST_F(WAHBitmapColumnTest, INTERNAL_DATA_STRUCTURES_ZERO_ONE_ALTERNATING) {
        ASSERT_EQ(3, getSizeOfWordsVector(wah_column_zero_one));

        ASSERT_EQ(715827882, getVectorWord(wah_column_zero_one, 0));
        ASSERT_EQ(1431655765, getVectorWord(wah_column_zero_one, 1));
        ASSERT_EQ(715827882, getVectorWord(wah_column_zero_one, 2));

        ASSERT_EQ(85, getTailingWord(wah_column_zero_one));
    };

    TEST_F(WAHBitmapColumnTest, INTERNAL_DATA_STRUCTURES_100_ONES) {
        ASSERT_EQ(1, getSizeOfWordsVector(wah_column_ones));

        ASSERT_EQ(3221225475, getVectorWord(wah_column_ones, 0));

        ASSERT_EQ(127, getTailingWord(wah_column_ones));
    };

    TEST_F(WAHBitmapColumnTest, INTERNAL_DATA_STRUCTURES_100_ZEROES) {
        ASSERT_EQ(1, getSizeOfWordsVector(wah_column_zeroes));

        ASSERT_EQ(2147483651, getVectorWord(wah_column_zeroes, 0));

        ASSERT_EQ(0, getTailingWord(wah_column_zeroes));
    };

    TEST_F(WAHBitmapColumnTest, INTERNAL_DATA_STRUCTURES_64_ZEROES_60_ONES_3_ZEROES_3_ONES_70_ZEROES) {
        ASSERT_EQ(5, getSizeOfWordsVector(wah_column_mixed));

        ASSERT_EQ(2147483650, getVectorWord(wah_column_mixed, 0));
        ASSERT_EQ(536870911, getVectorWord(wah_column_mixed, 1));
        ASSERT_EQ(3221225473, getVectorWord(wah_column_mixed, 2));
        ASSERT_EQ(234881024, getVectorWord(wah_column_mixed, 3));
        ASSERT_EQ(2147483649, getVectorWord(wah_column_mixed, 4));

        ASSERT_EQ(0, getTailingWord(wah_column_mixed));
    };

    TEST_F(WAHBitmapColumnTest, PREFIXSUM_ZERO_ONE_ALTERNATING) {
        ASSERT_EQ(0, wah_column_zero_one->getPrefixSum(0));
        ASSERT_EQ(0, wah_column_zero_one->getPrefixSum(1));
        ASSERT_EQ(1, wah_column_zero_one->getPrefixSum(2));
        ASSERT_EQ(15, wah_column_zero_one->getPrefixSum(31));
        ASSERT_EQ(16, wah_column_zero_one->getPrefixSum(32));
        ASSERT_EQ(25, wah_column_zero_one->getPrefixSum(50));
        ASSERT_EQ(48, wah_column_zero_one->getPrefixSum(97));
        // TODO create death test for this later on
        //EXPECT_FATAL_FAILURE(bitmap_column->getPrefixSum(100), "INDEX_OUT_OF_BOUNDS");
    };

    TEST_F(WAHBitmapColumnTest, PREFIXSUM_100_ONES) {
        ASSERT_EQ(0, wah_column_ones->getPrefixSum(0));
        ASSERT_EQ(1, wah_column_ones->getPrefixSum(1));
        ASSERT_EQ(2, wah_column_ones->getPrefixSum(2));
        ASSERT_EQ(31, wah_column_ones->getPrefixSum(31));
        ASSERT_EQ(32, wah_column_ones->getPrefixSum(32));
        ASSERT_EQ(50, wah_column_ones->getPrefixSum(50));
        ASSERT_EQ(97, wah_column_ones->getPrefixSum(97));
    };

    TEST_F(WAHBitmapColumnTest, PREFIXSUM_100_ZEROES) {
        ASSERT_EQ(0, wah_column_zeroes->getPrefixSum(0));
        ASSERT_EQ(0, wah_column_zeroes->getPrefixSum(1));
        ASSERT_EQ(0, wah_column_zeroes->getPrefixSum(2));
        ASSERT_EQ(0, wah_column_zeroes->getPrefixSum(31));
        ASSERT_EQ(0, wah_column_zeroes->getPrefixSum(32));
        ASSERT_EQ(0, wah_column_zeroes->getPrefixSum(50));
        ASSERT_EQ(0, wah_column_zeroes->getPrefixSum(97));
    };

    TEST_F(WAHBitmapColumnTest, PREFIXSUM_64_ZEROES_60_ONES_3_ZEROES_3_ONES_70_ZEROES) {
        ASSERT_EQ(0, wah_column_mixed->getPrefixSum(0));
        ASSERT_EQ(0, wah_column_mixed->getPrefixSum(1));
        ASSERT_EQ(0, wah_column_mixed->getPrefixSum(2));
        ASSERT_EQ(0, wah_column_mixed->getPrefixSum(31));
        ASSERT_EQ(0, wah_column_mixed->getPrefixSum(32));
        ASSERT_EQ(0, wah_column_mixed->getPrefixSum(50));
        ASSERT_EQ(33, wah_column_mixed->getPrefixSum(97));
        ASSERT_EQ(56, wah_column_mixed->getPrefixSum(120));
        ASSERT_EQ(59, wah_column_mixed->getPrefixSum(123));
        ASSERT_EQ(63, wah_column_mixed->getPrefixSum(197));
    };
    ...
    TEST_F(WAHBitmapColumnTest, IS_ROW_SET_64_ZEROES_60_ONES_3_ZEROES_3_ONES_70_ZEROES) {
