...
bool insertTest(){
    ...
    std::vector<const char*> reference_data_c_string;
    boost::shared_ptr<Column<std::string> > string_col(new Column<std::string>("std::string",VARCHAR));
    boost::shared_ptr<CStringColumn> c_string_col(new CStringColumn("c_string",max_string_length));
    boost::shared_ptr<DictionaryCompressedColumn<std::string> > dict_comp_col(new DictionaryCompressedColumn<std::string>("std::string",VARCHAR));
    
    if(call_reserve_before_insert){
        try{
        string_col->reserve(num_elements);
        c_string_col->reserve(num_elements);
//        dict_comp_col->reserve(num_elements);
    ...
    for(size_t i=0;i<num_elements;++i){
        std::string str = generateRandomString(min_string_length, max_string_length);
        reference_data.push_back(str);
        reference_data_c_string.push_back(str.c_str());
    }
    ...
    if(validate_each_insert_c_string_col){
        ...
        for(size_t i=0;i<num_elements;++i){
//            const char* c_str = reference_data[i].c_str();
            c_string_col->push_back(reference_data_c_string[i]);
        }
    }
    ...
    if(unittest_print_checkpoints){
        ...
        double string_col_time_in_sec = double(end_string_col-begin_string_col)/(1024*1024*1024);
        double c_string_col_time_in_sec = double(end_c_string_col-begin_c_string_col)/(1024*1024*1024);
        double dict_comp_col_time_in_sec = double(end_dict_comp_col-begin_dict_comp_col)/(1024*1024*1024);
        std::cout << "Column<std::string>: " << string_col_time_in_sec << "s" << std::endl;
        std::cout << "CStringColumn: " << c_string_col_time_in_sec << "s" << std::endl;
        std::cout << "DictionaryCompressedColumn<std::string>: " << dict_comp_col_time_in_sec << "s" << std::endl;
    }
    ...
    for(size_t i=0;i<num_elements;++i){
        ...
        if(reference_data[i]!=string_col_data[i]){
            COGADB_FATAL_ERROR("Unittest Failed!: Result of Column<std::string> is wrong!","");
            return false;
        }
    }
    ...
    for(size_t i=0;i<num_elements;++i){
        ...
        if(reference_data[i]!=std::string(c_string_col_data[i])){
            COGADB_ERROR("Unittest Failed!: Result of CStringColumn is wrong!" << std::endl
...
//            }
//            std::cout << " === REFERENCE END   ========================= " << std::endl;            
//            c_string_col->printHeap();
//            for(size_t i=0;i<num_elements;++i){
//                std::cout << "'" << reference_data[i] << "'   '" << std::string(c_string_col_data[i]) << "'" << std::endl;
//            }
            return false;
        }
    }
    ...
    boost::shared_ptr<Column<std::string> > dict_string_col = dict_comp_col->copyIntoDenseValueColumn(proc_spec);
    for(size_t i=0;i<num_elements;++i){
        std::string* dict_string_col_data = dict_string_col->data();
        if(reference_data[i]!=dict_string_col_data[i]){
            COGADB_ERROR("Unittest Failed!: Result of DictionaryCompressedColumn<std::string> is wrong!","");
            return false;
        }
    }
    if(unittest_print_checkpoints)
        std::cout << "Insert Test DictionaryCompressedColumn<std::string> passed..." << std::endl;
...
bool persistenceTest(){
    ...
    for(size_t i=0;i<num_elements;++i){
        std::string str = generateRandomString(min_string_length, max_string_length).c_str();
        reference_data.push_back(str);
    ...
    if(boost::filesystem::exists(path_to_database)){
        std::cerr << "Error: directory './temp_database_unittest' exists already, delete it and restart." << std::endl;
        return false;
    }
    boost::filesystem::create_directory(path_to_database);
    if(!boost::filesystem::exists(path_to_database)){
        std::cerr << "Error: Failed to create directory './temp_database_unittest', ensure you have write access to the working directory." << std::endl;
        return false;
    }
    ...
    boost::shared_ptr<CStringColumn> loaded_c_string_col(new CStringColumn(c_string_col->getName(),0));
    if(!loaded_c_string_col->load(path_to_database)){
    ...
    if(c_string_col->size()!=loaded_c_string_col->size()){
        COGADB_ERROR("Loaded column unequal to reference column! Something went wrong other with load or store.","");
        boost::filesystem::remove_all(path_to_database);
        return false;
    }
...
//    c_string_col->print();
//    std::cout << "====================================================" << std::endl;
//    loaded_c_string_col->print();
    }
    
    /* now check whether load still works when the original column does no longer exist */
    boost::shared_ptr<CStringColumn> loaded_c_string_col(new CStringColumn("c_string",0));
    if(!loaded_c_string_col->load(path_to_database)){
        COGADB_ERROR("Failed to load column!","");
        boost::filesystem::remove_all(path_to_database);
        return false;
