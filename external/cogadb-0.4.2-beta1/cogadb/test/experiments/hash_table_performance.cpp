...
//#pragma omp parallel for
        for(size_t i=0;i<num_build_elements;++i){
            ...
            ht.put(t);
#else
//           std::pair<HashTableIterator,bool> ret = ht.insert(std::make_pair(build_keys[i], i));
...
//           HashTableIterator ret = ht.insert(std::make_pair(build_keys[i], i));
//           assert(ret!=ht.end());            
#endif
        }  
...
#else 
        std::pair<HashTableIterator,HashTableIterator> range = ht.equal_range(probe_keys[i]);
