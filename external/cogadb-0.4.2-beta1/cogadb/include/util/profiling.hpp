...
#define COGADB_PCM_STOP_PROFILING(name, outstream, num_elements, size_of_element, show_core_output, show_socket_output, show_system_output) \
...
print_pcm_measurements(pcm, cstates1, cstates2, sktstate1, sktstate2, sstate1, sstate2, \
cpu_model, show_core_output, show_socket_output, show_system_output, outstream); \
outstream << "[PROFILER]: Time for '" << name << "' for " << num_elements << " elements: " << double(end-begin)/(1000*1000) << "ms" << std::endl; \
outstream << "[PROFILER]: Bandwidth: " << (double(num_elements*size_of_element)/(1024*1024*1024))/(double(end-begin)/(1000*1000*1000)) << "GB/s" << std::endl;
