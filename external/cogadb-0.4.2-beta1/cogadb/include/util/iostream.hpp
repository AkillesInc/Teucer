...
namespace CoGaDB {
    ...
    inline std::ostream& operator<<(std::ostream& os, const boost::any& v) {
        ...
        if (typeid (int) == v.type()) {
            ...
            COGADB_FATAL_ERROR("In std::ostream& operator << (std::ostream& os, const boost::any& v)" << std::endl
                    << "Unkown Type stored in boost::any, cannot print value!", "");
        }
        return os;
    }

    /*! \brief A overload of std::vector<T> for operator <<, so that vector objects can be printed in CoGaDB by using operator <<.*/
    template < class T >
    ...
    /*! \brief Total template specialization for operator <<, so that different semantic can be implemented in contrast to std::vector<T>. This overload is a convinience function, so that Tuple objects can be printed by using operator <<. */
    template <>
    inline std::ostream& operator<<(std::ostream& os, const Tuple& v) {
        ...
        for (unsigned int i = 0; i < v.size(); i++) {
            if (v[i].type() == typeid (int)) {
                ...
                COGADB_FATAL_ERROR("Found Invalid Type in Tuple: " << v[i].type().name(), "");
            }
            if (i < v.size() - 1) os << ",";
        }
        os << ")"; // << std::endl;
