...
namespace CoGaDB{
   ...
   template <typename T>
   const ColumnPtr getMatchingIDsFromDictionary(const std::string& regular_expression, const typename DictionaryCompressedColumn<T>::Dictionary& dict){
       COGADB_FATAL_ERROR("Called unimplemented method!","");
       return ColumnPtr();
   }    
    
   template <>
   const ColumnPtr getMatchingIDsFromDictionary<std::string>(const std::string& regular_expression, const typename DictionaryCompressedColumn<std::string>::Dictionary& dict){
        ...
        {
           COGADB_FATAL_ERROR("'" << regular_expression << "' is not a valid regular expression: '"
                << e.what() << "'","");
           return ColumnPtr();
        }
        
        boost::shared_ptr<Column<CodeWordType> > column = boost::make_shared<Column<CodeWordType> >("", UINT32);
