...
namespace CoGaDB{
    ...
    template <typename T> 
    bool getValueFromAny(const std::string& name_, const boost::any& value, T& typed_value, const AttributeType& value_database_type){
                ...
                if(value.type()==typeid(int)){
                    ...
                    if(value_database_type==DATE && typeid(T)==typeid(uint32_t)){
                        ...
                        if(!convertStringToInternalDateType(v,val)){
                            COGADB_FATAL_ERROR("The string '" << v << "' is not representing a DATE!" << std::endl
                                               << "Typecast Failed!","");
                        }
                    ...
                    COGADB_ERROR("Typemismatch for column " << name_ << std::endl
                              << "Column Type: " << typeid(T).name() << " filter value type: " << value.type().name(), "");
                    return false;
                }
            }catch(boost::bad_any_cast& e){
                COGADB_ERROR("Typemismatch for column " << name_ << std::endl
                          << "Column Type: " << typeid(T).name() << " filter value type: " << value.type().name() << " (bad any cast)", "");   
                return false;
            }catch(boost::bad_lexical_cast& e){
                COGADB_ERROR("Typemismatch for column " << name_ << std::endl
                          << "Column Type: " << typeid(T).name() << " filter value type: " << value.type().name() << " (bad lexical cast)", "");                   
                return false;
            }
        }else{
    ...
    template <> 
    inline bool getValueFromAny<char*>(const std::string& name_, const boost::any& value, 
            char*& typed_value, const AttributeType& value_database_type){
