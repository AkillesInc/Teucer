...
namespace CoGaDB{
    ...
    const std::string SAM_EXPORTER_SIMPLE_INPUT_API_DEFINITION = "<contigID_1>,<contigID_2>,...,<contigID_n> <start> <end> -<param_1> ... -<param_n> (<Filename>)";
    ...
    public:
        ...
        virtual bool addArguments(std::vector<std::string> newArg, std::ostream& clientOut)=0;
        int getStartValue();
        int getEndValue();
        
    protected:
        typedef bool (SamExportParser::*SamExportParameterHandlerPtr)(); //Member Function Pointer takes no arguments and returns a bool
        typedef std::map<std::string, SamExportParameterHandlerPtr> SamExportParameter;
        typedef std::map<std::string, SamExportParameterHandlerPtr>::iterator SamExportParameterIterator;
