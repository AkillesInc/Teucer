...
const std::string QUERY_SELECT = "select sb_read_id, r_qname, r_flag, c_name, rb_position, r_mapq, sb_base_value, "
                "sb_insert_offset, sb_base_call_quality, rg_name ";
const std::string QUERY_FROM = "from sample_base join reference_base on sb_rb_id = "
                "rb_id join contig on rb_c_id = c_id join read on sb_read_id = r_id ";
const std::string QUERY_ORDER_BY = "order by c_id, sb_read_id, rb_position, sb_insert_offset;";
//const std::string QUERY_ORDER_BY = "order by rb_c_id, sb_read_id, sb_rb_id, sb_insert_offset;";
