...
namespace CoGaDB {
    ...
    class SequenceCentric_SimpleKey_WithStash_Importer : public GenomeDataImporter {
    private:
        ...
        bool _importAlignment(TID sample_genome_id, TID id_of_contig, bam1_t *alignment,
                              const bam1_core_t c, TID read_id, TID mate_id, TID &raux_id,
                              string converted_cigar);

        bool _importUnalignedAlignment(TID sample_genome_id,
                                       TID id_of_contig,
                                       bam1_t *alignment,
        ...
        bool _importSampleGenomeData(string sample_genome_name, TID reference_genome_id,
                                     string path_to_sam_bam_file, TID &numberOfUnmappedReads, TID &numberOfReads);

        /**
         * Initiates connection to database by initializing column pointers. If database does not exist, this method creates it.
         */
        bool _initDatabaseConnection();

        std::vector<GenomeDataImporter::aux_info> getAuxiliaryStringVector(bam1_t *alignment);

        string convertCIGAR2String(uint32_t *cigar, uint32_t cigarLength);

    public:
        SequenceCentric_SimpleKey_WithStash_Importer(ostream &, bool compress_data = true, bool verbose = true);

        bool importReferenceGenomeData(const string path_to_fasta_file, const string reference_genome_name);

        bool importSampleGenomeData(const string path_to_sambam_file, const string reference_genome_name,
