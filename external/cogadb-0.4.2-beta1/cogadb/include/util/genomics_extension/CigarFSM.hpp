...
//    void handleInput(char input);
     ...
     * Sets all states to initial Values. Starts a new sequence for cigar calculation.
     */ 
    void reset();
    ...
    const std::string getCigar();
      ...
      * Handles the case, when there is an Insertion at current position of Sample Genome
      ...
      * Handles the case, when there is an Deletion at current position of Sample Genome
      * in comparison to the Reference Genome.
      */
     void handleDelete();
...
private:
    ...
    //Counts the insertions in the current subsequence. Resets, if the state changes.
    unsigned int insertCount;
    
    //Counts the matches in the current subsequence. Resets, if the state changes.
    unsigned int matchCount;
    
    //Counts the deletions in the current subsequence. Resets, if the state changes.
