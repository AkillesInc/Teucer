...
class HardwareDetector{
	public:
		...
		const DeviceSpecifications& getDeviceSpecifications();	
                ...
                size_t getFreeMemorySizeInByte(const hype::ProcessingDeviceMemoryID& mem_id) const;
                size_t getTotalMemorySizeInByte(const hype::ProcessingDeviceMemoryID& mem_id) const;
...
bool printGPUs(std::ostream& out);
		...
		/* \brief convinience function, adds an algorithm specified by AlgorithmSpecification to all available processing devices detected by the HardwareDetector */
