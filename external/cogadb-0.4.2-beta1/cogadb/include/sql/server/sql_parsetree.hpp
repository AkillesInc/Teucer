...
public:
	...
	virtual void explain(std::ostream &os) {}
        virtual CoGaDB::query_processing::LogicalQueryPlanPtr getLogicalQueryPlan(){return CoGaDB::query_processing::LogicalQueryPlanPtr();}
};
...
struct ColumnExpression : public ScalarExpression {
	...
	ColumnExpression(String &_column) : ScalarExpression(), column(_column) {}
	void setColumnName(const String &name);
        String getColumnName() const;
	String getQueryPlan(TypedNodePtr &io_node);
};
...
struct FunctionExpression : public ScalarExpression {
	...
	FunctionExpression(AggregationFunction _function,
			  ...
			  : ScalarExpression(), function(_function), param(_param), new_name(_param->getColumnName()) {

			  }
	FunctionExpression(AggregationFunction _function,
			   ScalarExpressionPtr _param,
                           std::string _new_name)
			  : ScalarExpression(), function(_function), param(_param), new_name(_new_name) {
...
struct ComparisonPredicate : public Predicate {
	...
	ComparisonPredicate(ScalarExpressionPtr _lvalue,
			    ValueComparator _op,
			    ScalarExpressionPtr _rvalue)
...
struct BetweenPredicate : public Predicate {
	...
	BetweenPredicate(ScalarExpressionPtr _exp,
...
struct LikePredicate : public Predicate {
	...
	LikePredicate(ScalarExpressionPtr _lvalue,
                      CoGaDB::ValueComparator _op,
...
struct TableExpression {
	...
	TableExpression(TableReferenceList &from, SearchConditionPtr where,
			ColumnListPtr _group_by, OrderByPtr _order_by, SearchConditionPtr having = SearchConditionPtr(), LimitPtr limit = LimitPtr());
};
...
public:
	...
	void explain(std::ostream &os);
        virtual CoGaDB::query_processing::LogicalQueryPlanPtr getLogicalQueryPlan();
        TypedNodePtr getQueryPlan();
};
...
public:
	...
	std::list<CoGaDB::query_processing::LogicalQueryPlanPtr> getLogicalQueryPlans();
};
