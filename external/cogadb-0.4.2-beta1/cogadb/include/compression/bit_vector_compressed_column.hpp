...
//        bool load(const sstring& path_);
	...
	template<class T>
	BitVectorCompressedColumn<T>::BitVectorCompressedColumn(const sstring& name, AttributeType db_type) : CompressedColumn<T>(name, db_type, BIT_VECTOR_COMPRESSED), bitVectorMap() {}
...
//        }
	...
	template<class T>
	bool BitVectorCompressedColumn<T>::store_impl(const std::string&, boost::archive::binary_oarchive& oa){
		oa << bitVectorMap;
		return true;
	}
    
	template<class T>
	bool BitVectorCompressedColumn<T>::load_impl(const std::string&, boost::archive::binary_iarchive& ia){
            ia >> bitVectorMap;
            return true;
	}
