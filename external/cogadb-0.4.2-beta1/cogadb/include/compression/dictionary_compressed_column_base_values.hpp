...
namespace CoGaDB {
    ...
    class DictionaryCompressedColumnForBaseValues
            : public CompressedColumn<T>, public DictionaryCompressedColForBaseValues {
    public:
        ...
        DictionaryCompressedColumnForBaseValues(const std::string &name, AttributeType db_type,
                                                const hype::ProcessingDeviceMemoryID &mem_id = hype::PD_Memory_0);

        DictionaryCompressedColumnForBaseValues(const std::string &name, AttributeType db_type, Dictionary dictionary,
                                                std::vector<T> reverse_lookup_vector,
                                                const hype::ProcessingDeviceMemoryID &mem_id = hype::PD_Memory_0);

        DictionaryCompressedColumnForBaseValues(const DictionaryCompressedColumnForBaseValues &);

        DictionaryCompressedColumnForBaseValues &operator=(const DictionaryCompressedColumnForBaseValues &);
        ...
        virtual const ColumnPtr copy(const hype::ProcessingDeviceMemoryID &mem_id) const;

        virtual const typename ColumnBaseTyped<T>::DenseValueColumnPtr copyIntoDenseValueColumn(
                const ProcessorSpecification &proc_spec) const;

        const ColumnPtr gather(PositionListPtr tid_list, const GatherParam &);

        virtual const ColumnGroupingKeysPtr createColumnGroupingKeys(const ProcessorSpecification &proc_spec) const;
    ...
    template<class T>
    const typename DictionaryCompressedColumnForBaseValues<T>::Dictionary &DictionaryCompressedColumnForBaseValues<T>::getDictionary() const throw() {
        return dictionary_;
    }

    template<class T>
    const std::vector<uint32_t> &DictionaryCompressedColumnForBaseValues<T>::getCompressedValues() const throw() {
        //workaround, implement proper groupby which does not need something like this!
...
//        return ids_;
    ...
    template<class T>
    std::pair<bool, uint32_t> DictionaryCompressedColumnForBaseValues<T>::getDictionaryID(const T &value) const {
        //                    std::string filter_val = boost::any_cast<std::string>(comp_val);
        //                    const typename DictionaryCompressedColumnForBaseValues<std::string>::Dictionary& dictionary = host_col->getDictionary();
        //                    typename DictionaryCompressedColumnForBaseValues<std::string>::Dictionary::const_iterator it;
    ...
    template<class T>
    unsigned int DictionaryCompressedColumnForBaseValues<T>::getNumberofDistinctValues() const {
    ...
    template<class T>
    DictionaryCompressedColumnForBaseValues<T>::DictionaryCompressedColumnForBaseValues(const std::string &name,
    ...
    DictionaryCompressedColumnForBaseValues<T>::DictionaryCompressedColumnForBaseValues(const std::string &name,
                                                                                        ...
                                                                                        const hype::ProcessingDeviceMemoryID &mem_id)
            : CompressedColumn<T>(name, db_type, DICTIONARY_COMPRESSED),
              ids_(new IDColumn(name + "_ENCODING_IDS", UINT32, mem_id)), dictionary_(dictionary),
    ...
    template<class T>
    DictionaryCompressedColumnForBaseValues<T>::DictionaryCompressedColumnForBaseValues(
            const DictionaryCompressedColumnForBaseValues<T> &other)
            : CompressedColumn<T>(other.getName(), other.getType(), other.getColumnType()),
              ids_(boost::dynamic_pointer_cast<IDColumn>(other.ids_->copy())), dictionary_(other.dictionary_),
              reverse_lookup_vector_(other.reverse_lookup_vector_), maximal_id_(other.maximal_id_) {
        assert(ids_ != NULL);
    }

    template<class T>
    DictionaryCompressedColumnForBaseValues<T> &DictionaryCompressedColumnForBaseValues<T>::operator=(
            const DictionaryCompressedColumnForBaseValues<T> &other) {
    ...
    //	template<class T>
    //	DictionaryCompressedColumnForBaseValues<T>::DictionaryCompressedColumnForBaseValues(DictionaryCompressedColumnForBaseValues& other)
    //        : ids_(other.ids_), dictionary_(other.dictionary_), reverse_lookup_vector_(other.reverse_lookup_vector_), maximal_id_(other.maximal_id_)
    //        {
    //            
    //        }
    ...
    template<class T>
    bool DictionaryCompressedColumnForBaseValues<T>::insert(const boost::any &new_Value) {
    ...
    template<>
    inline bool DictionaryCompressedColumnForBaseValues<string>::insert(const string &value) {
    ...
    template<typename T>
    template<typename InputIterator>
    bool DictionaryCompressedColumnForBaseValues<T>::insert(InputIterator first, InputIterator last) {
    ...
    void DictionaryCompressedColumnForBaseValues<T>::print() const throw() {
        ...
        for (unsigned int i = 0; i < this->size(); i++) {
            std::cout << "| " << reverse_lookup_vector_[(*ids_)[i]] << " |" << std::endl;
        }
    }
    ...
    const ColumnPtr DictionaryCompressedColumnForBaseValues<T>::copy(
        ...
        IDColumnPtr new_typed_id_col = boost::dynamic_pointer_cast<IDColumn>(new_id_col);

        if (!new_typed_id_col) return ColumnPtr();

        DictionaryCompressedColumnForBaseValues<T> *new_col = new DictionaryCompressedColumnForBaseValues<T>(
    ...
    template<class T>
    const typename ColumnBaseTyped<T>::DenseValueColumnPtr DictionaryCompressedColumnForBaseValues<T>::copyIntoDenseValueColumn(
            const ProcessorSpecification &proc_spec) const {
        ...
        DictionaryCompressedColumnForBaseValues<T> *this_col = const_cast<DictionaryCompressedColumnForBaseValues<T> *>(this);
    ...
    template<class T>
    const ColumnPtr DictionaryCompressedColumnForBaseValues<T>::gather(PositionListPtr tid_list,
                                                                       const GatherParam &param) {
        DictionaryCompressedColumnForBaseValues<T> *result = new DictionaryCompressedColumnForBaseValues<T>(this->name_,
                                                                                                            this->db_type_);

//        PositionListPtr copied_tids = copy_if_required(tid_list, hype::PD_Memory_0);//this->mem_alloc->getMemoryID());
//        if(!copied_tids) return ColumnPtr();

        ColumnPtr result_ids = this->ids_->gather(tid_list, param);
        if (!result_ids) return ColumnPtr();
        IDColumnPtr typed_result_ids = boost::dynamic_pointer_cast<IDColumn>(result_ids);
...
//        return ColumnPtr(result);
    ...
    template<class T>
    const ColumnGroupingKeysPtr DictionaryCompressedColumnForBaseValues<T>::createColumnGroupingKeys(
            const ProcessorSpecification &proc_spec) const {
        return this->ids_->createColumnGroupingKeys(proc_spec);
    }

    template<class T>
    size_t DictionaryCompressedColumnForBaseValues<T>::getNumberOfRequiredBits() const {
        return this->ids_->getNumberOfRequiredBits();
    }

    template<class T>
    bool DictionaryCompressedColumnForBaseValues<T>::update(TID index, const boost::any &new_Value) {
        T value = boost::any_cast<T>(new_Value);
    ...
    template<class T>
    bool DictionaryCompressedColumnForBaseValues<T>::update(PositionListPtr tids, const boost::any &new_Value) {
        if (!tids)
            return false;
    ...
    template<class T>
    bool DictionaryCompressedColumnForBaseValues<T>::remove(PositionListPtr tids) {
...
//        return true;
    ...
    template<class T>
    bool DictionaryCompressedColumnForBaseValues<T>::store_impl(const std::string& path, boost::archive::binary_oarchive& oa) {
        oa << dictionary_;
    ...
    template<class T>
    bool DictionaryCompressedColumnForBaseValues<T>::load_impl(const std::string& path, boost::archive::binary_iarchive& ia) {
        ia >> dictionary_;
        ia >> reverse_lookup_vector_;
        ia >> this->maximal_id_;
        return ids_->load(path);
    }

    template<class T>
    bool DictionaryCompressedColumnForBaseValues<T>::isMaterialized() const throw() {
        return false;
    }

    template<class T>
    const ColumnPtr DictionaryCompressedColumnForBaseValues<T>::materialize() throw() {
        return this->copy();
    }

    template<class T>
    hype::ProcessingDeviceMemoryID DictionaryCompressedColumnForBaseValues<T>::getMemoryID() const {
        //column is dormant in memory where ever our ids_ column is
        return ids_->getMemoryID();
    }

    template<class T>
    const PositionListPtr DictionaryCompressedColumnForBaseValues<T>::selection(const SelectionParam &param) {
    ...
    template<class T>
    const BitmapPtr DictionaryCompressedColumnForBaseValues<T>::bitmap_selection(const SelectionParam &param) {
        if (param.pred_type == ValueConstantPredicate && param.comp == EQUAL) {
            T value = boost::any_cast<T>(param.value);
            typename Dictionary::iterator it = this->dictionary_.find(value);
            if (it == dictionary_.end()) {
    ...
    template<class T>
    bool DictionaryCompressedColumnForBaseValues<T>::isCompressed() const throw() {
        return true;
    }
    ...
    template<class T>
    size_t DictionaryCompressedColumnForBaseValues<T>::getSizeinBytes() const throw() {
    ...
    template<>
    inline size_t DictionaryCompressedColumnForBaseValues<std::string>::getSizeinBytes() const throw() {
