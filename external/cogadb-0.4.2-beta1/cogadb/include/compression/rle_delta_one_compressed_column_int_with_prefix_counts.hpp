...
namespace CoGaDB {
    ...
    template<class T>
    class RLEDeltaOneCompressedColumnNumberWithPrefixCounts : public CompressedColumn<T> {
    public:
        RLEDeltaOneCompressedColumnNumberWithPrefixCounts(const std::string& name, AttributeType db_type);
...
//        virtual const PositionListPtr selection(const SelectionParam& param);
        ...
        const typename ColumnBaseTyped<T>::DenseValueColumnPtr copyIntoDenseValueColumn(const ProcessorSpecification& proc_spec) const;
    ...
    template <typename T>
    template <typename InputIterator>
    bool RLEDeltaOneCompressedColumnNumberWithPrefixCounts<T>::insert(InputIterator first, InputIterator last) {
