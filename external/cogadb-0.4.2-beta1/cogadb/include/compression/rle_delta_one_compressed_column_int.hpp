...
namespace CoGaDB {
    ...
    public:
        RLEDeltaOneCompressedColumnNumber(const std::string& name, AttributeType db_type);
...
//        virtual const PositionListPtr selection(const SelectionParam& param);
    ...
    template <typename T>
    template <typename InputIterator>
    bool RLEDeltaOneCompressedColumnNumber<T>::insert(InputIterator first, InputIterator last) {
