...
namespace CoGaDB {
	...
	inline DeltaCompressedColumn<std::string>::DeltaCompressedColumn(const std::string& name, AttributeType db_type) 
		: CompressedColumn<std::string>(name, db_type, DELTA_COMPRESSED) {}

	inline DeltaCompressedColumn<std::string>::~DeltaCompressedColumn() {}

	inline bool DeltaCompressedColumn<std::string>::insert(const boost::any& new_value){
		if (new_value.empty()) {
			return false;
		}
	...
	inline bool DeltaCompressedColumn<std::string>::insert(const std::string& new_value) {
		compress_update(cvalues_.size(), new_value);
		return true;
	}

	inline void DeltaCompressedColumn<std::string>::compress_update(TID tid, std::string value) {
	...
	template <typename InputIterator>
	inline bool DeltaCompressedColumn<std::string>::insert(InputIterator first, InputIterator last) {
	...
	inline bool DeltaCompressedColumn<std::string>::update(TID tid, const boost::any& new_value) {
		if (new_value.empty()) {
			return false;
		}
		if (typeid(std::string) == new_value.type()) {
	...
	inline bool DeltaCompressedColumn<std::string>::update(PositionListPtr tids, const boost::any& new_value){
	...
	inline bool DeltaCompressedColumn<std::string>::store_impl(const std::string& path, boost::archive::binary_oarchive& oa) {
		oa << cvalues_;
		return true;
	}

	inline bool DeltaCompressedColumn<std::string>::load_impl(const std::string& path, boost::archive::binary_iarchive& ia) {
		ia >> cvalues_;
		return true;
	}

	inline std::string& DeltaCompressedColumn<std::string>::operator[](const TID index) {
	...
	inline size_t DeltaCompressedColumn<std::string>::getSizeinBytes() const throw(){
	...
	template<class T>
	bool DeltaCompressedColumn<T>::store_impl(const std::string& path_to_table_dir, boost::archive::binary_oarchive& oa){
		oa << cvalues_;
		return true;
	}

	template<class T>
	bool DeltaCompressedColumn<T>::load_impl(const std::string& path_to_table_dir, boost::archive::binary_iarchive& ia){
		ia >> cvalues_;
		size_ = 0;
