...
#define gpuAssertCheck(ans) { (gpuAssertionCheck((ans), __FILE__, __LINE__)); }
...
inline bool gpuAssertionCheck(cudaError_t code, const char *file, int line, bool abort = false) {
    if (code != cudaSuccess) {
...
namespace CoGaDB {
    ...
    public:
        ...
        static bool generateConstantSequence(T* dest_column, 
                size_t num_elements, 
                T value, 
                const ProcessorSpecification& proc_spec);      
        
        static bool generateAscendingSequence(T* dest_column, 
                size_t num_elements, 
                T begin_value, 
                const ProcessorSpecification& proc_spec);    
        
        static bool gather(T* dest_column, T* source_column,
...
// works for N > 0
template<typename T>
COGADB_HOST_DEVICE_DECLARATION TID binary_search(T* haystack, size_t N, T needle) {
    TID high = N - 1;
    TID mid;
    TID low = 0;
    T sample;
...
// works for N > 0
template<typename T>
COGADB_HOST_DEVICE_DECLARATION TID binary_search_first_occurence(T* haystack, size_t N, T needle) {
    TID high = N - 1;
    TID mid;
    TID low = 0;
    TID index = N;
...
// works for N > 0
template<typename T>
COGADB_HOST_DEVICE_DECLARATION TID binary_search_last_occurrence(T* haystack, size_t N, T needle) {
    TID high = N - 1;
    TID mid;
    TID low = 0;
    TID index = N;
...
// Finds the nearest occurence that is greater than the needle in a sorted sequence haystack
// works for N > 0
template<typename T>
COGADB_HOST_DEVICE_DECLARATION TID binary_search_find_nearest_greater(T* haystack, size_t N, T needle) {
    TID high = N - 1;
    TID mid;
    TID low = 0;
    T sample;
