...
namespace CoGaDB {
    ...
    public:
        ...
        virtual bool column_algebra_operation(T* column, size_t num_elements,
                T value, const AlgebraOperationParam&) = 0;
        
        virtual bool double_precision_column_algebra_operation(double* target_column, T* source_column, 
                                                       size_t num_elements, 
                                                       const AlgebraOperationParam&) = 0;
        ...
        virtual const ColumnGroupingKeysPtr createColumnGroupingKeys(T* column,
                size_t num_elements,
        ...
        virtual size_t getNumberOfRequiredBits(T* column, size_t num_elements,
                const ProcessorSpecification& proc_spec) const = 0;
        ...
        virtual bool bit_shift(ColumnGroupingKeysPtr keys, const BitShiftParam& param) = 0;

        /*! \brief computes target_keys[i] = target_keys[i] & source_keys[i] or target_keys[i] = target_keys[i] | source_keys[i] */
        virtual bool bitwise_combination(ColumnGroupingKeysPtr target_keys, ColumnGroupingKeysPtr source_keys, const BitwiseCombinationParam& param) = 0;
         ...
         */
        virtual const AggregationResult aggregateByGroupingKeys(ColumnGroupingKeysPtr grouping_keys,
        ...
        /* SET OPERATIONS */
        virtual const BitmapPtr computeBitmapSetOperation(BitmapPtr left_bitmap,
        ...
        virtual const PositionListPtr computePositionListSetOperation(PositionListPtr tids1, PositionListPtr tids2, const SetOperationParam&) = 0;
        ...
        /* MISC */
        virtual bool gather(T* dest_column, T* source_column,
                PositionListPtr tids, const GatherParam&) = 0;
        
        virtual bool generateConstantSequence(T* dest_column, 
                size_t num_elements, 
                T value, 
                const ProcessorSpecification& proc_spec) = 0;        
        virtual bool generateAscendingSequence(T* dest_column, 
                size_t num_elements, 
                T begin_value, 
                const ProcessorSpecification& proc_spec) = 0;        
        
        virtual const BitmapPtr convertPositionListToBitmap(PositionListPtr tids,
                size_t num_rows_base_table,
                const ProcessorSpecification& proc_spec) = 0;
        virtual const PositionListPtr convertBitmapToPositionList(BitmapPtr bitmap,
                const ProcessorSpecification& proc_spec) = 0;
        
        virtual const DoubleDenseValueColumnPtr convertToDoubleDenseValueColumn(const std::string& column_name, T* array, size_t value, const ProcessorSpecification& proc_spec) const = 0;
