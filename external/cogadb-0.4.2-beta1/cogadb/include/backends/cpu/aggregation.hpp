...
//            unsigned long hash = HASH(keys[i]) & hashtable->mask;
...
//                        break;
        ...
        std::cout << "Pure Aggregation Time for Hash based aggregation for " << num_elements << " elements: " << double(end-begin)/(1000*1000) << "ms" << std::endl;
        std::cout << "Bandwidth: " << (double(num_elements*sizeof(double))/(1024*1024*1024))/(double(end-begin)/(1000*1000*1000)) << "GB/s" << std::endl;

        Column<TID>* new_keys = new Column<TID>(grouping_keys->keys->getName(), OID);
        
        boost::shared_ptr<Column<double> > aggregated_values(new Column<double>("", DOUBLE));
        ...
        for (size_t bucketid = 0; bucketid < hashtable.hashtable->num_buckets; ++bucketid) {
    ...
    inline void hash_aggregation_sum_thread(const GroupingKeys::value_type* keys, 
            const double* values, 
//            size_t num_elements, 
            size_t begin_index, 
            size_t end_index,
        ...
        typedef TypedHashTable<GroupingKeys::value_type,AggregationPayload> TypedHashTable;
...
//        Timestamp end = getTimestamp();
//        
//        double size_gb = (double(num_elements*sizeof(double))/(1024*1024*1024));
//        double time_in_seconds = (double(end-begin)/(1000*1000*1000)); 
//        
//        std::cout << "Best Case Aggregation Time for Hash based aggregation for " << num_elements << " elements: " << double(end-begin)/(1000*1000) << "ms" << std::endl;
//        std::cout << "Size of Input Data in GB: " << size_gb  << std::endl;
//        std::cout << "Time in seconds: " << time_in_seconds << std::endl;
//        std::cout << "Bandwidth: " << size_gb/time_in_seconds  << "GB/s" << std::endl;            
//        std::cout << sum1 << sum2 << std::endl;
//        }
//        
//        {
//            HashTable ht(num_elements);
//            
//            double* hash_table = (double*) malloc(num_elements*sizeof(double));
//            
//            Timestamp begin = getTimestamp();
//            TID* keys = grouping_keys->keys->data();
//            size_t sum1=0;
//            double sum2=0;
//            
//            
...
//        double time_in_seconds = (double(end-begin)/(1000*1000*1000)); 
//        
//        std::cout << "Time inserting " << num_elements << " elements in hash table: " << double(end-begin)/(1000*1000) << "ms" << std::endl;
//        std::cout << "Size of Input Data in GB: " << size_gb  << std::endl;
//        std::cout << "Time in seconds: " << time_in_seconds << std::endl;
//        std::cout << "Bandwidth: " << size_gb/time_in_seconds  << "GB/s" << std::endl;            
//        }        
//        
        ...
        size_t thread_id, partition_size;
        TID begin_index, end_index;
        #pragma omp parallel default(shared) private(thread_id, num_threads, partition_size, begin_index, end_index)
        {
            
//        for(size_t thread_id=0;thread_id<num_threads;++thread_id){
            thread_id = omp_get_thread_num();
            num_threads = omp_get_num_threads();
            hashtables[thread_id]=boost::make_shared<TypedHashTable>(num_elements/num_threads);
...
//            num_threads = 1; //omp_get_num_threads();
            ...
            hash_aggregation_sum_thread(keys, 
            values,  
            begin_index, 
            end_index,
        ...
        for(size_t i=0;i<num_threads;++i){
            ...
            for (size_t bucketid = 0; bucketid < hashtables[i]->hashtable->num_buckets; ++bucketid) {
                ...
                while (bucket) {
                    for (size_t j = 0; j < bucket->count; j++) {
                        ...
                        hash_bucket_t* ht_result_bucket = hashtable.getBucket(bucket->tuples[j].key);
                        bool found = false;
                        while (ht_result_bucket) {
                            for (size_t k = 0; k < ht_result_bucket->count; k++) {
                                if (ht_result_bucket->tuples[k].key == bucket->tuples[j].key) {
                                    ht_result_bucket->tuples[k].payload.value += bucket->tuples[j].payload.value;
                                    found = true;
                                    break;
                                }
                            }
                        ...
                        //and initialize with the current value 
                        if (!found) {
//                            AggregationPayload p = {i, values[j]};
//                            tuple_t t = {keys[i], p};
...
//
//        assert(grouping_keys->keys->size() == num_elements);
//        //take a dictionary compressed column in case key column is a VARCHAR column, and a plain column otherwise
//        //typedef typename boost::mpl::if_<boost::mpl::equal_to<U,std::string>,DictionaryCompressedColumn<U>,Column<U> >::type KeyColumnType;
//        //        typedef typename TypedColumnImplementationGenerator<U>::ColumnType KeyColumnType;
//
//        typedef GroupingKeys::value_type GroupingKeysType;
//
//        GroupingKeysType* keys = grouping_keys->keys->data();
...
//            } else {
//                std::pair<typename HashTable::iterator, bool> ret = hash_table.insert(std::make_pair(keys[i], Payload(i, double(0))));
//                if (ret.second) {
...
//
//
//        Column<U>* new_keys = new Column<U>(grouping_keys->keys->getName(), grouping_keys->keys->getType());
//        //aggregated_values->setName(values->getName());
//        //Column<float>* aggregated_values = new Column<float>(values->getName(),FLOAT); 
//
//        boost::shared_ptr<Column<double> > aggregated_values(new Column<double>("", DOUBLE));
//        
//        //if there is nothing to do, return valid but empty colums
