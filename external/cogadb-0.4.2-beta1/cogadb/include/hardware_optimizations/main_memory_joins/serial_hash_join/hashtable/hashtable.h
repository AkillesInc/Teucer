...
//test code, took from: http://stackoverflow.com/questions/664014/what-integer-hash-function-are-good-that-accepts-an-integer-hash-key
...
inline uint64_t simple_hash_function(uint64_t x) {
//    x = ((x >> 16) ^ x) * 0x45d9f3b;
//    x = ((x >> 16) ^ x) * 0x45d9f3b;
//    x = ((x >> 16) ^ x);
...
bool hash_get_first (const hashtable_t *hashtable,
                     const ht_key_t key, ht_value_t *val);
