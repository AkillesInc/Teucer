...
namespace CoGaDB {
    ...
    //    TablePtr reconstruct_tuples_reverse_join_index(TablePtr fact_table, PositionListPtr fact_table_tids, IndexedTupleReconstructionParam dimensions, hype::DeviceConstraint dev_constr, std::ostream& out);
    ...
    namespace query_processing {
        ...
        namespace physical_operator {
            ...
            class IndexedTupleReconstructionOperator : public hype::queryprocessing::UnaryOperator<TablePtr, TablePtr> {
            public:
                typedef hype::queryprocessing::OperatorMapper_Helper_Template<TablePtr>::TypedOperatorPtr TypedOperatorPtr;

                IndexedTupleReconstructionOperator(const hype::SchedulingDecision& sched_dec,
            ...
            TypedOperatorPtr create_Indexed_Tuple_Reconstruction_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision&, TypedOperatorPtr left_child, TypedOperatorPtr right_child);

        }//end namespace physical_operator

        //extern Map_Init_Function init_function_InvisibleJoin_operator;

        //Map_Init_Function init_function_InvisibleJoin_operator=physical_operator::map_init_function_InvisibleJoin_operator; //boost::bind();

        namespace logical_operator {

            class Logical_IndexedTupleReconstruction : public hype::queryprocessing::TypedNode_Impl<TablePtr, physical_operator::map_init_function_indexed_tuple_reconstruction_operator> //init_function_InvisibleJoin_operator>
            {
            public:

                Logical_IndexedTupleReconstruction(const IndexedTupleReconstructionParam& dimensions,
                        MaterializationStatus mat_stat = LOOKUP,
                        hype::DeviceConstraint dev_constr = CoGaDB::RuntimeConfiguration::instance().getGlobalDeviceConstraint() // = hype::DeviceConstraint()
                        ) : TypedNode_Impl<TablePtr, physical_operator::map_init_function_indexed_tuple_reconstruction_operator>(false, dev_constr),
                param_(dimensions),
                mat_stat_(mat_stat) {
                }
                ...
                std::string toString(bool verbose) const {
                    ...
                    //                        for(unsigned int i=0;i<chain_join_specification_.join_path.second.size();++i)
                    //                            result+=" (";
                    //                        
                    ...
                    //                               if(!sel_map_it->second.disjunctions.empty()){
                    //                                    result+="( SELECTION ";
                    //                                    result+=sel_map_it->second.toString(); //  knf_sel_expr.toString();
                    //                                    result+=" ON "; 
                    //                               }     
                    //                               
                    //                            }
                    ...
                    //                    if(verbose){
                    //                        result+=" (";
                    //                        IndexedTupleReconstructionParam::const_iterator it;
                ...
                const IndexedTupleReconstructionParam& getIndexedTupleReconstructionParam() {
                    return param_;
                }
