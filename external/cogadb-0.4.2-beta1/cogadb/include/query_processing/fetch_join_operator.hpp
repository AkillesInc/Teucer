...
namespace CoGaDB {
    namespace query_processing {
        namespace physical_operator {
            ...
            class CPU_Fetch_Join_Operator : public hype::queryprocessing::BinaryOperator<TablePtr, TablePtr, TablePtr> {
            public:
                ...
                virtual bool execute() {
                    if (!quiet && verbose && debug) std::cout << "Execute Fetch_Join CPU" << std::endl;
                    ...
                    this->result_ = BaseTable::fetch_join(this->getInputDataLeftChild(), join_column1_name_, this->getInputDataRightChild(), join_column2_name_, param);
                    if (this->result_) {
			...
			TypedOperatorPtr create_CPU_Fetch_Join_Operator(TypedLogicalNode& logical_node, const hype::SchedulingDecision&, TypedOperatorPtr left_child, TypedOperatorPtr right_child);	

        }//end namespace physical_operator

        //extern Map_Init_Function init_function_Fetch_Join_operator;

        //Map_Init_Function init_function_Fetch_Join_operator=physical_operator::map_init_function_Fetch_Join_operator; //boost::bind();

        namespace logical_operator {

			class Logical_Fetch_Join : public hype::queryprocessing::TypedNode_Impl<TablePtr,physical_operator::map_init_function_fetch_join_operator> //init_function_Fetch_Join_operator>
            {
            public:

                Logical_Fetch_Join(const std::string& join_pk_column_name,
                        ...
                        ) : TypedNode_Impl<TablePtr, physical_operator::map_init_function_fetch_join_operator>(false, dev_constr),
