...
namespace CoGaDB {
    namespace query_processing {
        namespace physical_operator {
            ...
            class CPU_ColumnConstantOperator : public hype::queryprocessing::UnaryOperator<TablePtr, TablePtr> {
				public:
                ...
                operation_(operation) {
					...
					virtual bool execute() {
                                            ...
                                            this->result_=BaseTable::ColumnConstantOperation(this->getInputData(),column_name_,value_,result_column_name_, algebra_param);
		...
		//Map_Init_Function init_function_sort_operator=physical_operator::map_init_function_column_constant_operator; //boost::bind();

        namespace logical_operator {

			class Logical_ColumnConstantOperator : public hype::queryprocessing::TypedNode_Impl<TablePtr,physical_operator::map_init_function_column_constant_operator> //init_function_sort_operator>
			{
				public:

					Logical_ColumnConstantOperator(std::string column_name,
					             const boost::any& value, 
                                                     const std::string& result_col_name, 
                                                     ColumnAlgebraOperation operation,
                                                     hype::DeviceConstraint dev_constr = CoGaDB::RuntimeConfiguration::instance().getGlobalDeviceConstraint())
								 : TypedNode_Impl<TablePtr,physical_operator::map_init_function_column_constant_operator>(false,dev_constr), 
									column_name_(column_name), 
									value_(value),
                                                                        result_column_name_(result_col_name),
                operation_(operation) {
