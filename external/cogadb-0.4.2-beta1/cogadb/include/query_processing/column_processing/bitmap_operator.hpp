...
namespace CoGaDB {
    namespace query_processing {
        ...
        namespace physical_operator {
            ...
            class CPU_Bitmap_Operator : public hype::queryprocessing::BinaryOperator<ColumnPtr, ColumnPtr, ColumnPtr>, public BitmapOperator {
            public:
            ...
            column_processing::cpu::TypedOperatorPtr create_CPU_Bitmap_Operator(column_processing::cpu::TypedLogicalNode& logical_node, const hype::SchedulingDecision&, column_processing::cpu::TypedOperatorPtr left_child, column_processing::cpu::TypedOperatorPtr right_child);
//            column_processing::cpu::TypedOperatorPtr create_GPU_Bitmap_Operator(column_processing::cpu::TypedLogicalNode& logical_node, const hype::SchedulingDecision&, column_processing::cpu::TypedOperatorPtr left_child, column_processing::cpu::TypedOperatorPtr right_child);
        ...
        namespace logical_operator {
            ...
            class Logical_Bitmap_Operator : public hype::queryprocessing::TypedNode_Impl<ColumnPtr, physical_operator::map_init_function_cpu_bitmap_operator> //init_function_Join_operator>
            {
            public:

                Logical_Bitmap_Operator(BitmapOperation op,
                        MaterializationStatus mat_stat = MATERIALIZE,
                        hype::DeviceConstraint dev_constr = hype::DeviceConstraint(hype::CPU_ONLY))
                : TypedNode_Impl<ColumnPtr, physical_operator::map_init_function_cpu_bitmap_operator>(false, dev_constr),
                op_(op),
                mat_stat_(mat_stat) {
                }
                ...
                virtual BitmapOperation getBitmapOperation() const {
                    return op_;
                }
            ...
            const hype::Tuple getFeatureVector() const{
                ...
                if (this->left_) { //if left child is valid (has to be by convention!), add input data size
                    ...
                    if(this->left_->getPhysicalOperator() && this->right_->getPhysicalOperator()){
                        
                        BitmapOperator* left_bitmap_op = dynamic_cast<BitmapOperator*>(this->left_->getPhysicalOperator().get());
                        BitmapOperator* right_bitmap_op = dynamic_cast<BitmapOperator*>(this->right_->getPhysicalOperator().get());
                        ...
                        //we use as feature vector <size of bitmap in mega byte,number bitmaps cached on GPU>
                        if(left_bitmap_op->hasResultBitmap()){
                            t.push_back(left_bitmap_op->getResultBitmap()->size()/(1000*1000));
                        }
                        
//                        if(right_bitmap_op->hasResultBitmap()){
//                            t.push_back(right_bitmap_op->getResultBitmap()->size());
//                        }else if(right_bitmap_op->hasCachedResult_GPU_Bitmap()){
//                            t.push_back(right_bitmap_op->getResult_GPU_Bitmap()->size());
//                        }
                        double num_of_cached_GPU_bitmaps=0;
                        if(left_bitmap_op->hasResultBitmap() && CoGaDB::isGPUMemory(left_bitmap_op->getResultBitmap()->getMemoryID())) num_of_cached_GPU_bitmaps++;
                        if(right_bitmap_op->hasResultBitmap() && CoGaDB::isGPUMemory(right_bitmap_op->getResultBitmap()->getMemoryID())) num_of_cached_GPU_bitmaps++;
