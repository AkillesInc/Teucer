...
namespace CoGaDB {
    namespace query_processing {
        namespace physical_operator {
            ...
            class ColumnComparatorOperation : public hype::queryprocessing::BinaryOperator<ColumnPtr, ColumnPtr, ColumnPtr>, public PositionListOperator{
            public:
                ...
                virtual bool execute() {
                    if (!quiet && debug && verbose) std::cout << "Execute Column Operator CPU" << std::endl;
                     //assure we have a Value Value Predicate, meaning we compare to columns
                     ...
                     SelectionParam param(proc_spec, op_.getPredicateType(), this->getInputDataRightChild(), op_.getValueComparator());
                     ...
                     //create a LookupArray, the improtant thing is that the tids are returned
                     //since we need a ColumnPtr to cosntruct a Lookup Array, we pass it the pointer of one of the childs
            ...
            //			GPU_ColumnWise_TypedOperatorPtr create_GPU_ColumnComparatorOperator(GPU_ColumnWise_TypedLogicalNode& logical_node, const hype::SchedulingDecision&, GPU_ColumnWise_TypedOperatorPtr left_child, GPU_ColumnWise_TypedOperatorPtr right_child);

        }//end namespace physical_operator

        namespace logical_operator {

            class Logical_ColumnComparatorOperation : public hype::queryprocessing::TypedNode_Impl<ColumnPtr, physical_operator::map_init_function_cpu_column_comparison_operator> //init_function_Join_operator>
            {
            public:

                Logical_ColumnComparatorOperation(Predicate pred,
                        hype::DeviceConstraint dev_constr = hype::DeviceConstraint(hype::CPU_ONLY))
                : TypedNode_Impl<ColumnPtr, physical_operator::map_init_function_cpu_column_comparison_operator>(false, dev_constr),
                op_(pred),
                mat_stat_() {
                }
