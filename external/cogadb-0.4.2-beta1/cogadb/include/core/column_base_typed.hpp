...
class ColumnBaseTyped : public ColumnBase{
	...
	virtual const ColumnPtr copy(const hype::ProcessingDeviceMemoryID&) const;
        ...
        virtual const DenseValueColumnPtr copyIntoDenseValueColumn(const ProcessorSpecification& proc_spec) const = 0;
	...
	virtual const StringDenseValueColumnPtr convertToDenseValueStringColumn() const;
        ...
        virtual const ColumnPtr decompress(const ProcessorSpecification& proc_spec) const; 
	...
	virtual const PositionListPairPtr hash_join(ColumnPtr join_column);
        ...
        static void hash_join_pruning_thread(ColumnBaseTyped<T>* join_column, HashTable* hashtable, TID* join_tids_table1, TID* join_tids_table2, size_t thread_id, size_t number_of_threads, size_t* result_size);
        static void join_write_result_chunk_thread(ColumnBaseTyped<T>* join_column, TID* join_tids_table1, TID* join_tids_table2, TID* join_tids_result_table1, TID* join_tids_result_table2, size_t thread_id, size_t number_of_threads, TID begin_index_result, TID end_index_result);
	...
	virtual bool is_equal(ColumnPtr column);	
        ...
        virtual const ExtendedColumnStatistics<T>& getExtendedColumnStatistics() const;
	...
	virtual bool operator==(ColumnBaseTyped<T>& column);
        ...
        private:
            virtual bool load_impl(const std::string& path, boost::archive::binary_iarchive& ia) = 0;
            virtual bool store_impl(const std::string& path, boost::archive::binary_oarchive& oa) = 0;
};
