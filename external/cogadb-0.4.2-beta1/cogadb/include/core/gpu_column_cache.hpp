...
//                typedef std::map<JoinIndexPtr,gpu::GPU_JoinIndexPtr> JoinIndexMap;
//
//
//		/* \brief if GPU column already exists, then return pointer to it, otherwise create new GPU column by transferring the data the GPU memory
//			\detail if the GPU Buffer is full, then delete least recently used Column*/
...
//                
//		private:
//		GPU_Column_Cache(unsigned int max_gpu_buffer_size_in_byte); //no constructor call outside of this class
//		GPU_Column_Cache(const GPU_Column_Cache&); //no copy constructor
//		GPU_Column_Cache& operator=(const GPU_Column_Cache&); //nocopy assignment
//                bool cleanupGPUMemory(size_t number_of_bytes_to_cleanup);
//                const gpu::GPU_Base_ColumnPtr getGPUColumn_internal(ColumnPtr);
//		const gpu::GPU_JoinIndexPtr getGPUJoinIndex_internal(JoinIndexPtr);
