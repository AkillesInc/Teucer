...
namespace CoGaDB {
    ...
    enum StringHeapType{
        FIXED_LENGTH_STRING_HEAP,
        VARIABLE_LENGTH_STRING_HEAP
    };
    ...
    public:
        ...
        /* \brief returns an array of char* that point to a string in the heap 
         The order of pointers is the same as the order the pointers where inserted into the heap */
        ...
        virtual bool initStringArray(char** c_string_ptr_array, 
            size_t num_elements, 
        ...
        virtual const StringHeapPtr copy(const hype::ProcessingDeviceMemoryID& mem_id) const = 0;
        ...
        bool load(const std::string& base_path, const std::string& column_name, boost::archive::binary_iarchive& ia);
        /* \brief stores heap on disk */        
        bool store(const std::string& base_path, const std::string& column_name, boost::archive::binary_oarchive& oa);
    ...
    private:
        virtual bool load_impl(const std::string& base_path, const std::string& column_name, boost::archive::binary_iarchive& ia) = 0;
        virtual bool store_impl(const std::string& base_path, const std::string& column_name, boost::archive::binary_oarchive& oa) = 0;
...
//        size_t number_of_elements_;
    ...
    public:
        FixedLengthStringHeap(const size_t max_string_length, 
                const hype::ProcessingDeviceMemoryID mem_id = hype::PD_Memory_0);
        FixedLengthStringHeap(const size_t max_string_length, 
                const size_t num_elements,
                const hype::ProcessingDeviceMemoryID mem_id = hype::PD_Memory_0);
        char* getPointerArray();
        bool initStringArray(char** c_string_ptr_array, 
            size_t num_elements, 
        ...
        const StringHeapPtr copy(const hype::ProcessingDeviceMemoryID& mem_id) const;
        ...
        bool load_impl(const std::string& base_path, const std::string& column_name, boost::archive::binary_iarchive& ia);
        bool store_impl(const std::string& base_path, const std::string& column_name, boost::archive::binary_oarchive& oa);
    ...
    const StringHeapPtr createStringHeap(StringHeapType type, 
            size_t max_string_length, const hype::ProcessingDeviceMemoryID mem_id);
    ...
    public:
        ...
        CStringColumn(const std::string& name, size_t maximal_string_length, hype::ProcessingDeviceMemoryID mem_id = hype::PD_Memory_0);
        ...
        bool insert(const char*& new_value);
...
//        virtual void print() const throw ();
        ...
        virtual const typename Column<char*>::DenseValueColumnPtr copyIntoDenseValueColumn(const ProcessorSpecification& proc_spec) const;
        virtual const DoubleDenseValueColumnPtr convertToDenseValueDoubleColumn(const ProcessorSpecification& proc_spec) const;
        virtual const ColumnPtr materialize() throw ();
        virtual hype::ProcessingDeviceMemoryID getMemoryID() const;
        virtual const ColumnPtr gather(PositionListPtr tid_list, const GatherParam&);
        ...
        virtual const AggregationResult aggregateByGroupingKeys(ColumnGroupingKeysPtr grouping_keys, const AggregationParam&); 
	...
	virtual const PositionListPairPtr join(ColumnPtr join_column, const JoinParam&);
        virtual const PositionListPairPtr join(Column<char*>& join_column, const JoinParam& param);
        
        virtual const PositionListPtr tid_semi_join(ColumnPtr join_column, const JoinParam&);
        virtual const PositionListPtr tid_semi_join(CStringColumn& join_column, const JoinParam&);
        
        virtual const BitmapPtr bitmap_semi_join(ColumnPtr join_column, const JoinParam&);  
        virtual const BitmapPtr bitmap_semi_join(CStringColumn& join_column, const JoinParam&);  

        virtual const ColumnPtr column_algebra_operation(ColumnPtr source_column, const AlgebraOperationParam&);
        virtual const ColumnPtr column_algebra_operation(CStringColumn& source_column, const AlgebraOperationParam&);
        virtual const ColumnPtr column_algebra_operation(const boost::any& value, const AlgebraOperationParam&);
        ...
        virtual void push_back(const char*& val); // __attribute__((always_inline)){
        ...
        void printHeap();
    private:
	virtual bool load_impl(const std::string& path, boost::archive::binary_iarchive& ia);
	virtual bool store_impl(const std::string& path, boost::archive::binary_oarchive& oa);
