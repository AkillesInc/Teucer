...
class BaseTable{
	...
	virtual const TablePtr materialize() const = 0;
        ...
        static const TablePtr createResultTable(TablePtr table, PositionListPtr tids, MaterializationStatus mat_stat, const std::string& operation_name, const ProcessorSpecification& proc_spec);
        //create result table for binary operation
        static const TablePtr createResultTable(TablePtr table1, const std::string& join_column_table1, TablePtr table2, const std::string& join_column_table2, PositionListPairPtr join_tids, TableSchema result_schema, MaterializationStatus mat_stat, const std::string& operation_name, const ProcessorSpecification& proc_spec);
	/***************** status report *****************/
	...
	static const TablePtr selection(TablePtr table, const std::string& column_name, const SelectionParam& param);
        ...
        static const TablePtr selection(TablePtr table, const Disjunction& disjunction, const ProcessorSpecification& proc_spec, const hype::DeviceConstraint&);
	...
	static const TablePtr join(TablePtr table1, const std::string& join_column_table1, TablePtr table2, const std::string& join_column_table2, const JoinParam&);
//											 JoinAlgorithm join_alg=SORT_MERGE_JOIN, MaterializationStatus mat_stat=LOOKUP, const ComputeDevice comp_dev=CPU);

	static const TablePtr semi_join(TablePtr table1, const std::string& join_column_table1, TablePtr table2, const std::string& join_column_table2, const JoinParam&);
        
	static const TablePtr pk_fk_join(TablePtr table1, const std::string& join_column_table1, TablePtr table2, const std::string& join_column_table2, 
											 JoinAlgorithm join_alg=SORT_MERGE_JOIN, MaterializationStatus mat_stat=LOOKUP, const ComputeDevice comp_dev=CPU);
        
	static const TablePtr fetch_join(TablePtr table1, const std::string& join_column_table1, TablePtr table2, const std::string& join_column_table2, const FetchJoinParam& param);
	...
	static const TablePtr sort(TablePtr table, const std::string& column_name, SortOrder order=ASCENDING, MaterializationStatus mat_stat=MATERIALIZE, ComputeDevice comp_dev=CPU);
        ...
        static const TablePtr sort(TablePtr table, const std::list<SortAttribute>& sort_attributes, MaterializationStatus mat_stat=MATERIALIZE, ComputeDevice comp_dev=CPU);
...
//	static const TablePtr groupby(TablePtr table, const std::list<std::string>& grouping_columns, std::list<std::pair<std::string,AggregationMethod> > aggregation_functions, ComputeDevice comp_dev=CPU);    
	static const TablePtr groupby(TablePtr table, const GroupbyParam& param);
        /***************** Aggregation Functions *****************/
         /*! \brief adds a new column named result_col_name to the table, which is the result of col_name <operation> value*/
        static TablePtr ColumnConstantOperation(TablePtr tab, const std::string& col_name, const boost::any& value, const std::string& result_col_name, const AlgebraOperationParam& param);
         /*! \brief adds a new column named result_col_name to the table, which is the result of col1_name <operation> col2_name*/
        static TablePtr ColumnAlgebraOperation(TablePtr tab, const std::string& col1_name, const std::string& col2_name, const std::string& result_col_name, const AlgebraOperationParam& param);
         /*! \brief fills a Column #rows times with value and append to table */ 
        static TablePtr AddConstantValueColumnOperation(TablePtr tab, const std::string& col_name, AttributeType type, const boost::any& value, const ProcessorSpecification& proc_spec);
               
        static const TablePtr user_defined_function(TablePtr table, const std::string& function_name, const std::vector<boost::any>& function_parameters, const ProcessorSpecification& proc_spec);
	...
	bool hasColumn(const std::string& column_name);
        ...
        virtual bool replaceColumn(const std::string& column_name, const ColumnPtr new_column) = 0;
...
//        bool setForeignKeyConstraint(const std::string& column_name, const ForeignKeyConstraint& prim_foreign_key_reference);
	...
	const HashTablePtr getHashTablebyName(const std::string& column_name) const throw();
