...
//fill constructor	
explicit PositionList(size_t number_of_elements, const TID& val=TID(0)) : tid_ptr_(0), num_elements_(0),buffer_size_(0){
...
PositionList(InputIterator first, InputIterator last) : tid_ptr_(0), num_elements_(0),buffer_size_(0){
    assert(last>=first);
...
PositionList(const PositionList& x) : tid_ptr_(0), num_elements_(0),buffer_size_(0){
...
PositionList & operator= (const PositionList & other)
{
    if (this != &other) // protect against invalid self-assignment
    {
        ...
        assert(tid_ptr_!=NULL);
        std::memcpy(tid_ptr_,other.tid_ptr_,other.num_elements_*sizeof(TID));
...
inline TID& operator[](const int index){
...
//            cout << "Reference Vector: " <<  reference_vector[i] << endl;
//            return false;
//        }
//    }
//    return true;
...
TID* end(){
    if(this->empty()){
        ...
        return &tid_ptr_[num_elements_]; //one element further than last element
    }
}
...
//
////insert fill	
//void insert (iterator position, size_t number_of_new_elements, const TID& val){
//    //we currently only support insertion at the end of the PositionList!
//    assert(position==this->end());
...
inline bool operator==(const PositionList& poslist1, const PositionList& poslist2){
...
//template <>
//void PositionList::insert<TID*>(iterator position, TID* first, TID* last){
//    //we currently only support insertion at the end of the PositionList!
//    assert(position==this->end());
//    assert(last>=first);
//    unsigned int number_of_new_elements=last-first;
