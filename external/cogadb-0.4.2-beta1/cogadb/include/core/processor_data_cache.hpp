...
namespace CoGaDB{
	...
	class DataCache{
		...
		/* \brief if olumn already exists, then return pointer to it, otherwise create new column by transferring the data the (co-)processor memory
			\detail if the Buffer is full, then delete least recently used Column*/
		...
		private:
		DataCache(size_t max_buffer_size_in_byte, const hype::ProcessingDeviceMemoryID& mem_id); //no constructor call outside of this class
		...
		bool caching_enabled_;
                 ...
                 *  \details mutex needs to be locked even in cosnt functions, so we make it mutable
                 */
