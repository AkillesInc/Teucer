...
namespace CoGaDB{
	...
	class MemoryAllocator{
		...
		typedef boost::shared_ptr<MemoryAllocator<T> > MemoryAllocatorPtr;
	        MemoryAllocator() : mem_id_(hype::PD_Memory_0), name_() {}
                MemoryAllocator(hype::ProcessingDeviceMemoryID mem_id, const std::string name) : mem_id_(mem_id), name_(name){}
		...
		static const MemoryAllocatorPtr getMemoryAllocator(hype::ProcessingDeviceMemoryID mem_id);
		private:
    ...
    struct AllocationManager{
        public:
        static T* malloc(hype::ProcessingDeviceMemoryID mem_id, size_t number_of_elements);
        static void free(hype::ProcessingDeviceMemoryID mem_id, T*& data);
        static T* realloc(hype::ProcessingDeviceMemoryID mem_id, T* data, size_t number_of_elements);
        
    };
        
    template<typename T> 
    T* customMalloc(hype::ProcessingDeviceMemoryID mem_id, size_t number_of_elements){
    ...
    template<typename T> 
    T* customRealloc(hype::ProcessingDeviceMemoryID mem_id, T* data, size_t number_of_elements){
...
//		//FIXME: for now we only create CPU allocators, plain C allocators for elemtary types, and a c++ allocator for obejct types 
//		if(typeid(T)==typeid(std::string)){
//			allocator=MemoryAllocatorPtr(new CPUMemoryAllocator_CPP<T>());
//		}else{
//			allocator=MemoryAllocatorPtr(new CPUMemoryAllocator_C<T>());
//		}	
//		return allocator;
//	}
