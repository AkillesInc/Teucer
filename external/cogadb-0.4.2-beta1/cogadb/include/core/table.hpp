...
struct ColumnAccessStatistics{
    ...
    : table_name(_table_name), column_name(_column_name), statistics(_statistics) {}
...
class Table : public BaseTable{
	...
	Table(const std::string& name, const TableSchema& schema, const CompressionSpecifications& compression_specifications);

        Table(const std::string& name, const std::string& path_to_database, bool& error_occured);
        
	Table(const std::string& name, const std::vector<ColumnPtr>& columns); //if we already have columns, we can use this constructor to pass them in the table without any copy effort
	virtual ~Table();
	/***************** utility functions *****************/
...
//	virtual const TablePtr selection(const std::string& column_name, const boost::any& value_for_comparison, const ValueComparator& comp, const ComputeDevice& comp_dev) const;// = 0;

//	virtual const TablePtr projection(const std::list<std::string>& columns_to_select, const ComputeDevice comp_dev) const;// = 0;

//	virtual const TablePtr join(TablePtr table, const std::string& join_column_table1, const std::string& join_column_table2, const ComputeDevice comp_dev) const;// = 0;

//	virtual const TablePtr sort(const std::string& column_name, SortOrder order, ComputeDevice comp_dev) const;// = 0;

//	virtual const TablePtr groupby(const std::string& grouping_column, const std::string& aggregation_column,  AggregationMethod agg_meth=SUM, ComputeDevice comp_dev=CPU) const;// = 0;
	...
	bool remove(const std::string& attribute_name, const boost::any& value);
        ...
        bool replaceColumn(const std::string& column_name, const ColumnPtr new_column);
...
//	const std::vector<TID> lookup_selection(const std::string& column_name, const boost::any& value_for_comparison, const ValueComparator& comp, const ComputeDevice& comp_dev) const;

//	//const TablePtr projection(const std::list<std::string>& columns_to_select, const ComputeDevice comp_dev) const;

//	const std::vector<TID_Pair> lookup_join(TablePtr table, const std::string& join_column_table1, const std::string& join_column_table2, const ComputeDevice comp_dev) const;

//	const TablePtr lookup_sort(const std::string& column_name, SortOrder order, ComputeDevice comp_dev) const;
