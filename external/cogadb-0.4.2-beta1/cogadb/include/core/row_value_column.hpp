...
namespace CoGaDB
{	
/*! \brief	Interface for column based access to the early materialized table RowTable */

template <typename T>
class RowValueColumn : public ColumnBaseTyped<T>
{
private:
    /*! \brief Pointer to RowTable (early materialized table) to which this Column depents */    
...
template <typename T>
RowValueColumn<T>::RowValueColumn(RowTablePtr row_table_ptr, string& name, const AttributeType& type) : ColumnBaseTyped<T>(name, type), _row_table_ptr(), _column_index(0), _type()
{	
...
{	
	...
	for(size_t index = 0; index < _row_table_ptr->getNumberofRows(); index++)
	{
	    ...
	    switch(_type)
		{
		   case INT: cout << *reinterpret_cast<int*>(res) << endl; break;
		   case FLOAT: cout << *reinterpret_cast<float*>(res) << endl; break;
		   case BOOLEAN: cout << *reinterpret_cast<bool*>(res) << endl; break;
		   case VARCHAR:
		   for(unsigned short index = 0; index < *stringLength; index++)
			   s.push_back(res[index]);
...
template <typename T>
char* RowValueColumn<T>::getValue(const int index, unsigned short& stringLength) const
{
...
template <typename T>
bool RowValueColumn<T>::isCompressed() const throw(){ return false; }	

template <typename T>
bool RowValueColumn<T>::update(PositionListPtr tid, const boost::any& new_value)
{ 
