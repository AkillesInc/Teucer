...
bool checkStringIsFloat (const std::string&);
    ...
    class VariableState{
    public:
        VariableState(const std::string& value, 
                       AttributeType type,
...
public:
    ...
    bool addVariable(const std::string& variable_name, const VariableState& var_stat);
...
private:
    ...
    template <typename Type>
    Type VariableManager::getVariableValue(const std::string& variable_name) const{
