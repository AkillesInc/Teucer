...
#define COGADB_WARNING(X, Y)  {std::cout << "WARNING: " << X << ": In " <<  __PRETTY_FUNCTION__ << std::endl;\
std::cout << "In File: " << __FILE__ << " Line: " << __LINE__ << std::endl;}

#define COGADB_ERROR(X, Y)  {std::cout << "ERROR: " << X << ": In " << __PRETTY_FUNCTION__ << std::endl;\
std::cout << "In File: " << __FILE__ << " Line: " << __LINE__ << std::endl;\
CoGaDB::printStackTrace(std::cout);}

#define COGADB_FATAL_ERROR(X, Y)  {std::cout << "FATAL ERROR: " << X << ": In " << __PRETTY_FUNCTION__ << std::endl;\
std::cout << "In File: " << __FILE__ << " Line: " << __LINE__ << std::endl;\
...
#define COGADB_INSTANTIATE_CLASS_TEMPLATE_FOR_SUPPORTED_TYPES(TEMPLATE) template class TEMPLATE<int>; \
        template class TEMPLATE<float>; \
        template class TEMPLATE<double>; \
        template class TEMPLATE<uint64_t>; \
        template class TEMPLATE<uint32_t>; \
...
namespace CoGaDB {
    ...
    enum AttributeType {
        INT, FLOAT, VARCHAR, BOOLEAN, UINT32, OID, DOUBLE, CHAR, DATE
    };
    ...
    enum ColumnType {
        ...
        VOID_COMPRESSED_NUMBER, REFERENCE_BASED_COMPRESSED, RUN_LENGTH_COMPRESSED_PREFIX,
        RUN_LENGTH_DELTA_ONE_COMPRESSED_NUMBER_PREFIX
    };

    enum TableLoaderMode {
        LOAD_ALL_COLUMNS, LOAD_NO_COLUMNS, LOAD_ELEMENTARY_COLUMNS
    };
    ...
    enum AggregationMethod {
        COUNT, SUM, MIN, MAX, AVERAGE, AGG_GENOTYPE, AGG_CONCAT_BASES, AGG_IS_HOMOPOLYMER, 
        AGG_GENOTYPE_STATISTICS
    };
    
    enum AggregationAlgorithm {
        SORT_BASED_AGGREGATION, HASH_BASED_AGGREGATION
    };    

    enum ValueComparator {
        LESSER, GREATER, EQUAL, LESSER_EQUAL, GREATER_EQUAL, UNEQUAL
    };

    enum PredicateType {
        ValueValuePredicate, ValueConstantPredicate, ValueRegularExpressionPredicate
    };
    ...
    enum Operation {
        SELECTION, PROJECTION, JOIN, GROUPBY, SORT, COPY, AGGREGATION, FULL_SCAN, INDEX_SCAN
    };

//    enum JoinAlgorithm {
//        SORT_MERGE_JOIN, SORT_MERGE_JOIN_2, NESTED_LOOP_JOIN, HASH_JOIN, PARALLEL_HASH_JOIN, RADIX_JOIN
//    };
    enum JoinAlgorithm{SORT_MERGE_JOIN,SORT_MERGE_JOIN_2,NESTED_LOOP_JOIN,HASH_JOIN, PARALLEL_HASH_JOIN, RADIX_JOIN, INDEX_NESTED_LOOP_JOIN};


    enum JoinType {INNER_JOIN, LEFT_SEMI_JOIN, RIGHT_SEMI_JOIN, LEFT_ANTI_SEMI_JOIN, RIGHT_ANTI_SEMI_JOIN, LEFT_OUTER_JOIN, RIGHT_OUTER_JOIN, FULL_OUTER_JOIN, GATHER_JOIN};
    ...
    enum PositionListOperation {
        POSITIONLIST_INTERSECTION, POSITIONLIST_UNION
    };
    ...
    enum BitShiftOperation{
        SHIFT_BITS_LEFT, SHIFT_BITS_RIGHT
    };
    
    enum BitwiseCombinationOperation{
        BITWISE_AND, BITWISE_OR
    };

    enum GPUBufferManagementStrategy {
        LEAST_RECENTLY_USED, LEAST_FREQUENTLY_USED, DISABLED_GPU_BUFFER
    };
    
    typedef GPUBufferManagementStrategy BufferManagementStrategy;

    enum SelectivityEstimationStrategy {
        NO_SELECTIVITY_ESTIMATION, EQUI_HEIGHT_HISTOGRAM
    };
    ...
    //a SortAttribute determines a specific SortOrder for a specific attribute name
