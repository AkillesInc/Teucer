...
namespace CoGaDB {
    ...
    public:
        ...
        Column(const std::string& name, AttributeType db_type, hype::ProcessingDeviceMemoryID mem_id = hype::PD_Memory_0);

        //fill constructor	
        Column(const std::string& name, AttributeType db_type, size_t number_of_elements, const T& val=T(), hype::ProcessingDeviceMemoryID mem_id = hype::PD_Memory_0);

        //range	constructor
        template <class InputIterator>
        Column(const std::string& name, AttributeType db_type, InputIterator first, InputIterator last) : ColumnBaseTyped<T>(name, db_type, PLAIN_MATERIALIZED), type_tid_comparator(), values_(0), num_elements_(0),buffer_size_(0), mem_alloc(MemoryAllocator<T>::getMemoryAllocator(hype::PD_Memory_0))
        {
            assert(last>=first);
        ...
        virtual const StringDenseValueColumnPtr convertToDenseValueStringColumn() const;
...
//        virtual bool load(const std::string& path);
	...
	virtual const PositionListPairPtr join(ColumnPtr join_column, const JoinParam&);
        virtual const PositionListPairPtr join(Column<T>& join_column, const JoinParam& param);
        
        virtual const PositionListPtr tid_semi_join(ColumnPtr join_column, const JoinParam&);
        virtual const PositionListPtr tid_semi_join(Column<T>& join_column, const JoinParam&);
        
        virtual const BitmapPtr bitmap_semi_join(ColumnPtr join_column, const JoinParam&);  
        virtual const BitmapPtr bitmap_semi_join(Column<T>& join_column, const JoinParam&);  

        virtual const ColumnPtr column_algebra_operation(ColumnPtr source_column, const AlgebraOperationParam&);
        virtual const ColumnPtr column_algebra_operation(Column<T>& source_column, const AlgebraOperationParam&);
        ...
        virtual inline T& operator[](const TID index){
        ...
        virtual void push_back(const T& val) __attribute__((always_inline)){
            ...
            if(num_elements_<buffer_size_-1){
                ...
                this->Column<T>::reserve(new_size);
                //std::cout << "#elements: " << num_elements_ << " buffer_size: " << buffer_size_ << std::endl;
