...
class ColumnStatistics; //forward declaration
...
//	typedef shared_pointer_namespace::shared_ptr<ColumnBase> ColumnPtr;
	...
	ColumnBase(const std::string& name, AttributeType db_type, ColumnType column_type = PLAIN_MATERIALIZED);
	virtual ~ColumnBase();
	/***************** methods *****************/	
	...
	/*! \brief generic function for fetching a string representation for a value form a column
	 *  \return string representing the value on position tid*/
	virtual std::string getStringValue(TID tid)=0; //not const, because operator [] does not provide const return type and the child classes rely on []
	 ...
	 * \return a ColumnPtr to an copy of the current column which is compressed according to col_type*/	
	virtual const ColumnPtr changeCompression(const ColumnType& col_type) const=0;
	 ...
	 * \return a ColumnPtr to an exact copy of the current column*/	
	virtual const ColumnPtr copy(const hype::ProcessingDeviceMemoryID&) const=0;
         ...
         */
        virtual const DoubleDenseValueColumnPtr convertToDenseValueDoubleColumn(const ProcessorSpecification& proc_spec) const = 0;
         ...
         */
	virtual const StringDenseValueColumnPtr convertToDenseValueStringColumn() const = 0;
         ...
         */
        virtual const ColumnPtr decompress(const ProcessorSpecification& proc_spec) const = 0;        
        /*! \brief returns the ID of the memory the column is stored, e.g., in main memory
         or in GPU memory*/
        virtual hype::ProcessingDeviceMemoryID getMemoryID() const = 0;
	/*! \brief creates a new column by fetching all values identified by the tid_list
	 * \return a ColumnPtr that contains only values from the tid_list*/	
	virtual const ColumnPtr gather(PositionListPtr tid_list, const GatherParam&)=0;        
	/*! \brief materializes a column to a normal uncompressed column with dense values 
	...
	/*! \brief filters the values of a column according to a filter condition consisting of a comparison value and a ValueComparator (=,<,>)
	 * \return BitmapPtr to a Bitmap, which represents the result, by having one Bit per per row, indicating whether the row matches the predicate or not*/		
	virtual const BitmapPtr bitmap_selection(const SelectionParam& param)= 0;
	/*! \brief filters the values of a column according to a filter condition consisting of a comparison column and a ValueComparator (=,<,>). 
...
//	virtual const PositionListPtr selection(ColumnPtr, const ValueComparator comp)= 0;        
	...
	virtual const PositionListPairPtr hash_join(ColumnPtr join_column)=0;
 	/*! \brief joins two columns using the hash join algorithm with a parallel pruning phase
	 * \return PositionListPairPtr to a PositionListPair, which represents the result*/       
	 ...
	 * \return PositionListPairPtr to a PositionListPair, which represents the result*/
	virtual const PositionListPairPtr join(ColumnPtr join_column, const JoinParam&)=0;
        virtual const PositionListPtr tid_semi_join(ColumnPtr join_column, const JoinParam&)=0;
        virtual const BitmapPtr bitmap_semi_join(ColumnPtr join_column, const JoinParam&)=0;
        
        /*! creates a compact version of the values in the column, which is used by groupby*/
        virtual const ColumnGroupingKeysPtr createColumnGroupingKeys(const ProcessorSpecification& proc_spec) const = 0;
        /*! returns the number of bits we need to represent the values stored in the column*/        
        virtual size_t getNumberOfRequiredBits() const = 0;
        /*! \brief aggregates the column values according to grouping keys
         *         and AggregationMethod agg_meth using aggregation algorithm agg_alg
         *  \details using aggregation algorithm sort based requires prior sorting of column!
         */
        virtual const AggregationResult aggregateByGroupingKeys(ColumnGroupingKeysPtr grouping_keys, const AggregationParam&) = 0; 
	...
	/***************** column algebra operations *****************/	
        virtual const ColumnPtr column_algebra_operation(ColumnPtr source_column, const AlgebraOperationParam&) = 0;
        virtual const ColumnPtr column_algebra_operation(const boost::any& value, const AlgebraOperationParam&) = 0;
	...
	/*! \brief set database type of column (as defined in "SQL" statement)*/
        ...
        /*! \brief returns type of column (e.g., plain (uncompressed) or dictionary compressed)*/
        ColumnType getColumnType() const throw();

	/*! \brief returns true if column is loaded in memory, otherwise we return false, 
	     because the column resides on disk*/
	bool isLoadedInMainMemory() const throw();
	/*! \brief sets the status whether this column is currently memory resident or on disk*/
	...
	/*! \brief compares the values of this column on position id1 with value at position id2 */
...
const ColumnPtr createColumn(AttributeType type, const std::string& name, ColumnType column_type);

//bool operator== (const ColumnBase& lhs, const ColumnBase& rhs);
bool operator!= (const ColumnBase& lhs, const ColumnBase& rhs);

const PositionListPtr createPositionList(size_t num_of_elements=0, 
        const hype::ProcessingDeviceMemoryID& mem_id=hype::PD_Memory_0);
const PositionListPtr createPositionList(size_t num_of_elements, 
        const ProcessorSpecification& proc_spec);
const PositionListPtr createAscendingPositionList(size_t num_of_elements, 
        const ProcessorSpecification& proc_spec, 
...
/*! create a copy of a data object and transfer data between processors if required*/
ColumnPtr copy(ColumnPtr col, const hype::ProcessingDeviceMemoryID& mem_id);
std::vector<ColumnPtr> copy(const std::vector<ColumnPtr>& col_vec, const hype::ProcessingDeviceMemoryID& mem_id);
PositionListPtr copy(PositionListPtr tids, const hype::ProcessingDeviceMemoryID& mem_id);
PositionListPairPtr copy(PositionListPairPtr pair_tids, const hype::ProcessingDeviceMemoryID& mem_id);
LookupColumnPtr copy(LookupColumnPtr lookup_column, const hype::ProcessingDeviceMemoryID& mem_id);
JoinIndexPtr copy(JoinIndexPtr join_index, const hype::ProcessingDeviceMemoryID& mem_id);
...
ColumnPtr copy_if_required(ColumnPtr col, const hype::ProcessingDeviceMemoryID& mem_id);
std::vector<ColumnPtr> copy_if_required(const std::vector<ColumnPtr>& col, const hype::ProcessingDeviceMemoryID& mem_id);
PositionListPtr copy_if_required(PositionListPtr tids, const hype::ProcessingDeviceMemoryID& mem_id);
PositionListPairPtr copy_if_required(PositionListPairPtr pair_tids, const hype::ProcessingDeviceMemoryID& mem_id);
LookupColumnPtr copy_if_required(LookupColumnPtr lookup_column, const hype::ProcessingDeviceMemoryID& mem_id);
JoinIndexPtr copy_if_required(JoinIndexPtr join_index, const hype::ProcessingDeviceMemoryID& mem_id);
BitmapPtr copy_if_required(BitmapPtr bitmap, const hype::ProcessingDeviceMemoryID& mem_id);

ColumnPtr copy_if_required(ColumnPtr col, const ProcessorSpecification& proc_spec);
std::vector<ColumnPtr> copy_if_required(const std::vector<ColumnPtr>& col, const ProcessorSpecification& proc_spec);
PositionListPtr copy_if_required(PositionListPtr tids, const ProcessorSpecification& proc_spec);
PositionListPairPtr copy_if_required(PositionListPairPtr pair_tids, const ProcessorSpecification& proc_spec);
LookupColumnPtr copy_if_required(LookupColumnPtr lookup_column, const ProcessorSpecification& proc_spec);
JoinIndexPtr copy_if_required(JoinIndexPtr join_index, const ProcessorSpecification& proc_spec);
BitmapPtr copy_if_required(BitmapPtr bitmap, const ProcessorSpecification& proc_spec);

/*! \brief decompresses input column if required and return pointer to 
 * decompressed column 
 * \detail if the input column is not compressed, it is returned as output column
 *  and no work is done. */
ColumnPtr decompress_if_required(ColumnPtr col);

/*! \brief computes the union of two position lists, e.g., to compute a logical OR in a complex selection, the tid lists resulting from several selections need to be united*/
PositionListPtr computePositionListUnion(PositionListPtr tids1, PositionListPtr tids2);
/*! \brief computes the union of two position lists, e.g., to compute a logical AND in a complex selection, the tid lists resulting from several selections need to be intersected*/        
