...
namespace CoGaDB {
    ...
    inline const std::string KNF_Selection_Expression::toString() const{
        ...
        for(unsigned int i=0;i<disjunctions.size();++i){
            for(unsigned int j=0;j<disjunctions[i].size();++j){
                ...
                if(j+1<disjunctions[i].size())    
                        result+=" OR ";                                
            }

            if(i+1<disjunctions.size())
                result+=") AND (";
        }
        result+=")";
        return result;
    ...
    inline const std::list<std::string> KNF_Selection_Expression::getReferencedColumnNames() const{
       std::list<std::string> result;
        for(unsigned int i=0;i<disjunctions.size();++i){
            for(unsigned int j=0;j<disjunctions[i].size();++j){
                if(disjunctions[i][j].getPredicateType()==ValueConstantPredicate || disjunctions[i][j].getPredicateType()==ValueRegularExpressionPredicate){
                    result.push_back(disjunctions[i][j].getColumn1Name());                              
                }else if(disjunctions[i][j].getPredicateType()==ValueValuePredicate){
                    result.push_back(disjunctions[i][j].getColumn1Name());
